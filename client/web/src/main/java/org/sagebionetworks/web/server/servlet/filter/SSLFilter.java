 ...
 * 
 * This is a little tricky.  The load balancer actually does all
 * https communication with clients and talks to the protal-servlets using http:
 * 
 * Request: client->https->load-balancer->http->portal
 * Response: client<-https<-load-balancer<-http<-portal
 * 
 * This is important, as load-balancer can take on the resource intensive
 * encryption work involved in SSL communication.
 * 
 * Q: So if we want to re-direct clients from http to https how do we do so
 * from the portal?
 * A: Look for the header: "X-Forwarded-Proto - a de facto standard for identifying the originating 
 * protocol of an HTTP request, since a reverse proxy (load balancer) may communicate
