...
public class DateTimeUtilsImpl implements DateTimeUtils {
  ...
  public String getDateTimeString(Date toFormat) {
    DateTimeFormat formatter = isShowingUTCTime()
      ? SMALL_DATE_FORMAT_UTC
      : SMALL_DATE_FORMAT;
  ...
  public String getRelativeTime(Date toFormat, boolean forceRelative) {
    if (
      !forceRelative &&
      (toFormat.before(getDaysFromNow(-1)) || toFormat.after(getDaysFromNow(1)))
    ) {
      // older than a day (or more than a day into the future), show a long date (show in UTC if user
      // wants)
