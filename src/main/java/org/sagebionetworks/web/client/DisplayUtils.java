...
public class DisplayUtils {
	...
	private static void focusOnChild(Widget w, String tagName) {
		NodeList<com.google.gwt.dom.client.Element> children =
				w.getElement().getElementsByTagName(tagName);
	...
	private static String getNotifyTemplate(String href, String buttonText) {
		...
		sb.append(
				"<div data-notify=\"container\" class=\"alert fullWidthAlert alert-{0}\" role=\"alert\" style=\"height: 50px;\">\n");
		if (href != null && buttonText != null) {
			sb.append("<a class=\"margin-top-5 right color-white margin-right-20\" href=" + href + ">"
					+ buttonText.toUpperCase() + "</a>\n");
		} else {
			sb.append(
					"<button type=\"button\" aria-hidden=\"true\" class=\"close margin-top-5\" data-notify=\"dismiss\">x</button>\n");
		}
		sb.append(
				"<span data-notify=\"icon\" class=\"black-25-percent font-size-17 margin-top-8 margin-right-5 margin-left-5 \"></span>\n"
	...
	public static void confirm(String trustedHtmlMessage, Callback cancelCallback,
			Callback okCallback) {
	...
	public static void showPopup(String title, String message, DisplayUtils.MessagePopup iconStyle,
			final Callback primaryButtonCallback, final Callback secondaryButtonCallback) {
	...
	public static SafeHtml getPopupSafeHtml(String title, String message,
	...
	public static String getDisplayName(UserGroupHeader header) {
		return DisplayUtils.getDisplayName(header.getFirstName(), header.getLastName(),
				header.getUserName());
	}
	...
	public static String getAlertHtml(String title, String text, BootstrapAlertType type) {
		return "<div class=\"alert alert-" + type.toString().toLowerCase()
				+ "\"><span class=\"boldText\">" + title + "</span> " + SafeHtmlUtils.htmlEscape(text)
				+ "</div>";
	}
	...
	public static String getVersionDisplay(VersionableEntity versionable) {
		...
		if (versionable.getVersionLabel() != null
				&& !versionable.getVersionNumber().toString().equals(versionable.getVersionLabel())) {
	 ...
	 * @return
	 */
	public static String createWikiAttachmentUrl(String baseFileHandleUrl, WikiPageKey wikiKey,
		...
		String wikiIdParam = wikiKey.getWikiPageId() == null ? ""
				: "&" + WebConstants.WIKI_ID_PARAM_KEY + "=" + wikiKey.getWikiPageId();

		return baseFileHandleUrl + "?" + WebConstants.WIKI_OWNER_ID_PARAM_KEY + "="
				+ wikiKey.getOwnerObjectId() + "&" + WebConstants.WIKI_OWNER_TYPE_PARAM_KEY + "="
				+ wikiKey.getOwnerObjectType() + "&" + WebConstants.WIKI_FILENAME_PARAM_KEY + "=" + fileName
				+ "&" + WebConstants.FILE_HANDLE_PREVIEW_PARAM_KEY + "=" + Boolean.toString(preview)
				+ wikiIdParam;
	}

	public static String createFileEntityUrl(String baseFileHandleUrl, String entityId,
			Long versionNumber, boolean preview) {
	 ...
	 */
	public static String createFileEntityUrl(String baseFileHandleUrl, String entityId,
			Long versionNumber, boolean preview, boolean proxy) {
		String versionParam = versionNumber == null ? ""
				: "&" + WebConstants.ENTITY_VERSION_PARAM_KEY + "=" + versionNumber.toString();
		return baseFileHandleUrl + "?" + WebConstants.ENTITY_PARAM_KEY + "=" + entityId + "&"
				+ WebConstants.FILE_HANDLE_PREVIEW_PARAM_KEY + "=" + Boolean.toString(preview) + "&"
				+ WebConstants.PROXY_PARAM_KEY + "=" + Boolean.toString(proxy) + versionParam;
	}
	...
	public static void updateWidgetSelectionState(WidgetSelectionState state, String text,
			int cursorPos) {
		...
		if (cursorPos > -1) {
			...
			if (possibleWidget.startsWith(WidgetConstants.WIDGET_START_MARKDOWN)) {
				...
				while (endWord < text.length()
						&& !WidgetConstants.WIDGET_END_MARKDOWN.equals(String.valueOf(text.charAt(endWord)))) {
					endWord++;
				}
				...
				if (endWord < text.length()) {
					...
					state.setInnerWidgetText(
							possibleWidget.substring(WidgetConstants.WIDGET_START_MARKDOWN.length(),
									possibleWidget.length() - WidgetConstants.WIDGET_END_MARKDOWN.length()));
	...
	/**
	 * Surround the selectedText with the given markdown. Or, if the selected text is already
	 * surrounded by the markdown, then remove it.
	 * 
	 * @param text
	 * @param markdown
	 * @param startPos
	 ...
	 * @return
	 */
	public static String surroundText(String text, String startTag, String endTag,
			boolean isMultiline, int startPos, int selectionLength) throws IllegalArgumentException {
		if (text != null && selectionLength > -1 && startPos >= 0 && startPos <= text.length()
				&& isDefined(startTag)) {
			if (endTag == null)
				endTag = "";
			...
			if (beforeSelectedTextPos > -1 && afterSelectedTextPos <= text.length()) {
				if (startTag.equals(text.substring(beforeSelectedTextPos, startPos))
						&& endTag.equals(text.substring(endPos, afterSelectedTextPos))) {
					// strip off markdown instead
					return text.substring(0, beforeSelectedTextPos) + selectedText
							+ text.substring(afterSelectedTextPos);
				}
			}
			return text.substring(0, startPos) + startTag + selectedText + endTag
	...
	public static com.google.gwt.user.client.ui.Button createIconButton(String title, ButtonType type,
			String iconClass) {
	...
	public static void relabelIconButton(com.google.gwt.user.client.ui.Button btn, String title,
			String iconClass) {
	...
	public static void getPublicPrincipalIds(UserAccountServiceAsync userAccountService,
			final AsyncCallback<PublicPrincipalIds> callback) {
		if (publicPrincipalIds == null) {
			userAccountService
					.getPublicAndAuthenticatedGroupPrincipalIds(new AsyncCallback<PublicPrincipalIds>() {
						@Override
	...
	/**
	 * return true if the widget is in the visible part of the page paddingBottom is the extra space
	 * (in px) to enlarge the viewport (in order to preemptively load the widget before scrolling into
	 * view).
	 */
	...
	public static boolean isAnyModifierKeyDown(ClickEvent event) {
		...
		return event.isAltKeyDown() || event.isControlKeyDown() || event.isMetaKeyDown()
