...
public class DisplayUtils {
  ...
  private static Logger displayUtilsLogger = Logger.getLogger(
    DisplayUtils.class.getName()
  );
  ...
  public static KeyDownHandler getPreventTabHandler(Focusable lastWidget) {
    return event -> {
      if (
        KeyCodes.KEY_TAB == event.getNativeKeyCode() && !event.isShiftKeyDown()
      ) {
        event.preventDefault();
  ...
  private static void focusOnChild(Widget w, String tagName) {
    NodeList<com.google.gwt.dom.client.Element> children = w
      .getElement()
  ...
  public static String getFriendlySize(double size, boolean abbreviatedUnits) {
    ...
    if (size >= TB) {
      return (
        decimalFormat.format(size / TB) +
        (abbreviatedUnits ? " TB" : " Terabytes")
      );
    }
    if (size >= GB) {
      return (
        decimalFormat.format(size / GB) +
        (abbreviatedUnits ? " GB" : " Gigabytes")
      );
    }
    if (size >= MB) {
      return (
        fileSizeFormat.format(size / MB) +
        (abbreviatedUnits ? " MB" : " Megabytes")
      );
    }
    if (size >= KB) {
      return (
        fileSizeFormat.format(size / KB) +
        (abbreviatedUnits ? " KB" : " Kilobytes")
      );
    }
  ...
  public static void notify(
    ...
    notify(
      message,
      variant,
      new ToastMessageOptions.Builder().setTitle(title).build()
    );
  }
  ...
  public static void notify(
    ...
    try {
      ...
      if (!recentNotificationMessages.contains(key)) {
        ...
        SRC.SynapseComponents.displayToast(
          message,
          variant.toString(),
          options
        );
  ...
  public static void confirmDelete(
    ...
    options.addButton(
      ...
      DANGER_BUTTON_STYLE,
      () -> {
        if (yesCallback != null) yesCallback.invoke();
      }
    );
      ...
      Bootbox.alert(
        ...
        new SimpleCallback() {
          ...
          public void callback() {
            if (primaryButtonCallback != null) primaryButtonCallback.invoke();
          }
        }
      );
    }
  ...
  public static SafeHtml getPopupSafeHtml(
    String title,
    String message,
    DisplayUtils.MessagePopup iconStyle
  ) {
    String iconHtml = "";
    if (MessagePopup.INFO.equals(iconStyle)) iconHtml =
      getFontAwesomeIcon("info-circle font-size-32 col-xs-1"); else if (
      MessagePopup.WARNING.equals(iconStyle)
    ) iconHtml =
      getFontAwesomeIcon("exclamation-circle font-size-32 col-xs-1"); else if (
      MessagePopup.QUESTION.equals(iconStyle)
    ...
    if (iconHtml.length() > 0) builder.appendHtmlConstant(iconHtml);
    String messageWidth = DisplayUtils.isDefined(iconHtml)
      ? "col-xs-11"
      : "col-xs-12";
  ...
  public static String getPrimaryEmail(UserProfile userProfile) {
    ...
    if (
      emailAddresses == null || emailAddresses.isEmpty()
  ...
  public static String getDisplayName(UserGroupHeader header) {
    return DisplayUtils.getDisplayName(
      header.getFirstName(),
      header.getLastName(),
      header.getUserName()
    );
  ...
  public static String getMarkdownWidgetWarningHtml(String warningText) {
    return getWarningHtml(
      DisplayConstants.MARKDOWN_WIDGET_WARNING,
      warningText
    );
  }
  ...
  public static String getAlertHtml(
    String title,
    String text,
    BootstrapAlertType type
  ) {
    return (
   ...
   */
  public static Tooltip addTooltip(
    Widget widget,
    String tooltipText,
    Placement pos
  ) {
  ...
  public static String getVersionDisplay(VersionableEntity versionable) {
    String version = "";
    if (
      versionable == null || versionable.getVersionNumber() == null
    ) return version;

    if (
      versionable.getVersionLabel() != null &&
      !versionable
        .getVersionNumber()
        .toString()
        .equals(versionable.getVersionLabel())
    ) {
      version =
        versionable.getVersionLabel() +
        " (" +
        versionable.getVersionNumber() +
        ")";
    } else {
  ...
  public static native JavaScriptObject newWindow(
    String url,
    String name,
    String features
  ) /*-{
  ...
  public static void setTestWebsite(
    boolean testWebsite,
    CookieProvider cookies
  ) {
    setInCookies(
      testWebsite,
      DisplayUtils.SYNAPSE_TEST_WEBSITE_COOKIE_KEY,
      cookies
    );
  }

  public static final String SYNAPSE_TEST_WEBSITE_COOKIE_KEY =
    "SynapseTestWebsite";
  ...
  public static void setInCookies(
    boolean value,
    String cookieKey,
    CookieProvider cookies
  ) {
   ...
   * @return
   */
  public static String createWikiAttachmentUrl(
    ...
    return (
      baseFileHandleUrl +
      "?" +
      WebConstants.WIKI_OWNER_ID_PARAM_KEY +
      "=" +
      wikiKey.getOwnerObjectId() +
      "&" +
      ...
      "&" +
      WebConstants.WIKI_FILENAME_PARAM_KEY +
      "=" +
      fileName +
      "&" +
   ...
   * @return
   */
  public static String createTeamIconUrl(
    String baseFileHandleUrl,
    String teamId
  ) {
  ...
  public static String createEntityVersionString(Reference ref) {
    return createEntityVersionString(
      ref.getTargetId(),
  ...
  public static String createEntityVersionString(String id, Long version) {
    ...
    if (version != null) return (
      idNotNull + WebConstants.ENTITY_VERSION_STRING + version
    ); else return idNotNull;
  }
   ...
   * @return
   */
  public static CloudProviderFileHandleInterface getPreviewFileHandle(
    EntityBundle bundle
  ) {
  ...
  public static void updateWidgetSelectionState(
    WidgetSelectionState state,
    String text,
    int cursorPos
  ) {
    ...
    if (cursorPos > -1) {
      ...
      if (possibleWidget.startsWith(WidgetConstants.WIDGET_START_MARKDOWN)) {
        ...
        while (
          endWord < text.length() &&
          !WidgetConstants.WIDGET_END_MARKDOWN.equals(
            String.valueOf(text.charAt(endWord))
          )
        ) {
          endWord++;
        }
        ...
        if (endWord < text.length()) {
          ...
          state.setInnerWidgetText(
            possibleWidget.substring(
              WidgetConstants.WIDGET_START_MARKDOWN.length(),
              possibleWidget.length() -
              WidgetConstants.WIDGET_END_MARKDOWN.length()
            )
          );
   ...
   * @return
   */
  public static String surroundText(
    String text,
    String startTag,
    String endTag,
    boolean isMultiline,
    ...
    if (
      ...
      if (eolPos < 0) eolPos = text.length();
      int endPos = startPos + selectionLength;

      if (eolPos < endPos && !isMultiline) throw new IllegalArgumentException(
        DisplayConstants.SINGLE_LINE_COMMAND_MESSAGE
      );
      ...
      if (beforeSelectedTextPos > -1 && afterSelectedTextPos <= text.length()) {
        if (
          startTag.equals(text.substring(beforeSelectedTextPos, startPos)) &&
          endTag.equals(text.substring(endPos, afterSelectedTextPos))
        ) {
          // strip off markdown instead
          return (
            text.substring(0, beforeSelectedTextPos) +
            selectedText +
  ...
  public static com.google.gwt.user.client.ui.Button createButton(
    String title
  ) {
  ...
  public static com.google.gwt.user.client.ui.Button createIconButton(
    String title,
    ButtonType type,
    String iconClass
  ) {
  ...
  public static void relabelIconButton(
    com.google.gwt.user.client.ui.Button btn,
    String title,
    String iconClass
  ) {
    String style = iconClass == null
      ? ""
      : " class=\"glyphicon " + iconClass + "\"";
    btn.setHTML(
      SafeHtmlUtils.fromSafeConstant("<span" + style + "></span> " + title)
    );
  }
  ...
  public static void getPublicPrincipalIds(
    UserAccountServiceAsync userAccountService,
    final AsyncCallback<PublicPrincipalIds> callback
  ) {
    if (publicPrincipalIds == null) {
      userAccountService.getPublicAndAuthenticatedGroupPrincipalIds(
        new AsyncCallback<PublicPrincipalIds>() {
          @Override
    ...
    } else callback.onSuccess(publicPrincipalIds);
  }
  ...
  public static void showFormError(
  ...
  public static void hideFormError(
    DivElement parentElement,
    DivElement messageElement
  ) {
