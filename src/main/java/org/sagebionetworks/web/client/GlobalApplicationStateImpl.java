...
public class GlobalApplicationStateImpl implements GlobalApplicationState {
	...
	static {
		...
		SAFE_TO_IGNORE_ERRORS
				.add("the node before which the new node is to be inserted is not a child of this node.");
	...
	@Inject
	public GlobalApplicationStateImpl(GlobalApplicationStateView view, CookieProvider cookieProvider,
			EventBus eventBus, StackConfigServiceAsync stackConfigService,
			SynapseJSNIUtils synapseJSNIUtils, ClientCache localStorage, GWTWrapper gwt,
			DateTimeUtils dateTimeUtils, SynapseJavascriptClient jsClient,
			SynapseProperties synapseProperties, SessionStorage sessionStorage) {
	...
	public void handleUncaughtException(Throwable e) {
		try {
			...
			synapseJSNIUtils
					.consoleError("Unable to log uncaught exception to server: " + t.getMessage());
		} finally {
	...
	private Place fixIfNull(Place defaultPlace) {
		if (defaultPlace == null)
			return AppActivityMapper.getDefaultPlace();
		else
			return defaultPlace;
	}
	...
	public void setLastPlace(Place lastPlace) {
		sessionStorage.setItem(GlobalApplicationStateImpl.LAST_PLACE,
	...
	public void checkVersionCompatibility(final AsyncCallback<VersionState> callback) {
		...
		localStorage.put(RECENTLY_CHECKED_SYNAPSE_VERSION, Boolean.TRUE.toString(),
	...
	private final static native void _initializeDragDrop(GlobalApplicationStateImpl globalAppState,
			Element dropZone, Element rootPanel) /*-{
		try {
