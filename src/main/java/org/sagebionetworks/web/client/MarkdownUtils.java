...
public class MarkdownUtils {
	...
	public static String markdown2Html(String markdown, String attachmentUrl, MarkdownProcessor markdownProcessor) {
		...
		MarkdownUtils.sendAllLinksToNewWindow(doc);
		Elements anchors = doc.getElementsByTag("a");
		anchors.addClass("link");
		MarkdownUtils.applyCssClass(doc, DisplayConstants.MARKDOWN_CSS_CLASSNAME);
		MarkdownUtils.resolveAttachmentImages(doc, attachmentUrl);
		MarkdownUtils.addSynapseLinks(doc);
		MarkdownUtils.addUrlLinks(doc);
		MarkdownUtils.addYouTubeVideos(doc);
		return doc.html();
	}
			...
			for (Iterator iterator2 = element.childNodes().iterator(); iterator2.hasNext();) {
				...
				if (childNode instanceof TextNode) {
					...
					while (matcher.find() && matcher.groupCount() == 1) {
						...
						sb.append(MarkdownUtils.getSynAnchorHtml(matcher.group(1))); //the actual synapse Id group (not the non-word characters that might surround it)
			...
			for (Iterator iterator2 = element.childNodes().iterator(); iterator2.hasNext();) {
				...
				if (childNode instanceof TextNode) {
					...
					while (matcher.find() && matcher.groupCount() == 2) {
						sb.append(MarkdownUtils.getYouTubeHTML(matcher.group(2)));
						previousFoundIndex = matcher.end(1);
					}
					if (previousFoundIndex < oldText.length() - 1)
						// substring, go from the previously found index to the end
			...
			for (Iterator iterator2 = element.childNodes().iterator(); iterator2.hasNext();) {
				...
				if (childNode instanceof TextNode) {
					...
					while (matcher.find()) {
						...
						sb.append(MarkdownUtils.getUrlHtml(matcher.group()));
