		...
		if (initToken.length() > 0 && !initToken.startsWith("!")) {
			...
			GWT.runAsync(new RunAsyncCallback() {
				...
				public void onSuccess() {
					try {
						// previous session will be detected on place change (if there is one). do not block app
						// load to check.
						...
						AppActivityMapper activityMapper =
								new AppActivityMapper(ginjector, new SynapseJSNIUtilsImpl(), null);
						...
						globalApplicationState.init(new Callback() {
							...
							public void invoke() {
								...
								// start timer to check for user session state change (session expired, or user
