...
public class SessionDetector {
	...
	@Inject
	public SessionDetector(AuthenticationController authController,
			GlobalApplicationState globalAppState, GWTWrapper gwt, ClientCache clientCache) {
	...
	public void start() {
		gwt.scheduleFixedDelay(() -> {
			...
			if (!Objects.equals(authController.getCurrentUserPrincipalId(), currentSessionUserId)) {
				// session token state mismatch, update the app state by reloading (refresh leads to auth
				// controller detecting app reload)
	...
	public void initializeSessionTokenState() {
		...
		if (authController.isLoggedIn()) {
			...
			clientCache.put(SESSION_MARKER, authController.getCurrentUserPrincipalId(),
