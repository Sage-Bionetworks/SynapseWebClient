...
public interface SynapseClientAsync {
	...
	void addEmail(EmailValidationSignedToken emailValidationSignedToken,
	...
	void updateAcl(AccessControlList acl, boolean recursive,
	...
	void hasAccess(String ownerId, String ownerType, String accessType,
	...
	void createAccessApproval(AccessApproval aaEW, AsyncCallback<AccessApproval> callback);

	void updateExternalFile(String entityId, String externalUrl, String name, String contentType,
			Long fileSize, String md5, Long storageLocationId, AsyncCallback<Entity> callback)
			throws RestServiceException;

	void createExternalFile(String parentEntityId, String externalUrl, String name,
			String contentType, Long fileSize, String md5, Long storageLocationId,
			AsyncCallback<Entity> callback) throws RestServiceException;

	void getWikiAttachmentHandles(WikiPageKey key, AsyncCallback<FileHandleResults> callback);

	void restoreV2WikiPage(String ownerId, String ownerType, String wikiId, Long versionToUpdate,
			AsyncCallback<V2WikiPage> callback);

	void deleteV2WikiPage(WikiPageKey key, AsyncCallback<Void> callback);

	void getV2WikiAttachmentHandles(WikiPageKey key, AsyncCallback<FileHandleResults> callback);
	...
	void createV2WikiPageWithV1(String ownerId, String ownerType, WikiPage wikiPage,
			AsyncCallback<WikiPage> callback);

	void updateV2WikiPageWithV1(String ownerId, String ownerType, WikiPage wikiPage,
			AsyncCallback<WikiPage> callback);
	...
	void getTeamBundle(String userId, String teamId, boolean isLoggedIn,
			AsyncCallback<TeamBundle> callback);

	void getOpenInvitations(String userId,
			AsyncCallback<ArrayList<OpenUserInvitationBundle>> callback);

	void getOpenTeamInvitations(String teamId, Integer limit, Integer offset,
			AsyncCallback<ArrayList<OpenTeamInvitationBundle>> callback);

	void resendTeamInvitation(String membershipInvitationId, String hostPageBaseURL,
			AsyncCallback<Void> callback);
	...
	void requestMembership(String currentUserId, String teamId, String message,
			String hostPageBaseURL, Date expiresOn, AsyncCallback<TeamMembershipStatus> callback);

	void inviteMember(String userGroupId, String teamId, String message, String hostPageBaseURL,
			AsyncCallback<Void> callback);

	void inviteNewMember(String email, String teamId, String message, String hostPageBaseURL,
			AsyncCallback<Void> callback);
	/////////////////
	...
	void getFileEntityIdWithSameName(String fileName, String parentEntityId,
			AsyncCallback<String> callback);

	void setFileEntityFileHandle(String fileHandleId, String entityId, String parentEntityId,
			AsyncCallback<String> callback);

	void handleSignedToken(SignedTokenInterface signedToken, String hostPageBaseURL,
			AsyncCallback<ResponseMessage> callback);

	void hexDecodeAndDeserialize(String signedTokenString,
			AsyncCallback<SignedTokenInterface> callback);

	void hexDecodeAndDeserializeAccountCreationToken(String tokenString,
			AsyncCallback<AccountCreationToken> callback);

	void getAPIKey(AsyncCallback<String> callback);

	void sendMessage(Set<String> recipients, String subject, String message, String hostPageBaseURL,
			AsyncCallback<String> callback);

	void sendMessageToEntityOwner(String entityId, String subject, String messageBody,
			String hostPageBaseURL, AsyncCallback<String> callback);
	...
	/**
	 * Set a table's schema. Creates any necessary ColumnModels (for create and update), and figures
	 * out necessary ColumnChanges to transform oldSchema into newSchema.
	 * 
	 * @param tableId
	 * @param oldSchema
	 * @param newSchema
	 * @param callback
	 */
	void getTableUpdateTransactionRequest(String tableId, List<ColumnModel> oldSchema,
			List<ColumnModel> newSchema, AsyncCallback<TableUpdateTransactionRequest> callback);
	...
	void createTableColumns(List<ColumnModel> value, AsyncCallback<List<ColumnModel>> asyncCallback);

	void getHost(String urlString, AsyncCallback<String> callback);

	void updateFileEntity(FileEntity toUpdate, FileHandleCopyRequest copyRequest,
			AsyncCallback<Entity> callback);

	void moveEntity(String entityId, String newParentEntityId, AsyncCallback<Entity> callback);

	void createStorageLocationSetting(String parentEntityId, StorageLocationSetting setting,
			AsyncCallback<Void> callback);

	void getStorageLocationSetting(String parentEntityId,
			AsyncCallback<StorageLocationSetting> callback);
	...
	void setIsTeamAdmin(String currentUserId, String targetUserId, String teamId, boolean isTeamAdmin,
			AsyncCallback<Void> callback);
	...
	void generateSqlWithFacets(String basicSql, List<FacetColumnRequest> selectedFacets,
			List<ColumnModel> schema, AsyncCallback<String> callback);
	 ...
	 * @param callback
	 */
	void createExternalObjectStoreFileHandle(ExternalObjectStoreFileHandle fileHandle,
			AsyncCallback<String> callback);
