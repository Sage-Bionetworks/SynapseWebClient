...
public class SynapseJSNIUtilsImpl implements SynapseJSNIUtils {
	...
	private static native void _sendAnalyticsEvent(String eventCategoryValue, String eventActionValue,
	...
	@Override
	public String getFileHandleAssociationUrl(String objectId, FileHandleAssociateType objectType,
			String fileHandleId) {
	...
	private final static native LayoutResultJso _nChartlayout(NChartLayersArray layers,
	...
	public void setPageDescription(String newDescription) {
		if (Document.get() != null) {
			NodeList<com.google.gwt.dom.client.Element> tags =
					Document.get().getElementsByTagName("meta");
	...
	@Override
	public void uploadFileChunk(String contentType, JavaScriptObject blob, Long startByte,
			Long endByte, String url, XMLHttpRequest xhr, ProgressCallback callback) {
	...
	private final static native JavaScriptObject _getFileBlob(int index,
			JavaScriptObject fileList) /*-{
		return fileList[index];
	}-*/;

	private final static native void _directUploadBlob(String contentType,
			JavaScriptObject fileToUpload, Long startByte, Long endByte, String url,
			XMLHttpRequest xhr) /*-{
	...
	private final static native String _getWebkitRelativePath(JavaScriptObject fileList,
			int index) /*-{
	...
	private final static native void _getFileMd5(JavaScriptObject file,
	 ...
	 */
	@Override
	public void getFilePartMd5(JavaScriptObject blob, int currentChunk, Long chunkSize,
			MD5Callback md5Callback) {
		_getFilePartMd5(blob, currentChunk, chunkSize.doubleValue(), md5Callback);
	}

	private final static native void _getFilePartMd5(JavaScriptObject file, int currentChunk,
			double chunkSize, MD5Callback md5Callback) /*-{
																									var blobSlice = file.slice || file.mozSlice || file.webkitSlice;
																									spark = new $wnd.SparkMD5.ArrayBuffer();
																									$wnd.frOnload = function(e) {
																									spark.append(e.target.result); // append array buffer
	...
	private final static native boolean _elementSupportsAttribute(String tagName,
			String attribute) /*-{
	...
	@Override
	public void showJiraIssueCollector(String issueSummary, String issueDescription,
			String jiraIssueCollectorURL, String principalId, String userDisplayName,
			String userEmailAddress, String synapseDataObjectId, String componentID,
			String accessRequirementId, String issuePriority) {
		_showJiraIssueCollector(issueSummary, issueDescription, jiraIssueCollectorURL, principalId,
				userDisplayName, userEmailAddress, synapseDataObjectId, componentID, accessRequirementId,
				issuePriority);
	}
