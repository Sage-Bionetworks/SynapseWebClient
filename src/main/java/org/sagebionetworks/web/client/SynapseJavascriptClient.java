...
/**
 * Used to call Synapse backend services directly from the client. Reflection is not supported (you
 * can't call Class.newInstance() in this js code), so each method has a construction callback where
 * they create the return object.
 * 
 * @author Jay
 *
 */
public class SynapseJavascriptClient {
	...
	@Inject
	public SynapseJavascriptClient(JSONObjectAdapter jsonObjectAdapter,
			SynapseProperties synapseProperties, GWTWrapper gwt, SynapseJavascriptFactory jsFactory,
			PortalGinInjector ginInjector, SynapseJSNIUtils jsniUtils,
			EntityId2BundleCache entityIdBundleCache) {
	...
	private Request doDelete(String url, AsyncCallback callback) {
		...
		return sendRequest(requestBuilder, null, OBJECT_TYPE.None, INITIAL_RETRY_REQUEST_DELAY_MS,
				canCancel, callback);
	}

	private Request doGet(String url, OBJECT_TYPE responseType, AsyncCallback callback) {
		return doGet(url, responseType, APPLICATION_JSON_CHARSET_UTF8,
				authController.getCurrentUserSessionToken(), callback);
	}
	...
	private Request doGet(String url, OBJECT_TYPE responseType, String acceptedResponseType,
			String sessionToken, AsyncCallback callback) {
		RequestBuilderWrapper requestBuilder = ginInjector.getRequestBuilder();
		...
		return sendRequest(requestBuilder, null, responseType, INITIAL_RETRY_REQUEST_DELAY_MS,
				canCancel, callback);
	}

	private Request doPostOrPut(RequestBuilder.Method method, String url, JSONEntity requestObject,
			OBJECT_TYPE responseType, boolean canCancel, AsyncCallback callback) {
		...
		return sendRequest(requestBuilder, requestData, responseType, INITIAL_RETRY_REQUEST_DELAY_MS,
				canCancel, callback);
	}

	private Request doPost(String url, JSONEntity requestObject, OBJECT_TYPE responseType,
			boolean canCancel, AsyncCallback callback) {
		return doPostOrPut(POST, url, requestObject, responseType, canCancel, callback);
	}

	private Request doPut(String url, JSONEntity requestObject, OBJECT_TYPE responseType,
	...
	private Request sendRequest(final RequestBuilderWrapper requestBuilder, final String requestData,
			final OBJECT_TYPE responseType, final int retryDelay, boolean canCancel,
					...
					if (statusCode > 199 && statusCode < 300) {
						try {
							...
							if (OBJECT_TYPE.None.equals(responseType)) {
								...
								// SWC-4114: remove requestBody, attempted construction can result in a json object
								// adapter exception in PartialRow
						...
						// Status code could be 0 if the preflight request failed, or if the network connection
						// is down.
						if (statusCode == TOO_MANY_REQUESTS_STATUS_CODE || statusCode == 0) {
							...
							gwt.scheduleExecution(new Callback() {
								@Override
								public void invoke() {
									sendRequest(requestBuilder, requestData, responseType, retryDelay * 2, canCancel,
											callback);
								}
							}, retryDelay);
						} else {
							// getException() based on status code,
							// instead of using org.sagebionetworks.client.ClientUtils.throwException() and
							// ExceptionUtil.convertSynapseException() (neither of which can be referenced here)
	...
	public void getEntityBundle(String entityId, EntityBundleRequest request,
	...
	/**
	 * If bundle is found in local js cache, then this will immediately call onSuccess() with the
	 * cached version. Note that the current entity bundle will still be retrieved, and onSuccess()
	 * may be called again if there's a newer version (so write your onSuccess accordingly)!
	 * 
	 * @param entityId
	 * @param partsMask
	 * @param callback
	 */
	public void getEntityBundleFromCache(String entityId,
			final AsyncCallback<EntityBundle> callback) {
		...
		getEntityBundleForVersion(entityId, null, EntityPageTop.ALL_PARTS_REQUEST,
						...
						if (!latestEntityBundle.equals(cachedBundle)) {
							if (cachedBundle != null) {
								jsniUtils.consoleLog(
										"Cache out of date. Updating cache and calling onSuccess() again: " + entityId);
							}
	...
	public void getEntityBundleForVersion(String entityId, Long versionNumber,
			EntityBundleRequest request, final AsyncCallback<EntityBundle> callback) {
	...
	public FluentFuture<DoiAssociation> getDoiAssociation(String objectId, ObjectType objectType,
			Long objectVersion) {
		String url = getRepoServiceUrl() + DOI_ASSOCIATION + "?" + ID_PARAMETER + objectId + "&"
	...
	public FluentFuture<Doi> getDoi(String objectId, ObjectType objectType, Long objectVersion) {
		String url = getRepoServiceUrl() + DOI + "?" + ID_PARAMETER + objectId + "&" + TYPE_PARAMETER
				+ objectType;
		if (objectVersion != null) {
			url += "&" + VERSION_PARAMETER + objectVersion;
		}
	...
	public void getRestrictionInformation(String subjectId, RestrictableObjectType type,
			final AsyncCallback<RestrictionInformationResponse> callback) {
	...
	public Request getEntityChildren(EntityChildrenRequest request,
	...
	public void getVersionOfV2WikiPageAsV1(final WikiPageKey key, final Long versionNumber,
	...
	public void getV2WikiPage(final WikiPageKey key, final Long versionNumber,
	...
	private void getV2WikiPageWithPageId(WikiPageKey key, Long versionNumber,
			final AsyncCallback<V2WikiPage> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/"
	...
	public void getRootWikiPageKey(String ownerObjectType, String ownerObjectId,
			final AsyncCallback<String> wikiPageIdKeyCallback) {
		// get the root wiki page id first
		String url =
				getRepoServiceUrl() + "/" + ownerObjectType.toLowerCase() + "/" + ownerObjectId + WIKIKEY;

		AsyncCallback<org.sagebionetworks.repo.model.dao.WikiPageKey> wikiPageKeyCallback =
				new AsyncCallback<org.sagebionetworks.repo.model.dao.WikiPageKey>() {
					@Override
					public void onFailure(Throwable caught) {
	...
	private void getVersionOfV2WikiPageAsV1WithWikiPageId(WikiPageKey key, Long versionNumber,
			final AsyncCallback<WikiPage> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/"
	...
	public void getWikiAttachmentFileHandles(WikiPageKey key, Long versionNumber,
			AsyncCallback<List<FileHandle>> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/"
				+ key.getOwnerObjectId() + WIKI + key.getWikiPageId() + "/" + ATTACHMENT_HANDLES;
		if (versionNumber != null) {
			url += WIKI_VERSION_PARAMETER + versionNumber;
		}
	...
	public void getUserGroupHeadersByPrefix(String prefix, TypeFilter type, long limit, long offset,
			final AsyncCallback<UserGroupHeaderResponsePage> callback) {
	...
	public void getFavorites(final AsyncCallback<List<EntityHeader>> callback) {
		String url = getRepoServiceUrl() + FAVORITE_URI_PATH + "?" + OFFSET_PARAMETER + "0" + "&"
				+ LIMIT_PARAMETER + "200";
	...
	public void getChallenges(String userId, Integer limit, Integer offset,
			AsyncCallback<List<Challenge>> callback) {
		String url = getRepoServiceUrl() + CHALLENGE + "?participantId=" + userId;
		if (limit != null) {
	...
	public void getUserGroupHeadersByAlias(ArrayList<String> aliases,
	...
	public void getUserGroupHeadersById(List<String> ids,
	...
	public void getEntityForVersion(String entityId, Long versionNumber,
			AsyncCallback<Entity> callback) {
		getEntityByID(entityId, OBJECT_TYPE.Entity, versionNumber, callback);
	}

	private void getEntityByID(String entityId, OBJECT_TYPE type, Long versionNumber,
			AsyncCallback<Entity> callback) {
		String url = getRepoServiceUrl() + ENTITY + "/" + entityId;
		if (versionNumber != null) {
			url += REPO_SUFFIX_VERSION + "/" + versionNumber;
	...
	public void isFileOrFolder(String projectId, AsyncCallback<Boolean> callback) {
		EntityChildrenRequest request =
				getEntityChildrenRequest(projectId, EntityType.file, EntityType.folder);
		getEntityChildren(request, getEntityChildrenExistCallback(callback));
	}

	public void isTable(String projectId, AsyncCallback<Boolean> callback) {
		EntityChildrenRequest request =
				getEntityChildrenRequest(projectId, EntityType.table, EntityType.entityview);
		getEntityChildren(request, getEntityChildrenExistCallback(callback));
	}

	private AsyncCallback<EntityChildrenResponse> getEntityChildrenExistCallback(
	...
	public void getThreadCountForForum(String forumId, DiscussionFilter filter,
			AsyncCallback<Long> callback) {
		String url = getRepoServiceUrl() + FORUM + "/" + forumId + THREAD_COUNT + "?filter=" + filter;
		doGet(url, OBJECT_TYPE.ThreadCount, callback);
	}

	public void getReplyCountForThread(String threadId, DiscussionFilter filter,
			AsyncCallback<Long> callback) {
		String url = getRepoServiceUrl() + THREAD + "/" + threadId + REPLY_COUNT + "?filter=" + filter;
		doGet(url, OBJECT_TYPE.ThreadCount, callback);
	}

	public void getModerators(String forumId, Long limit, Long offset,
			AsyncCallback<PaginatedIds> callback) {
		String url = getRepoServiceUrl() + FORUM + "/" + forumId + MODERATORS + "?" + LIMIT_PARAMETER
				+ limit + "&" + OFFSET_PARAMETER + offset;
		doGet(url, OBJECT_TYPE.PaginatedIds, callback);
	}

	public void getSubscribers(Topic topic, String nextPageToken,
			AsyncCallback<SubscriberPagedResults> callback) {
	...
	public void getFileHandleAndUrlBatch(BatchFileRequest request,
			AsyncCallback<BatchFileResult> callback) {
		String url = getFileServiceUrl() + FILE_HANDLE_BATCH;
		doPost(url, request, OBJECT_TYPE.BatchFileResult, true, callback);
	}

	public void getEntityHeaderBatch(List<String> entityIds,
			AsyncCallback<ArrayList<EntityHeader>> callback) {
	...
	public void getEntityHeaderBatchFromReferences(List<Reference> list,
			AsyncCallback<ArrayList<EntityHeader>> callback) {
	...
	public FluentFuture<MembershipInvitation> getMembershipInvitation(
			MembershipInvtnSignedToken token) {
		String url =
				getRepoServiceUrl() + MEMBERSHIP_INVITATION + "/" + token.getMembershipInvitationId();
	...
	public FluentFuture<InviteeVerificationSignedToken> getInviteeVerificationSignedToken(
		...
		String url = getRepoServiceUrl() + MEMBERSHIP_INVITATION + "/" + membershipInvitationId
	...
	public FluentFuture<Void> updateInviteeId(InviteeVerificationSignedToken token) {
		String url = getRepoServiceUrl() + MEMBERSHIP_INVITATION + "/"
				+ token.getMembershipInvitationId() + INVITEE_ID;
	...
	public void updateV2WikiPage(WikiPageKey key, V2WikiPage toUpdate,
	...
	public void updateEntity(Entity toUpdate, String generatedByID, Boolean newVersion,
			AsyncCallback<Entity> callback) {
	...
	public void createSnapshot(String entityId, String comment, String label, String activityId,
			AsyncCallback<SnapshotResponse> callback) {
	...
	public void updateV2WikiOrderHint(WikiPageKey key, V2WikiOrderHint toUpdate,
			AsyncCallback<V2WikiOrderHint> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/"
	...
	public FluentFuture<Void> logError(Throwable ex) {
		...
		String outputExceptionString =
				exceptionString.substring(0, Math.min(exceptionString.length(), MAX_LOG_ENTRY_LABEL_SIZE));
		entry.setLabel(versionInfo + ":" + outputExceptionString);
		entry.setMessage(gwt.getCurrentURL() + " : \n" + ex.getMessage());
		return logError(entry);
	}

	public FluentFuture<PaginatedTeamIds> getUserTeams(String userId, boolean isAscendingOrder,
			String nextPageToken) {
		String urlBuilder = getRepoServiceUrl() + USER + "/" + userId + TEAM + "/id?" + ASCENDING_PARAM
				+ Boolean.toString(isAscendingOrder) + "&sort=TEAM_NAME";
		if (nextPageToken != null) {
	...
	public void getMyProjects(ProjectListType projectListType, int limit, String nextPageToken,
			ProjectListSortColumn sortBy, SortDirection sortDir,
			AsyncCallback<ProjectHeaderList> projectHeadersCallback) {
		getProjects(projectListType, null, null, limit, nextPageToken, sortBy, sortDir,
				projectHeadersCallback);
	}

	public void getProjectsForTeam(String teamId, int limit, String nextPageToken,
			ProjectListSortColumn sortBy, SortDirection sortDir,
			AsyncCallback<ProjectHeaderList> projectHeadersCallback) {
		getProjects(ProjectListType.TEAM, null, teamId, limit, nextPageToken, sortBy, sortDir,
				projectHeadersCallback);
	}

	public void getUserProjects(String userId, int limit, String nextPageToken,
			ProjectListSortColumn sortBy, SortDirection sortDir,
			AsyncCallback<ProjectHeaderList> projectHeadersCallback) {
		getProjects(ProjectListType.ALL, userId, null, limit, nextPageToken, sortBy, sortDir,
				projectHeadersCallback);
	}

	private void getProjects(ProjectListType projectListType, String userId, String teamId, int limit,
			String nextPageToken, ProjectListSortColumn sortBy, SortDirection sortDir,
			AsyncCallback<ProjectHeaderList> projectHeadersCallback) {
	...
	public void getAsynchJobResults(AsynchType type, String jobId, AsynchronousRequestBody request,
	...
	public void startAsynchJob(AsynchType type, AsynchronousRequestBody request,
			AsyncCallback<String> callback) {
		String url = type.getStartUrl(request);
		doPost(getEndpoint(type) + url, request, OBJECT_TYPE.AsyncJobId, false, callback);
	}

	public void getEntitiesGeneratedBy(String activityId, Integer limit, Integer offset,
			AsyncCallback<ArrayList<Reference>> callback) {
		String url = getRepoServiceUrl() + ACTIVITY_URI_PATH + "/" + activityId + GENERATED_PATH + "?"
				+ OFFSET_PARAMETER + offset + "&" + LIMIT_PARAMETER + limit;
		doGet(url, OBJECT_TYPE.PaginatedResultReference, callback);
	}

	public void createActivityAndLinkToEntity(Activity activity, Entity entity,
			AsyncCallback<Entity> callback) {
	...
	public void getActivityForEntityVersion(String entityId, Long versionNumber,
			AsyncCallback<Activity> callback) {
		String url = getRepoServiceUrl() + ENTITY + "/" + entityId;
		if (versionNumber != null) {
			url += REPO_SUFFIX_VERSION + "/" + versionNumber;
		}
	...
	public void getDockerTaggedCommits(String entityId, Long limit, Long offset,
	...
	public void startMultipartUpload(MultipartUploadRequest request, Boolean forceRestart,
	...
	public void getMultipartPresignedUrlBatch(BatchPresignedUploadUrlRequest request,
			AsyncCallback<BatchPresignedUploadUrlResponse> callback) {
		String url =
				getFileServiceUrl() + "/file/multipart/" + request.getUploadId() + "/presigned/url/batch";
		doPost(url, request, OBJECT_TYPE.BatchPresignedUploadUrlResponse, false, callback);
	}

	public void addPartToMultipartUpload(String uploadId, int partNumber, String partMD5Hex,
			AsyncCallback<AddPartResponse> callback) {
		String url = getFileServiceUrl() + "/file/multipart/" + uploadId + "/add/" + partNumber
	...
	public void getOpenMembershipRequestCount(String teamId, AsyncCallback<Long> callback) {
		String url = getRepoServiceUrl() + TEAM + "/" + teamId + OPEN_MEMBERSHIP_REQUEST + "?"
				+ OFFSET_PARAMETER + "0&" + LIMIT_PARAMETER + "1";
		doGet(url, OBJECT_TYPE.PaginatedResultsTotalNumberOfResults, callback);
	}

	public void getPrincipalAlias(PrincipalAliasRequest request,
			AsyncCallback<PrincipalAliasResponse> callback) {
		String url = getRepoServiceUrl() + PRINCIPAL + "/alias/";
		doPost(url, request, OBJECT_TYPE.PrincipalAliasResponse, true, callback);
	}

	public void unbindOAuthProvidersUserId(OAuthProvider provider, String alias,
			AsyncCallback<Void> callback) {
		String url = getAuthServiceUrl() + AUTH_OAUTH_2_ALIAS + "?provider="
				+ gwt.encodeQueryString(provider.name()) + "&" + "alias=" + gwt.encodeQueryString(alias);
		doDelete(url, callback);
	}

	public void lookupChild(String entityName, String containerEntityId,
			AsyncCallback<String> callback) {
	...
	public void getUploadDestinations(String parentEntityId,
			AsyncCallback<List<UploadDestination>> callback) {
	...
	public void getAllSubscriptions(SubscriptionObjectType objectType, Long limit, Long offset,
			SortByType sortByType,
			org.sagebionetworks.repo.model.subscription.SortDirection sortDirection,
			AsyncCallback<SubscriptionPagedResults> callback) {
		String url = getRepoServiceUrl() + SUBSCRIPTION + "/all?" + OBJECT_TYPE_PARAMETER
				+ objectType.name() + "&" + LIMIT_PARAMETER + limit + "&" + OFFSET_PARAMETER + offset;
		if (sortByType != null) {
	...
	public void addFileToDownloadList(String fileHandleId, String fileEntityId,
	...
	public void addFilesToDownloadList(List<FileHandleAssociation> toAdd,
			AsyncCallback<DownloadList> callback) {
	...
	public void removeFileFromDownloadList(FileHandleAssociation fha,
			AsyncCallback<DownloadList> callback) {
	...
	public void removeFilesFromDownloadList(List<FileHandleAssociation> toRemove,
			AsyncCallback<DownloadList> callback) {
		FileHandleAssociationList request = new FileHandleAssociationList();
	...
	public void createDownloadOrderFromUsersDownloadList(String zipFileName,
			AsyncCallback<DownloadOrder> callback) {
	...
	public void getDownloadOrderHistory(DownloadOrderSummaryRequest request,
			AsyncCallback<DownloadOrderSummaryResponse> callback) {
	...
	public void listSubscription(SubscriptionRequest request,
			AsyncCallback<SubscriptionPagedResults> callback) {
	...
	public void getColumnModelsForTableEntity(String tableEntityId,
			AsyncCallback<List<ColumnModel>> callback) {
		String url = getRepoServiceUrl() + ENTITY + "/" + tableEntityId + COLUMN;
		doGet(url, OBJECT_TYPE.PaginatedColumnModelsResults, callback);
	}

	public Request getEntityVersions(String entityId, int offset, int limit,
			AsyncCallback<List<VersionInfo>> callback) {
	...
	private String getThreadsURL(String associatedObjectType, String objectId, Long limit,
			Long offset, DiscussionThreadOrder order, Boolean ascending, DiscussionFilter filter) {
	...
	public void getRepliesForThread(String threadId, Long limit, Long offset,
			DiscussionReplyOrder order, Boolean ascending, DiscussionFilter filter,
			AsyncCallback<List<DiscussionReplyBundle>> callback) {

		String url = getRepoServiceUrl() + THREAD + "/" + threadId + REPLIES + "?" + LIMIT_PARAMETER
				+ limit + "&" + OFFSET_PARAMETER + offset;
		if (order != null) {
			url += "&sort=" + order.name();
		}
	...
	private void getV2WikiHeaderTree(String ownerId, String ownerType,
			AsyncCallback<List<V2WikiHeader>> finalCallback, int offset,
			List<V2WikiHeader> fullWikiHeaderList) {
		// continue asking for v2 wiki headers until result is empty
		String url = getRepoServiceUrl() + "/" + ownerType.toLowerCase() + "/" + ownerId
				+ WIKI_HEADER_TREE + "?" + LIMIT_PARAMETER + LIMIT_50 + "&" + OFFSET_PARAMETER + offset;
		doGet(url, OBJECT_TYPE.PaginatedResultsV2WikiHeader, new AsyncCallback<List<V2WikiHeader>>() {
			...
			public void onSuccess(List<V2WikiHeader> results) {
				...
				if (results.isEmpty() || results.size() < LIMIT_50) {
					...
					getV2WikiHeaderTree(ownerId, ownerType, finalCallback, offset + LIMIT_50,
							fullWikiHeaderList);
				}
			}
		});
	...
	public void sendPasswordResetEmail(String emailAddress, AsyncCallback<Void> cb) {
		...
		url += "?passwordResetEndpoint="
				+ gwt.encodeQueryString(gwt.getHostPageBaseURL() + "#!PasswordResetSignedToken:");
		doPost(url, username, OBJECT_TYPE.None, false, cb);
	}

	public void changePassword(ChangePasswordInterface changePasswordRequest,
	...
	public void addTeamMember(String userId, String teamId, AsyncCallback<Void> cb) {
		...
		String url =
				getRepoServiceUrl() + TEAM + "/" + teamId + MEMBER + "/" + userId + "?teamEndpoint="
						+ teamEndpoint + "&notificationUnsubscribeEndpoint=" + signedTokenEndpoint;
	...
	public void getTeamMembers(String teamId, String fragment, TeamMemberTypeFilterOptions memberType,
			Integer limit, Integer offset, AsyncCallback<TeamMemberPagedResults> callback) {
		// first gather the team members
		String url = getRepoServiceUrl() + TEAM_MEMBERS + teamId + "?" + OFFSET_PARAMETER + offset + "&"
	...
	public void getTeamMembersStep2(List<TeamMember> teamMembers, Integer limit, Integer offset,
			AsyncCallback<TeamMemberPagedResults> callback) {
		...
		AsyncCallback<List> profilesCallback = new AsyncCallback<List>() {
			...
			public void onSuccess(List profiles) {
				...
				for (int i = 0; i < userIds.size(); i++) {
					teamMemberBundles.add(new TeamMemberBundle((UserProfile) profiles.get(i),
							teamMembers.get(i).getIsAdmin(), teamMembers.get(i).getTeamId()));
				}
	...
	public void updateAnnotations(String entityId, Annotations annotations,
			AsyncCallback<Annotations> cb) {
	...
	public Request getFileEntityTemporaryUrlForVersion(String entityId, Long versionNumber,
			boolean preview, AsyncCallback<String> cb) {
		String filePath = preview ? FILE_PREVIEW : FILE;
		String url = getRepoServiceUrl() + ENTITY + "/" + entityId + VERSION_INFO + "/" + versionNumber
				+ filePath + "?" + REDIRECT_PARAMETER + "false";
		return doGet(url, OBJECT_TYPE.String, cb);
	}

	public void additionalEmailValidation(String userId, String emailAddress, String callbackUrl,
			AsyncCallback<Void> cb) {
		String url = getRepoServiceUrl() + ACCOUNT + "/" + userId + "/" + EMAIL_VALIDATION + "?"
				+ PORTAL_ENDPOINT_PARAM + callbackUrl;
		Username username = new Username();
		username.setEmail(emailAddress);
		doPost(url, username, OBJECT_TYPE.None, false, cb);
	}
