...
public class SynapseJavascriptClient {
	...
	public static final String DOWNLOAD_LIST_PACKAGE_V2 = "/download/list/package/";
	public static final String DOWNLOAD_LIST_MANIFEST_V2 = "/download/list/package/";
	...
	public FluentFuture<DoiAssociation> getDoiAssociation(String objectId, ObjectType objectType, Long objectVersion) {
		String url = getRepoServiceUrl() + DOI_ASSOCIATION + "?" + ID_PARAMETER + objectId + "&" + TYPE_PARAMETER + objectType;
	...
	public void getRootWikiPageKey(String ownerObjectType, String ownerObjectId, final AsyncCallback<String> wikiPageIdKeyCallback) {
		...
		String url = getRepoServiceUrl() + "/" + ownerObjectType.toLowerCase() + "/" + ownerObjectId + WIKIKEY;
	...
	private void getVersionOfV2WikiPageAsV1WithWikiPageId(WikiPageKey key, Long versionNumber, final AsyncCallback<WikiPage> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/" + key.getOwnerObjectId() + WIKI + key.getWikiPageId();
		if (versionNumber != null) {
			url += WIKI_VERSION_PARAMETER + versionNumber;
		}
	...
	public void getWikiAttachmentFileHandles(WikiPageKey key, Long versionNumber, AsyncCallback<List<FileHandle>> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/" + key.getOwnerObjectId() + WIKI + key.getWikiPageId() + "/" + ATTACHMENT_HANDLES;
		if (versionNumber != null) {
			url += WIKI_VERSION_PARAMETER + versionNumber;
		}
	...
	public void getFavorites(final AsyncCallback<List<EntityHeader>> callback) {
		String url = getRepoServiceUrl() + FAVORITE_URI_PATH + "?" + OFFSET_PARAMETER + "0" + "&" + LIMIT_PARAMETER + "200";
	...
	public void getUserGroupHeadersByAlias(ArrayList<String> aliases, AsyncCallback<List<UserGroupHeader>> callback) {
	...
	public void getUserGroupHeadersById(List<String> ids, AsyncCallback<UserGroupHeaderResponsePage> callback) {
	...
	public void updateV2WikiOrderHint(WikiPageKey key, V2WikiOrderHint toUpdate, AsyncCallback<V2WikiOrderHint> callback) {
		String url = getRepoServiceUrl() + "/" + key.getOwnerObjectType().toLowerCase() + "/" + key.getOwnerObjectId() + WIKI_ORDER_HINT;
	...
	public FluentFuture<PaginatedTeamIds> getUserTeams(String userId, boolean isAscendingOrder, String nextPageToken) {
		String urlBuilder = getRepoServiceUrl() + USER + "/" + userId + TEAM + "/id?" + ASCENDING_PARAM + Boolean.toString(isAscendingOrder) + "&sort=TEAM_NAME";
		if (nextPageToken != null) {
	...
	private void getProjects(ProjectListType projectListType, String userId, String teamId, int limit, String nextPageToken, ProjectListSortColumn sortBy, SortDirection sortDir, AsyncCallback<ProjectHeaderList> projectHeadersCallback) {
		...
		url += '?' + LIMIT_PARAMETER + limit + "&sort=" + sortBy.name() + "&sortDirection=" + sortDir.name();
	...
	public void getDockerTaggedCommits(String entityId, Long limit, Long offset, DockerCommitSortBy sortBy, Boolean ascending, AsyncCallback<ArrayList<DockerCommit>> callback) {
	...
	public void unbindOAuthProvidersUserId(OAuthProvider provider, String alias, AsyncCallback<Void> callback) {
		String url = getAuthServiceUrl() + AUTH_OAUTH_2_ALIAS + "?provider=" + gwt.encodeQueryString(provider.name()) + "&" + "alias=" + gwt.encodeQueryString(alias);
		doDelete(url, callback);
	}
	...
	public void getThreadsForForum(String forumId, Long limit, Long offset, DiscussionThreadOrder order, Boolean ascending, DiscussionFilter filter, AsyncCallback<List<DiscussionThreadBundle>> callback) {
		String url = getThreadsURL(FORUM, forumId, limit, offset, order, ascending, filter);
		doGet(url, OBJECT_TYPE.PaginatedResultsDiscussionThreadBundle, callback);
	}

	private String getThreadsURL(String associatedObjectType, String objectId, Long limit, Long offset, DiscussionThreadOrder order, Boolean ascending, DiscussionFilter filter) {
		String url = getRepoServiceUrl() + associatedObjectType + "/" + objectId + THREADS + "?" + LIMIT_PARAMETER + limit + "&" + OFFSET_PARAMETER + offset;
		if (order != null) {
			url += "&sort=" + order.name();
		}
		if (ascending != null) {
	...
	public void getRepliesForThread(String threadId, Long limit, Long offset, DiscussionReplyOrder order, Boolean ascending, DiscussionFilter filter, AsyncCallback<List<DiscussionReplyBundle>> callback) {

		String url = getRepoServiceUrl() + THREAD + "/" + threadId + REPLIES + "?" + LIMIT_PARAMETER + limit + "&" + OFFSET_PARAMETER + offset;
		if (order != null) {
			url += "&sort=" + order.name();
		}
		if (ascending != null) {
	...
	public void getV2WikiHeaderTree(String ownerId, String ownerType, AsyncCallback<List<V2WikiHeader>> callback) {
		getV2WikiHeaderTree(ownerId, ownerType, callback, 0, new ArrayList<V2WikiHeader>());
	}

	private void getV2WikiHeaderTree(String ownerId, String ownerType, AsyncCallback<List<V2WikiHeader>> finalCallback, int offset, List<V2WikiHeader> fullWikiHeaderList) {
		// continue asking for v2 wiki headers until result is empty
		String url = getRepoServiceUrl() + "/" + ownerType.toLowerCase() + "/" + ownerId + WIKI_HEADER_TREE + "?" + LIMIT_PARAMETER + LIMIT_50 + "&" + OFFSET_PARAMETER + offset;
	...
	public Request getFileEntityTemporaryUrlForVersion(String entityId, Long versionNumber, boolean preview, AsyncCallback<String> cb) {
		...
		String url = getRepoServiceUrl() + ENTITY + "/" + entityId + VERSION_INFO + "/" + versionNumber + filePath + "?" + REDIRECT_PARAMETER + "false";
	...
	public void additionalEmailValidation(String userId, String emailAddress, String callbackUrl, AsyncCallback<Void> cb) {
		String url = getRepoServiceUrl() + ACCOUNT + "/" + userId + "/" + EMAIL_VALIDATION + "?" + PORTAL_ENDPOINT_PARAM + gwt.encodeQueryString(callbackUrl);
		Username username = new Username();
		username.setEmail(emailAddress);
		doPost(url, username, OBJECT_TYPE.None, false, cb);
	}
