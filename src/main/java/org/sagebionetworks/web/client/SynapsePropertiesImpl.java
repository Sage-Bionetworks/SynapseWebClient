...
public class SynapsePropertiesImpl implements SynapseProperties {
	...
	@Inject
	public SynapsePropertiesImpl(StackConfigServiceAsync stackConfigService,
	...
	public PublicPrincipalIds getPublicPrincipalIds() {
		if (publicPrincipalIds == null) {
			...
			publicPrincipalIds.setAnonymousUserId(
					Long.parseLong(getSynapseProperty(WebConstants.ANONYMOUS_USER_PRINCIPAL_ID)));
			publicPrincipalIds.setAuthenticatedAclPrincipalId(
					Long.parseLong(getSynapseProperty(WebConstants.AUTHENTICATED_ACL_PRINCIPAL_ID)));
		}
