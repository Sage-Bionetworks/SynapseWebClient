...
public class ProvenanceGraphProps extends ReactComponentProps {
	...
	@JsOverlay
	public static ProvenanceGraphProps create(List<Reference> refs, String containerHeight, JavaScriptObject initialNodes, JavaScriptObject initialEdges, OnUpdateJavaScriptObject nodesListener, OnUpdateJavaScriptObject edgesListener) {
		ProvenanceGraphProps props = new ProvenanceGraphProps();
		props.entityRefs = new ReferenceJsObject[refs.size()];
		for (int i = 0; i < refs.size(); i++) {
			ReferenceJsObject newRef = ReferenceJsObject.create(refs.get(i).getTargetId(),
