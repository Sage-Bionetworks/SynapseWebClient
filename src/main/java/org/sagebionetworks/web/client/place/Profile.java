...
public class Profile extends Place {
      ...
      try {
        ...
        if (Synapse.ProfileArea.PROJECTS.equals(area)) {
          ...
          if (tokens.length > 2) {
            ...
            if (
              ProjectFilterEnum.TEAM.equals(projectFilter) && tokens.length > 3
            ) {
              teamId = tokens[3];
            }
          }
  ...
  public Profile(String userId, ProfileArea area) {
    String areaToken = area == null
      ? ""
      : DELIMITER + area.toString().toLowerCase();
  ...
  public void setArea(
    ProfileArea area,
    ProjectFilterEnum projectFilter,
    String teamId
  ) {
    this.area = area;
