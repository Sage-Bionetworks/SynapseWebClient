...
public class BulkPresenterProxy extends AbstractActivity {
  ...
  private static Logger log = Logger.getLogger(
    BulkPresenterProxy.class.getName()
  );
  Place place;
  ...
  public void start(final AcceptsOneWidget panel, final EventBus eventBus) {
    ...
    // NOTE: DO NOT USE SHARED METHOD FOR runAsync(). GWT split points defined by GWT.runAsync() line
    // (attempt to pull out into shared method results in a single fragment!
    if (place instanceof Home) {
    ...
    } else if (place instanceof SearchV2Place) {
      GWT.runAsync(
        SearchV2Place.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
            SearchV2Presenter presenter = ginjector.getSearchV2Presenter();
            presenter.setPlace((SearchV2Place) place);
            presenter.start(panel, eventBus);
          }
    ...
    } else if (place instanceof DownloadCartPlace) {
      GWT.runAsync(
        DownloadCartPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
      ...
      GWT.runAsync(
        ...
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
            GridPagePresenter presenter = ginjector.getGridPagePresenter();
            presenter.setPlace((GridPlace) place);
            presenter.start(panel, eventBus);
          }
      ...
      GWT.runAsync(
        ACTDataAccessSubmissionsPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
      ...
      GWT.runAsync(
        PasswordResetSignedTokenPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
      ...
      GWT.runAsync(
        ACTDataAccessSubmissionDashboardPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
      ...
      GWT.runAsync(
        DataAccessApprovalTokenPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
    ...
    } else if (place instanceof PersonalAccessTokenPlace) {
      GWT.runAsync(
        PersonalAccessTokenPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
    ...
    } else if (place instanceof DataAccessManagementPlace) {
      GWT.runAsync(
        DataAccessManagementPlace.class,
        new RunAsyncCallback() {
          @Override
          public void onSuccess() {
  ...
  public String mayStop() {
    ...
    ) return DisplayConstants.NAVIGATE_AWAY_CONFIRMATION_MESSAGE; else return null;
  }
}
