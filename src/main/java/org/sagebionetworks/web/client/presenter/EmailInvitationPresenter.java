...
public class EmailInvitationPresenter
  ...
  public void setPlace(EmailInvitation place) {
    ...
    synapseClient.hexDecodeAndDeserialize(
            ...
            new FutureCallback<MembershipInvitation>() {
              ...
              public void onSuccess(MembershipInvitation mis) {
                ...
                if (authController.isLoggedIn()) {
                  if (
                    mis.getInviteeId() != null &&
                    authController
                      .getCurrentUserPrincipalId()
                      .equals(mis.getInviteeId())
                  ) {
  ...
  private void verifyEmailAssociatedToAuthenticatedUser() {
    ...
    jsClient
      .getInviteeVerificationSignedToken(membershipInvitation.getId())
      .addCallback(
  ...
  private void bindInvitationToAuthenticatedUser(
    InviteeVerificationSignedToken token
  ) {
    jsClient
      .updateInviteeId(token)
      .addCallback(
  ...
  private void addTeamMember() {
    jsClient.addTeamMember(
      authController.getCurrentUserPrincipalId(),
      membershipInvitation.getTeamId(),
      new AsyncCallback<Void>() {
        @Override
        public void onSuccess(Void result) {
          view.showInfo("Successfully joined the team.");
          placeChanger.goTo(
            new org.sagebionetworks.web.client.place.Team(
              membershipInvitation.getTeamId()
            )
          );
        }
  ...
  private void gotoProfilePageTeamArea() {
    ...
    placeChanger.goTo(
      new Profile(authController.getCurrentUserPrincipalId(), ProfileArea.TEAMS)
    );
  }
  ...
  private void initializeView(final MembershipInvitation mis) {
    ...
    FluentFuture
      .from(
        whenAllComplete(teamFuture, userProfileFuture)
          .call(
            () -> {
