...
public class EmailInvitationPresenter extends AbstractActivity
		implements EmailInvitationView.Presenter, Presenter<EmailInvitation> {
	...
	@Inject
	public EmailInvitationPresenter(EmailInvitationView view, SynapseJavascriptClient jsClient,
			SynapseFutureClient futureClient, SynapseAlert synapseAlert,
	...
	public void setPlace(EmailInvitation place) {
		...
		futureClient.hexDecodeAndDeserialize(encodedMISignedToken)
				.transformAsync(
						...
						directExecutor())
				.addCallback(new FutureCallback<MembershipInvitation>() {
					@Override
					public void onSuccess(MembershipInvitation mis) {
						...
						if (authController.isLoggedIn()) {
							if (mis.getInviteeId() != null
									&& authController.getCurrentUserPrincipalId().equals(mis.getInviteeId())) {
	...
	private void verifyEmailAssociatedToAuthenticatedUser() {
		...
		jsClient.getInviteeVerificationSignedToken(membershipInvitation.getId())
				.addCallback(new FutureCallback<InviteeVerificationSignedToken>() {
					@Override
					...
					public void onFailure(Throwable t) {
						...
						if (t instanceof ForbiddenException) {
							// SWC-4721: fix message in the case where membership invitation email is not
							...
							// SWC-4741: invitation not associated to the current user, send user to the Settings
							// page to add the new email address
							placeChanger.goTo(
									new Profile(authController.getCurrentUserPrincipalId(), ProfileArea.SETTINGS));
						} else {
	...
	private void addTeamMember() {
		jsClient.addTeamMember(authController.getCurrentUserPrincipalId(),
				membershipInvitation.getTeamId(), new AsyncCallback<Void>() {
					@Override
					public void onSuccess(Void result) {
						view.showInfo("Successfully joined the team.");
						placeChanger.goTo(
								new org.sagebionetworks.web.client.place.Team(membershipInvitation.getTeamId()));
					}
					...
					public void onFailure(Throwable caught) {
						...
						synapseAlert.consoleError(
								"Unable to add user directly to team, sending to team area of profile: "
										+ caught.getMessage());
						gotoProfilePageTeamArea();
					}
				});
	...
	private void gotoProfilePageTeamArea() {
		// SWC-4668: take the user to their profile page with the join request for them to accept (using
		// the Join button, which handles any ARs)
