...
public class LoginPresenter extends AbstractActivity
	...
	public LoginPresenter(LoginView view, AuthenticationController authenticationController,
			GlobalApplicationState globalApplicationState, SynapseAlert synAlert) {
		this.view = view;
	...
	public void showView(final LoginPlace place) {
		...
		} else if (LoginPlace.SHOW_TOU.equals(token)
				&& authenticationController.getCurrentUserSessionToken() != null) {
			showTermsOfUse(false);
		} else if (LoginPlace.SHOW_SIGNED_TOU.equals(token)
				&& authenticationController.getCurrentUserSessionToken() != null) {
			showTermsOfUse(true);
		} else if (!ClientProperties.DEFAULT_PLACE_TOKEN.equals(token)
				&& !LoginPlace.CHANGE_USERNAME.equals(token) && !"".equals(token) && token != null) {
