...
public class NewAccountPresenter extends AbstractActivity
		implements NewAccountView.Presenter, Presenter<NewAccount> {
	...
	@Inject
	public NewAccountPresenter(NewAccountView view, SynapseClientAsync synapseClient,
			GlobalApplicationState globalAppState, UserAccountServiceAsync userAccountService,
	...
	public void setPlace(NewAccount place) {
		...
		synapseClient.hexDecodeAndDeserializeAccountCreationToken(place.toToken(),
				new AsyncCallback<AccountCreationToken>() {
					@Override
					public void onSuccess(AccountCreationToken result) {
						...
						if (!accountCreationTokenIsValid()) {
							...
							view.showErrorMessage(
									DisplayConstants.ACCOUNT_CREATION_FAILURE + " token is not valid");
							return;
						}
	...
	public void completeRegistration(String userName, String fName, String lName, String password) {
		...
		userAccountService.createUserStep2(userName.trim(), fName.trim(), lName.trim(), password,
				emailValidationSignedToken, new AsyncCallback<String>() {
					@Override
					public void onSuccess(String sessionToken) {
						...
						if (accountCreationToken.getEncodedMembershipInvtnSignedToken() != null) {
							globalAppState.setLastPlace(
									new EmailInvitation(accountCreationToken.getEncodedMembershipInvtnSignedToken()));
						}
	...
	public void checkEmailAvailable(String email) {
		synapseClient.isAliasAvailable(email, AliasType.USER_EMAIL.toString(),
				new AsyncCallback<Boolean>() {
					...
					public void onFailure(Throwable e) {
						// do nothing. validation has failed, but updating the email will fail if it's already
						// taken.
						e.printStackTrace();
					}
				});
	...
	public void checkUsernameAvailable(String username) {
		if (username.trim().length() > 3) {
			synapseClient.isAliasAvailable(username, AliasType.USER_NAME.toString(),
					new AsyncCallback<Boolean>() {
						@Override
						public void onSuccess(Boolean isAvailable) {
							if (!isAvailable)
						...
						@Override
						public void onFailure(Throwable e) {
							// do nothing. validation has failed, but updating the username will fail if it's
							// already taken.
							e.printStackTrace();
						}
					});
