...
public class NewAccountPresenter
  ...
  public void setPlace(NewAccount place) {
    ...
    synAlert.clear();
    synapseClient.hexDecodeAndDeserializeAccountCreationToken(
      place.toToken(),
      new AsyncCallback<AccountCreationToken>() {
        ...
        public void onSuccess(AccountCreationToken result) {
          ...
          if (!accountCreationTokenIsValid()) {
            ...
            synAlert.showError(
              DisplayConstants.ACCOUNT_CREATION_FAILURE + " Token is invalid."
            );
            return;
          }
          String email = accountCreationToken
            .getEmailValidationSignedToken()
            .getEmail();
          view.setEmail(email);
        ...
        public void onFailure(Throwable caught) {
          view.setLoading(false);
          synAlert.showError(
            DisplayConstants.ACCOUNT_CREATION_FAILURE +
            " Token is invalid: " +
            caught.getMessage()
          );
        }
    ...
    userAccountService.createUserStep2(
      ...
      new AsyncCallback<String>() {
        ...
        public void onSuccess(String accessToken) {
          ...
          if (
            accountCreationToken.getEncodedMembershipInvtnSignedToken() != null
          ) {
            globalAppState.setLastPlace(
              new EmailInvitation(
                accountCreationToken.getEncodedMembershipInvtnSignedToken()
              )
            );
          }
          // After creating the new user, update the access token
          authController.setNewAccessToken(
            accessToken,
            getSetNewAccessTokenCallback()
          );
        }
  ...
  public AsyncCallback<UserProfile> getSetNewAccessTokenCallback() {
    return new AsyncCallback<UserProfile>() {
      ...
      public void onSuccess(UserProfile result) {
        globalAppState
          .getPlaceChanger()
          .goTo(new Profile(Profile.VIEW_PROFILE_TOKEN));
      }
  ...
  public void checkUsernameAvailable(String username) {
    if (username.trim().length() > 3) {
      synapseClient.isAliasAvailable(
        username,
        AliasType.USER_NAME.toString(),
        new AsyncCallback<Boolean>() {
          @Override
          public void onSuccess(Boolean isAvailable) {
            if (!isAvailable) view.markUsernameUnavailable();
          }
