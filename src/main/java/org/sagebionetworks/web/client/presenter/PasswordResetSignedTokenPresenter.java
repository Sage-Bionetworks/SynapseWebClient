...
public class PasswordResetSignedTokenPresenter extends AbstractActivity
		implements PasswordResetSignedTokenView.Presenter, Presenter<PasswordResetSignedTokenPlace> {
	public static final String INVALID_PASSWORD_RESET_SIGNED_TOKEN =
			"Invalid PasswordResetSignedToken";
	...
	public PasswordResetSignedTokenPresenter(PasswordResetSignedTokenView view,
			SynapseClientAsync synapseClient, SynapseJavascriptClient jsClient, SynapseAlert synAlert,
			AuthenticationController authController, GlobalApplicationState globalAppState) {
		this.view = view;
		...
		if (!checkPasswordDefined(password1) || !checkPasswordDefined(password2)) {
			...
			jsClient.changePassword(changePasswordRequest, new AsyncCallback<Void>() {
				...
				public void onSuccess(Void result) {
					...
					view.setChangePasswordEnabled(true);
					globalAppState.getPlaceChanger()
							.goTo(new LoginPlace(ClientProperties.DEFAULT_PLACE_TOKEN));
				}
			});
		}
	}
