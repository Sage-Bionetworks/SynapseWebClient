...
public class ProfilePresenter extends AbstractActivity
	...
	public ProfilePresenter(ProfileView view, AuthenticationController authenticationController,
			GlobalApplicationState globalApplicationState, GWTWrapper gwt, TeamListWidget myTeamsWidget,
			OpenTeamInvitationsWidget openInvitesWidget, PortalGinInjector ginInjector,
			SynapseJavascriptClient jsClient, IsACTMemberAsyncHandler isACTMemberAsyncHandler) {
		this.view = view;
	...
	private void getUserProfile() {
		...
		isACTMemberAsyncHandler.isACTActionAvailable(isACT -> {
			if (isACT) {
				ProfileCertifiedValidatedWidget certifiedValidatedWidget =
						ginInjector.getProfileCertifiedValidatedWidget();
		...
		if (isOwner) {
			...
			switch (filterType) {
				...
				case CREATED_BY_ME:
					...
					getMyProjects(ProjectListType.CREATED, ProjectFilterEnum.CREATED_BY_ME,
							projectNextPageToken);
					break;
				case SHARED_DIRECTLY_WITH_ME:
					view.setSharedDirectlyWithMeFilterSelected();
					getMyProjects(ProjectListType.PARTICIPATED, ProjectFilterEnum.SHARED_DIRECTLY_WITH_ME,
							projectNextPageToken);
					break;
				case ALL_MY_TEAM_PROJECTS:
					view.setTeamsFilterSelected();
					getMyProjects(ProjectListType.TEAM, ProjectFilterEnum.ALL_MY_TEAM_PROJECTS,
							projectNextPageToken);
					break;
				case FAVORITES:
	...
	public void sort(ProjectListSortColumn column) {
		...
		currentSortDirection =
				SortDirection.ASC.equals(currentSortDirection) ? SortDirection.DESC : SortDirection.ASC;
	...
	public void getMoreTeams() {
		jsClient.getUserTeams(currentUserId, true, teamNextPageToken)
				.addCallback(new FutureCallback<PaginatedTeamIds>() {
					@Override
	...
	public void getMoreChallenges() {
		...
		jsClient.getChallenges(currentUserId, CHALLENGE_PAGE_SIZE, currentChallengeOffset,
	...
	public void getMyProjects(ProjectListType projectListType, final ProjectFilterEnum filter,
			String nextPageToken) {
		projectSynAlert.clear();
		jsClient.getMyProjects(projectListType, PROJECT_PAGE_SIZE, nextPageToken, currentSortColumn,
				currentSortDirection, new AsyncCallback<ProjectHeaderList>() {
					@Override
	...
	public void getTeamProjects(String nextPageToken) {
		projectSynAlert.clear();
		jsClient.getProjectsForTeam(filterTeamId, PROJECT_PAGE_SIZE, nextPageToken, currentSortColumn,
				currentSortDirection, new AsyncCallback<ProjectHeaderList>() {
					@Override
					public void onSuccess(ProjectHeaderList results) {
	...
	public void getUserProjects(String nextPageToken) {
		projectSynAlert.clear();
		jsClient.getUserProjects(currentUserId, PROJECT_PAGE_SIZE, nextPageToken, currentSortColumn,
				currentSortDirection, new AsyncCallback<ProjectHeaderList>() {
					@Override
					public void onSuccess(ProjectHeaderList results) {
						addProjectResults(results.getResults());
	...
	public void getFavorites() {
		projectSynAlert.clear();
		EntityBrowserUtils.loadFavorites(jsClient, globalApplicationState,
	...
	public void createProject() {
		...
		getPromptDialog().configureAndShow("Create a New Project", "Project Name", null,
				projectName -> {
	...
	private void showView(Profile place) {
		...
		if (token.equals("oauth_bound")) {
			...
			globalApplicationState.getPlaceChanger().goTo(
					new Profile(authenticationController.getCurrentUserPrincipalId(), ProfileArea.SETTINGS));
			return;
		}
		if (token.equals(Profile.VIEW_PROFILE_TOKEN)
				|| token.startsWith(Profile.VIEW_PROFILE_TOKEN + "/") || token.isEmpty()) {
			viewMyProfile(token.substring(1));
			return;
		}
		if (authenticationController.isLoggedIn()
				&& authenticationController.getCurrentUserPrincipalId().equals(place.getUserId())) {
			// View my profile
