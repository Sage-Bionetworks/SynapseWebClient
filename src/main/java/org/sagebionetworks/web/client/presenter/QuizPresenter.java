...
public class QuizPresenter extends AbstractActivity
		implements QuizView.Presenter, Presenter<org.sagebionetworks.web.client.place.Quiz> {
	...
	@Inject
	public QuizPresenter(QuizView view, AuthenticationController authenticationController,
			GlobalApplicationState globalApplicationState, SynapseClientAsync synapseClient,
			AdapterFactory adapterFactory, JSONObjectAdapter jsonObjectAdapter,
	...
	public void showQuizFromPassingRecord(PassingRecord passingRecord) {
		...
		for (ResponseCorrectness response : responseCorrections) {
			QuestionContainerWidget newQuestion = ginInjector.getQuestionContainerWidget();
			questionIndexToQuestionWidget.put(questionNumber, newQuestion);
			newQuestion.configure(questionNumber, response.getQuestion(),
	...
	private void scoreQuiz(PassingRecord passingRecord) {
		...
		if (passingRecord.getCorrections() != null) {
			view.showScore(
					"Score: " + passingRecord.getScore() + "/" + passingRecord.getCorrections().size());
		}
	}
	...
	public void getIsCertified() {
			...
			public void onSuccess(String passingRecordJson) {
				try {
					...
					PassingRecord passingRecord =
							new PassingRecord(adapterFactory.createNew(passingRecordJson));
					view.hideLoading();
					showSuccess(passingRecord);
		...
		synapseClient.getCertifiedUserPassingRecord(
				authenticationController.getCurrentUserPrincipalId(), callback);
	}
