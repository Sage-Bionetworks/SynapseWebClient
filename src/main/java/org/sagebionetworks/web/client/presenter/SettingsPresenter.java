...
public class SettingsPresenter implements SettingsView.Presenter {
  ...
  public void updateMyNotificationSettings(
    ...
    AsyncCallback<UserProfile> callback = new AsyncCallback<UserProfile>() {
      ...
      public void onSuccess(final UserProfile myProfile) {
        ...
        synapseClient.updateUserProfile(
          myProfile,
          new AsyncCallback<Void>() {
            @Override
            public void onSuccess(Void result) {
  ...
  private void getUserProfile() {
    ...
    jsClient.getUserBundle(
      Long.parseLong(authenticationController.getCurrentUserPrincipalId()),
      mask,
  ...
  public void newVerificationSubmissionClicked() {
    ...
    if (currentUserBundle.getVerificationSubmission() != null) {
      attachments =
        currentUserBundle.getVerificationSubmission().getAttachments();
    }
  ...
  public void linkOrcIdClicked() {
    ...
    if (orcId != null && orcId.length() > 0) {
      ...
      view.showErrorMessage(
        "An ORC ID has already been linked to your Synapse account."
      );
    } else {
      DisplayUtils.newWindow(
        "/Portal/oauth2AliasCallback?oauth2provider=ORCID",
        "_self",
        ""
      );
    }
  ...
  public void unbindOrcIdAfterConfirmation() {
    jsClient.unbindOAuthProvidersUserId(
      OAuthProvider.ORCID,
      currentUserBundle.getORCID(),
      new AsyncCallback<Void>() {
        @Override
