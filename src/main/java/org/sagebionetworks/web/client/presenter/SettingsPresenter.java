...
public class SettingsPresenter implements SettingsView.Presenter {
	...
	public static final String MUST_BE_CERTIFIED_TO_SUBMIT_PROFILE_VALIDATION_MESSAGE =
			"Only Certified Users can apply to have their user profile validated.  Please get certified and try again.";
	...
	@Inject
	public SettingsPresenter(SettingsView view, AuthenticationController authenticationController,
			GlobalApplicationState globalApplicationState, SynapseClientAsync synapseClient,
			PortalGinInjector ginInjector, UserProfileModalWidget userProfileModalWidget,
			SubscriptionListWidget subscriptionListWidget, EmailAddressesWidget emailAddressesWidget,
			PopupUtilsView popupUtils, SynapseJavascriptClient jsClient) {
		this.view = view;
		...
		if (authenticationController.isLoggedIn()) {
			if (authenticationController.getCurrentUserProfile() != null
				...
				changePasswordRequest
						.setUsername(authenticationController.getCurrentUserProfile().getUserName());
	...
	@Override
	public void updateMyNotificationSettings(final boolean sendEmailNotifications,
	...
	private void getUserProfile() {
		...
		jsClient.getUserBundle(Long.parseLong(authenticationController.getCurrentUserPrincipalId()),
				mask, new AsyncCallback<UserBundle>() {
					@Override
	...
	public void onEditProfile() {
		userProfileModalWidget.showEditProfile(authenticationController.getCurrentUserPrincipalId(),
				new Callback() {
					@Override
					public void invoke() {
	...
	public void editVerificationSubmissionClicked() {
		...
		getVerificationSubmissionWidget()
				.configure(currentUserBundle.getVerificationSubmission(), false, // is ACT
						true) // isModal
	...
	public void newVerificationSubmissionClicked() {
		...
		getVerificationSubmissionWidget()
				.configure(currentUserBundle.getUserProfile(), currentUserBundle.getORCID(), true, // isModal
						attachments)
				.show();
	}
	...
	public void showVerificationUI(VerificationSubmission submission) {
		VerificationState currentState =
				submission.getStateHistory().get(submission.getStateHistory().size() - 1);
	...
	public void unbindOrcId() {
		popupUtils.showConfirmDialog("Unlink",
				"Are you sure you want to unlink this ORCID from your Synapse user profile?",
				new Callback() {
					@Override
					public void invoke() {
	...
	public void unbindOrcIdAfterConfirmation() {
		jsClient.unbindOAuthProvidersUserId(OAuthProvider.ORCID, currentUserBundle.getORCID(),
