...
public class SettingsPresenter implements SettingsView.Presenter {
	...
	public void resetPassword(final String existingPassword,
		...
		if (authenticationController.isLoggedIn()) {
			if (authenticationController.getCurrentUserSessionData() != null
					&& authenticationController.getCurrentUserSessionData()
							.getProfile() != null
					&& authenticationController.getCurrentUserSessionData()
							.getProfile().getUserName() != null) {
				final String username = authenticationController
						.getCurrentUserSessionData().getProfile().getUserName();
	...
	public void updateMyNotificationSettings(
		...
		AsyncCallback<UserProfile> callback = new AsyncCallback<UserProfile>() {
			...
			public void onSuccess(final UserProfile myProfile) {
				...
				synapseClient.updateUserProfile(myProfile,
						new AsyncCallback<Void>() {
							@Override
							public void onSuccess(Void result) {
								view.showInfo(
										DisplayConstants.UPDATED_NOTIFICATION_SETTINGS,
										"");
								authenticationController
										.updateCachedProfile(myProfile);
							}
	...
	private void updateView() {
		...
		getAPIKey();
		view.updateNotificationCheckbox(authenticationController
				.getCurrentUserSessionData().getProfile());
	}
	...
	public void addEmail(String emailAddress) {
		...
		List<String> emailAddresses = authenticationController
				.getCurrentUserSessionData().getProfile().getEmails();
