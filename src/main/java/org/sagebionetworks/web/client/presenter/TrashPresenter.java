...
public class TrashPresenter extends AbstractActivity
		implements TrashView.Presenter, Presenter<Trash> {
	...
	public static final String ERROR_RESTORING_ENTITY_TITLE =
			"Sorry, an error occurred while restoring this entity.";
	public static final String ERROR_DELETING_SELECTED_TITLE =
			"Sorry, an error occured while deleting your selected entities.";
	public static final String ERROR_FETCHING_TRASH_TITLE =
			"Sorry, an error occured while fetching your trash.";
	...
	public void getTrash(Integer offset) {
		...
		synapseClient.viewTrashForUser(this.offset, TRASH_LIMIT,
				new AsyncCallback<PaginatedResults<TrashedEntity>>() {
					@Override
	...
	public void purgeEntities(Set<TrashedEntity> trashedEntities) {
		...
		synapseClient.purgeMultipleTrashedEntitiesForUser(entityIds, new AsyncCallback<Void>() {
			...
			public void onSuccess(Void result) {
				...
				view.showInfo(TRASH_PURGED_TITLE
						+ entityNames.toString().substring(1, entityNamesSet.length() - 1) + ".");
				// Refresh table.
				view.refreshTable();
			}
	...
	public void restoreEntity(final TrashedEntity trashedEntity) {
		synAlert.clear();
		synapseClient.restoreFromTrash(trashedEntity.getEntityId(), trashedEntity.getOriginalParentId(),
