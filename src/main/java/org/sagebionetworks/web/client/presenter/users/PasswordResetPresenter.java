...
public class PasswordResetPresenter
  ...
  public void setPlace(PasswordReset place) {
    ...
    if (
      ...
      view.showLoading();
      authenticationController.setNewAccessToken(
        place.toToken(),
  ...
  public void requestPasswordReset(String emailAddress) {
    synAlert.clear();
    jsClient.sendPasswordResetEmail(
      emailAddress,
    ...
    jsClient.changePassword(
      ...
      new AsyncCallback<Void>() {
        ...
        public void onSuccess(Void result) {
          ...
          ) reloginUser(profile.getUserName(), newPassword); else { // re-login like we do on the Settings page (when changing the password)
