...
public class ResourceLoaderImpl implements ResourceLoader {
	...
	public void requires(List<WebResource> resources, final AsyncCallback<Void> loadedCallback) {
		...
		for (final WebResource resource : resources) {
			if (!loaded.contains(resource)) {
				...
				ScriptInjector.fromUrl(resource.getUrl()).setCallback(callback)
	...
	private Callback<Void, Exception> getCallback(final AsyncCallback<Void> loadedCallback,
			final Set<WebResource> downloading, final WebResource resource) {
