...
public class ResourceLoaderImpl implements ResourceLoader {
  ...
  public void requires(
    ...
    final AsyncCallback<Void> loadedCallback
  ) {
    ...
    for (final WebResource resource : resources) {
      if (!loaded.contains(resource)) {
        // Attach resources
        Callback<Void, Exception> callback = getCallback(
          loadedCallback,
          downloading,
          resource
        );
        ScriptInjector
          .fromUrl(resource.getUrl())
  ...
  private Callback<Void, Exception> getCallback(
    final AsyncCallback<Void> loadedCallback,
    final Set<WebResource> downloading,
