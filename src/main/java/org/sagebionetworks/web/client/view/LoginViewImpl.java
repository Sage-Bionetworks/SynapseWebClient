...
public class LoginViewImpl extends Composite implements LoginView {
			...
			takePledgeButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					if (validatePledge()) {
						...
						showErrorMessage(
								"To accept these Terms and Conditions for Use, you must first agree to all of the statements.");
					}
				}
			});
		}
	...
	private boolean validatePledge() {
		return actEthicallyCb.getValue() && protectPrivacyCb.getValue() && noHackCb.getValue()
				&& shareCb.getValue() && responsibilityCb.getValue() && lawsCb.getValue()
				&& responsibleDataUseCb.getValue();
	}
