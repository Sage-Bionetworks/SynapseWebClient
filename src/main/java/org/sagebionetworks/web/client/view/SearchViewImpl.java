...
public class SearchViewImpl extends Composite implements SearchView {
  ...
  public interface SearchViewImplUiBinder
  ...
  public void setSearchResults(SearchResults searchResults, String searchTerm) {
    ...
    String pageTitleSearchTerm = searchTerm != null && searchTerm.length() > 0
      ? "'" + searchTerm + "' "
      : "";
    synapseJSNIUtils.setPageTitle(
      "Search: " + pageTitleSearchTerm + facetNames
    );
    narrowResultsPanel.setVisible(
      searchResults.getFacets() != null && !searchResults.getFacets().isEmpty()
    );
  ...
  public Widget getResults(
    ...
    if (hits != null && hits.size() > 0) {
      searchResultsPanel =
        createSearchResults(hits, searchResults.getStart().intValue());
      if (isFirstPage) {
      ...
      searchResultsPanel =
        new HTMLPanel(
          new SafeHtmlBuilder()
            ...
            .appendHtmlConstant(
              DisplayConstants.LABEL_NO_SEARCH_RESULTS_PART2 + "</h4>"
            )
            .toSafeHtml()
        );
      facetPanel.clear();
  ...
  private String createShownFacets(SearchResults searchResults) {
    ...
    for (final KeyValue facet : presenter.getAppliedFacets()) {
      ...
      if (
        "link".equals(facet.getValue()) && "node_type".equals(facet.getKey())
      ) continue;

      // show project facet differently
      if (
        "project".equals(facet.getValue()) && "node_type".equals(facet.getKey())
      ) {
        Button btn = new Button(
          DisplayConstants.SHOW_ALL_RESULTS,
          IconType.SEARCH,
          new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
      ...
      if (
        "created_by".equalsIgnoreCase(facet.getKey()) ||
        "modified_by".equalsIgnoreCase(facet.getKey())
      ) {
        String buttonText = "created_by".equalsIgnoreCase(facet.getKey())
          ? "Created by "
          : "Last modified by ";
  ...
  private void addTimeFacets(
    SearchResults searchResults,
    StringBuilder facetNames,
    FlowPanel currentFacets
  ) {
    for (final KeyRange facet : presenter.getAppliedTimeFacets()) {
      String text = presenter.getDisplayForTimeFacet(
        facet.getKey(),
        facet.getMin()
      );
      if (text != null) {
        ...
        try {
          ...
          formattedDateString =
            valueInMiliseconds == 0
              ? "any time"
              : dateTimeUtils.getDateString(new Date(valueInMiliseconds));
  ...
  /**
   * stack-27 temporary change (until the index is updated, there may be usernames in the created_by
   * and modified_by values). Can remove
   *
   * @param userId
   * @return
   */
  private String getSearchUserId(String userId) {
    ...
    if (DisplayUtils.isTemporaryUsername(createdBy)) {
      createdBy =
        createdBy.substring(WebConstants.TEMPORARY_USERNAME_PREFIX.length());
    }
    return createdBy;
  }
  ...
  private Panel getResult(Hit hit) {
    ...
    inlineHtml =
      new InlineHTML(
        " on " +
        dateTimeUtils.getDateString(new Date(hit.getCreated_on() * 1000)) +
        ", Updated by "
      );
    ...
    inlineHtml =
      new InlineHTML(
        " on " +
        dateTimeUtils.getDateString(new Date(hit.getModified_on() * 1000))
      );
    ...
    if (null != hit.getPath()) {
      resultBuilder
        .append(getPathHtml(hit.getPath()))
    ...
    if (null != hit.getDescription()) {
      ...
      description =
        new HTML(description)
          .getText()
          .replace("<Synapse widget>", "")
          .replace("|", "")
          .replace("#", "");
      resultBuilder
        .appendHtmlConstant("<span class=\"hitdescription\">")
        .appendEscaped(
          DisplayUtils.stubStr(description, HIT_DESCRIPTION_LENGTH_CHAR)
        )
  ...
  private SafeHtml getPathHtml(EntityPath path) {
    ...
    for (int i = 0; i < headers.size(); i++) {
      ...
      String safeLink =
        "<a href=\"" +
        DisplayUtils.getSynapseHistoryToken(header.getId()) +
        "\"";
      if (i >= headers.size() - 1) {
  ...
  private FlowPanel createDateFacet(final Facet facet) {
    ...
    // if(facet.getMin() == null || facet.getMax() == null || facet.getMin() >= facet.getMax()) return
    // null;

    FlowPanel lc = new FlowPanel();
    Heading facetTitle = new Heading(
      HeadingSize.H4,
      formatFacetName(facet.getName())
    );
    lc.add(facetTitle);
  ...
  private Anchor createTimeFacet(
    final Facet facet,
  ...
  private FlowPanel createLiteralFacet(final Facet facet) {
    ...
    if (
      facet != null &&
      facet.getConstraints() != null &&
      facet.getConstraints().size() > 0
    ) {
      lc = new FlowPanel();
      String displayName = facetToDisplay.containsKey(facet.getName())
        ? formatFacetName(facetToDisplay.get(facet.getName()))
        : formatFacetName(facet.getName());
      ...
      for (int i = 0; i < facet.getConstraints().size(); i++) {
        ...
        if (i >= MAX_FACET_VALUES_SHOWN) {
          ...
          boolean isMax =
            facet.getConstraints().size() ==
            SearchQueryUtils.MAX_FACET_VALUES_COUNT;
  ...
  private void showAllFacetValues(FlowPanel p, Facet facet) {
    for (
      int i = MAX_FACET_VALUES_SHOWN;
      i < facet.getConstraints().size();
      i++
    ) {
      FlowPanel valueContainer = getFacetValue(facet, i);
      if (valueContainer != null) {
  ...
  private FlowPanel getFacetValue(Facet facet, int i) {
    ...
    String stub = DisplayUtils.stubStr(
      constraint.getValue(),
      FACET_NAME_LENGTH_CHAR
    );
    ...
    if (
      !stub.equalsIgnoreCase(constraint.getValue()) &&
      !isCreatedByFacet &&
      !isModifiedByFacet
    ) {
      valueContainer.add(
