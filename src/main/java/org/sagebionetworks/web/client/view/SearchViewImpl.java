...
public class SearchViewImpl extends Composite implements SearchView {
	...
	@Inject
	public SearchViewImpl(SearchViewImplUiBinder binder, Header headerWidget, SynapseJSNIUtils synapseJSNIUtils, PortalGinInjector ginInjector, DateTimeUtils dateTimeUtils, MarkdownIt markdownIt) {
	...
	public void setSearchResults(SearchResults searchResults, String searchTerm) {
		...
		narrowResultsPanel.setVisible(searchResults.getFacets() != null && !searchResults.getFacets().isEmpty());
	...
	public Widget getResults(SearchResults searchResults, String searchTerm, boolean isFirstPage) {
		...
		if (hits != null && hits.size() > 0) {
			...
			searchResultsPanel = new HTMLPanel(new SafeHtmlBuilder().appendHtmlConstant("<h4>" + DisplayConstants.LABEL_NO_SEARCH_RESULTS_PART1).appendEscaped(searchTerm).appendHtmlConstant(DisplayConstants.LABEL_NO_SEARCH_RESULTS_PART2 + "</h4>").toSafeHtml());
			facetPanel.clear();
		} else {
	...
	private Panel getResult(int i, Hit hit) {
		...
		if (null != hit.getDescription()) {
			...
			resultBuilder.appendHtmlConstant("<span class=\"hitdescription\">").appendEscaped(DisplayUtils.stubStr(description, HIT_DESCRIPTION_LENGTH_CHAR)).appendHtmlConstant("</span><br>\n");
		}
