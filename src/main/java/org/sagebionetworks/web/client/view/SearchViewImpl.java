...
public class SearchViewImpl extends Composite implements SearchView {
	...
	public interface SearchViewImplUiBinder extends UiBinder<Widget, SearchViewImpl> {
	}
	...
	public SearchViewImpl(SearchViewImplUiBinder binder, Header headerWidget,
			SynapseJSNIUtils synapseJSNIUtils, PortalGinInjector ginInjector, DateTimeUtils dateTimeUtils,
	...
	public void setSearchResults(SearchResults searchResults, String searchTerm) {
		...
		String pageTitleSearchTerm =
				searchTerm != null && searchTerm.length() > 0 ? "'" + searchTerm + "' " : "";
	...
	public Widget getResults(SearchResults searchResults, String searchTerm, boolean isFirstPage) {
		...
		if (hits != null && hits.size() > 0) {
			...
			searchResultsPanel = new HTMLPanel(new SafeHtmlBuilder()
					.appendHtmlConstant("<h4>" + DisplayConstants.LABEL_NO_SEARCH_RESULTS_PART1)
					.appendEscaped(searchTerm)
					.appendHtmlConstant(DisplayConstants.LABEL_NO_SEARCH_RESULTS_PART2 + "</h4>")
					.toSafeHtml());
			facetPanel.clear();
		} else {
	...
	private String createShownFacets(SearchResults searchResults) {
		...
		for (final KeyValue facet : presenter.getAppliedFacets()) {
			...
			if ("project".equals(facet.getValue()) && "node_type".equals(facet.getKey())) {
				Button btn =
						new Button(DisplayConstants.SHOW_ALL_RESULTS, IconType.SEARCH, new ClickHandler() {
			...
			if ("created_by".equalsIgnoreCase(facet.getKey())
				...
				String buttonText =
						"created_by".equalsIgnoreCase(facet.getKey()) ? "Created by " : "Last modified by ";
	...
	private void addTimeFacets(SearchResults searchResults, StringBuilder facetNames,
		...
		for (final KeyRange facet : presenter.getAppliedTimeFacets()) {
			...
			if (text != null) {
				...
				try {
					...
					formattedDateString = valueInMiliseconds == 0 ? "any time"
	...
	private Panel getResult(int i, Hit hit) {
		...
		inlineHtml = new InlineHTML(" on "
				+ dateTimeUtils.getDateString(new Date(hit.getCreated_on() * 1000)) + ", Updated by ");
		inlineHtml.addStyleName("hitattribution");
		...
		inlineHtml =
				new InlineHTML(" on " + dateTimeUtils.getDateString(new Date(hit.getModified_on() * 1000)));
		inlineHtml.addStyleName("hitattribution");
		...
		h4.add(headingPanel);
		org.gwtbootstrap3.client.ui.Anchor link = new org.gwtbootstrap3.client.ui.Anchor(hit.getName(),
		...
		if (null != hit.getDescription()) {
			...
			description = new HTML(description).getText().replace("<Synapse widget>", "").replace("|", "")
					.replace("#", "");
	...
	private FlowPanel createDateFacet(final Facet facet) {
		...
		// if(facet.getMin() == null || facet.getMax() == null || facet.getMin() >= facet.getMax())
		// return null;
