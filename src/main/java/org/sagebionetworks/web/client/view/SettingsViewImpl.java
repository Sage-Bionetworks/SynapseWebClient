...
public class SettingsViewImpl extends Composite implements SettingsView {
  ...
  public SettingsViewImpl(
    ...
    ClickHandler newVerificationSubmissionCallback = event ->
      presenter.newVerificationSubmissionClicked();
    ClickHandler editVerificationSubmissionCallback = event ->
      presenter.editVerificationSubmissionClicked();

    requestProfileValidationLink1.addClickHandler(
      newVerificationSubmissionCallback
    );
    requestProfileValidationLink2.addClickHandler(
      newVerificationSubmissionCallback
    );
    verificationApprovedButton.addClickHandler(
      editVerificationSubmissionCallback
    );
    verificationSubmittedButton.addClickHandler(
      editVerificationSubmissionCallback
    );
    verificationSuspendedButton.addClickHandler(
      editVerificationSubmissionCallback
    );
    verificationRejectedButton.addClickHandler(
      editVerificationSubmissionCallback
    );
    resubmitProfileValidationButton.addClickHandler(
      newVerificationSubmissionCallback
    );

    unbindButton.addClickHandler(event -> presenter.unbindOrcId());
    certificationButton.addClickHandler(event ->
  ...
  public void render() {
    currentPasswordField
      .getElement()
      .setAttribute("placeholder", "Enter current password");
    password1Field
      .getElement()
  ...
  public void updateNotificationCheckbox(UserProfile profile) {
    ...
    if (profile.getNotificationSettings() != null) {
      if (
        profile.getNotificationSettings().getSendEmailNotifications() != null
      ) isNotify =
        profile.getNotificationSettings().getSendEmailNotifications();
    }
    ...
    return new ClickHandler() {
      ...
      public void onClick(ClickEvent event) {
        ...
        presenter.updateMyNotificationSettings(
          emailNotificationsCheckbox.getValue(),
          false
        );
      }
    };
