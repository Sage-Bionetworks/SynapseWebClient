...
public class DownloadSpeedTesterImpl implements DownloadSpeedTester {
	 ...
	 * presigned url and file size. 3. Run the test - how much time does it take to download the
	 * target file?
	 * 
	 ...
	 */
	@Inject
	public DownloadSpeedTesterImpl(AuthenticationController authController,
			PresignedAndFileHandleURLAsyncHandler presignedUrlAsyncHandler,
			RequestBuilderWrapper requestBuilder, SynapseJavascriptClient jsClient,
			ClientCache clientCache) {
		...
		presignedUrlAsyncHandler.getFileResult(fha, new AsyncCallback<FileResult>() {
			...
			public void onSuccess(FileResult fileResult) {
				if (fileResult.getFailureCode() == null) {
					...
					onFailure(new Exception("Unable to get the test file presigned url and file handle: "
							+ fileResult.getFailureCode().toString()));
				}
			};
		});
	}
	...
	public void updateCachedDownloadSpeed(Double downloadSpeed) {
		...
		clientCache.put(ESTIMATED_DOWNLOAD_SPEED_CACHE_KEY, downloadSpeed.toString(),
				new Date(System.currentTimeMillis() + 1000 * 60 * 10).getTime());
	}
			...
			requestBuilder.sendRequest(null, new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					...
					if (statusCode == Response.SC_OK) {
						...
						onError(null, new IllegalArgumentException(
								"Unable to retrieve test file. Reason: " + response.getStatusText()));
					}
				}
