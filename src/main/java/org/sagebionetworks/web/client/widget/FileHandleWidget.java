...
public class FileHandleWidget implements IsWidget {
  ...
  public static final String UNABLE_TO_LOAD_FILE_DATA =
  ...
  public void configure(final FileHandleAssociation fha) {
    ...
    fileHandleAsynHandler.getFileResult(
      fha,
      new AsyncCallback<FileResult>() {
        ...
        public void onSuccess(FileResult result) {
          if (view.isAttached() && result != null) {
            ...
            if (result.getFileHandle() != null) {
              view.setAnchor(
                result.getFileHandle().getFileName(),
                createAnchorHref(fha)
              );
            } else if (result.getFailureCode() != null) {
              // failed
              view.setErrorText(
                UNABLE_TO_LOAD_FILE_DATA +
                ": " +
                result.getFailureCode().toString()
              );
            }
          }
        }
        ...
        public void onFailure(Throwable caught) {
          if (view.isAttached()) {
            view.setLoadingVisible(false);
            view.setErrorText(
              UNABLE_TO_LOAD_FILE_DATA + ": " + caught.getMessage()
            );
          }
        }
      }
  ...
  public String createAnchorHref(FileHandleAssociation fha) {
    return jsniUtils.getFileHandleAssociationUrl(
  ...
  public String createAnchorHref(String rawFileHandleId) {
    return (
      jsniUtils.getBaseFileHandleUrl() + "?rawFileHandleId=" + rawFileHandleId
    );
  }
