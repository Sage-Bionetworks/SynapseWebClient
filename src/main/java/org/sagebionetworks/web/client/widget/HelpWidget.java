...
public class HelpWidget implements IsWidget {
	...
	public HelpWidget() {
		...
		closeHTML = "<button class=\"btn btn-default btn-xs right margin-right-5\" onClick=\""
				+ closePopoverJs + "\">Close</button>";
		POPOVERS.add(this);
	...
	public void setHref(String fullHelpHref) {
		if (DisplayUtils.isDefined(fullHelpHref)) {
			this.moreHelpHTML = "<button class=\"btn btn-primary btn-xs right\" onClick=\"window.open('"
					+ SafeHtmlUtils.htmlEscape(fullHelpHref) + "');" + closePopoverJs
					+ "\">More info</button>";
		}
	}
	...
	// SWC-4292: On static initialization of class, add a listener for the bootstrap modal close
	// event. If the event fires, make sure all popovers are hidden.
	static {
