...
public class ACTAccessRequirementWidget
	...
	public ACTAccessRequirementWidget(ACTAccessRequirementWidgetView view,
			SynapseJavascriptClient jsClient, WikiPageWidget wikiPageWidget, SynapseAlert synAlert,
			PortalGinInjector ginInjector, SubjectsWidget subjectsWidget,
			CreateAccessRequirementButton createAccessRequirementButton,
			DeleteAccessRequirementButton deleteAccessRequirementButton,
			DataAccessClientAsync dataAccessClient, LazyLoadHelper lazyLoadHelper,
			AuthenticationController authController, ReviewAccessorsButton manageAccessButton,
			ConvertACTAccessRequirementButton convertACTAccessRequirementButton,
			SynapseJSNIUtils jsniUtils) {
		this.view = view;
	...
	public void onRequestAccess() {
		...
		jsniUtils.showJiraIssueCollector(REQUEST_ACCESS_ISSUE_SUMMARY, REQUEST_ACCESS_ISSUE_DESCRIPTION,
				REQUEST_ACCESS_ISSUE_COLLECTOR_URL, userProfile.getOwnerId(),
				DisplayUtils.getDisplayName(userProfile), primaryEmail, ar.getSubjectIds().get(0).getId(),
				GRANT_ACCESS_REQUEST_COMPONENT_ID, ar.getId().toString(), ISSUE_PRIORITY_MINOR);
	}
