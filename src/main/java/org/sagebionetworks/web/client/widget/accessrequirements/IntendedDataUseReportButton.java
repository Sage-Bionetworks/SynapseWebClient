...
public class IntendedDataUseReportButton implements IsWidget {
	...
	@Inject
	public IntendedDataUseReportButton(Button button, IsACTMemberAsyncHandler isACTMemberAsyncHandler,
			DataAccessClientAsync dataAccessClient, PopupUtilsView popupUtils,
			BigPromptModalView copyTextModal, DateTimeUtils dateTimeUtils) {
		this.button = button;
	...
	public void gatherAllSubmissions(String nextPageToken) {
		dataAccessClient.getDataAccessSubmissions(ar.getId(), nextPageToken, SubmissionState.APPROVED,
				SubmissionOrder.CREATED_ON, true, new AsyncCallback<SubmissionPage>() {
					@Override
					public void onSuccess(SubmissionPage page) {
						...
						for (Submission submission : newSubmissions) {
							researchProjectId2Submission.put(submission.getResearchProjectSnapshot().getId(),
									submission);
						}
