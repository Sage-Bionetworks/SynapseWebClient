...
public class CreateDataAccessSubmissionStep2 implements ModalPage {
  ...
  public static final String SAVE_CHANGES_MESSAGE =
    "Would you want to save your recent changes?";
  ...
  public CreateDataAccessSubmissionStep2(
    ...
    peopleSuggestWidget.setPlaceholderText(
      "Enter the user name of other data requestors..."
    );
    ducUploader.configure(
      "Browse...",
      new CallbackP<FileUpload>() {
        @Override
        public void invoke(FileUpload fileUpload) {
          setDUCFileHandle(
            fileUpload.getFileMeta().getFileName(),
            fileUpload.getFileHandleId()
          );
        }
      }
    );

    irbUploader.configure(
      "Browse...",
      new CallbackP<FileUpload>() {
        @Override
        public void invoke(FileUpload fileUpload) {
    ...
    peopleSuggestWidget.addItemSelectedHandler(
      new CallbackP<UserGroupSuggestion>() {
        public void invoke(UserGroupSuggestion suggestion) {
          ...
          CreateDataAccessSubmissionStep2.this.accessorChangesList.addAccessorChange(
              change
            );
        }
      }
  ...
  public void configure(
    ...
    view.setOtherDocumentUploadVisible(
      ValidationUtils.isTrue(ar.getAreOtherAttachmentsRequired())
    );
    ...
    client.getDataAccessRequest(
      ar.getId(),
      new AsyncCallback<RequestInterface>() {
        @Override
        ...
        public void onSuccess(RequestInterface dataAccessRequest) {
          CreateDataAccessSubmissionStep2.this.dataAccessRequest =
            dataAccessRequest;
          ...
          if (isRenewal) {
            view.setPublications(
              ((Renewal) dataAccessRequest).getPublication()
            );
            view.setSummaryOfUse(
              ((Renewal) dataAccessRequest).getSummaryOfUse()
            );
          }
          if (dataAccessRequest.getDucFileHandleId() != null) {
            FileHandleWidget fileHandleWidget = getFileHandleWidget(
  ...
  public void initAccessors() {
    ...
    if (dataAccessRequest.getAccessorChanges() != null) {
      for (AccessorChange change : dataAccessRequest.getAccessorChanges()) {
        if (
          change.getUserId().equals(authController.getCurrentUserPrincipalId())
        ) {
          submitterFound = true;
  ...
  public void updateDataAccessRequest(final boolean isSubmit) {
    ...
    dataAccessRequest.setAccessorChanges(
      accessorChangesList.getAccessorChanges()
    );
    ...
    client.updateDataAccessRequest(
      dataAccessRequest,
      new AsyncCallback<RequestInterface>() {
        @Override
        public void onFailure(Throwable caught) {
  ...
  public void submitDataAccessRequest() {
    ...
    client.submitDataAccessRequest(
      submissionRequest,
      ar.getId(),
      new AsyncCallback<Void>() {
        ...
        public void onSuccess(Void result) {
          popupUtils.showInfoDialog(
            SUCCESSFULLY_SUBMITTED_TITLE,
            SUCCESSFULLY_SUBMITTED_MESSAGE,
            null
          );
          modalPresenter.setLoading(false);
          modalPresenter.onFinished();
  ...
  public void setModalPresenter(ModalPresenter modalPresenter) {
    ...
    ((ModalWizardWidget) modalPresenter).addCallback(
