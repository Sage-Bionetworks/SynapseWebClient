...
public class ACTDataAccessSubmissionWidget
	...
	public ACTDataAccessSubmissionWidget(ACTDataAccessSubmissionWidgetView view,
			SynapseAlert synAlert, DataAccessClientAsync dataAccessClient,
			BigPromptModalView promptDialog, FileHandleWidget ducFileRenderer,
			FileHandleWidget irbFileRenderer, FileHandleList otherDocuments, SynapseJSNIUtils jsniUtils,
	...
	public void configure(Submission submission) {
		...
		switch (submission.getState()) {
			...
			case REJECTED:
				String reason =
						submission.getRejectedReason() == null ? "" : submission.getRejectedReason();
	...
	public void updateDataAccessSubmissionState(SubmissionState state, String reason) {
		dataAccessClient.updateDataAccessSubmissionState(submission.getId(), state, reason,
