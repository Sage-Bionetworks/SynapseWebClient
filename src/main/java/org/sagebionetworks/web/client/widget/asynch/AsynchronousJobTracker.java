 ...
 * 
 */
public class AsynchronousJobTracker {
	...
	/**
	 * Create a new tracker for each job. Note: This Constructor does not
	 * include an @Inject annotation because it is not designed to be re-used.
	 * Re-use is prevented since all parameters must be passed to the
	 * constructor.
	 * If we used @Inject and included a configure() method there would be
	 * nothing to stop someone from calling the configure method with new data.
	 * 
	 * @param synapseClient
	 *            Synapse java client.
	 * @param timerProvider
	 ...
	 * @param waitTimeMS
	 *            The amount of time in milliseconds to wait between status
	 *            update checks.
	 * @param statusToTrack
	 *            The job status to track.
	 * @param handler
	 *            The handler is notified of all critical events that occur
	 *            while tracking a job.
	 */
	public AsynchronousJobTracker(SynapseClientAsync synapseClient,
			TimerProvider timerProvider, AdapterFactory adapterFactory,
			int waitTimeMS, AsynchronousJobStatus statusToTrack,
