...
public class AsynchronousProgressWidget
  ...
  @Inject
  public AsynchronousProgressWidget(
    AsynchronousProgressView view,
  ...
  > void startAndTrackJob(
    ...
    final AsynchronousProgressHandler<TResponse> handler
  ) {
    ...
    jobTracker.startAndTrack(
      ...
      requestBody,
      WAIT_MS,
      new UpdatingAsynchProgressHandler<TResponse>() {
        @Override
  ...
  private double calculateProgressPercent(AsynchronousJobStatus status) {
    if (
      status.getProgressCurrent() == null ||
