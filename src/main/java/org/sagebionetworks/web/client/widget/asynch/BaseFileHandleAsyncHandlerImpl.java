...
public abstract class BaseFileHandleAsyncHandlerImpl {
	...
	private List<FileHandleAssociation> fileHandleAssociations =
			new ArrayList<FileHandleAssociation>();
	...
	public void getFileResult(FileHandleAssociation fileHandleAssociation,
			AsyncCallback<FileResult> callback) {
		if (isIncludeFileHandles() && !isIncludePreSignedURLs() && clientCache
				.contains(fileHandleAssociation.getFileHandleId() + WebConstants.FILE_HANDLE_SUFFIX)) {
			FileResult fileResult = getFileResultFromCache(fileHandleAssociation.getFileHandleId(),
					adapterFactory, clientCache);
			if (fileResult != null) {
				// done!
		...
		List<AsyncCallback<FileResult>> list =
				reference2Callback.get(fileHandleAssociation.getFileHandleId());
		if (list == null) {
	...
	public void executeRequests() {
		if (!reference2Callback.isEmpty()) {
			final Map<String, List<AsyncCallback<FileResult>>> reference2CallbackCopy =
					new HashMap<String, List<AsyncCallback<FileResult>>>();
			reference2CallbackCopy.putAll(reference2Callback);
			List<FileHandleAssociation> fileHandleAssociationsCopy =
					new ArrayList<FileHandleAssociation>();
	...
	public static FileResult getFileResultFromCache(String fileHandleId,
			AdapterFactory adapterFactory, ClientCache clientCache) {
	...
	public void cacheFileResult(FileResult result) {
		...
		try {
			...
			clientCache.put(result.getFileHandleId() + WebConstants.FILE_HANDLE_SUFFIX,
