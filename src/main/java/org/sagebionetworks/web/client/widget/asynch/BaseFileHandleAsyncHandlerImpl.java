...
public abstract class BaseFileHandleAsyncHandlerImpl {
  ...
  protected abstract boolean isIncludeFileHandles();
  ...
  public BaseFileHandleAsyncHandlerImpl(
    SynapseJavascriptClient jsClient,
    GWTWrapper gwt,
  ...
  public void getFileResult(
    ...
    if (
      isIncludeFileHandles() &&
      !isIncludePreSignedURLs() &&
      clientCache.contains(
        fileHandleAssociation.getFileHandleId() +
        WebConstants.FILE_HANDLE_SUFFIX
      )
    ) {
      FileResult fileResult = getFileResultFromCache(
        fileHandleAssociation.getFileHandleId(),
        adapterFactory,
        clientCache
      );
    ...
    List<AsyncCallback<FileResult>> list = reference2Callback.get(
      fileHandleAssociation.getFileHandleId()
    );
    if (list == null) {
  ...
  public void executeRequests() {
    if (!reference2Callback.isEmpty()) {
      ...
      jsClient.getFileHandleAndUrlBatch(
        request,
        new AsyncCallback<BatchFileResult>() {
          @Override
  ...
  public static FileResult getFileResultFromCache(
    String fileHandleId,
    AdapterFactory adapterFactory,
    ClientCache clientCache
  ) {
    String fileResultString = clientCache.get(
      fileHandleId + WebConstants.FILE_HANDLE_SUFFIX
    );
  ...
  public void cacheFileResult(FileResult result) {
    ...
    try {
      ...
      clientCache.put(
        result.getFileHandleId() + WebConstants.FILE_HANDLE_SUFFIX,
