...
public class IsACTMemberAsyncHandlerImpl implements IsACTMemberAsyncHandler {
  ...
  @Inject
  public IsACTMemberAsyncHandlerImpl(
    UserProfileClientAsync userProfileClient,
    SessionStorage sessionStorage,
    AuthenticationController authController,
    SynapseJSNIUtils jsniUtils
  ...
  public FluentFuture<Boolean> isACTActionAvailable() {
    return isACTMember()
      .transform(isActMember -> isActMember && visible, directExecutor());
  }
  ...
  public FluentFuture<Boolean> isACTMember() {
    ...
    String cachedValue = sessionStorage.getItem(
      SESSION_KEY_PREFIX + authController.getCurrentUserPrincipalId()
    );
    if (cachedValue != null) {
    ...
    // do rpc
    return getFuture(cb ->
      userProfileClient.getMyOwnUserBundle(
        IS_ACT_MEMBER,
