...
public class IsACTMemberAsyncHandlerImpl implements IsACTMemberAsyncHandler {
	...
	public IsACTMemberAsyncHandlerImpl(UserProfileClientAsync userProfileClient,
			SessionStorage sessionStorage, AuthenticationController authController,
			SynapseJSNIUtils jsniUtils) {
	...
	public void isACTMember(final CallbackP<Boolean> callback) {
		...
		String cachedValue =
				sessionStorage.getItem(SESSION_KEY_PREFIX + authController.getCurrentUserPrincipalId());
		if (cachedValue != null) {
		...
		userProfileClient.getMyOwnUserBundle(IS_ACT_MEMBER, new AsyncCallback<UserBundle>() {
			...
			public void onSuccess(UserBundle userBundle) {
				sessionStorage.setItem(SESSION_KEY_PREFIX + authController.getCurrentUserPrincipalId(),
						userBundle.getIsACTMember().toString());
				callback.invoke(userBundle.getIsACTMember());
			}

			@Override
			public void onFailure(Throwable caught) {
				// log the error, and tell client that user is not part of the ACT (give the developer
				// something to look at if something goes wrong for an ACT member)
