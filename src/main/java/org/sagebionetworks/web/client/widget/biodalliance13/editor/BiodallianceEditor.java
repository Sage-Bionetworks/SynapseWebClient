...
public class BiodallianceEditor
  implements BiodallianceEditorView.Presenter, WidgetEditorPresenter {
  ...
  @Inject
  public BiodallianceEditor(
    BiodallianceEditorView view,
  ...
  public void configure(
    ...
    if (descriptor.containsKey(WidgetConstants.BIODALLIANCE_SPECIES_KEY)) {
      species =
        Species.valueOf(
          descriptor.get(WidgetConstants.BIODALLIANCE_SPECIES_KEY).toUpperCase()
        );
    }
    ...
    if (descriptor.containsKey(WidgetConstants.BIODALLIANCE_VIEW_START_KEY)) {
      viewStart =
        Integer.parseInt(
          descriptor.get(WidgetConstants.BIODALLIANCE_VIEW_START_KEY)
        );
    }
    ...
    if (descriptor.containsKey(WidgetConstants.BIODALLIANCE_VIEW_END_KEY)) {
      viewEnd =
        Integer.parseInt(
          descriptor.get(WidgetConstants.BIODALLIANCE_VIEW_END_KEY)
        );
    }

    selectableItemList.clear();
    if (
      descriptor.containsKey(WidgetConstants.BIODALLIANCE_SOURCE_PREFIX + 0)
    ) {
  ...
  public void updateDescriptorFromView() {
    ...
    descriptor.put(
      WidgetConstants.BIODALLIANCE_VIEW_START_KEY,
      view.getViewStart()
    );
    descriptor.put(
      WidgetConstants.BIODALLIANCE_VIEW_END_KEY,
      view.getViewEnd()
    );
    ...
    try {
      ...
      if (chr < 1) {
        throw new IllegalArgumentException(
          "chr must be greater than or equal to 1."
        );
      }
      ...
      if (viewStart < 0) {
        throw new IllegalArgumentException(
          "View start must be a positive integer."
        );
      }
      if (viewEnd < viewStart) {
        throw new IllegalArgumentException(
          "View start must less than view end."
        );
      }
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(
        "Chr, view start, and view end must be integers."
      );
    }
  }
