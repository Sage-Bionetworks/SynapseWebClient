 ...
 */
public class BiodallianceSourceEditor
  implements
    BiodallianceSourceEditorView.Presenter, IsWidget, SelectableListItem {
  ...
  public BiodallianceSourceEditor(
    ...
    this.entityFinder =
      entityFinderBuilder
        ...
        .setHelpMarkdown(
          "Search or Browse Synapse to find supported Files for this Genome Browser"
        )
  ...
  public void checkParams() throws IllegalArgumentException {
    ...
    try {
      ...
      if (heightInt < 1) {
        throw new IllegalArgumentException(
          "Track height must be a positive integer."
        );
      }
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(
        "Invalid track height: " + e.getMessage()
      );
    }
    ...
    if (
      SourceType.VCF.equals(source.getSourceType()) ||
      SourceType.BED.equals(source.getSourceType())
    ) {
      if (
        source.getIndexEntityId() == null || source.getIndexVersion() == null
      ) {
        throw new IllegalArgumentException(
          "An index file must be specified for a tabix source."
        );
      }
    }
  }
    ...
    AsyncCallback<EntityBundle> callback = new AsyncCallback<EntityBundle>() {
      ...
      public void onSuccess(EntityBundle bundle) {
        try {
          ...
          view.setEntityFinderText(
            getEntityFinderText(newEntityId, newVersion)
          );
          entityFinder.hide();
        } catch (Exception e) {
          onFailure(e);
    ...
    AsyncCallback<EntityBundle> callback = new AsyncCallback<EntityBundle>() {
      ...
      public void onSuccess(EntityBundle bundle) {
        try {
          ...
          BiodallianceWidget.updateSourceURIs(source);
          view.setIndexEntityFinderText(
            getEntityFinderText(newIndexEntityId, newIndexVersion)
          );
  ...
  public SourceType getSourceType(String fileName)
    ...
    if (fileName != null) {
      ...
      if (lastDot > -1) {
        ...
        // else
        if (
          ".vcf".equals(extension) || fileName.toLowerCase().endsWith(".vcf.gz")
        ) {
          return SourceType.VCF;
        }
        // else
        if (
          ".bed".equals(extension) || fileName.toLowerCase().endsWith(".bed.gz")
        ) {
  ...
  public void assertIndexFile(String fileName) throws IllegalArgumentException {
    if (!(fileName.toLowerCase().endsWith(".tbi"))) {
      throw new IllegalArgumentException(
        "Unrecognized index file: " + fileName
      );
    }
  }
  ...
  public void updateViewFromSource() {
    ...
    view.setEntityFinderText(
      getEntityFinderText(source.getEntityId(), source.getVersion())
    );
    view.setIndexEntityFinderText(
      getEntityFinderText(source.getIndexEntityId(), source.getIndexVersion())
    );
