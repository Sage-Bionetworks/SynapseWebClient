...
public class ForumWidget implements ForumWidgetView.Presenter {
  ...
  public static final Long MODERATOR_LIMIT = 20L;
  ...
  public static final String DEFAULT_THREAD_ID_KEY =
    "org.sagebionetworks.portal.default_thread_id";
  ...
  public ForumWidget(
    ...
    String defaultThreadId = synapseProperties.getSynapseProperty(
      DEFAULT_THREAD_ID_KEY
    );
  ...
  public DiscussionThreadBundle createDefaultThread(
    DiscussionThreadBundle stuThread
  ) {
  ...
  public void loadForum(String entityId, final Callback callback) {
    stuAlert.clear();
    jsClient.getForumByProjectId(
      entityId,
      new AsyncCallback<Forum>() {
        @Override
        public void onFailure(Throwable caught) {
          stuAlert.handleException(caught);
        }
  ...
  public void loadModerators(
    ...
    stuAlert.clear();
    jsClient.getModerators(
      forumId,
      MODERATOR_LIMIT,
      offset,
      new AsyncCallback<PaginatedIds>() {
        @Override
        public void onFailure(Throwable caught) {
          stuAlert.handleException(caught);
        }
  ...
  public void showForum() {
    ...
    subscribeToForumButton.clear();
    updatePlaceToForum();
    subscribeToForumButton.configure(
      SubscriptionObjectType.FORUM,
      forumId,
      actionMenu
    );
    newThreadModal.configure(
      forumId,
      new Callback() {
        @Override
        public void invoke() {
          threadListWidget.configure(
            forumId,
            isCurrentUserModerator,
            moderatorIds,
            emptyListCallback,
  ...
  public void onClickDeletedThreadCommand() {
    if (view.isDeletedThreadListVisible()) {
      ...
      deletedThreadListWidget.configure(
        forumId,
        isCurrentUserModerator,
        moderatorIds,
        null,
  ...
  private void updateActionMenuDeletedThreadsCommand() {
    if (actionMenu != null) {
      String commandName = view.isDeletedThreadListVisible()
