...
public class ForumWidget implements ForumWidgetView.Presenter {
	...
	@Inject
	public ForumWidget(final ForumWidgetView view, StuAlert stuAlert,
			DiscussionForumClientAsync discussionForumClient, DiscussionThreadListWidget threadListWidget,
			DiscussionThreadListWidget deletedThreadListWidget, NewDiscussionThreadModal newThreadModal,
			AuthenticationController authController, GlobalApplicationState globalApplicationState,
			SingleDiscussionThreadWidget singleThreadWidget, SubscribeButtonWidget subscribeToForumButton,
			SingleDiscussionThreadWidget defaultThreadWidget, SubscribersWidget forumSubscribersWidget,
			SynapseJavascriptClient jsClient, SynapseProperties synapseProperties) {
		this.view = view;
	...
	public void initDefaultThreadWidget() {
		...
		String replyId = null;
		defaultThreadWidget.configure(defaultThreadBundle, replyId, isCurrentUserModerator,
				moderatorIds, null, deleteCallback);
	...
	public void configure(String entityId, final ParameterizedToken params,
			Boolean isCurrentUserModerator, ActionMenuWidget actionMenu,
	...
	public void showThread(String threadId, final String replyId) {
		...
		jsClient.getThread(threadId, new AsyncCallback<DiscussionThreadBundle>() {
			...
			public void onSuccess(DiscussionThreadBundle result) {
				...
				singleThreadWidget.configure(result, replyId, isCurrentUserModerator, moderatorIds,
						actionMenu, new Callback() {
							@Override
							public void invoke() {
								isForumConfigured = false;
	...
	public void configureActionMenu() {
		if (actionMenu != null) {
			...
			actionMenu.setActionListener(Action.SHOW_DELETED_THREADS,
					new ActionMenuWidget.ActionListener() {
						@Override
						public void onAction(Action action) {
	...
	public void updateActionMenuCommands() {
		if (actionMenu != null) {
			...
			actionMenu.setActionVisible(Action.SHOW_DELETED_THREADS,
					!isSingleThread && isCurrentUserModerator);
			actionMenu.setActionVisible(Action.EDIT_THREAD, isSingleThread
					&& currentThreadBundle.getCreatedBy().equals(authController.getCurrentUserPrincipalId()));
	...
	public void onClickDeletedThreadCommand() {
		if (view.isDeletedThreadListVisible()) {
			...
			deletedThreadListWidget.configure(forumId, isCurrentUserModerator, moderatorIds, null,
					DiscussionFilter.DELETED_ONLY);
		}
		updateActionMenuDeletedThreadsCommand();
	}
	...
	private void updateActionMenuDeletedThreadsCommand() {
		if (actionMenu != null) {
			String commandName =
					view.isDeletedThreadListVisible() ? "Hide Deleted Threads" : "Show Deleted Threads";
