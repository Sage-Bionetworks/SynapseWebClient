...
public class SingleDiscussionThreadWidget implements SingleDiscussionThreadWidgetView.Presenter {
	...
	public static final String RESTORE_CONFIRM_MESSAGE =
			"Are you sure you want to restore this thread?";
	...
	ActionMenuWidget.ActionListener editActionListener, unpinActionListener, pinActionListener,
			deleteActionListener, restoreActionListener;
	Boolean isPinned;
	...
	@Inject
	public SingleDiscussionThreadWidget(SingleDiscussionThreadWidgetView view, SynapseAlert synAlert,
			UserBadge authorWidget, DiscussionForumClientAsync discussionForumClientAsync,
			PortalGinInjector ginInjector, DateTimeUtils dateTimeUtils,
			RequestBuilderWrapper requestBuilder, AuthenticationController authController,
			GlobalApplicationState globalApplicationState, MarkdownWidget markdownWidget,
			LoadMoreWidgetContainer loadMoreWidgetContainer, SubscribeButtonWidget subscribeButtonWidget,
			NewReplyWidget newReplyWidget, NewReplyWidget secondNewReplyWidget,
			SubscribersWidget threadSubscribersWidget, SynapseJavascriptClient jsClient,
			PopupUtilsView popupUtils, ClientCache clientCache) {
	...
	public void configure(DiscussionThreadBundle bundle, String replyId,
			Boolean isCurrentUserModerator, Set<String> moderatorIds, ActionMenuWidget actionMenu,
		...
		configureMessage();
		if (!bundle.getId().equals(
				ginInjector.getSynapseProperties().getSynapseProperty(ForumWidget.DEFAULT_THREAD_ID_KEY))) {
	...
	/**
	 * After configuring this widget, call this method to pop up an alert when the thread etag changes
	 * upstream.
	 */
	...
	public void reconfigureThread() {
		...
		jsClient.getThread(threadId, new AsyncCallback<DiscussionThreadBundle>() {
			...
			public void onSuccess(DiscussionThreadBundle result) {
				configure(result, replyId, isCurrentUserModerator, moderatorIds, actionMenu,
						deleteOrRestoreCallback);
			}
		});
	}
			...
			requestBuilder.sendRequest(null, new RequestCallback() {
				...
				public void onResponseReceived(Request request, Response response) {
					...
					if (statusCode == Response.SC_OK) {
						...
						onError(null, new IllegalArgumentException("Unable to retrieve message for thread "
								+ threadId + ". Reason: " + response.getStatusText()));
					}
				}
	...
	public void loadMore() {
		synAlert.clear();
		jsClient.getRepliesForThread(threadId, LIMIT, offset, order, ascending, DEFAULT_FILTER,
	...
	public void onClickRestore() {
		popupUtils.showConfirmDialog(CONFIRM_RESTORE_DIALOG_TITLE, RESTORE_CONFIRM_MESSAGE,
				new Callback() {
					@Override
					public void invoke() {
						restoreThread();
