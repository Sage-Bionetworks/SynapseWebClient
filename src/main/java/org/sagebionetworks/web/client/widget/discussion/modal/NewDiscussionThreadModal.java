 ...
 */
public class NewDiscussionThreadModal
  implements DiscussionThreadModalView.Presenter {
  ...
  public void configure(String forumId, Callback newThreadCallback) {
    ...
    this.titleKey =
      forumId + "_" + authController.getCurrentUserPrincipalId() + "_title";
    this.messageKey =
      forumId + "_" + authController.getCurrentUserPrincipalId() + "_message";
  }
  ...
  private void checkForSavedThread() {
    ...
    if (
      storage.getItem(titleKey) == null || storage.getItem(messageKey) == null
    ) {
  ...
  public void onSave() {
    ...
    result
      .requiredField("Title", threadTitle)
    ...
    discussionForumClient.createThread(
      toCreate,
