...
public class DockerCommitListWidget implements IsWidget, DockerCommitListWidgetView.Presenter {
	...
	@Inject
	public DockerCommitListWidget(DockerCommitListWidgetView view, SynapseJavascriptClient jsClient,
			SynapseAlert synAlert, LoadMoreWidgetContainer commitsContainer,
			PortalGinInjector ginInjector, GWTWrapper gwtWrapper) {
		this.view = view;
	...
	public void loadMore() {
		synAlert.clear();
		jsClient.getDockerTaggedCommits(entityId, LIMIT, offset, order, ascending,
				new AsyncCallback<ArrayList<DockerCommit>>() {
