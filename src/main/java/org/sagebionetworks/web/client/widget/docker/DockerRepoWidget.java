...
public class DockerRepoWidget {
	...
	@Inject
	public DockerRepoWidget(DockerRepoWidgetView view, WikiPageWidget wikiPageWidget,
			ProvenanceWidget provWidget, DockerTitleBar dockerTitleBar, EntityMetadata metadata,
			ModifiedCreatedByWidget modifiedCreatedBy, DockerCommitListWidget dockerCommitListWidget,
			CookieProvider cookies, EventBus eventBus) {
		this.view = view;
	...
	private void configureWikiPage(EntityBundle bundle) {
		...
		wikiPageWidget.configure(
				new WikiPageKey(entityId, ObjectType.ENTITY.toString(), bundle.getRootWikiId(), null),
				canEdit, wikiCallback);
		CallbackP<String> wikiReloadHandler = new CallbackP<String>() {
			@Override
			public void invoke(String wikiPageId) {
				wikiPageWidget.configure(
						new WikiPageKey(entityId, ObjectType.ENTITY.toString(), wikiPageId, null), canEdit,
						wikiCallback);
			}
		};
