...
public class CreateOrUpdateDoiModalViewImpl
  implements CreateOrUpdateDoiModalView {

  public interface CreateOrUpdateDoiModalViewImplUiBinder
    extends UiBinder<Widget, CreateOrUpdateDoiModalViewImpl> {}
  ...
  @Inject
  public CreateOrUpdateDoiModalViewImpl(
    CreateOrUpdateDoiModalViewImplUiBinder binder
  ) {
  ...
  private void initializeResourceTypeGeneralSelect() {
    ...
    for (DoiResourceTypeGeneral rtg : DoiResourceTypeGeneral.values()) {
      if (
        !rtg.equals(DoiResourceTypeGeneral.Dataset) &&
        !rtg.equals(DoiResourceTypeGeneral.Collection)
      ) {
  ...
  public void reset() {
    ...
    String entityTypeDisplay = EntityTypeUtils.getDisplayName(
      EntityTypeUtils.getEntityTypeForClass(this.entity.getClass())
    );
    String versionHelpMarkdown =
      "A DOI can be associated with a specific version of this " +
      entityTypeDisplay +
      ".\n\n" +
      "Versioned DOIs will link to the specified version of the " +
      entityTypeDisplay +
      ".\n\n" +
      "A DOI without a version will always link to the newest version of this " +
      entityTypeDisplay +
  ...
  public void setVersions(
    ...
    for (int i = 0; i < versions.size(); i++) {
      ...
      versionSelection.addItem(
        label,
        String.valueOf(version.getVersionNumber())
      );
      if (
        selectedVersion.isPresent() &&
        selectedVersion.get().equals(version.getVersionNumber())
      ) {
