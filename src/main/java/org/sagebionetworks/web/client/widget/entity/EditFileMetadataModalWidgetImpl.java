...
public class EditFileMetadataModalWidgetImpl
  implements EditFileMetadataModalView.Presenter, EditFileMetadataModalWidget {

  public static final String FILE_NAME_MUST_INCLUDE_AT_LEAST_ONE_CHARACTER =
    "File name must include at least one character.";
  public static final String CURRENT_VERSION_ONLY_MESSAGE =
    "Metadata can only be modified on the most current version of the file.";
  ...
  @Inject
  public EditFileMetadataModalWidgetImpl(
    EditFileMetadataModalView view,
    SynapseClientAsync synapseClient,
    SynapseJavascriptClient jsClient
  ) {
  ...
  private void updateFileEntityFileHandle() {
    ...
    synapseClient.updateFileEntity(
      fileEntity,
      copyRequest,
  ...
  public void onPrimary() {
    if (getEntityNameFromView() == null) {
      view.showError(
        RenameEntityModalWidgetImpl.NAME_MUST_INCLUDE_AT_LEAST_ONE_CHARACTER
      );
  ...
  private boolean isFileHandleChange() {
    return (
      !this.fileHandle.getFileName().equals(getFileNameFromView()) ||
      !Objects.equal(
        this.fileHandle.getContentType(),
        getFileContentTypeFromView()
      )
    );
  }
