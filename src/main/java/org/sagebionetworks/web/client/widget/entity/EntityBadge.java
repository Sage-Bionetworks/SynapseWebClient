...
public class EntityBadge implements SynapseWidgetPresenter, EntityBadgeView.Presenter {
	...
	@Inject
	public EntityBadge(EntityBadgeView view, GlobalApplicationState globalAppState,
			AnnotationTransformer transformer, SynapseJavascriptClient jsClient,
			LazyLoadHelper lazyLoadHelper, PopupUtilsView popupUtils, SynapseProperties synapseProperties,
			EventBus eventBus, AuthenticationController authController, SynapseJSNIUtils jsniUtils) {
		this.view = view;
	...
	public void setEntityBundle(EntityBundle eb) {
		...
		if (PublicPrivateBadge.isPublic(eb.getBenefactorAcl(),
	...
	public void onAddToDownloadList() {
		...
		// to download list.
		jsClient.addFileToDownloadList(dataFileHandleId, entityHeader.getId(),
				new AsyncCallback<DownloadList>() {
					...
					public void onSuccess(DownloadList result) {
						...
						popupUtils.showInfo(entityHeader.getName() + EntityBadge.ADDED_TO_DOWNLOAD_LIST, href,
								VIEW_DOWNLOAD_LIST, IconType.CHECK_CIRCLE);
