...
public class EntityMetadata implements Presenter {
	...
	@Inject
	public EntityMetadata(EntityMetadataView view, DoiWidgetV2 doiWidgetV2,
			AnnotationsRendererWidget annotationsWidget, SynapseJavascriptClient jsClient,
			SynapseJSNIUtils jsni, RestrictionWidget restrictionWidgetV2,
			ContainerItemCountWidget containerItemCountWidget, PortalGinInjector ginInjector) {
		this.view = view;
	...
	public void configure(EntityBundle bundle, Long versionNumber, ActionMenuWidget actionMenu) {
		...
		if (EntityActionControllerImpl.isVersionSupported(bundle.getEntity(),
			...
			view.setRestrictionPanelVisible(
					en instanceof TableEntity || en instanceof Folder || en instanceof DockerRepository);
		}
	...
	public void configureStorageLocation(Entity en) {
		...
		if (en instanceof Folder || en instanceof Project) {
			...
			jsClient.getUploadDestinations(containerEntityId,
						...
						public void onSuccess(List<UploadDestination> uploadDestinations) {
							if (uploadDestinations == null || uploadDestinations.isEmpty()
								...
								ExternalGoogleCloudUploadDestination externalUploadDestination =
										(ExternalGoogleCloudUploadDestination) uploadDestinations.get(0);
