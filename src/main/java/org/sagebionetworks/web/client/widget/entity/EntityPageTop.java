...
public class EntityPageTop implements SynapseWidgetPresenter, IsWidget {
	...
	private EntityBundle currentTargetEntityBundle, projectBundle, filesEntityBundle,
			tablesEntityBundle, dockerEntityBundle;
	...
	@Inject
	public EntityPageTop(EntityPageTopView view, SynapseClientAsync synapseClient, Tabs tabs,
			EntityMetadata projectMetadata, WikiTab wikiTab, FilesTab filesTab, TablesTab tablesTab,
			ChallengeTab adminTab, DiscussionTab discussionTab, DockerTab dockerTab,
			EntityActionController projectActionController, ActionMenuWidget projectActionMenu,
			EntityActionController entityActionController, ActionMenuWidget entityActionMenu,
			CookieProvider cookies, SynapseJavascriptClient synapseJavascriptClient,
			GlobalApplicationState globalAppState, EntityId2BundleCache entityId2BundleCache,
			EventBus eventBus) {
		this.view = view;
		...
		if (entity.getId().equals(place.getEntityId())) {
			...
			if (bundle1 != null && bundle2 != null) {
				...
				if (projectHeader1 != null && projectHeader1.equals(projectHeader2)) {
					// skip full reload - use the same logic as clicking on an entity in the same tab (in the
					// same project)
	...
	private void initTabs() {
		...
		filesTab.setTabClickedCallback(tab -> {
			...
			configureFilesTab();
			projectMetadata
					.setVisible(projectBundle != null && filesEntityBundle.getEntity() instanceof Project);
		});
		tablesTab.setTabClickedCallback(tab -> {
			...
			configureTablesTab();
			projectMetadata
					.setVisible(projectBundle != null && tablesEntityBundle.getEntity() instanceof Project);
		});
		dockerTab.setTabClickedCallback(tab -> {
			...
			configureDockerTab();
			projectMetadata
					.setVisible(projectBundle != null && dockerEntityBundle.getEntity() instanceof Project);
		});
	}
	...
	public void configure(EntityBundle targetEntityBundle, Long versionNumber,
			EntityHeader projectHeader, Synapse.EntityArea initArea, String areaToken) {
	...
	public void initDefaultTabPlaces() {
		...
		if (projectHeader != null) {
			...
			wikiTab.asTab().setEntityNameAndPlace(projectName,
					new Synapse(projectId, versionNumber, EntityArea.WIKI, wikiAreaToken));
			filesTab.asTab().setEntityNameAndPlace(projectName,
					new Synapse(projectId, versionNumber, EntityArea.FILES, null));
			tablesTab.asTab().setEntityNameAndPlace(projectName,
					new Synapse(projectId, versionNumber, EntityArea.TABLES, tablesAreaToken));
			adminTab.asTab().setEntityNameAndPlace(projectName,
					new Synapse(projectId, versionNumber, EntityArea.CHALLENGE, null));
			discussionTab.asTab().setEntityNameAndPlace(projectName,
					new Synapse(projectId, versionNumber, EntityArea.DISCUSSION, discussionAreaToken));
			dockerTab.asTab().setEntityNameAndPlace(projectName,
					new Synapse(projectId, versionNumber, EntityArea.DOCKER, dockerAreaToken));
		}
	}
	...
	public void configureEntity(String entityId, final Long version) {
		...
		if (entityId != null && projectBundle != null
			...
			if (version == null) {
				...
				synapseJavascriptClient.getEntityBundleForVersion(entityId, version, ALL_PARTS_REQUEST,
						callback);
			}
		}
	}
	...
	public void showSelectedTabs() {
		...
		if (projectBundle == null || projectBundle.getPermissions() == null
			...
			synapseJavascriptClient.isWiki(projectHeader.getId(),
					getTabVisibilityCallback(EntityArea.WIKI, wikiTab.asTab()));
			synapseJavascriptClient.isFileOrFolder(projectHeader.getId(),
					getTabVisibilityCallback(EntityArea.FILES, filesTab.asTab()));
			synapseJavascriptClient.isTable(projectHeader.getId(),
					getTabVisibilityCallback(EntityArea.TABLES, tablesTab.asTab()));
			synapseJavascriptClient.isDocker(projectHeader.getId(),
					getTabVisibilityCallback(EntityArea.DOCKER, dockerTab.asTab()));
		}
		synapseClient.isChallenge(projectHeader.getId(),
				getTabVisibilityCallback(EntityArea.CHALLENGE, adminTab.asTab()));
	}

	public AsyncCallback<Boolean> getTabVisibilityCallback(final EntityArea entityArea,
			final Tab tab) {
	...
	private EntityArea getAreaForEntity(Entity entity) {
		...
		if (entity instanceof Project) {
			...
			area = EntityArea.DOCKER;
		} else { // if (entity instanceof FileEntity || entity instanceof Folder, or any other entity
							// type)
			area = EntityArea.FILES;
		}
		return area;
	...
	public void configureTablesTab() {
		...
		entityActionController.configure(entityActionMenu, tablesEntityBundle, isCurrentVersion, null,
				area);
	}
	...
	public void configureFilesTab() {
		...
		entityActionController.configure(entityActionMenu, filesEntityBundle, isCurrentVersion, null,
				area);
	}
	...
	public void configureWikiTab() {
		if (wikiTab.asTab().isContentStale()) {
			...
			final WikiPageWidget.Callback callback = new WikiPageWidget.Callback() {
				...
				public void noWikiFound() {
					if (isWikiTabShown && projectBundle.getRootWikiId() != null
						...
						// attempted to load a wiki, but it was not found. Show a message, and redirect to the
						// root.
						view.showInfo(
								"Wiki not found (id=" + wikiAreaToken + "), loading root wiki page instead.");
						wikiTab.asTab().setContentStale(true);
			...
			wikiTab.configure(projectHeader.getId(), projectHeader.getName(), wikiId, canEdit, callback,
					entityActionMenu);
			if (isWikiTabShown) {
			...
			CallbackP<String> wikiReloadHandler = new CallbackP<String>() {
				...
				public void invoke(String wikiPageId) {
					...
					// update the tab link. Note that WikiSubpageNavigationTree will push the link into the
					// history.
					Entity project = projectBundle.getEntity();
					wikiTab.asTab().setEntityNameAndPlace(project.getName(),
							new Synapse(project.getId(), null, EntityArea.WIKI, wikiPageId));
		...
		// on configure of wiki tab, always update the entity action controller with the correct wiki
		// page
