 ...
 *
 */
public class EntitySearchBox
  implements EntitySearchBoxView.Presenter, IsWidget {
  ...
  public void setEntitySelectedHandler(
    EntitySelectedHandler handler,
    boolean retrieveVersions
  ) {
  ...
  public void setSelectedSuggestion(EntitySearchBoxSuggestion suggestion) {
    ...
    if (suggestion != null) entitySelected(
      selectedSuggestion.getHit().getId(),
      selectedSuggestion.getHit().getName()
    );
  }
    ...
    if (handler != null) {
      if (retrieveVersions) {
        jsClient.getEntityVersions(
          entityId,
          WebConstants.ZERO_OFFSET.intValue(),
          20,
