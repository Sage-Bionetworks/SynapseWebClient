...
public class FileMetadataViewImpl extends Composite implements FileMetadataView {
	...
	interface FileMetadataViewImplUiBinder extends UiBinder<Widget, FileMetadataViewImpl> {
	}

	private static FileMetadataViewImplUiBinder uiBinder = GWT
			.create(FileMetadataViewImplUiBinder.class);
	...
	@Inject
	public FileMetadataViewImpl(IconsImageBundle iconsImageBundle,
		...
		fileHistory.setLayout(new FlowLayout(10));
	}

	@Override
	public void setEntityBundle(EntityBundle bundle, Long highlightVersionNumber) {
		clear();
		...
		Long currentVersionNumber = vb.getVersionNumber(); //doi to the current version
		favoriteWidget.configure(bundle.getEntity().getId());
		
		//doi widget
		doiWidget.configure(bundle.getEntity().getId(), bundle.getPermissions().getCanEdit(), currentVersionNumber);
	}
	...
	public void setPreviousVersions(ContentPanel versions) {
		fileHistory.add(versions);
		fileHistory.layout(true);
	}
						...
						new AsyncCallback<PaginatedResults<VersionInfo>>() {
							...
							public void onSuccess(
								...
								for (VersionInfo version : result.getResults()) {
									...
									model.set(
											FileMetadataViewImpl.VERSION_KEY_ID,
											version.getId());
									model.set(
											FileMetadataViewImpl.VERSION_KEY_NUMBER,
											version.getVersionNumber());
									model.set(
											FileMetadataViewImpl.VERSION_KEY_LABEL,
											version.getVersionLabel());
									model.set(
											FileMetadataViewImpl.VERSION_KEY_COMMENT,
											version.getVersionComment());
									model.set(
											FileMetadataViewImpl.VERSION_KEY_MOD_ON,
											version.getModifiedOn());
									model.set(
											FileMetadataViewImpl.VERSION_KEY_MOD_BY,
