...
public class MarkdownEditorWidget
  ...
  public void configureStep2() {
    getFormattingGuide()
      .loadMarkdownFromWikiPage(formattingGuideWikiPageKey, true);
    ...
    gwt.scheduleExecution(
      new Callback() {
        ...
        public void invoke() {
          resizeMarkdownTextArea();
          if (view.isEditorAttachedAndVisible()) gwt.scheduleExecution(
            this,
            500
          );
        }
  ...
  public void insertMarkdown(String md) {
    ...
    if (cursorPos < 0 || cursorPos > currentValue.length()) cursorPos =
      currentValue.length();
    view.setMarkdown(
  ...
  public void handleCommand(MarkdownEditorAction action) {
    ...
    switch (action) {
      ...
      case INSERT_TOC:
        insertMarkdown(
          WIDGET_START_MARKDOWN + TOC_CONTENT_TYPE + WIDGET_END_MARKDOWN
        );
        break;
      ...
      case INSERT_WIKI_FILES_PREVIEW:
        insertMarkdown(
          WIDGET_START_MARKDOWN +
          WIKI_FILES_PREVIEW_CONTENT_TYPE +
          WIDGET_END_MARKDOWN
        );
        break;
      case INSERT_TUTORIAL_WIZARD:
        insertMarkdown(
          WIDGET_START_MARKDOWN +
          TUTORIAL_WIZARD_CONTENT_TYPE +
          "?" +
          WIDGET_ENTITY_ID_KEY +
          "=syn123&" +
          TEXT_KEY +
          "=Tutorial" +
          WIDGET_END_MARKDOWN
      ...
      case INSERT_INTENDED_DATA_USE_REPORT:
        insertMarkdown(
          WIDGET_START_MARKDOWN +
          IDU_REPORT_CONTENT_TYPE +
          "?" +
          IntendedDataUseReportWidget.ACCESS_RESTRICTION_ID +
          "=12345" +
          WIDGET_END_MARKDOWN
        );
        break;
      case INSERT_REGISTER_CHALLENGE_TEAM:
        insertMarkdown(
          WIDGET_START_MARKDOWN +
          REGISTER_CHALLENGE_TEAM_CONTENT_TYPE +
          "?" +
          CHALLENGE_ID_KEY +
          "=123&" +
          BUTTON_TEXT_KEY +
      ...
      case INSERT_CHALLENGE_PARTICIPANTS:
        insertMarkdown(
          WIDGET_START_MARKDOWN +
          CHALLENGE_PARTICIPANTS_CONTENT_TYPE +
          "?" +
          CHALLENGE_ID_KEY +
          "=123&" +
          IS_IN_CHALLENGE_TEAM_KEY +
          "=false" +
          WIDGET_END_MARKDOWN
        );
        break;
  ...
  public void surroundWithTag(
    ...
    String newText = DisplayUtils.surroundText(
      text,
      startTag,
      endTag,
      isMultiline,
    ...
    if (widgetSelectionState.isWidgetSelected()) {
      ...
      WidgetDescriptorUpdatedHandler handler = event -> {
        ...
        if (widgetStartIndex > -1 && widgetEndIndex > -1) {
          view.setMarkdown(
            text.substring(0, widgetStartIndex) + text.substring(widgetEndIndex)
          );
      ...
      widgetDescriptorEditor.addWidgetDescriptorUpdatedHandler(handler);
      widgetDescriptorEditor.editExisting(
        wikiKey,
        contentTypeKey,
        widgetDescriptor
      );
  ...
  public void addFileHandles(List<String> fileHandleIds) {
    ...
    if (
      fileHandleIds != null &&
      fileHandleIds.size() > 0 &&
      filesAddedCallback != null
    ) {
