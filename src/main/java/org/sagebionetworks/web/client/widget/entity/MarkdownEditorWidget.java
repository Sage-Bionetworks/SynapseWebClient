 ...
 *
 */
public class MarkdownEditorWidget
		implements MarkdownEditorWidgetView.Presenter, SynapseWidgetPresenter {
	...
	@Inject
	public MarkdownEditorWidget(MarkdownEditorWidgetView view, SynapseClientAsync synapseClient,
			CookieProvider cookies, GWTWrapper gwt,
			BaseEditWidgetDescriptorPresenter widgetDescriptorEditor, WidgetRegistrar widgetRegistrar,
			UserTeamSelector userTeamSelector, PortalGinInjector ginInjector) {
		super();
		this.view = view;
	...
	public void markdownEditorClicked() {
		...
		DisplayUtils.updateWidgetSelectionState(widgetSelectionState, view.getMarkdown(),
				view.getCursorPos());
	...
	public void handleCommand(MarkdownEditorAction action) {
		...
		switch (action) {
			...
			case INSERT_WIKI_FILES_PREVIEW:
				insertMarkdown(
						WIDGET_START_MARKDOWN + WIKI_FILES_PREVIEW_CONTENT_TYPE + WIDGET_END_MARKDOWN);
				break;
			case INSERT_TUTORIAL_WIZARD:
				insertMarkdown(WIDGET_START_MARKDOWN + TUTORIAL_WIZARD_CONTENT_TYPE + "?"
						+ WIDGET_ENTITY_ID_KEY + "=syn123&" + TEXT_KEY + "=Tutorial" + WIDGET_END_MARKDOWN);
				break;
			case INSERT_REGISTER_CHALLENGE_TEAM:
				insertMarkdown(
						WIDGET_START_MARKDOWN + REGISTER_CHALLENGE_TEAM_CONTENT_TYPE + "?" + CHALLENGE_ID_KEY
								+ "=123&" + BUTTON_TEXT_KEY + "=Register team" + WIDGET_END_MARKDOWN);
				break;
			case INSERT_CHALLENGE_TEAMS:
				insertMarkdown(WIDGET_START_MARKDOWN + CHALLENGE_TEAMS_CONTENT_TYPE + "?" + CHALLENGE_ID_KEY
						+ "=123" + WIDGET_END_MARKDOWN);
				break;
			case INSERT_CHALLENGE_PARTICIPANTS:
				insertMarkdown(
						WIDGET_START_MARKDOWN + CHALLENGE_PARTICIPANTS_CONTENT_TYPE + "?" + CHALLENGE_ID_KEY
								+ "=123&" + IS_IN_CHALLENGE_TEAM_KEY + "=false" + WIDGET_END_MARKDOWN);
				break;
	...
	public void surroundWithTag(String startTag, String endTag, boolean isMultiline) {
		...
		String newText =
				DisplayUtils.surroundText(text, startTag, endTag, isMultiline, currentPos, selectionLength);
	...
	public void onKeyPress(KeyPressEvent event) {
		if ('@' == event.getCharCode()) {
			// only insert a user link if this is the first character, or the character before it is
			// whitespace
	...
	public void insertUserLink() {
		// pop up suggest box. on selection, userSelector has been configured to add the username to the
		// md.
		insertMarkdown("@");
