...
public class MarkdownWidget implements MarkdownWidgetView.Presenter, IsWidget {
	...
	@Inject
	public MarkdownWidget(SynapseJavascriptClient jsClient, SynapseJSNIUtils synapseJSNIUtils,
			WidgetRegistrar widgetRegistrar, CookieProvider cookies, ResourceLoader resourceLoader,
			GWTWrapper gwt, PortalGinInjector ginInjector, MarkdownWidgetView view, SynapseAlert synAlert,
	...
	public void loadMath(String suffix) {
		ClientProperties.fixResourceToCdnEndpoint(ClientProperties.MATH_PROCESSOR_JS,
	...
	public Set<String> loadWidgets(WikiPageKey wikiKey, Long wikiVersionInView, String suffix) {
		...
		String currentWidgetDiv =
				org.sagebionetworks.markdown.constants.WidgetConstants.DIV_ID_WIDGET_PREFIX + i + suffix;
		ElementWrapper el = view.getElementById(currentWidgetDiv);
		while (el != null) {
			...
			if (innerText != null) {
				try {
					...
					IsWidget presenter = widgetRegistrar.getWidgetRendererForWidgetDescriptor(wikiKey,
							contentType, widgetDescriptor, new Callback() {
								@Override
								public void invoke() {
									refresh();
								}
					...
					view.addWidget(new HTMLPanel(DisplayUtils.getMarkdownWidgetWarningHtml(message)),
							currentWidgetDiv);
				}
			}
	...
	public void loadMarkdownFromWikiPage(final WikiPageKey wikiKey,
			final boolean isIgnoreLoadingFailure) {
		synAlert.clear();
		// get the wiki page
