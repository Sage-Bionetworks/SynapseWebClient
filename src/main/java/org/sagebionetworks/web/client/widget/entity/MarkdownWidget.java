...
public class MarkdownWidget implements MarkdownWidgetView.Presenter, IsWidget {
  ...
  public void configure(
    ...
    final String uniqueSuffix =
      "-" + new Date().getTime() + "" + gwt.nextRandomInt();
    view.setLoadingVisible(false);
    view.callbackWhenAttached(
      new Callback() {
        @Override
        public void invoke() {
          try {
    ...
    while (el != null) {
      ...
      if (innerText != null) {
        try {
          ...
          view.addWidget(
            new HTMLPanel(DisplayUtils.getMarkdownWidgetWarningHtml(message)),
            currentWidgetDiv
          );
        }
      }
  ...
  public void loadMarkdownFromWikiPage(
    final WikiPageKey wikiKey,
    ...
    jsClient.getV2WikiPageAsV1(
      ...
      new AsyncCallback<WikiPage>() {
        ...
        public void onFailure(Throwable caught) {
          view.setLoadingVisible(false);
          if (!isIgnoreLoadingFailure) synAlert.showError(
