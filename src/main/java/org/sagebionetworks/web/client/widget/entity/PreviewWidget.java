...
public class PreviewWidget implements PreviewWidgetView.Presenter, WidgetRendererPresenter {
	...
	@Inject
	public PreviewWidget(PreviewWidgetView view, RequestBuilderWrapper requestBuilder,
			SynapseJSNIUtils synapseJSNIUtils, SynapseAlert synapseAlert,
			SynapseClientAsync synapseClient, AuthenticationController authController,
			SynapseJavascriptClient jsClient, PortalGinInjector ginInjector) {
		this.view = view;
	...
	public PreviewFileType getPreviewFileType(CloudProviderFileHandleInterface previewHandle,
		...
		if (originalFileHandle != null && originalFileHandle instanceof S3FileHandle) {
			...
			if (VideoConfigEditor.isRecognizedVideoFileName(fileName)) {
				...
				return PreviewFileType.CODE;
					...
					&& (fileName.toLowerCase().endsWith(".md") || fileName.toLowerCase().endsWith(".rmd"))) {
	...
	public void configure(String entityId, String version) {
		...
		if (EntityPresenter.isValidEntityId(entityId)) {
			if (version == null) {
				...
				jsClient.getEntityBundleForVersion(entityId, Long.parseLong(version), bundleRequest,
						entityBundleCallback);
			}
		} else {
			view.addSynapseAlertWidget(synapseAlert.asWidget());
			synapseAlert.showError(
					"Preview error: " + entityId + " does not appear to be a valid Synapse identifier.");
		}
	}
		...
		if (!synapseAlert.isUserLoggedIn()) {
			...
			if (bundle.getEntity() instanceof Link) {
				...
				String targetVersion =
						ref.getTargetVersionNumber() == null ? null : ref.getTargetVersionNumber() + "";
			...
			if (!(bundle.getEntity() instanceof FileEntity)) {
				// not a file!
				view.addSynapseAlertWidget(synapseAlert.asWidget());
				synapseAlert.showError("Preview unavailable for \"" + bundle.getEntity().getName() + "\" ("
	...
	private void renderFilePreview(EntityBundle bundle) {
		...
		if (originalFileHandlePreviewType != PreviewFileType.NONE) {
			...
			view.showNoPreviewAvailable(bundle.getEntity().getId(),
					((FileEntity) bundle.getEntity()).getVersionNumber());
		}
	}
	...
	private void renderFilePreview(PreviewFileType previewType, FileHandle fileHandleToShow) {
		...
		switch (previewType) {
			case IMAGE:
				// add a html panel that contains the image src from the attachments server (to pull
				// asynchronously)
				// create img
				String fullFileUrl =
						DisplayUtils.createFileEntityUrl(synapseJSNIUtils.getBaseFileHandleUrl(),
								fileEntity.getId(), ((Versionable) fileEntity).getVersionNumber(), false);
			...
			case VIDEO:
				...
				videoWidget.configure(bundle.getEntity().getId(), fileHandleToShow.getFileName(),
						VIDEO_WIDTH, VIDEO_HEIGHT);
				view.setPreviewWidget(videoWidget);
				break;
			case HTML:
				if (fileHandleToShow.getContentSize() != null
						&& fileHandleToShow.getContentSize() < MAX_HTML_FILE_SIZE) {
			...
			default:
				view.showLoading();
				boolean isGetPreviewFile =
						!fileHandleToShow.getId().equals(fileEntity.getDataFileHandleId());
				// must be a text type of some kind
				// try to load the text of the preview, if available
				jsClient.getFileEntityTemporaryUrlForVersion(fileEntity.getId(),
						((Versionable) fileEntity).getVersionNumber(), isGetPreviewFile,
						new AsyncCallback<String>() {
							...
							public void onSuccess(String url) {
								...
								// SWC-5014: Setting the request header "Content-Type" messes up the signed string
								// verification in Google storage files (causes a 403).
								// We must set another request header so RequestBuilder does not auto-set
								// "Content-Type" to "text/plain; charset=utf-8"
								// (see
								// http://www.gwtproject.org/javadoc/latest/com/google/gwt/http/client/RequestBuilder.html#send--)
								...
								try {
									requestBuilder.sendRequest(null, new RequestCallback() {
										...
										public void onResponseReceived(final Request request, final Response response) {
											// if this response is not for the file entity associated to this preview
											// widget, then ignore it!
											...
											if (statusCode > 199 && statusCode < 300) {
												...
												if (responseText != null && responseText.length() > 0) {
													...
													switch (previewType) {
														case CODE:
															String codePreview =
																	SafeHtmlUtils.htmlEscapeAllowEntities(responseText);
															String extension =
																	ContentTypeUtils.getExtension(fileHandleToShow.getFileName());
												...
												if (statusText == null || statusText.length() == 0) {
													statusText =
															"Unable to directly download a preview for " + fileEntity.getName();
												}
												synapseAlert
														.consoleError("Attempt to download \"" + fileHandleToShow.getFileName()
																+ "\" (file handle ID = " + fileHandleToShow.getId()
