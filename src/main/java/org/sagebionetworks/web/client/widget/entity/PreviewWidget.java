...
public class PreviewWidget
  implements PreviewWidgetView.Presenter, WidgetRendererPresenter {
  ...
  public PreviewFileType getPreviewFileType(
    CloudProviderFileHandleInterface previewHandle,
      ...
      if (contentType != null) {
        if (isRecognizedImageContentType(contentType)) {
          ...
          if (isCSV(contentType)) {
            if (
              APPLICATION_ZIP.equals(originalFileHandle.getContentType())
            ) previewFileType = PreviewFileType.ZIP; else previewFileType =
              PreviewFileType.CSV;
          } else if (isCSV(originalFileHandle.getContentType())) {
            previewFileType = PreviewFileType.CSV;
          } else if (
            isTAB(contentType) || isTAB(originalFileHandle.getContentType())
          ) {
  ...
  public PreviewFileType getOriginalFileType(FileHandle originalFileHandle) {
    if (
      originalFileHandle != null && originalFileHandle instanceof S3FileHandle
    ) {
      ...
      if (VideoConfigEditor.isRecognizedVideoFileName(fileName)) {
        ...
        contentType != null && isRecognizedImageContentType(contentType)
      ) {
        ...
        contentType != null && contentType.toLowerCase().equals("image/tiff")
      ) {
  ...
  public void configure(
    ...
    view.clear();
    String entityId = widgetDescriptor.get(
      WidgetConstants.WIDGET_ENTITY_ID_KEY
    );
    String version = widgetDescriptor.get(
      WidgetConstants.WIDGET_ENTITY_VERSION_KEY
    );
  ...
  public void configure(String entityId, String version) {
    ...
    if (EntityPresenter.isValidEntityId(entityId)) {
      ...
      synapseAlert.showError(
        "Preview error: " +
        entityId +
        " does not appear to be a valid Synapse identifier."
      );
    }
  }
  ...
  public void configure(EntityBundle bundle) {
    ...
    if (bundle != null) {
      ...
      if (bundle.getEntity() instanceof Link) {
        ...
        String targetVersion = ref.getTargetVersionNumber() == null
          ? null
      ...
      if (!(bundle.getEntity() instanceof FileEntity)) {
        ...
        if (
          DisplayUtils.getFileHandle(bundle) != null ||
          synapseAlert.isUserLoggedIn()
        ) {
  ...
  private void renderFilePreview(EntityBundle bundle) {
    ...
    if (originalFileHandlePreviewType != PreviewFileType.NONE) {
      ...
      PreviewFileType previewType = getPreviewFileType(
        previewFileHandle,
        originalFileHandle
      );
  ...
  private void renderFilePreview(
    ...
    switch (previewType) {
      case IMAGE:
        ...
        // create img
        String fullFileUrl = DisplayUtils.createFileEntityUrl(
          synapseJSNIUtils.getBaseFileHandleUrl(),
          fileEntity.getId(),
          ((Versionable) fileEntity).getVersionNumber(),
          false
        );
      ...
      default:
        view.showLoading();
        boolean isGetPreviewFile = !fileHandleToShow
          .getId()
          .equals(fileEntity.getDataFileHandleId());
        // must be a text type of some kind
        // try to load the text of the preview, if available
        jsClient.getFileEntityTemporaryUrlForVersion(
          fileEntity.getId(),
          ((Versionable) fileEntity).getVersionNumber(),
          isGetPreviewFile,
          new AsyncCallback<String>() {
            ...
            public void onSuccess(String url) {
              ...
              // SWC-5014: Setting the request header "Content-Type" messes up the signed string verification in
              // Google storage files (causes a 403).
              // We must set another request header so RequestBuilder does not auto-set "Content-Type" to
                    ...
                    public void onResponseReceived(
                      ...
                      if (statusCode > 199 && statusCode < 300) {
                        ...
                        if (statusText == null || statusText.length() == 0) {
                          statusText =
                            "Unable to directly download a preview for " +
                            fileEntity.getName();
                        }
