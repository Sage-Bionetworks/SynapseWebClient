  ...
  public void configure(ProjectHeader header) {
    ...
    if (header != null) {
      if (header.getLastActivity() != null) {
        try {
          String dateString = view.getSimpleDateString(
            header.getLastActivity()
          );
  ...
  public void updateTooltip(UserProfile modifiedBy) {
    ...
    if (header.getModifiedOn() != null) {
      ...
      map.put(
        "Modified On",
        gwt
          .getDateTimeFormat(PredefinedFormat.DATE_TIME_MEDIUM)
          .format(header.getModifiedOn())
      );
    }
