...
public class RegisterTeamDialog implements RegisterTeamDialogView.Presenter {
	...
	@Inject
	public RegisterTeamDialog(RegisterTeamDialogView view, ChallengeClientAsync challengeClient,
	...
	public void configure(String challengeId, Callback callback) {
		...
		view.setNewTeamLink("#!Profile:" + authenticationController.getCurrentUserPrincipalId()
	...
	public void refreshRegistratableTeams() {
		challengeClient.getRegistratableTeams(authenticationController.getCurrentUserPrincipalId(),
				challengeId, new AsyncCallback<List<Team>>() {
					@Override
					public void onSuccess(List<Team> result) {
						teams = result;
						// if there is a team, then select the first team by default. otherwise, show no teams
						// visible ui
					...
					public void onFailure(Throwable caught) {
						// note: call will result in a NotFoundException if the current user is not registered
						// for the challenge
