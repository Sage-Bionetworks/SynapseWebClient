...
public class RegisterTeamDialog implements RegisterTeamDialogView.Presenter {
  ...
  public void configure(String challengeId, Callback callback) {
    if (!authenticationController.isLoggedIn()) {
      view.showConfirmDialog(
        DisplayConstants.ANONYMOUS_JOIN_EVALUATION,
        getConfirmCallback()
      );
      return;
    }
  ...
  public void refreshRegistratableTeams() {
    challengeClient.getRegistratableTeams(
      authenticationController.getCurrentUserPrincipalId(),
      challengeId,
  ...
  public void onOk() {
    if (isValid()) {
      ...
      challengeClient.registerChallengeTeam(
        challengeTeam,
        new AsyncCallback<ChallengeTeam>() {
          @Override
          public void onSuccess(ChallengeTeam result) {
            view.showInfo(
              "Successfully registered your team for the challenge."
            );
            if (callback != null) {
              callback.invoke();
            }
