...
public class SharingAndDataUseConditionWidgetViewImpl extends FlowPanel
	...
	public SharingAndDataUseConditionWidgetViewImpl(SynapseJSNIUtils synapseJSNIUtils,
			GlobalApplicationState globalApplicationState, IconsImageBundle iconsImageBundle,
			PublicPrivateBadge publicPrivateBadge,
			AccessControlListModalWidget accessControlListModalWidget,
			RestrictionWidget restrictionWidgetV2, CookieProvider cookies) {
	...
	public void configure(EntityBundle bundle) {
		...
		container.add(new InlineHTML(
				"<h5 class=\"inline-block\">" + DisplayConstants.SHARING_PUBLIC_TITLE + "</h5>"));
		...
		publicPrivateBadge.configure(bundle.getEntity(), new AsyncCallback<Boolean>() {
			...
			public void onSuccess(Boolean isPublic) {
				...
				String description = isPublic ? DisplayConstants.SHARING_PUBLIC_DESCRIPTION
						: DisplayConstants.SHARING_PRIVATE_DESCRIPTION;
				sharingDescriptionContainer.setWidget(
						new HTML("<p class=\"margin-left-20 nobottommargin\">" + description + "</p>"));
			}
		...
		container
				.add(new InlineHTML("<h5 class=\"inline-block\">" + DisplayConstants.DATA_USE + "</h5>"));
		restrictionWidgetV2.setShowChangeLink(false);
		restrictionWidgetV2.configure(bundle.getEntity(),
