...
public class TutorialWizard
  implements TutorialWizardView.Presenter, WidgetRendererPresenter {
      ...
      new AsyncCallback<List<V2WikiHeader>>() {
        ...
        public void onSuccess(List<V2WikiHeader> wikiHeaders) {
          ...
          for (V2WikiHeader header : wikiHeaders) {
            ...
            if (header.getParentId() != null) sortedHeaders.add(header);
          }
          if (sortedHeaders.size() == 0) {
            onFailure(
              new NotFoundException(
                "Wiki subpages not found for tutorial: " + entityId
              )
            );
            return;
          }
