...
public class VersionHistoryWidget implements VersionHistoryWidgetView.Presenter, IsWidget {
	...
	@Inject
	public VersionHistoryWidget(VersionHistoryWidgetView view, SynapseClientAsync synapseClient,
			SynapseJavascriptClient jsClient, GlobalApplicationState globalApplicationState,
			PreflightController preflightController, SynapseAlert synAlert) {
		super();
	...
	public void deleteVersion(final Long versionNumber) {
		synAlert.clear();
		synapseClient.deleteEntityVersionById(bundle.getEntity().getId(), versionNumber,
				new AsyncCallback<Void>() {
					...
					public void onSuccess(Void result) {
						view.showInfo("Version " + versionNumber + " of " + bundle.getEntity().getId() + " "
								+ DisplayConstants.LABEL_DELETED);
	...
	public void gotoCurrentVersion() {
		...
		Synapse synapse =
				new Synapse(bundle.getEntity().getId(), targetVersion, EntityArea.FILES, null);
		globalApplicationState.getPlaceChanger().goTo(synapse);
	}

	public void onMore() {
		currentRequest = jsClient.getEntityVersions(bundle.getEntity().getId(), currentOffset,
				VERSION_LIMIT, new AsyncCallback<List<VersionInfo>>() {
					@Override
					public void onSuccess(List<VersionInfo> results) {
						...
						if (currentOffset == 0) {
							...
							view.setEditVersionInfoButtonVisible(
									isCurrentVersion && canEdit && !(bundle.getEntity() instanceof Table));
							if (results.size() == 0) {
								view.showNoResults();
							}
						}
						...
						for (VersionInfo versionInfo : results) {
							view.addVersion(bundle.getEntity().getId(), versionInfo, canEdit,
									versionInfo.getVersionNumber().equals(versionNumber));
						}
