...
public class WikiHistoryWidget
		implements WikiHistoryWidgetView.Presenter, SynapseWidgetPresenter, IsWidget {
	...
	public WikiHistoryWidget(GlobalApplicationState globalApplicationState,
			WikiHistoryWidgetView view, SynapseClientAsync synapseClient,
	...
	@Override
	public void configure(final WikiPageKey key, final boolean canEdit,
	...
	public void configureNextPage(final Long offset, final Long limit) {
		synAlert.clear();
		synapseClient.getV2WikiHistory(wikiKey, limit, offset,
				new AsyncCallback<PaginatedResults<V2WikiHistorySnapshot>>() {
					@Override
					public void onFailure(Throwable caught) {
						if (caught.getMessage() != null
								&& caught.getMessage().contains(NO_HISTORY_IS_FOUND_FOR_A_WIKI)) {
					...
					public void onSuccess(PaginatedResults<V2WikiHistorySnapshot> result) {
						...
						if (historyAsListOfHeaders == null || historyAsListOfHeaders.isEmpty()) {
							...
							for (int i = 0; i < historyAsListOfHeaders.size(); i++) {
								...
								if (mapIdToName != null && !idsToSearch.contains(modifiedById)
										&& !modifiedById.trim().isEmpty()) {
									idsToSearch.add(modifiedById);
								}
							}
							// Call to get user headers from the list of needed ids
							synAlert.clear();
							jsClient.getUserGroupHeadersById(idsToSearch,
									new AsyncCallback<UserGroupHeaderResponsePage>() {
										...
										public void onSuccess(UserGroupHeaderResponsePage response) {
											...
											for (int i = 0; i < headers.size(); i++) {
												mapIdToName.put(idsToSearch.get(i),
														DisplayUtils.getDisplayName(headers.get(i)));
											}
