...
public class WikiMarkdownEditor implements IsWidget, WikiMarkdownEditorView.Presenter {
	...
	@Inject
	public WikiMarkdownEditor(WikiMarkdownEditorView view, MarkdownEditorWidget editor,
			SynapseClientAsync synapseClient, GlobalApplicationState globalApplicationState,
			SynapseJavascriptClient jsClient, PortalGinInjector ginInjector, PopupUtilsView popupUtils) {
		this.view = view;
		this.editor = editor;
	...
	public void initWikiPage() {
		...
		jsClient.getV2WikiPageAsV1(wikiKey, new AsyncCallback<WikiPage>() {
			...
			public void onFailure(Throwable caught) {
				// if it is because of a missing root (and we have edit permission), then the pages browser
				// should have a Create Wiki button
	...
	public void createNewWiki() {
		...
		synapseClient.createV2WikiPageWithV1(wikiKey.getOwnerObjectId(), wikiKey.getOwnerObjectType(),
				page, new AsyncCallback<WikiPage>() {
					@Override
					public void onFailure(Throwable caught) {
	...
	public void saveClicked() {
		...
		synapseClient.updateV2WikiPageWithV1(wikiKey.getOwnerObjectId(), wikiKey.getOwnerObjectType(),
				currentPage, new AsyncCallback<WikiPage>() {
					@Override
					public void onSuccess(WikiPage result) {
	...
	public void cancelAfterConfirm() {
		globalApplicationState.setIsEditing(false);
		view.hideEditorModal();
		// TODO: update should not be necessary, but widget loading is based on div ids that are
		// overloaded when the formatting guide is initialized
		if (wikiPageUpdatedHandler != null)
	...
	public void deleteClicked() {
		getWikiPageDeleteConfirmationDialog().show(wikiKey, parentWikiId -> {
			...
			globalApplicationState.getPlaceChanger()
					.goTo(new Synapse(wikiKey.getOwnerObjectId(), null, EntityArea.WIKI, parentWikiId));
		});
	}
