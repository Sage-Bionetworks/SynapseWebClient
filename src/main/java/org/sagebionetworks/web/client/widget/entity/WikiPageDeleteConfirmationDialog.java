...
public class WikiPageDeleteConfirmationDialog
	...
	public static final String DELETE_WIKI_PAGE_CONFIRMATION_MESSAGE =
			EntityActionControllerImpl.ARE_YOU_SURE_YOU_WANT_TO_DELETE + "this wiki page?";
	...
	public void onDeleteWikiGetHeaderTree(WikiPageKey key) {
		jsClient.getV2WikiHeaderTree(key.getOwnerObjectId(), key.getOwnerObjectType(),
				new AsyncCallback<List<V2WikiHeader>>() {
					...
					public void onSuccess(List<V2WikiHeader> wikiHeaders) {
						if (wikiHeaders.size() == 1) {
							parentWikiPageId = null; // if there's a single wiki in the tree, then the parent wiki
																				// must be null
