...
public class WikiPageDeleteConfirmationDialog
  implements WikiPageDeleteConfirmationDialogView.Presenter, IsWidget {

  public static final String DELETE_WIKI_PAGE_CONFIRMATION_MESSAGE =
    EntityActionControllerImpl.ARE_YOU_SURE_YOU_WANT_TO_DELETE +
    "this wiki page?";
  ...
  @Inject
  public WikiPageDeleteConfirmationDialog(
    WikiPageDeleteConfirmationDialogView view,
      ...
      new AsyncCallback<List<V2WikiHeader>>() {
        ...
        public void onSuccess(List<V2WikiHeader> wikiHeaders) {
          if (wikiHeaders.size() == 1) {
            ...
            popupUtilsView.showConfirmDelete(
              DELETE_WIKI_PAGE_CONFIRMATION_MESSAGE,
              () -> {
                onDeleteWiki();
              }
            );
          } else {
            Map<String, V2WikiHeader> wikiHeaderMap = getWikiHeaderMap(
              wikiHeaders
            );
            parentWikiPageId =
              wikiHeaderMap.get(key.getWikiPageId()).getParentId();
            view.showModal(
              key.getWikiPageId(),
              wikiHeaderMap,
              getWikiChildrenMap(wikiHeaders)
            );
          }
        }
      }
  ...
  public Map<String, V2WikiHeader> getWikiHeaderMap(
  ...
  public Map<String, List<V2WikiHeader>> getWikiChildrenMap(
    List<V2WikiHeader> wikiHeaders
  ) {
    Map<String, List<V2WikiHeader>> id2Children = new HashMap<>();
    for (V2WikiHeader v2WikiHeader : wikiHeaders) {
      if (v2WikiHeader.getParentId() != null) {
        List<V2WikiHeader> children = id2Children.get(
          v2WikiHeader.getParentId()
        );
        if (children == null) {
