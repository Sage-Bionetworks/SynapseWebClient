...
public class WikiPageWidget implements WikiPageWidgetView.Presenter, SynapseWidgetPresenter {
	...
	@Inject
	public WikiPageWidget(WikiPageWidgetView view, SynapseClientAsync synapseClient,
			StuAlert stuAlert, WikiHistoryWidget historyWidget, MarkdownWidget markdownWidget,
			WikiSubpagesWidget wikiSubpages, PortalGinInjector ginInjector, SessionStorage sessionStorage,
			AuthenticationController authController, AdapterFactory adapterFactory,
			DateTimeUtils dateTimeUtils, SynapseJavascriptClient jsClient, CookieProvider cookies,
			EventBus eventBus) {
		this.view = view;
	...
	public String getSessionCacheKey(WikiPageKey wikiKey) {
		...
		return currentUserId + "_" + wikiKey.getOwnerObjectId() + "_" + wikiKey.getOwnerObjectType()
				+ "_" + wikiKey.getWikiPageId() + WebConstants.WIKIPAGE_SUFFIX;
	}
	...
	public void showRestoreWarning(final Long versionToRestore) {
		...
		view.showPopup(DisplayConstants.RESTORING_WIKI_VERSION_WARNING_TITLE,
				DisplayConstants.RESTORING_WIKI_VERSION_WARNING_MESSAGE, MessagePopup.WARNING, okCallback,
				cancelCallback);
	}
	...
	public void restoreConfirmed() {
		...
		synapseClient.restoreV2WikiPage(wikiKey.getOwnerObjectId(), wikiKey.getOwnerObjectType(),
				wikiKey.getWikiPageId(), versionInView, new AsyncCallback<V2WikiPage>() {
					@Override
