...
public class AnnotationTransformerImpl implements AnnotationTransformer {
  ...
  public AnnotationTransformerImpl(GWTWrapper gwt) {
    standardFormatter =
      gwt.getDateTimeFormat(PredefinedFormat.DATE_TIME_MEDIUM);
  }
  ...
  public String getFriendlyValues(AnnotationsValue annotation) {
    ...
    boolean isDate = AnnotationsValueType.TIMESTAMP_MS.equals(
      annotation.getType()
    );
    for (String value : values) {
      ...
      isAfterFirst = true;
      if (isDate) value = friendlyDate(value);
  ...
  public String friendlyDate(String value) {
    return value == null
      ? null
      : standardFormatter.format(new Date(Long.parseLong(value)));
  }
}
