...
public enum EntityFilter {
	ALL(project, folder, file, link), CONTAINER(project, folder), PROJECT(project), FOLDER(
			folder), FILE(
					file), ALL_BUT_LINK(project, folder, file), PROJECT_OR_TABLE(project, table, entityview);
	...
	private EntityFilter(EntityType... values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			entityTypeClassNamesSet
					.add(EntityTypeUtils.getEntityClassNameForEntityType(values[i].name()));
		}
	}
