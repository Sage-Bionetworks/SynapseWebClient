...
public class EntityFinderImpl implements EntityFinder, EntityFinderView.Presenter, IsWidget {
	...
	@Inject
	public EntityFinderImpl(EntityFinderView view, GlobalApplicationState globalApplicationState, AuthenticationController authenticationController, ClientCache cache, SynapseAlert synAlert, SynapseJavascriptClient jsClient) {
		this.view = view;
	...
	@Override
	public void configure(EntityFilter viewable, EntityFilter selectable, boolean showVersions, SelectedHandler<Reference> handler) {
		// Setting the 'viewable' scope is not possible in this implementation.
		// This method just exists on the interface to simplify migration to EntityFinderV2
		configure(selectable, showVersions, handler);
	}
	...
	@Override
	public void configureMulti(EntityFilter viewable, EntityFilter selectable, boolean showVersions, SelectedHandler<List<Reference>> handler) {
		// Setting the 'viewable' scope is not possible in this implementation.
		// This method just exists on the interface to simplify migration to EntityFinderV2
		configureMulti(selectable, showVersions, handler);
	}
