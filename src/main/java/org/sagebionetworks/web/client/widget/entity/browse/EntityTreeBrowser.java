...
public class EntityTreeBrowser implements EntityTreeBrowserView.Presenter, SynapseWidgetPresenter {
	...
	@Inject
	public EntityTreeBrowser(PortalGinInjector ginInjector, EntityTreeBrowserView view,
			SynapseJavascriptClient synapseClient, IconsImageBundle iconsImageBundle,
			AdapterFactory adapterFactory, SynapseAlert synAlert) {
		this.view = view;
	...
	public void configure(List<EntityHeader> headers) {
		...
		// if we are only configuring with a single entity, then we can show the sorting UI for the
		// child queries
	}
	...
	public void addHeaders(List<EntityHeader> headers) {
		...
		for (EntityHeader header : headers) {
			...
			view.appendRootEntityTreeItem(
					makeTreeItemFromQueryResult(header, true, isExpandable(header)));
		}
	}
	...
	@Override
	public void getChildren(final String parentId, final EntityTreeItem parent,
				...
				jsClient.getEntityChildren(request, new AsyncCallback<EntityChildrenResponse>() {
					...
					public void onSuccess(EntityChildrenResponse results) {
						...
						if (!results.getPage().isEmpty()) {
							...
							// if this was a request for the first page (nextPageToken == null) and it was empty,
							// then set to true.
							isEmptyResults = true;
						}
	 ...
	 */
	@Override
	public void addMoreButton(MoreTreeItem moreItem, String parentId, EntityTreeItem parent,
			String nextPageToken) {
		if (parent == null) {
	...
	/**
	 * When a node is expanded, if its children have not already been fetched and placed into the
	 * tree, it will delete the dummy child node and fetch the actual children of the expanded node.
	 * During this process, a loading icon is appended below the folder.
	 */
	@Override
	...
	public EntityChildrenRequest createGetEntityChildrenRequest(String parentId,
			String nextPageToken) {
	...
	public EntityTreeItem makeTreeItemFromQueryResult(EntityHeader header, boolean isRootItem,
	...
	public void addResultsToParent(final EntityTreeItem parent, List<EntityHeader> results) {
		if (parent == null) {
			...
			for (EntityHeader header : results) {
				boolean isExpandable = isExpandable(header);
				addSynIdToList(header.getId());
				view.appendChildEntityTreeItem(makeTreeItemFromQueryResult(header, false, isExpandable),
						parent);
			}
		}
	}
	...
	public boolean isExpandable(EntityHeader header) {
		...
		return entityType
				.equals(EntityTypeUtils.getEntityClassNameForEntityType(EntityType.folder.name()))
				|| entityType
						.equals(EntityTypeUtils.getEntityClassNameForEntityType(EntityType.project.name()));
	}
