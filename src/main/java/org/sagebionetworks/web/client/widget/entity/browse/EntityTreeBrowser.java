...
public class EntityTreeBrowser
  implements EntityTreeBrowserView.Presenter, SynapseWidgetPresenter {
  ...
  public void onToggleSort(SortBy sortColumn) {
    currentDirection =
      Direction.ASC.equals(currentDirection) ? Direction.DESC : Direction.ASC;
  ...
  public void getChildren(
    ...
    EntityChildrenRequest request = createGetEntityChildrenRequest(
      parentId,
      nextPageToken
    );
    if (isInitializing) {
  ...
  public EntityChildrenRequest createGetEntityChildrenRequest(
    String parentId,
    String nextPageToken
  ) {
  ...
  public EntityTreeItem makeTreeItemFromQueryResult(
    EntityHeader header,
    boolean isRootItem,
  ...
  public void addResultsToParent(
    final EntityTreeItem parent,
    List<EntityHeader> results
  ) {
    if (parent == null) {
      for (EntityHeader header : results) {
        ...
        addSynIdToList(header.getId());
        view.appendRootEntityTreeItem(
          makeTreeItemFromQueryResult(header, true, isExpandable)
        );
      }
    } else {
      for (EntityHeader header : results) {
        ...
        view.appendChildEntityTreeItem(
          makeTreeItemFromQueryResult(header, false, isExpandable),
          parent
        );
      }
    }
