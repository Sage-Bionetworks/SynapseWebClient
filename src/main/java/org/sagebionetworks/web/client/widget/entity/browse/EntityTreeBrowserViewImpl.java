...
public class EntityTreeBrowserViewImpl
  extends FlowPanel
  implements EntityTreeBrowserView {
  ...
  @Inject
  public EntityTreeBrowserViewImpl(
    ...
    copyIDToClipboardIcon.addClickHandler(event ->
      presenter.copyIDsToClipboard()
    );
  }
  ...
  private Tree getTree() {
    if (entityTree == null) {
      ...
      entityTree.addOpenHandler(event -> {
        final EntityTreeItem target = getTreeItem2entityTreeItem()
          .get(event.getTarget());
  ...
  @Override
  public void appendChildEntityTreeItem(
    final EntityTreeItem childToAdd,
   ...
   */
  @Override
  public void placeRootMoreTreeItem(
    final MoreTreeItem childToCreate,
   ...
   */
  @Override
  public void placeChildMoreTreeItem(
    final MoreTreeItem childToCreate,
  ...
  public interface EntityTreeImageBundle
    extends ClientBundle, ClientBundleWithLookup {
  ...
  public void setLoadingVisible(boolean isShown) {
    ...
    if (entityTreeContainer != null) entityTreeContainer.setVisible(!isShown);
    if (loadingSpinner != null) loadingSpinner.setVisible(isShown);
  ...
  public void setSortUI(SortBy sortBy, Direction dir) {
    clearSortUI();
    SortDirection direction = Direction.ASC.equals(dir)
      ? SortDirection.ASC
      : SortDirection.DESC;
