...
public class MyEntitiesBrowser implements MyEntitiesBrowserView.Presenter, SynapseWidgetPresenter {
	...
	public MyEntitiesBrowser(MyEntitiesBrowserView view,
			...
			final GlobalApplicationState globalApplicationState, JSONObjectAdapter jsonObjectAdapter,
	...
	public boolean isSameContext() {
		if (globalApplicationState.getCurrentPlace() == null
				|| authenticationController.getCurrentUserPrincipalId() == null) {
			return false;
		}
		return globalApplicationState.getCurrentPlace().equals(cachedPlace)
				&& authenticationController.getCurrentUserPrincipalId().equals(cachedUserId);
	}
		...
		if (isSynapsePlace) {
			...
			jsClient.getEntityBundle(entityId, bundleRequest, new AsyncCallback<EntityBundle>() {
				...
				public void onSuccess(EntityBundle result) {
					...
					// add to the current context tree, and show all children of this container (or siblings
					// if leaf)
	...
	public void loadMoreUserUpdateable() {
		if (authenticationController.isLoggedIn()) {
			jsClient.getMyProjects(ProjectListType.CREATED, PROJECT_LIMIT, nextPageToken,
