...
public class EntityActionControllerImpl implements EntityActionController, ActionListener {
	...
	public EntityActionControllerImpl(EntityActionControllerView view,
			...
			AuthenticationController authenticationController, CookieProvider cookies,
			IsACTMemberAsyncHandler isACTMemberAsyncHandler, GWTWrapper gwt, EventBus eventBus) {
		super();
		this.view = view;
	...
	@Override
	public void configure(ActionMenuWidget actionMenu, EntityBundle entityBundle,
			boolean isCurrentVersion, String wikiPageId, EntityArea currentArea) {
		...
		this.enityTypeDisplay = EntityTypeUtils
				.getDisplayName(EntityTypeUtils.getEntityTypeForClass(entityBundle.getEntity().getClass()));
		...
		if (!(entity instanceof Project)) {
			...
			actionMenu.setToolsButtonIcon(DisplayUtils.capitalize(currentArea.name()) + TOOLS,
					IconType.GEAR);
		}
	}
	...
	private void configureAddExternalDockerRepo() {
		if (entityBundle.getEntity() instanceof Project && EntityArea.DOCKER.equals(currentArea)) {
			actionMenu.setActionVisible(Action.CREATE_EXTERNAL_DOCKER_REPO,
	...
	private void configureACTCommands() {
		...
		// show ACT commands if this is the Project Settings tools menu, or if the entity is not a
		// project (looking at a child entity)
			...
			isACTMemberAsyncHandler.isACTActionAvailable(new CallbackP<Boolean>() {
				...
				public void invoke(Boolean isACT) {
					if (isACT) {
						...
						actionMenu.setActionListener(Action.APPROVE_USER_ACCESS,
								EntityActionControllerImpl.this);
						actionMenu.setActionVisible(Action.MANAGE_ACCESS_REQUIREMENTS, true);
						actionMenu.setActionListener(Action.MANAGE_ACCESS_REQUIREMENTS,
								EntityActionControllerImpl.this);
	...
	private void configureChangeStorageLocation() {
		if (entityBundle.getEntity() instanceof Folder
			...
			actionMenu.setActionText(Action.CHANGE_STORAGE_LOCATION,
					"Change " + enityTypeDisplay + " Storage Location");
		...
		if (entityBundle.getEntity() instanceof Project && canEdit
			...
			getChallengeClient().getChallengeForProject(entity.getId(), new AsyncCallback<Challenge>() {
				...
				public void onSuccess(Challenge result) {
					...
					actionMenu.setActionVisible(Action.DELETE_CHALLENGE,
							EntityArea.CHALLENGE.equals(currentArea));
				}
	...
	private void onCreateOrUpdateDoi() {
		getCreateOrUpdateDoiModal().configureAndShow(entity, getVersion(),
	...
	private void configureTableCommands() {
		if (entityBundle.getEntity() instanceof Table) {
			...
			actionMenu.setActionVisible(Action.SHOW_VIEW_SCOPE,
	...
	private void configureReorderWikiSubpages() {
		if (isWikiableConfig(entityBundle.getEntity(), currentArea)
				&& entityBundle.getEntity() instanceof Project && permissions.getCanEdit()) {
			// shown if there's more than one page
			getSynapseJavascriptClient().getV2WikiHeaderTree(entityBundle.getEntity().getId(),
					ObjectType.ENTITY.name(), new AsyncCallback<List<V2WikiHeader>>() {
						@Override
						public void onFailure(Throwable caught) {
	...
	private void configureCreateTableViewSnapshot() {
		if (entityBundle.getEntity() instanceof Table
				&& EntityActionControllerImpl.isVersionSupported(entityBundle.getEntity(), cookies)) {
			String tableOrView = entityBundle.getEntity() instanceof TableEntity ? "Table" : "View";
			actionMenu.setActionText(Action.CREATE_TABLE_VERSION,
					"Create a New " + tableOrView + " Version");
	...
	private void configureAddWikiSubpage() {
		if (entityBundle.getEntity() instanceof Project
				&& isWikiableConfig(entityBundle.getEntity(), currentArea) && wikiPageId != null) {
	...
	private void configureSubmit() {
		if (isSubmittableType(entityBundle.getEntity())) {
			...
			actionMenu.setActionText(Action.SUBMIT_TO_CHALLENGE,
					"Submit " + enityTypeDisplay + " to Challenge");
		} else {
	...
	private void configureShareAction() {
		if (isTopLevelProjectToolsMenu(entityBundle.getEntity(), currentArea)) {
			...
			if (PublicPrivateBadge.isPublic(entityBundle.getBenefactorAcl(),
	...
	public void onCreateExternalDockerRepo() {
		...
		preflightController.checkCreateEntity(entityBundle, DockerRepository.class.getName(),
				new Callback() {
					@Override
					public void invoke() {
	...
	private void onManageAccessRequirements() {
		AccessRequirementsPlace place =
				new AccessRequirementsPlace(AccessRequirementsPlace.ID_PARAM + "=" + entity.getId() + "&"
						+ AccessRequirementsPlace.TYPE_PARAM + "=" + RestrictableObjectType.ENTITY.toString());
	...
	private void postCheckUploadFile() {
		...
		uploadDialogWidget.configure(DisplayConstants.TEXT_UPLOAD_FILE_OR_LINK,
				entityBundle.getEntity(), null, null, true);
	...
	private void onViewWikiSource() {
		WikiPageKey key = new WikiPageKey(this.entityBundle.getEntity().getId(),
				ObjectType.ENTITY.name(), wikiPageId);
	...
	private void postCheckCreateTableViewSnapshot() {
		...
		view.showMultiplePromptDialog("New Version", prompts, null, values -> {
			...
			if (entity instanceof TableEntity) {
				getSynapseJavascriptClient().createSnapshot(entityId, comment, label, activityId,
				...
				getJobTrackingWidget().startAndTrackJob(CREATING_A_NEW_VIEW_VERSION_MESSAGE, false,
						AsynchType.TableTransaction, transactionRequest, new AsynchronousProgressHandler() {
	...
	public void createWikiPage(final String name) {
		if (DisplayUtils.isDefined(name)) {
			...
			page.setTitle(name);
			getSynapseClient().createV2WikiPageWithV1(entityBundle.getEntity().getId(),
					ObjectType.ENTITY.name(), page, new AsyncCallback<WikiPage>() {
						@Override
						public void onSuccess(WikiPage result) {
							view.showInfo("'" + name + "' Page Added");
							Synapse newPlace = new Synapse(entityBundle.getEntity().getId(), getVersion(),
									EntityArea.WIKI, result.getId());
						...
						public void onFailure(Throwable caught) {
							view.showErrorMessage(
									DisplayConstants.ERROR_PAGE_CREATION_FAILED + ": " + caught.getMessage());
						}
					});
		}
	}
	...
	private void postCheckEditFileMetadata() {
		...
		getEditFileMetadataModalWidget().configure((FileEntity) entityBundle.getEntity(),
				originalFileHandle, () -> {
					fireEntityUpdatedEvent();
				});
	}
	...
	private void postCheckEditProjectMetadata() {
		...
		getEditProjectMetadataModalWidget().configure((Project) entityBundle.getEntity(),
				canChangeSettings, () -> {
					fireEntityUpdatedEvent();
				});
	}
	...
	public void onDeleteWiki() {
		final WikiPageKey key = new WikiPageKey(this.entityBundle.getEntity().getId(),
				ObjectType.ENTITY.name(), wikiPageId);
		getWikiPageDeleteConfirmationDialog().show(key, parentWikiId -> {
			getGlobalApplicationState().getPlaceChanger()
					.goTo(new Synapse(entityBundle.getEntity().getId(), null, EntityArea.WIKI, parentWikiId));
		});
	}

	@Override
	public void onDeleteEntity() {
		// Confirm the delete with the user. Mention that everything inside folder will also be deleted
		// if this is a folder entity.
		String display = ARE_YOU_SURE_YOU_WANT_TO_DELETE + this.enityTypeDisplay + " \""
				+ this.entity.getName() + "\"?";
	...
	public Place createDeletePlace() {
		...
		if (parentId != null && !(entityBundle.getEntity() instanceof Project)) {
			...
			gotoPlace =
					new Profile(authenticationController.getCurrentUserPrincipalId(), ProfileArea.PROJECTS);
		}
		return gotoPlace;
	}
