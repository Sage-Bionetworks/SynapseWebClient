...
public class ProvenanceEditorWidget
  implements ProvenanceEditorWidgetView.Presenter {
      ...
      new AsyncCallback<Activity>() {
        ...
        public void onFailure(Throwable caught) {
          if (caught instanceof NotFoundException) {
            ...
            usedProvenanceList.configure(
              new LinkedList<ProvenanceEntry>(),
              USED
            );
            executedProvenanceList.configure(
              new LinkedList<ProvenanceEntry>(),
              EXECUTED
            );
          } else {
          ...
          if (allProvenance != null) {
            ...
            for (Used provEntry : allProvenance) {
              ...
              if (provEntry instanceof UsedEntity) {
                ...
                ((EntityRefProvEntryView) toAdd).configure(
                    entityId,
                    versionString
                  );
                toAdd.setAnchorTarget(
  ...
  public void onSave() {
    ...
    if (isNewActivity) {
      ...
      jsClient.createActivityAndLinkToEntity(
        activity,
        entity,
  ...
  private Set<Used> provEntryListToUsedSet(
    List<ProvenanceEntry> entries,
    boolean wasExecuted
  ) {
    Set<Used> usedSet = new HashSet<Used>();
