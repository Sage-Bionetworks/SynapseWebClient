...
public class SynapseAlertImpl implements SynapseAlert {
	...
	@Inject
	public SynapseAlertImpl(SynapseAlertView view, GlobalApplicationState globalApplicationState,
			AuthenticationController authController, GWTWrapper gwt, PortalGinInjector ginInjector,
			SynapseJSNIUtils jsniUtils, JSONObjectAdapter jsonObjectAdapter) {
		this.view = view;
	...
	public void onCreateJiraIssue(String errorMessage) {
		String userId = WebConstants.ANONYMOUS, email = WebConstants.ANONYMOUS,
				displayName = WebConstants.ANONYMOUS;
		...
		String description =
				FLAG_ISSUE_DESCRIPTION_PART_1 + gwt.getCurrentURL() + "\n\n" + errorMessage;

		jsniUtils.showJiraIssueCollector("", description, WebConstants.SWC_ISSUE_COLLECTOR_URL, userId,
				displayName, email, "", "", "", ISSUE_PRIORITY_MINOR);
	}
