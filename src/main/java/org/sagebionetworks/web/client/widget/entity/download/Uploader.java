...
public class Uploader
		implements UploaderView.Presenter, SynapseWidgetPresenter, ProgressingFileUploadHandler {
	...
	@Inject
	public Uploader(UploaderView view, SynapseClientAsync synapseClient,
			SynapseJSNIUtils synapseJsniUtils, GWTWrapper gwt,
			AuthenticationController authenticationController, MultipartUploader multiPartUploader,
			GlobalApplicationState globalAppState, S3DirectUploader s3DirectUploader,
			SynapseJavascriptClient jsClient, SynapseProperties synapseProperties, EventBus eventBus) {
	...
	public Widget configure(Entity entity, String parentEntityId,
			CallbackP<String> fileHandleIdCallback, boolean isEntity) {
	...
	public void queryForUploadDestination() {
		...
		if (parentEntityId == null && entity == null) {
			...
			jsClient.getUploadDestinations(uploadDestinationsEntityId,
					new AsyncCallback<List<UploadDestination>>() {
						public void onSuccess(List<UploadDestination> uploadDestinations) {
							...
							if (uploadDestinations == null || uploadDestinations.isEmpty()) {
							...
							} else if (uploadDestinations
									.get(0) instanceof ExternalGoogleCloudUploadDestination) {
								...
								if (currentUploadType == UploadType.SFTP) {
									...
									onFailure(new org.sagebionetworks.web.client.exceptions.IllegalArgumentException(
								...
								// direct to s3(-like) storage
									...
									.get(0) instanceof ExternalObjectStoreUploadDestination) {
								ExternalObjectStoreUploadDestination externalUploadDestination =
										(ExternalObjectStoreUploadDestination) uploadDestinations.get(0);
								storageLocationId = externalUploadDestination.getStorageLocationId();
								currentUploadType = externalUploadDestination.getUploadType();
								String banner = externalUploadDestination.getBanner();
	...
	public void uploadBasedOnConfiguration() {
		// create necessary folders based on webkitRelativePath for the current item, and set parent
		// entity id to correct parent
	...
	private void mkdir() {
		...
		jsClient.lookupChild(currentFilePath[currentFilePathElement], currentFileParentEntityId,
				new AsyncCallback<String>() {
					...
					public void onFailure(Throwable caught) {
						if (caught instanceof NotFoundException) {
							...
							uploadError("Unable to create target folder structure: "
									+ synapseJsniUtils.getWebkitRelativePath(fileList, currIndex), caught);
						}
					}
	 ...
	 * @return
	 */
	public static String getSftpProxyLink(String fileNameOverride, String realSftpUrl,
			SynapseProperties synapseProperties, GWTWrapper gwt) {
		...
		if (sftpProxy != null) {
			...
			return sftpProxy + delimiter + "url=" + escapedRealSftpUrl + "&filename="
	...
	public void checkForExistingFileName(final String fileName, final Callback callback) {
		if (entity != null || currentFileParentEntityId == null) {
			...
			synapseClient.getFileEntityIdWithSameName(fileName, currentFileParentEntityId,
					new AsyncCallback<String>() {
						@Override
						public void onSuccess(final String result) {
							...
							view.showConfirmDialog("A file named \"" + fileName + "\" (" + result
						...
						public void onFailure(Throwable caught) {
							if (caught instanceof NotFoundException) {
								...
								view.showErrorMessage("An item named \"" + fileName
	...
	public void directUploadStep2(String fileName) {
		...
		if (endpointUrl != null) {
			s3DirectUploader.configure(view.getS3DirectAccessKey(), view.getS3DirectSecretKey(),
					bucketName, endpointUrl);
			s3DirectUploader.uploadFile(fileName, contentType, currentFile, this, keyPrefixUUID,
					storageLocationId, view);
		} else {
			this.multiPartUploader.uploadFile(fileName, contentType, currentFile, this, storageLocationId,
					view);
		}
	}
	...
	public void setFileEntityFileHandle(String fileHandleId) {
		if (entityId != null || currentFileParentEntityId != null) {
			synapseClient.setFileEntityFileHandle(fileHandleId, entityId, currentFileParentEntityId,
					new AsyncCallback<String>() {
						@Override
	...
	public void setSftpExternalFilePath(final String path, final Long storageLocationId) {
		...
		synapseJsniUtils.getFileMd5(blob, new MD5Callback() {
			...
			public void setMD5(String hexValue) {
				...
				if (isUpdating) {
					// existing entity
					updateExternalFileEntity(entityId, path, fileName, fileSize, contentType, hexValue,
							storageLocationId);
				} else {
					// new data, use the appropriate synapse call
					createNewExternalFileEntity(path, fileName, fileSize, contentType, hexValue,
							storageLocationId);
				}
			}
		});
	...
	public void updateExternalFileEntity(String entityId, String path, String name, Long fileSize,
			String contentType, String md5, Long storageLocationId) {
		try {
			synapseClient.updateExternalFile(entityId, path, name, contentType, fileSize, md5,
					storageLocationId, getExternalFileUpdatedCallback());
		} catch (Throwable t) {
			view.showErrorMessage(DisplayConstants.TEXT_LINK_FAILED);
		}
	}

	public void createNewExternalFileEntity(final String path, final String name, Long fileSize,
			String contentType, String md5, final Long storageLocationId) {
		try {
			synapseClient.createExternalFile(currentFileParentEntityId, path, name, contentType, fileSize,
					md5, storageLocationId, getExternalFileUpdatedCallback());
	...
	public void handleSubmitResult(UploadResult uploadResult) {
		if (uploadResult.getUploadStatus() == UploadStatus.SUCCESS) {
			...
			if (isJschAuthorizationError(uploadResult.getMessage())) {
				uploadError(DisplayConstants.INVALID_USERNAME_OR_PASSWORD,
						new UnauthorizedException(uploadResult.getMessage()));
			} else {
				uploadError(
						"Upload result status indicated upload was unsuccessful. " + uploadResult.getMessage(),
	...
	private void fireCancelEvent() {
		...
		// available,
		// so that this effectively cancels chunked upload too (after the current chunk upload
		// completes)
	...
	public void updateProgress(double currentProgress, String progressText, String uploadSpeed) {
		...
		double percentOfAllFiles =
				calculatePercentOverAllFiles(this.fileNames.length, this.currIndex, currentProgress);
	 ...
	 * @return
	 */
	public static double calculatePercentOverAllFiles(int numberFiles, int currentFileIndex,
			double percentOfCurrentFile) {
		double percentPerFile = 1.0 / (double) numberFiles;
		double percentOfAllFiles =
				percentPerFile * percentOfCurrentFile + (percentPerFile * currentFileIndex);
