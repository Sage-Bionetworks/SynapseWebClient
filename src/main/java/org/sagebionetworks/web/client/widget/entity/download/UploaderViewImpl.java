...
public class UploaderViewImpl extends FlowPanel implements UploaderView {
	...
	@Inject
	public UploaderViewImpl(SynapseJSNIUtils synapseJSNIUtils, SageImageBundle sageImageBundle,
	...
	protected void initMessageHandler() {
		...
		messageHandler = EventHandlerUtils.addEventListener("message", EventHandlerUtils.getWnd(),
	...
	public void showLoading() {
		spinningProgressContainer
				.add(DisplayUtils.getLoadingWidget(DisplayConstants.LABEL_INITIALIZING));
	...
	public void updateProgress(double value, String text, String uploadSpeed) {
		...
		uploadSpeedLabel
				.setHTML(SafeHtmlUtils.htmlEscapeAllowEntities(uploadSpeed.replaceAll(" ", "&nbsp;")));
	}
	...
	@Override
	public void showConfirmDialog(String message, final Callback yesCallback,
			final Callback noCallback) {
		DisplayUtils.showConfirmDialog(DisplayConstants.UPLOAD_DIALOG_TITLE, message, yesCallback,
				noCallback);
	}
	...
	public void showUploadingToExternalStorage(String host, String banner) {
		...
		uploadDestinationContainer.add(
				new HTML(DisplayConstants.UPLOAD_DESTINATION + "<strong>" + escapedHost + "</strong>"));
		if (banner != null)
			uploadDestinationContainer.add(new HTML(SafeHtmlUtils.htmlEscape(banner)));
