...
public class APITableColumnConfigViewImpl implements APITableColumnConfigView {
  ...
  public interface Binder
    extends UiBinder<Widget, APITableColumnConfigViewImpl> {}
  ...
  @Inject
  public APITableColumnConfigViewImpl() {
  ...
  public void configure(APITableColumnConfig data) {
    this.data = data;
    if (
      data.getInputColumnNames() != null &&
      data.getInputColumnNames().size() > 0
    ) {
  ...
  private void updateFromView() {
    ...
    data.setSort(
      COLUMN_SORT_TYPE.valueOf(sortField.getSelectedValue().toUpperCase())
    );
  }
