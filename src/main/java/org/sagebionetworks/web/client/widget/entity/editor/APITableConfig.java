...
public class APITableConfig {
  ...
  public APITableConfig(Map<String, String> descriptor) {
    ...
    if (uri != null) {
      ...
      if (
        ...
        isQueryTableResults =
          Boolean.parseBoolean(
            descriptor.get(WidgetConstants.API_TABLE_WIDGET_QUERY_TABLE_RESULTS)
          );
      }
      ...
      if (
        ...
        isShowOnlyIfLoggedIn =
          Boolean.parseBoolean(
            descriptor.get(WidgetConstants.API_TABLE_WIDGET_SHOW_IF_LOGGED_IN)
          );
      }

      if (
        descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_RESULTS_KEY)
      ) {
        jsonResultsArrayKeyName =
          descriptor.get(WidgetConstants.API_TABLE_WIDGET_RESULTS_KEY);
      }

      if (descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_CSS_STYLE)) {
        cssStyleName =
          descriptor.get(WidgetConstants.API_TABLE_WIDGET_CSS_STYLE);
      }
    }
  }
  ...
  public static List<APITableColumnConfig> parseTableColumnConfigs(
    ...
    while (
      ...
      String configString = descriptor.get(
        WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i
      );
      String[] parts = configString.split(FIELD_DELIMITER);
      if (parts.length < 3) {
        throw new IllegalArgumentException(
          WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX +
          i +
          ": Invalid configuration due to missing fields."
        );
      }
      try {
