...
public class APITableConfig {
	...
	public APITableConfig(Map<String, String> descriptor) {
		...
		if (uri != null) {
			if (descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_PAGING_KEY)) {
				isPaging =
						Boolean.parseBoolean(descriptor.get(WidgetConstants.API_TABLE_WIDGET_PAGING_KEY));
				if (isPaging) {
					// initialize the offset and pagesize
					offset = 0;
					if (descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_PAGESIZE_KEY))
						pageSize =
								Integer.parseInt(descriptor.get(WidgetConstants.API_TABLE_WIDGET_PAGESIZE_KEY));
					else
						pageSize = 10;
				}
			}
			if (descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_QUERY_TABLE_RESULTS)) {
				isQueryTableResults = Boolean
						.parseBoolean(descriptor.get(WidgetConstants.API_TABLE_WIDGET_QUERY_TABLE_RESULTS));
			}

			if (descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_SHOW_IF_LOGGED_IN)) {
				isShowOnlyIfLoggedIn = Boolean
						.parseBoolean(descriptor.get(WidgetConstants.API_TABLE_WIDGET_SHOW_IF_LOGGED_IN));
			}
	...
	public static List<APITableColumnConfig> parseTableColumnConfigs(Map<String, String> descriptor) {
		...
		while (descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i)) {
			String configString =
					descriptor.get(WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i);
			String[] parts = configString.split(FIELD_DELIMITER);
			if (parts.length < 3) {
				throw new IllegalArgumentException(WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i
			...
			try {
				...
				throw new RuntimeException(
						WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i + ":" + t.getMessage(), t);
			}
			i++;
		}
