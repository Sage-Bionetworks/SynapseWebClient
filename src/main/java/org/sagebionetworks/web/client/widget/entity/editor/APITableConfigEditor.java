...
public class APITableConfigEditor implements WidgetEditorPresenter {
  ...
  public void updateDescriptorFromView() {
    ...
    updateDescriptor(
      WidgetConstants.API_TABLE_WIDGET_PAGING_KEY,
      view.isPaging().toString()
    );
    updateDescriptor(
      WidgetConstants.API_TABLE_WIDGET_QUERY_TABLE_RESULTS,
      view.isQueryTableResults().toString()
    );
    updateDescriptor(
      WidgetConstants.API_TABLE_WIDGET_SHOW_IF_LOGGED_IN,
      view.isShowIfLoggedInOnly().toString()
    );
    updateDescriptor(
      WidgetConstants.API_TABLE_WIDGET_PAGESIZE_KEY,
      view.getPageSize()
    );
    updateDescriptor(
      WidgetConstants.API_TABLE_WIDGET_RESULTS_KEY,
      view.getJsonResultsKeyName()
    );
    updateDescriptor(
      WidgetConstants.API_TABLE_WIDGET_CSS_STYLE,
      view.getCssStyle()
    );
  ...
  private void updateDescriptor(String key, String value) {
    if (value != null && value.trim().length() > 0) descriptor.put(key, value);
  }
  ...
  public static void updateDescriptorWithColumnConfigs(
    Map<String, String> descriptor,
    List<APITableColumnConfig> configs
  ) {
    // clean up old column config definitions
    int index = 0;
    boolean foundConfig = descriptor.containsKey(
      WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + index
    );
    while (foundConfig) {
      descriptor.remove(
        WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + index
    ...
    if (configs != null) {
      for (int i = 0; i < configs.size(); i++) {
        ...
        if (config.getDisplayColumnName() != null) displayColumnName =
          config.getDisplayColumnName().trim();
        ...
        if (
          columnConfigString != null && columnConfigString.trim().length() > 0
        ) descriptor.put(
          WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i,
          columnConfigString
        );
      }
    }
