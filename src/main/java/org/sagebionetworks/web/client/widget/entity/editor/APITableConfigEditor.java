...
public class APITableConfigEditor implements WidgetEditorPresenter {
	...
	public void updateDescriptorFromView() {
		...
		updateDescriptor(WidgetConstants.API_TABLE_WIDGET_QUERY_TABLE_RESULTS,
				view.isQueryTableResults().toString());
		updateDescriptor(WidgetConstants.API_TABLE_WIDGET_SHOW_IF_LOGGED_IN,
	...
	public static void updateDescriptorWithColumnConfigs(Map<String, String> descriptor,
			List<APITableColumnConfig> configs) {
		// clean up old column config definitions
		int index = 0;
		boolean foundConfig =
				descriptor.containsKey(WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + index);
		while (foundConfig) {
		...
		if (configs != null) {
			for (int i = 0; i < configs.size(); i++) {
				...
				if (columnConfigString != null && columnConfigString.trim().length() > 0)
					descriptor.put(WidgetConstants.API_TABLE_WIDGET_COLUMN_CONFIG_PREFIX + i,
							columnConfigString);
			}
		}
	}
