...
public class AttachmentConfigEditor
  implements AttachmentConfigView.Presenter, WidgetEditorPresenter {
  ...
  public void updateDescriptorFromView() {
    ...
    if (!fileHandleIds.isEmpty()) {
      ...
      if (!wikiAttachments.isValid()) throw new IllegalArgumentException(
        DisplayConstants.ERROR_SELECT_ATTACHMENT_MESSAGE
      );
      descriptor.put(
        WidgetConstants.IMAGE_WIDGET_FILE_NAME_KEY,
        wikiAttachments.getSelectedFilename()
      );
    }
  }

  private String getFileName() {
    if (uploadedFile == null) return null; else return uploadedFile
      .getFileMeta()
      .getFileName();
  }
