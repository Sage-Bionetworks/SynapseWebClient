...
public class ButtonLinkConfigViewImpl implements ButtonLinkConfigView {
  ...
  public interface ButtonLinkConfigViewImplUiBinder
    extends UiBinder<Widget, ButtonLinkConfigViewImpl> {}
  ...
  private void updatePreviewButton() {
    ButtonType type = isHighlightButtonStyle()
      ? ButtonType.INFO
      : ButtonType.DEFAULT;
  ...
  public void configure(WikiPageKey wikiKey, Map<String, String> descriptor) {
    ...
    if (text != null) nameField.setValue(text);
    String url = descriptor.get(WidgetConstants.LINK_URL_KEY);
    if (url != null) urlField.setValue(url);
    boolean isHighlight = false;
    if (descriptor.containsKey(WebConstants.HIGHLIGHT_KEY)) {
      isHighlight =
        Boolean.parseBoolean(descriptor.get(WebConstants.HIGHLIGHT_KEY));
    }
  ...
  public void checkParams() throws IllegalArgumentException {
    ...
    if (
      !ValidationUtils.isValidWidgetName(nameField.getValue())
    ) throw new IllegalArgumentException(
      "Invalid name: " + nameField.getValue()
    );
  }
