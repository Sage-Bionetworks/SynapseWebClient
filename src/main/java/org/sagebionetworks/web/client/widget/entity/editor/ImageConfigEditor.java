...
public class ImageConfigEditor
  implements ImageConfigView.Presenter, WidgetEditorPresenter {
  ...
  @Inject
  public ImageConfigEditor(
    ImageConfigView view,
    ImageUploadWidget fileInputWidget,
  ...
  public void configure(
    ...
    if (descriptor.containsKey(IMAGE_WIDGET_SYNAPSE_ID_KEY)) {
      ...
      if (descriptor.containsKey(IMAGE_WIDGET_FILE_NAME_KEY)) {
        // existing attachment
        wikiAttachments.setSelectedFilename(
  ...
  public void configureWithoutUpload(
    WikiPageKey wikiKey,
    Map<String, String> widgetDescriptor,
  ...
  public void updateDescriptorFromView() {
    ...
    if (!view.isExternal()) {
      if (view.isSynapseEntity()) {
        ...
        descriptor.put(
          IMAGE_WIDGET_FILE_NAME_KEY,
          file.getFileMeta().getFileName()
        );
      } else {
  ...
  public String getTextToInsert() {
    if (view.isExternal()) return (
      "![" + view.getExternalAltText() + "](" + view.getImageUrl() + ")"
    ); else return null;
  }
