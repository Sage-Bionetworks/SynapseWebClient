...
public class PlotlyConfigEditor
  implements PlotlyConfigView.Presenter, WidgetEditorPresenter {

  private Map<String, String> descriptor;
  private PlotlyConfigView view;

  private static final String QUERY_FIRST_COLUMN_REG_EX =
    "select\\s+[\"']?([a-zA-Z0-9_ ]+)[\"']?[,]{1}";
  private static final RegExp X_COLUMN_PATTERN = RegExp.compile(
    QUERY_FIRST_COLUMN_REG_EX,
    "i"
  );
  private static final String QUERY_OTHER_COLUMNS_REG_EX =
    "select\\s+[\"']?([a-zA-Z0-9_ ]+)[\"']?[,]{1}(.+)from";
  private static final RegExp Y_COLUMNS_PATTERN = RegExp.compile(
    QUERY_OTHER_COLUMNS_REG_EX,
    "i"
  );
  ...
  @Inject
  public PlotlyConfigEditor(
    final PlotlyConfigView view,
    ...
    showHideAdvancedButton.addClickHandler(
      new ClickHandler() {
        @Override
    ...
    this.finder =
      entityFinderBuilder
        ...
        .setHelpMarkdown(
          "Search or Browse Synapse to find a Table or View to display in the Simple Plot"
        )
  ...
  public void setAdvancedModeVisible(boolean visible) {
    ...
    showHideAdvancedButton.setIcon(
      isAdvancedVisible ? IconType.TOGGLE_DOWN : IconType.TOGGLE_RIGHT
    );
  ...
  public void configure(
    ...
    if (descriptor.containsKey(TYPE)) {
      GraphType graphType = GraphType.valueOf(
        descriptor.get(TYPE).toUpperCase()
      );
    ...
    if (descriptor.containsKey(IS_HORIZONTAL)) {
      view.setBarOrientationHorizontal(
        Boolean.valueOf(descriptor.get(IS_HORIZONTAL))
      );
    }
    if (descriptor.containsKey(X_AXIS_TYPE)) {
      AxisType axisType = AxisType.valueOf(
        descriptor.get(X_AXIS_TYPE).toUpperCase()
      );
      view.setXAxisType(axisType);
    }
    if (descriptor.containsKey(Y_AXIS_TYPE)) {
      AxisType axisType = AxisType.valueOf(
        descriptor.get(Y_AXIS_TYPE).toUpperCase()
      );
  ...
  public void updateDescriptorFromView() {
    if (yColumnsList.isEmpty()) {
      throw new IllegalArgumentException(
        "Please select at least one y data column."
      );
    }
    ...
    if (GraphType.BAR.equals(view.getGraphType())) {
      ...
      descriptor.put(
        IS_HORIZONTAL,
        Boolean.toString(view.isBarOrientationHorizontal())
      );
    }
  ...
  public void setTableId(String synId) {
    ...
    // get the columns
    jsClient.getColumnModelsForTableEntity(
      synId,
  ...
  public void refreshAvailableColumnNames() {
    if (allAvailableColumnNames.size() > 0) {
      if (
        !DisplayUtils.isDefined(xColumnName) ||
        !allAvailableColumnNames.contains(xColumnName)
      ) {
        xColumnName = allAvailableColumnNames.get(0);
      }
    }

    List<String> availableColumnNames = new ArrayList<String>(
  ...
  public void onAddYColumn(String yColumnName) {
    ...
    if (
      DisplayUtils.isDefined(yColumnName) &&
      allAvailableColumnNames.contains(yColumnName)
    ) {
      // add to list
