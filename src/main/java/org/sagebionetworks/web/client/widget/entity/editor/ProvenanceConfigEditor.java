...
public class ProvenanceConfigEditor
  implements ProvenanceConfigView.Presenter, WidgetEditorPresenter {
  ...
  public void configure(
    ...
    if (
      descriptor.get(WidgetConstants.PROV_WIDGET_ENTITY_LIST_KEY) != null
    ) view.setEntityList(
    ...
    if (
      descriptor.get(WidgetConstants.PROV_WIDGET_DISPLAY_HEIGHT_KEY) != null
    ) {
      view.setProvDisplayHeight(
  ...
  public void updateDescriptorFromView() {
    ...
    if (!DisplayUtils.isDefined(entityList)) throw new IllegalArgumentException(
      DisplayConstants.ERROR_ENTER_AT_LEAST_ONE_ENTITY
    );
    String depth = view.getDepth();
    if (!DisplayUtils.isDefined(depth)) throw new IllegalArgumentException(
      DisplayConstants.ERROR_ENTER_DEPTH
    );
    ...
    descriptor.put(
      ...
      Boolean.toString(view.isExpanded())
    );
  }
