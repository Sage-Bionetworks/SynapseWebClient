...
public class AddToDownloadList implements IsWidget, AddToDownloadListView.Presenter {
	...
	public static final String ZERO_FILES_IN_FOLDER_MESSAGE =
	...
	public static final String SUCCESS_ADDED_FILES_MESSAGE =
			"Successfully added files to the Download List.";
	public static final String ADD_QUERY_FILES_CONFIRMATION_MESSAGE =
			"Add all files from this query result to the Download List?";
	public static final Long FILE_SIZE_QUERY_PART_MASK =
			BUNDLE_MASK_QUERY_SUM_FILE_SIZES | BUNDLE_MASK_QUERY_COUNT;
	public static final String PLEASE_LOGIN_TO_ADD_TO_DOWNLOAD_LIST =
			"Sign in to add files to your download list.";
	...
	@Inject
	public AddToDownloadList(AddToDownloadListView view, AsynchronousProgressWidget progress,
			InlineAsynchronousProgressViewImpl inlineProgressView, PopupUtilsView popupUtilsView,
			EventBus eventBus, SynapseAlert synAlert, PackageSizeSummary packageSizeSummary,
			SynapseJavascriptClient jsClient, SynapseJSNIUtils jsniUtils,
			AuthenticationController authController) {
	...
	public void confirmAddQueryResultsToDownloadList() {
		if (authController.isLoggedIn()) {
			...
			progress.startAndTrackJob("Calculating the total query result file size", false,
					AsynchType.TableQuery, queryBundleRequest, new AsynchronousProgressHandler() {
						@Override
						...
						public void onComplete(AsynchronousResponseBody response) {
							...
							if (queryResultBundle.getSumFileSizes() != null
									&& queryResultBundle.getSumFileSizes().getSumFileSizesBytes() != null) {
								sumFileSizesBytes =
										queryResultBundle.getSumFileSizes().getSumFileSizesBytes().doubleValue();
							}
	...
	public void confirmAddFolderChildrenToDownloadList() {
		if (authController.isLoggedIn()) {
			...
			jsClient.getEntityChildren(entityChildrenRequest,
					new AsyncCallback<EntityChildrenResponse>() {
						...
						public void onSuccess(EntityChildrenResponse entityChildrenResponse) {
							...
							if (fileCountToAdd == 0) {
								...
								packageSizeSummary.addFiles(fileCountToAdd,
										entityChildrenResponse.getSumFileSizesBytes().doubleValue());
								view.showConfirmAdd();
							}
						}
					});
	...
	public void startAddingFiles() {
		view.hideAll();
		view.showAsynchronousProgressWidget();
		progress.startAndTrackJob("Adding files to Download List...", false,
				AsynchType.AddFileToDownloadList, request, new AsynchronousProgressHandler() {
					@Override
					public void onFailure(Throwable failure) {
						synAlert.handleException(failure);
						view.hideAll();
					...
					public void onComplete(AsynchronousResponseBody response) {
						jsniUtils.sendAnalyticsEvent(AddToDownloadList.DOWNLOAD_ACTION_EVENT_NAME,
								AddToDownloadList.FILES_ADDED_TO_DOWNLOAD_LIST_EVENT_NAME,
								Integer.toString(fileCountToAdd));
						view.hideAll();
