...
public class FileDownloadMenuItem
	...
	public FileDownloadMenuItem(FileDownloadMenuItemView view, SynapseClientAsync synapseClient,
			LoginModalWidget loginModalWidget, PortalGinInjector ginInjector,
			SynapseJavascriptClient jsClient, AuthenticationController authController,
			SynapseJSNIUtils jsniUtils, GWTWrapper gwt, CookieProvider cookies, AwsSdk awsSdk,
			PopupUtilsView popupUtilsView) {
		this.view = view;
	...
	public void configure(final EntityBundle bundle) {
		...
		if (bundle.getRestrictionInformation() != null) {
			...
			jsClient.getRestrictionInformation(bundle.getEntity().getId(), RestrictableObjectType.ENTITY,
	...
	public void configure(EntityBundle bundle,
		...
		if (!authController.isLoggedIn()) {
			...
			view.setIsDirectDownloadLink(ACCESS_REQUIREMENTS_LINK + bundle.getEntity().getId() + "&"
			...
			if (dataFileHandle != null) {
				if (dataFileHandle instanceof ExternalObjectStoreFileHandle) {
					...
					String directDownloadUrl = getDirectDownloadURL((FileEntity) entityBundle.getEntity(),
							dataFileHandle, fileNameOverride);

					// special case, if this starts with sftp proxy, then handle
					String sftpProxy = ginInjector.getSynapseProperties()
					...
					if (directDownloadUrl.startsWith(sftpProxy)) {
						...
						loginModalWidget.configure(directDownloadUrl, FormPanel.METHOD_POST,
	...
	public String getDirectDownloadURL(FileEntity fileEntity, FileHandle fileHandle,
		...
		if (externalUrl == null)
			directDownloadURL = jsniUtils.getFileHandleAssociationUrl(fileEntity.getId(),
					FileHandleAssociateType.FileEntity, fileHandle.getId());
		else {
			if (externalUrl.toLowerCase().startsWith(WebConstants.SFTP_PREFIX)) {
				// point to sftp proxy instead
				directDownloadURL = Uploader.getSftpProxyLink(fileNameOverride, externalUrl,
						ginInjector.getSynapseProperties(), gwt);
			} else {
	...
	public void queryForSftpLoginInstructions(String url) {
		synapseClient.getHost(url, new AsyncCallback<String>() {
			...
			public void onSuccess(String host) {
				...
				loginModalWidget.setInstructionMessage(
						DisplayConstants.DOWNLOAD_CREDENTIALS_REQUIRED + SafeHtmlUtils.htmlEscape(host));
			}
	...
	public void onLoginS3DirectDownloadClicked(String accessKeyId, String secretAccessKey) {
		...
		CallbackP<JavaScriptObject> s3Callback = new CallbackP<JavaScriptObject>() {
			...
			public void invoke(JavaScriptObject s3JsObject) {
				s3 = s3JsObject;
				// NOTE: most browsers block the popup because the button click event is not directly
				// associated to the login popup.
				// Show the direct download button after authorization succeeds.
				view.showS3DirectDownloadDialog();
			}
		};
		awsSdk.getS3(accessKeyId, secretAccessKey, objectStoreFileHandle.getBucket(),
				objectStoreFileHandle.getEndpointUrl(), s3Callback);
	}
	...
	public void onAuthenticatedS3DirectDownloadClicked() {
		ExternalObjectStoreFileHandle objectStoreFileHandle =
				(ExternalObjectStoreFileHandle) dataFileHandle;
		String presignedUrl = awsSdk.getPresignedURL(objectStoreFileHandle.getFileKey(),
				objectStoreFileHandle.getBucket(), objectStoreFileHandle.getFileName(), s3);
	...
	public void onDirectDownloadClicked() {
		...
		jsniUtils.sendAnalyticsEvent(AddToDownloadList.DOWNLOAD_ACTION_EVENT_NAME,
				FILE_DIRECTLY_DOWNLOADED_EVENT_NAME);
	}
}
