...
public class FileTitleBar implements SynapseWidgetPresenter, FileTitleBarView.Presenter {
	...
	@Inject
	public FileTitleBar(FileTitleBarView view, SynapseProperties synapseProperties,
			FileDownloadMenuItem fileDownloadButton, SynapseJavascriptClient jsClient,
			FileClientsHelp fileClientsHelp, EventBus eventBus, SynapseJSNIUtils jsniUtils) {
		this.view = view;
	...
	public void configure(EntityBundle bundle) {
		...
		if (isFilenamePanelVisible) {
			...
			if (fileHandle.getContentSize() != null) {
				view.setFileSize(
						"| " + DisplayUtils.getFriendlySize(fileHandle.getContentSize().doubleValue(), true));
			}
	...
	public void getLatestVersion() {
		// determine if we should show report the version (only shows if we're looking at an older
		// version of the file).
	...
	public void configureCloudProviderFile(CloudProviderFileHandleInterface filehandle) {
		Long synapseStorageLocationId = Long.valueOf(
				synapseProperties.getSynapseProperty("org.sagebionetworks.portal.synapse_storage_id"));
	...
	public void configureExternalObjectStore(ExternalObjectStoreFileHandle externalFileHandle) {
		...
		view.setExternalObjectStoreInfo(externalFileHandle.getEndpointUrl(),
				externalFileHandle.getBucket(), externalFileHandle.getFileKey());
	...
	public void onAddToDownloadList() {
		...
		jsClient.addFileToDownloadList(entity.getDataFileHandleId(), entity.getId(),
				new AsyncCallback<DownloadList>() {
					...
					public void onSuccess(DownloadList result) {
						...
						view.showAddedToDownloadListAlert(
