...
public class S3DirectLoginDialogImpl implements S3DirectLoginDialog {
  ...
  @Inject
  public S3DirectLoginDialogImpl(AwsLoginView awsLoginView) {
    w = uiBinder.createAndBindUi(this);
    this.awsLoginView = awsLoginView;
    s3DirectLoginDialogBody.add(awsLoginView);

    s3DirectLoginDialogButton.addClickHandler(
      new ClickHandler() {
        @Override
        public void onClick(ClickEvent event) {
          s3DirectLoginDialog.hide();
          presenter.onLoginS3DirectDownloadClicked(
            awsLoginView.getAccessKey(),
            awsLoginView.getSecretKey()
          );
        }
      }
    );
    s3DirectDownloadButton.addClickHandler(
      new ClickHandler() {
        @Override
        public void onClick(ClickEvent event) {
