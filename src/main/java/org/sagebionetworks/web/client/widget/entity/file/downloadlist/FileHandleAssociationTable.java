...
public class FileHandleAssociationTable implements IsWidget, SortingListener {
	...
	@Inject
	public FileHandleAssociationTable(FileHandleAssociationTableView view,
			PortalGinInjector ginInjector) {
		this.view = view;
	...
	public void configure(List<FileHandleAssociation> fhas,
			CallbackP<Double> addToPackageSizeCallback, CallbackP<FileHandleAssociation> onRemove) {
		view.clear();
	...
	public void onToggleSort(String header) {
		...
		if (FILE_NAME_COLUMN_NAME.equals(header)) {
			comparator =
					SortDirection.ASC.equals(sortDir) ? fileNameAscComparator : fileNameDescComparator;
			...
			comparator =
					SortDirection.ASC.equals(sortDir) ? createdByAscComparator : createdByDescComparator;
		} else if (CREATED_ON_COLUMN_NAME.equals(header)) {
			comparator =
					SortDirection.ASC.equals(sortDir) ? createdOnAscComparator : createdOnDescComparator;
		} else if (SIZE_COLUMN_NAME.equals(header)) {
			comparator =
					SortDirection.ASC.equals(sortDir) ? fileSizeAscComparator : fileSizeDescComparator;
		}
