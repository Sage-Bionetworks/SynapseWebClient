...
public class FileHandleAssociationTableViewImpl
		implements FileHandleAssociationTableView, IsWidget {
	Widget w;

	interface FileHandleAssociationTableViewImplUiBinder
			extends UiBinder<Widget, FileHandleAssociationTableViewImpl> {
	}
	...
	private static FileHandleAssociationTableViewImplUiBinder uiBinder =
			GWT.create(FileHandleAssociationTableViewImplUiBinder.class);
	...
	public void setSort(String headerName, SortDirection sortDir) {
		...
		for (SortableTableHeaderImpl header : headers) {
			if (headerName.equals(header.getText())) {
				org.sagebionetworks.repo.model.table.SortDirection direction =
						SortDirection.ASC.equals(sortDir)
