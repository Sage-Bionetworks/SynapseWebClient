...
public class AttachmentPreviewWidgetViewImpl extends FlowPanel
		implements AttachmentPreviewWidgetView {
	...
	public void configure(WikiPageKey wikiKey, String fileName) {
		...
		sb.append(DisplayUtils.createWikiAttachmentUrl(synapseJsniUtils.getBaseFileHandleUrl(), wikiKey,
				fileName, false));
		sb.append("\">");
		...
		if (lastDotIndex > -1) {
			...
			if (ContentTypeUtils.isRecognizedImageContentType("image/" + extension)) {
				...
				sb.append(" src=\"");
				sb.append(DisplayUtils.createWikiAttachmentUrl(synapseJsniUtils.getBaseFileHandleUrl(),
						wikiKey, fileName, true));
				sb.append("\" ></img>");
				isPreviewed = true;
			}
		}
