...
public class ButtonLinkWidget implements WidgetRendererPresenter {
	...
	public ButtonLinkWidget(ButtonLinkWidgetView view, GWTWrapper gwt,
			AuthenticationController authController, AppPlaceHistoryMapper appPlaceHistoryMapper,
			SynapseJavascriptClient jsClient) {
		this.view = view;
		this.gwt = gwt;
	...
	public void configure(final WikiPageKey wikiKey, final Map<String, String> widgetDescriptor,
		...
		if (url.contains("#!Synapse:")) {
			Synapse synapsePlace =
					(Synapse) appPlaceHistoryMapper.getPlace(url.substring(url.indexOf('!')));
		...
		if (descriptor.containsKey(WidgetConstants.ALIGNMENT_KEY)) {
			view.addStyleNames(
					ImageWidget.getAlignmentStyleNames(descriptor.get(WidgetConstants.ALIGNMENT_KEY)));
		}
