...
public class ButtonLinkWidgetViewImpl extends Div implements ButtonLinkWidgetView {
	...
	public static final ClickHandler BUTTON_LINK_CLICK_HANDLER = event -> {
		if (!DisplayUtils.isAnyModifierKeyDown(event)) {
			...
			boolean openInNewWindow =
					button.getElement().hasAttribute(ButtonLinkWidget.LINK_OPENS_NEW_WINDOW);
			if (openInNewWindow) {
				newWindow(href, "_blank", "");
			} else {
				if (href.contains(SYNAPSE_PLACE_FRAGMENT)
	...
	public void configure(WikiPageKey wikiKey, String buttonText, final String url,
			boolean isHighlight, final boolean openInNewWindow) {
		clear();
