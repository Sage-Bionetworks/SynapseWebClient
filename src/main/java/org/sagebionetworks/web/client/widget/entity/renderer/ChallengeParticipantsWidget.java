...
public class ChallengeParticipantsWidget
		implements UserListView.Presenter, WidgetRendererPresenter, PageChangeListener {
	...
	@Inject
	public ChallengeParticipantsWidget(UserListView view, BasicPaginationWidget paginationWidget,
	...
	public void onPageChange(final Long newOffset) {
		...
		view.showLoading();
		view.clearUsers();
		challengeClient.getChallengeParticipants(isInTeam, challengeId,
				DEFAULT_PARTICIPANT_LIMIT.intValue(), newOffset.intValue(),
				new AsyncCallback<UserProfilePagedResults>() {
					...
					public void onSuccess(UserProfilePagedResults results) {
						...
						if (results.getTotalNumberOfResults() > 0) {
							...
							paginationWidget.configure(DEFAULT_PARTICIPANT_LIMIT, newOffset,
									results.getTotalNumberOfResults(), ChallengeParticipantsWidget.this);
