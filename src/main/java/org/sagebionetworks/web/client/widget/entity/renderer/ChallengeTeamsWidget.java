...
public class ChallengeTeamsWidget
  implements
    ChallengeTeamsView.Presenter, WidgetRendererPresenter, PageChangeListener {
  ...
  @Inject
  public ChallengeTeamsWidget(
    ChallengeTeamsView view,
  ...
  public void onPageChange(final Long newOffset) {
    ...
    view.showLoading();
    view.clearTeams();
    challengeClient.getChallengeTeams(
      authController.getCurrentUserPrincipalId(),
      challengeId,
      DEFAULT_TEAM_LIMIT.intValue(),
      newOffset.intValue(),
        ...
        public void onSuccess(ChallengeTeamPagedResults results) {
          ...
          if (results.getTotalNumberOfResults() > 0) {
            ...
            paginationWidget.configure(
              ...
              ChallengeTeamsWidget.this
            );
            for (ChallengeTeamBundle challenge : results.getResults()) {
              teamId2ChallengeTeam.put(
                challenge.getChallengeTeam().getTeamId(),
                challenge.getChallengeTeam()
              );
              view.addChallengeTeam(
                challenge.getChallengeTeam().getTeamId(),
                DisplayUtils.replaceWithEmptyStringIfNull(
                  challenge.getChallengeTeam().getMessage()
                ),
                challenge.isAdmin()
              );
            }
