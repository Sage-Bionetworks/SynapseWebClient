...
public class HtmlPreviewWidget implements IsWidget, HtmlPreviewView.Presenter {
	...
	@Inject
	public HtmlPreviewWidget(HtmlPreviewView view, PresignedURLAsyncHandler presignedURLAsyncHandler,
			SynapseJSNIUtils jsniUtils, RequestBuilderWrapper requestBuilder, SynapseAlert synAlert,
			SynapseClientAsync synapseClient, PopupUtilsView popupUtils, GWTWrapper gwt) {
		this.view = view;
	...
	public void configure(String synapseId, FileHandle fileHandle) {
		...
		if (fileHandle.getContentSize() != null && fileHandle.getContentSize() < MAX_HTML_FILE_SIZE) {
			...
			view.setLoadingVisible(false);
			synAlert.showError("The preview was not shown because the size ("
					+ gwt.getFriendlySize(fileHandle.getContentSize().doubleValue(), true)
					+ ") exceeds the maximum preview size (" + friendlyMaxFileSize + ")");
		}
	}
