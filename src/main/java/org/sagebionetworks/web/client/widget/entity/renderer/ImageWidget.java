...
public class ImageWidget implements ImageWidgetView.Presenter, WidgetRendererPresenter {
	...
	public ImageWidget(ImageWidgetView view, AuthenticationController authenticationController,
			PresignedURLAsyncHandler presignedURLAsyncHandler, SynapseJavascriptClient jsClient,
			SynapseAlert synAlert) {
		this.view = view;
	...
	private void loadFromFileHandleAssociation(FileHandleAssociation fha) {
		presignedURLAsyncHandler.getFileResult(fha, new AsyncCallback<FileResult>() {
			public void onSuccess(FileResult fileResult) {
				view.configure(fileResult.getPreSignedURL(), descriptor.get(IMAGE_WIDGET_FILE_NAME_KEY),
						descriptor.get(IMAGE_WIDGET_SCALE_KEY), descriptor.get(ALIGNMENT_KEY),
						descriptor.get(IMAGE_WIDGET_ALT_TEXT_KEY), descriptor.get(IMAGE_WIDGET_SYNAPSE_ID_KEY),
	...
	public void configure(final WikiPageKey wikiKey, final Map<String, String> widgetDescriptor,
		...
		if (synapseId != null) {
			...
			jsClient.getWikiAttachmentFileHandles(wikiKey, wikiVersionInView,
					new AsyncCallback<List<FileHandle>>() {
