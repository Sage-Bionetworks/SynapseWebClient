    ...
    image.addErrorHandler(
      new ErrorHandler() {
        ...
        public void onError(ErrorEvent event) {
          if (!hasTriedCache) {
            hasTriedCache = true;
            String newUrl = clientCache.get(
              fileName + WebConstants.TEMP_IMAGE_ATTACHMENT_SUFFIX
            );
          ...
          if (synapseId != null) {
            if (!isLoggedIn) p.handleLoadingError(
              DisplayConstants.IMAGE_FAILED_TO_LOAD +
              "You may need to log in to gain access to this image content (" +
              synapseId +
              ")"
            ); else p.handleLoadingError(
              DisplayConstants.IMAGE_FAILED_TO_LOAD +
              "Unable to view image " +
              synapseId
            );
          } else if (fileName != null) p.handleLoadingError(
            DisplayConstants.IMAGE_FAILED_TO_LOAD + fileName
          ); else p.handleLoadingError(
            DisplayConstants.IMAGE_FAILED_TO_LOAD + url
          );
        }
      }
    );
        ...
        public void onLoad(LoadEvent event) {
          try {
            ...
            p.handleLoadingError(
              DisplayConstants.IMAGE_FAILED_TO_LOAD + e.getMessage()
            );
          }
        }
