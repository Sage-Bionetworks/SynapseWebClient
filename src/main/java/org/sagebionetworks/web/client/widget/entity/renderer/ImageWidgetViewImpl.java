...
public class ImageWidgetViewImpl extends FlowPanel implements ImageWidgetView {
	...
	// if image fails to load from the given source, it will try to load from the cache (this is for
	// the case when the image has been uploaded, but the wiki has not yet been saved)
	...
	@Inject
	public ImageWidgetViewImpl(GlobalApplicationState globalApplicationState,
	...
	@Override
	public void configure(final String url, final String fileName, final String scale,
			String alignment, String altText, final String synapseId, final boolean isLoggedIn) {
		clear();
		add(synAlert);
		hasTriedCache = false;
		...
		// don't show until we have the correct size (otherwise it's initially shown at 100%, then
		// scaled down!).
		image.addErrorHandler(new ErrorHandler() {
			...
			public void onError(ErrorEvent event) {
				...
				if (synapseId != null) {
					if (!isLoggedIn)
						p.handleLoadingError(DisplayConstants.IMAGE_FAILED_TO_LOAD
								+ "You may need to log in to gain access to this image content (" + synapseId
								+ ")");
					else
						p.handleLoadingError(
								DisplayConstants.IMAGE_FAILED_TO_LOAD + "Unable to view image " + synapseId);
				}
