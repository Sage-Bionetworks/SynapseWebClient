...
import static org.sagebionetworks.web.client.widget.entity.tabs.AbstractTablesTab.TABLE_QUERY_PREFIX;
 ...
 *
 */
public class PlotlyWidget
  implements PlotlyWidgetView.Presenter, WidgetRendererPresenter {
  ...
  public static final Long DEFAULT_PART_MASK =
  ...
  public void configure(
    ...
    xAxisType =
      descriptor.containsKey(X_AXIS_TYPE)
        ? AxisType.valueOf(descriptor.get(X_AXIS_TYPE).toUpperCase())
        : AxisType.AUTO;
    yAxisType =
      descriptor.containsKey(Y_AXIS_TYPE)
        ? AxisType.valueOf(descriptor.get(Y_AXIS_TYPE).toUpperCase())
        : AxisType.AUTO;
    showLegend =
      descriptor.containsKey(SHOW_LEGEND)
        ? Boolean.valueOf(descriptor.get(SHOW_LEGEND))
        : true;
    isHorizontal =
      descriptor.containsKey(IS_HORIZONTAL)
  ...
  public void showChart() {
    ...
    try {
      ...
      if (fillColumnName != null) {
        plotlyGraphData =
          transform(xAxisColumnName, fillColumnName, graphType, graphData);
      } else {
   ...
   * @return
   */
  public static List<PlotlyTraceWrapper> transform(
  ...
  public static List<PlotlyTraceWrapper> transform(
    ...
    String[] fillColumnData = ArrayUtils.getStringArray(
      graphData.remove(fillColumnName)
    );
    ...
    for (String columnName : graphData.keySet()) {
      for (String targetFillColumnValue : uniqueFillColumnDataValues) {
        ...
        String traceName = targetFillColumnValue == null
          ? ""
          : targetFillColumnValue;
        if (yColumnCount > 1) {
  ...
  public void onClick(String x, String y) {
    boolean xNameIsAlias = sql
      .substring(0, sql.indexOf(','))
      .toUpperCase()
      .contains(" AS ");
    if (!xNameIsAlias) {
