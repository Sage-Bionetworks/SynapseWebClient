...
public class ShinySiteWidget implements WidgetRendererPresenter {
	...
	private static final String[] VALID_URL_BASES =
			{"http://spark.rstudio.com/", "https://s3.amazonaws.com/static.synapse.org/",
					"https://docs.google.com/a/sagebase.org/forms/", "https://calendar.google.com/calendar/",
					"https://sage-bionetworks.shinyapps.io/", "https://dce.seattlebiosoftware.com/",
	...
	@Inject
	public ShinySiteWidget(IFrameView view, AuthenticationController authenticationController,
	...
	public void configure(WikiPageKey wikiKey, Map<String, String> widgetDescriptor,
		...
		if (isValidShinySite(siteUrl, jsniUtils)) {
			...
			// if we should include the current user's principal id, then append ?principal=<principal> to
			// the siteUrl
			if (includePrincipleId && authenticationController.isLoggedIn()) {
				String delimiter = siteUrl.contains("?") ? "&" : "?";
				siteUrl = siteUrl + delimiter + "principalId="
	...
	public static boolean isIncludePrincipalId(Map<String, String> descriptor) {
		...
		if (descriptor.containsKey(WidgetConstants.INCLUDE_PRINCIPAL_ID_KEY)) {
			try {
				isIncludePrincipleId =
						Boolean.parseBoolean(descriptor.get(WidgetConstants.INCLUDE_PRINCIPAL_ID_KEY));
			} catch (Throwable e) {
				// fall back to default
			}
		}
