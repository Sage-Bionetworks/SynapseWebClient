...
public class SubmitToEvaluationWidget
  implements SubmitToEvaluationWidgetView.Presenter, WidgetRendererPresenter {
  ...
  @Inject
  public SubmitToEvaluationWidget(
    SubmitToEvaluationWidgetView view,
  ...
  public void configure(
    ...
    evaluationUnavailableMessage =
      descriptor.get(WidgetConstants.UNAVAILABLE_MESSAGE);
    ...
    getEvaluationIds(
      descriptor,
      new CallbackP<Set<String>>() {
        @Override
        public void invoke(Set<String> evalIds) {
          evaluationIds = evalIds;
          final String buttonText = descriptor.get(
            WidgetConstants.BUTTON_TEXT_KEY
          );
              ...
              public void onFailure(Throwable caught) {
                ...
                if (!(caught instanceof ForbiddenException)) {
                  view.showUnavailable(
                    DisplayConstants.EVALUATION_SUBMISSION_ERROR +
                    caught.getMessage()
                  );
                }
              }
   ...
   * @param callback
   */
  public void getEvaluationIds(
    Map<String, String> descriptor,
    final CallbackP<Set<String>> callback
  ) {
    ...
    if (
      ...
      descriptor.get(WidgetConstants.JOIN_WIDGET_SUBCHALLENGE_ID_LIST_KEY);
    if (subchallengeIdList != null) {
      String[] evaluationIdsArray = subchallengeIdList.split(
        WidgetConstants.JOIN_WIDGET_SUBCHALLENGE_ID_LIST_DELIMETER
      );
      Set<String> evaluationIds = new HashSet<String>();
      for (String evaluationId : evaluationIdsArray) {
        evaluationIds.add(evaluationId);
      }
      if (!evaluationIds.isEmpty()) callback.invoke(evaluationIds); else {
        view.showUnavailable(evaluationUnavailableMessage);
      }
      return;
    } else {
      AsyncCallback<Set<String>> asyncCallback = new AsyncCallback<Set<String>>() {
        ...
        public void onSuccess(Set<String> evaluationIds) {
          ...
          if (!evaluationIds.isEmpty()) callback.invoke(
            evaluationIds
          ); else view.showUnavailable(evaluationUnavailableMessage);
        }

        @Override
        public void onFailure(Throwable caught) {
          view.showErrorMessage(
            DisplayConstants.CHALLENGE_EVALUATIONS_ERROR + caught.getMessage()
          );
        }
      };
  ...
  public void showSubmissionDialog() {
    ...
    if (formContainerId != null) {
      formParams =
        new FormParams(formContainerId, formSchemaId, formUiSchemaId);
    }
