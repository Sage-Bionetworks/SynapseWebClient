...
public class SubmitToEvaluationWidget
	...
	@Inject
	public SubmitToEvaluationWidget(SubmitToEvaluationWidgetView view,
			ChallengeClientAsync challengeClient, AuthenticationController authenticationController,
			GlobalApplicationState globalApplicationState, PortalGinInjector ginInjector) {
		this.view = view;
	...
	public void configure(final WikiPageKey wikiKey, final Map<String, String> widgetDescriptor,
		...
		getEvaluationIds(descriptor, new CallbackP<Set<String>>() {
			...
			public void invoke(Set<String> evalIds) {
				...
				try {
					challengeClient.getAvailableEvaluations(evaluationIds,
							new AsyncCallback<PaginatedResults<Evaluation>>() {
								...
								public void onFailure(Throwable caught) {
									// if the user can't read the evaluation, then don't show the join button. if
									// there was some other error, then report it...
									if (!(caught instanceof ForbiddenException)) {
										view.showUnavailable(
												DisplayConstants.EVALUATION_SUBMISSION_ERROR + caught.getMessage());
									}
								}
							});
	 ...
	 * @param callback
	 */
	public void getEvaluationIds(Map<String, String> descriptor,
		...
		if (subchallengeIdList != null) {
			String[] evaluationIdsArray =
					subchallengeIdList.split(WidgetConstants.JOIN_WIDGET_SUBCHALLENGE_ID_LIST_DELIMETER);
