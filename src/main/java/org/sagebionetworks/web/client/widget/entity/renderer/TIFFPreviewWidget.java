...
public class TIFFPreviewWidget implements IsWidget {
	...
	@Inject
	public TIFFPreviewWidget(TIFFPreviewWidgetView view, ResourceLoader resourceLoader, SynapseJSNIUtils jsniUtils, GWTWrapper gwt) {
		this.view = view;
		...
		ClientProperties.fixResourceToCdnEndpoint(UTIF_JS, jsniUtils.getCdnEndpoint());
	...
	public void refreshContent() {
		if (fileHandle.getContentSize() != null && fileHandle.getContentSize() < MAX_TIFF_FILE_SIZE) {
			if (fha != null) {
				String url = jsniUtils.getFileHandleAssociationUrl(fha.getAssociateObjectId(), fha.getAssociateObjectType(), fha.getFileHandleId());
				view.configure(url);
			}
		} else {
			view.showError("The TIFF preview was not shown because the file size (" + gwt.getFriendlySize(fileHandle.getContentSize().doubleValue(), true) + ") exceeds the maximum preview size (" + friendlyMaxFileSize + ")");
		}
	}
}
