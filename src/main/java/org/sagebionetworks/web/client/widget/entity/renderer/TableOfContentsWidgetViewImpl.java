...
public class TableOfContentsWidgetViewImpl extends FlowPanel implements TableOfContentsWidgetView {
	...
	private RegExp widgetParamsRegEx =
			RegExp.compile("(data[-]widgetparams[=]{1}[\"]{1}[^\"]*)([\"]{1})", "i");
	...
	private RegExp widgetInnerHtmlRegEx =
			RegExp.compile("(<span data[-]widgetparams[^>]*>).*(<\\/span>)", "i");
	...
	protected void onAttach() {
		...
		if (!hasLoaded) {
			...
			// look for these special header ids (that were added by the markdown processor for us), and
			// create links to them

			if (headingElements.length() == 0) {
				// no entries. add an informative message
				linkContainer.add(
						new HTML("<p class=\"smallGreyText\">" + DisplayConstants.NO_HEADERS_FOUND + "</p>"));
			}
			...
			for (int j = 0; j < headingElements.length(); j++) {
				...
				if (heading.hasAttribute("toc-style") || heading.hasAttribute("toc")) {
					...
					if (widgetExistsRegEx.test(headingHtml)) {
						...
						// also add the isTOC parameter to the widget definition. Replace with the first group +
						// new param + second group.
						headingHtml = widgetParamsRegEx.replace(headingHtml,
