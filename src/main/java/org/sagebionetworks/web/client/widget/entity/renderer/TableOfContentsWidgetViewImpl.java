...
public class TableOfContentsWidgetViewImpl
  extends FlowPanel
  implements TableOfContentsWidgetView {

  private Presenter presenter;
  private boolean hasLoaded;
  private Map<String, String> tagName2Style;
  // Matches a widget element id. Group 1 contains the suffix (that all widgets will use for this
  // particular markdown renderer).
  private RegExp widgetIdRegEx = RegExp.compile(
    "^widget[-]{1}\\d+([-]{1}.*)$",
    "i"
  );
  // Matches a widget element id. Group 1 contains prefix, Group 2 contains suffix. Used to replace
  // widget index (with the next available index, so that the markdown renderer will find it).
  private RegExp widgetExistsRegEx = RegExp.compile(
    "(widget[-]{1})\\d+([-]{1})",
    "i"
  );
  // Matches widget parameters. Group 1 contains full param definition minus the end quote. Group 2
  // contains end quote.
  // Used to insert new isTOC parameter into widget definition (badge will ignore clicks).
  private RegExp widgetParamsRegEx = RegExp.compile(
    "(data[-]widgetparams[=]{1}[\"]{1}[^\"]*)([\"]{1})",
    "i"
  );
  // Matches any html inside of the widget span. Used to remove any other html (for example,
  // mentioning adds a child user/team anchor, used in notification emails).
  private RegExp widgetInnerHtmlRegEx = RegExp.compile(
    "(<span data[-]widgetparams[^>]*>).*(<\\/span>)",
    "i"
  );
  ...
  private int getNewWidgetIndex() {
    ...
    do {
      ...
      String currentWidgetDiv =
        WidgetConstants.DIV_ID_WIDGET_PREFIX + i + widgetSuffix;
  ...
  protected void onAttach() {
    ...
    if (!hasLoaded) {
      ...
      JsArray<Element> headingElements = _localHeaderElements(
        this.getElement()
      );

      // look for these special header ids (that were added by the markdown processor for us), and create
      // links to them
      ...
      if (headingElements.length() == 0) {
        ...
        linkContainer.add(
          new HTML(
            ...
            DisplayConstants.NO_HEADERS_FOUND +
            "</p>"
          )
        );
      }
      ...
      for (int j = 0; j < headingElements.length(); j++) {
        ...
        if (heading.hasAttribute("toc-style") || heading.hasAttribute("toc")) {
          ...
          if (widgetExistsRegEx.test(headingHtml)) {
            ...
            headingHtml =
              widgetExistsRegEx.replace(
                headingHtml,
                "$1" + newWidgetIndex + "$2"
              );
            // also add the isTOC parameter to the widget definition. Replace with the first group + new param +
            // second group.
            headingHtml =
              widgetParamsRegEx.replace(
                headingHtml,
                "$1&" + WidgetConstants.IS_TOC_KEY + "=true$2"
              );
            newWidgetIndex++;
          }

          HTML html = new HTML(
            "<a class=\"link " + tocStyle + "\">" + headingHtml + "</a>"
          );

          html.addClickHandler(
            new ClickHandler() {
              @Override
