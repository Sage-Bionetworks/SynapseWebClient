...
public class WikiSubpageNavigationTree
  ...
  @Inject
  public WikiSubpageNavigationTree(
    WikiSubpageNavigationTreeView view,
  ...
  public void configure(
    ...
    for (V2WikiHeader header : wikiHeaders) {
      ...
      if (header.getParentId() == null) {
        ...
        isCurrentWikiRoot =
          currentWikiKey.getWikiPageId() == null ||
          currentWikiKey.getWikiPageId().equals(header.getId());
      } else {
        targetPlace =
          WikiSubpagesWidget.getLinkPlace(
      ...
      SubpageNavTreeNode node = new SubpageNavTreeNode(
        pageTitle,
        targetPlace,
        wikiPageKey
      );
  ...
  public boolean isCurrentPage(SubpageNavTreeNode root) {
    return root
      .getWikiPageKey()
      .getWikiPageId()
      .equals(this.currentWikiKey.getWikiPageId());
  }
}
