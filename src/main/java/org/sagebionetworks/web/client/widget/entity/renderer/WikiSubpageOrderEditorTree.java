...
public class WikiSubpageOrderEditorTree
	...
	@Inject
	public WikiSubpageOrderEditorTree(WikiSubpageOrderEditorTreeView view, SynapseAlert synAlert,
			SynapseJavascriptClient jsClient) {
		this.view = view;
	...
	public void configure(String selectWikiPageId, WikiPageKey wikiKey,
			List<V2WikiHeader> wikiHeaders, String ownerObjectName, V2WikiOrderHint hint,
			CallbackP<String> refreshCallback) {
	...
	public void selectTreeItem(SubpageOrderEditorTreeNode node) {
		...
		if (movabilityCallback != null) {
			movabilityCallback.invoke(selectedCanMoveUpOrRight(), selectedCanMoveDown(),
					selectedCanMoveLeft());
		}
	}
	...
	public void moveRight() {
		...
		SubpageOrderEditorTreeNode siblingAboveSelected =
				parent.getChildren().get(selectedChildIndex - 1);
