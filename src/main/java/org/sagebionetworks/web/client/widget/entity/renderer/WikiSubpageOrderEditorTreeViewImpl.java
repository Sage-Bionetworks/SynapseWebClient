...
public class WikiSubpageOrderEditorTreeViewImpl
  extends FlowPanel
  implements WikiSubpageOrderEditorTreeView {
  ...
  public void moveTreeItem(SubpageOrderEditorTreeNode node, boolean moveUp) {
    ...
    UnorderedListPanel childrenList = listItem2childrenList.get(
      headerId2listItem.get(node.getHeader().getId())
    );

    SubpageOrderEditorTreeNode parent = presenter.getParent(node);
    UnorderedListPanel parentPanel = listItem2childrenList.get(
      headerId2listItem.get(node.getHeader().getParentId())
    );

    int insertIndex;
    if (moveUp) {
      SubpageOrderEditorTreeNode sibling = parent
        .getChildren()
        .get(parent.getChildren().indexOf(node) - 1);
      Widget siblingListItem = headerId2listItem.get(
        sibling.getHeader().getId()
      );
  ...
  private void swapWidgetInPanel(
    UnorderedListPanel parentPanel,
    Widget toInsert,
    int insertIndex,
    boolean moveUp
  ) {
    ...
    if (moveUp) parentPanel.insert(toInsert, insertIndex); else {
      if (willBeLastItem) {
  ...
  public void configure(
    SubpageOrderEditorTreeNode overallRoot,
    String ownerObjectId
  ) {
  ...
  private void addTreeItemsRecursive(
    UnorderedListPanel ul,
  ...
  public void selectTreeItem(SubpageOrderEditorTreeNode toSelect) {
    if (toSelect == null) throw new IllegalArgumentException(
      "Node to select cannot be null"
    );
    // Remove style from previously selected list item.
    SubpageOrderEditorTreeNode prevSelected = presenter.getSelectedTreeItem();
    if (prevSelected != null) {
      UnorderedListPanel.removeStyleNameFromListItem(
        headerId2listItem.get(prevSelected.getHeader().getId()),
        "active"
      );
    }

    // Add style to the selecting list item.
    UnorderedListPanel.addStyleNameToListItem(
      headerId2listItem.get(toSelect.getHeader().getId()),
      "active"
    );
