...
public class WikiSubpageOrderEditorTreeViewImpl extends FlowPanel
	...
	public void moveTreeItem(SubpageOrderEditorTreeNode node, boolean moveUp) {
		...
		UnorderedListPanel childrenList =
				listItem2childrenList.get(headerId2listItem.get(node.getHeader().getId()));


		SubpageOrderEditorTreeNode parent = presenter.getParent(node);
		UnorderedListPanel parentPanel =
				listItem2childrenList.get(headerId2listItem.get(node.getHeader().getParentId()));

		int insertIndex;
		if (moveUp) {
			SubpageOrderEditorTreeNode sibling =
					parent.getChildren().get(parent.getChildren().indexOf(node) - 1);
	...
	private void swapWidgetInPanel(UnorderedListPanel parentPanel, Widget toInsert, int insertIndex,
			boolean moveUp) {
	...
	public void selectTreeItem(SubpageOrderEditorTreeNode toSelect) {
		...
		if (prevSelected != null) {
			UnorderedListPanel.removeStyleNameFromListItem(
					headerId2listItem.get(prevSelected.getHeader().getId()), "active");
		}

		// Add style to the selecting list item.
		UnorderedListPanel.addStyleNameToListItem(headerId2listItem.get(toSelect.getHeader().getId()),
				"active");
	...
	private Widget makeListItem(final SubpageOrderEditorTreeNode node) {
		...
		l.addStyleName("link");
		l.setHref("#!Synapse:"
				+ new Synapse(ownerObjectId, null, EntityArea.WIKI, node.getHeader().getId()).toToken());
