...
public class WikiSubpagesViewImpl extends FlowPanel implements WikiSubpagesView {
	...
	public WikiSubpagesViewImpl(WikiSubpagesOrderEditor orderEditor,
			WikiSubpageNavigationTree navTree, EventBus eventBus, SynapseJSNIUtils jsniUtils,
	...
	@Override
	public void configure(final List<V2WikiHeader> wikiHeaders, final String ownerObjectName,
			Place ownerObjectLink, final WikiPageKey curWikiKey, boolean isEmbeddedInOwnerPage,
			CallbackP<WikiPageKey> wikiPageCallback, ActionMenuWidget actionMenu) {
		clear();
		...
		navTree.configure(wikiHeaders, ownerObjectName, ownerObjectLink, curWikiKey,
				isEmbeddedInOwnerPage, wikiPageCallback);
		...
		if (actionMenu != null) {
			actionMenu.setActionListener(Action.REORDER_WIKI_SUBPAGES,
