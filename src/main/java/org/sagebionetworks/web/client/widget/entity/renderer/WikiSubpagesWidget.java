...
public class WikiSubpagesWidget implements IsWidget, WikiSubpagesView.Presenter {
	...
	public WikiSubpagesWidget(WikiSubpagesView view,
			AuthenticationController authenticationController, SynapseJavascriptClient jsClient) {
		this.view = view;
		this.jsClient = jsClient;
		view.setPresenter(this);
	}

	public void configure(final WikiPageKey wikiKey, boolean embeddedInOwnerPage,
			CallbackP<WikiPageKey> reloadWikiPageCallback, ActionMenuWidget actionMenu) {

		// SWC-4177: if loading a page within the currently shown nav tree, don't reload the tree.
		if (this.wikiKey != null && this.wikiKey.getOwnerObjectId().equals(wikiKey.getOwnerObjectId())
	...
	public void refreshRootObject() {
		...
		if (wikiKey.getOwnerObjectType().equalsIgnoreCase(ObjectType.ENTITY.toString())) {
			...
			jsClient.getEntityBundle(wikiKey.getOwnerObjectId(), bundleRequest,
					new AsyncCallback<EntityBundle>() {
						...
						public void onSuccess(EntityBundle bundle) {
							...
							ownerObjectLink = getLinkPlace(bundle.getEntity().getId(), wikiKey.getVersion(), null,
	...
	public static Place getLinkPlace(String entityId, Long entityVersion, String wikiId,
	...
	public void refreshWikiHeaderTree() {
		...
		jsClient.getV2WikiHeaderTree(wikiKey.getOwnerObjectId(), wikiKey.getOwnerObjectType(),
				new AsyncCallback<List<V2WikiHeader>>() {
					...
					@Override
					public void onFailure(Throwable caught) {
						// if this is because the wiki header root was not found, and the parent wiki id is
						// null,
	...
	private void afterGetNewHeaderTreeAndOrderHint() {
		if (newWikiHeaders != null && newWikiOrderHint != null) {
			...
			if (currentWikiHeaders == null || !currentWikiHeaders.equals(newWikiHeaders)) {
				view.clear();
				view.configure(newWikiHeaders, ownerObjectName, ownerObjectLink, wikiKey,
						isEmbeddedInOwnerPage, reloadWikiPageCallback, actionMenu);
