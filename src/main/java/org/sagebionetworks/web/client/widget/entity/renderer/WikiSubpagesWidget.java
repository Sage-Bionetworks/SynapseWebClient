...
public class WikiSubpagesWidget
  implements IsWidget, WikiSubpagesView.Presenter {
  ...
  @Inject
  public WikiSubpagesWidget(
    WikiSubpagesView view,
  ...
  public void refreshRootObject() {
    ...
    if (
      wikiKey
        .getOwnerObjectType()
        .equalsIgnoreCase(ObjectType.ENTITY.toString())
    ) {
  ...
  public static Place getLinkPlace(
    String entityId,
    Long entityVersion,
    String wikiId,
    boolean isEmbeddedInOwnerPage
  ) {
    if (isEmbeddedInOwnerPage) return new Synapse(
      entityId,
      entityVersion,
      Synapse.EntityArea.WIKI,
      wikiId
    ); else return new Wiki(entityId, ObjectType.ENTITY.toString(), wikiId);
  }
  ...
  private void afterGetNewHeaderTreeAndOrderHint() {
    if (newWikiHeaders != null && newWikiOrderHint != null) {
      WikiOrderHintUtils.sortHeadersByOrderHint(
        newWikiHeaders,
        newWikiOrderHint
      );
      if (
        currentWikiHeaders == null || !currentWikiHeaders.equals(newWikiHeaders)
      ) {
        view.clear();
        view.configure(
          newWikiHeaders,
