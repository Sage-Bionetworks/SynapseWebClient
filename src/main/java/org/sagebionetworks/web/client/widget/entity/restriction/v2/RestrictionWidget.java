...
public class RestrictionWidget
	...
	public RestrictionWidget(RestrictionWidgetView view,
			AuthenticationController authenticationController, DataAccessClientAsync dataAccessClient,
			SynapseAlert synAlert, IsACTMemberAsyncHandler isACTMemberAsyncHandler,
			SynapseJavascriptClient jsClient, GWTWrapper gwt, SynapseJSNIUtils jsniUtils) {
		this.view = view;
	...
	public boolean includeRestrictionWidget() {
		return (entity instanceof FileEntity) || (entity instanceof TableEntity)
				|| (entity instanceof Folder) || (showIfProject && entity instanceof Project);
	}
	...
	public void loadRestrictionInformation() {
		view.clear();
		synAlert.clear();
		jsClient.getRestrictionInformation(entity.getId(), RestrictableObjectType.ENTITY,
	...
	public void configureUI(RestrictionInformationResponse restrictionInformation) {
		...
		// show the info link if there are any restrictions, or if we are supposed to show the flag link
		// (to allow people to flag or admin to "change" the data access level).
	...
	public void imposeRestrictionOkClicked() {
		...
		} else if ((isYesSelected == null || !isYesSelected)
	...
	public void reportIssueClicked() {
		...
		String userId = WebConstants.ANONYMOUS, email = WebConstants.ANONYMOUS,
				displayName = WebConstants.ANONYMOUS, synId = entity.getId();
		...
		jsniUtils.showJiraIssueCollector("", // summary
				...
				FLAG_ISSUE_COLLECTOR_URL, userId, displayName, email, synId, // Synapse data object ID
	...
	public void linkClicked() {
		view.open(
				"#!AccessRequirements:" + AccessRequirementsPlace.ID_PARAM + "=" + entity.getId() + "&"
