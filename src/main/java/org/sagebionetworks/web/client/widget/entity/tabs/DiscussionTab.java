...
public class DiscussionTab implements DiscussionTabView.Presenter {
	...
	public DiscussionTab(Tab tab, PortalGinInjector ginInjector) {
		...
		tab.configure("Discussion", "Engage your collaborators in project specific Discussions.",
				WebConstants.DOCS_URL + "discussion.html");
	}
	...
	public void configure(final String entityId, String entityName, String areaToken,
		...
		CallbackP<ParameterizedToken> updateParamsCallback = token -> {
			updatePlace(
					new Synapse(entityId, PROJECT_VERSION_NUMBER, EntityArea.DISCUSSION, token.toString()));
		};
		...
		forumWidget.configure(entityId, params, isCurrentUserModerator, actionMenu,
				updateParamsCallback, updateURLCallback);
	...
	public void checkForSynapseForum() {
		String forumSynapseId =
				synapseProperties.getSynapseProperty(WebConstants.FORUM_SYNAPSE_ID_PROPERTY);
