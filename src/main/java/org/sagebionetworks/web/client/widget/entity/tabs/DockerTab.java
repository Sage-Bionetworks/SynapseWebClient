...
public class DockerTab implements DockerTabView.Presenter {
	...
	public DockerTab(Tab tab, PortalGinInjector ginInjector) {
		...
		tab.configure(DOCKER_TAB_TITLE, "docker", "A [Docker](https://www.docker.com/what-docker) container is a convenient way to bundle up code and dependencies into a lightweight virtual machine to support reusable and reproducible analysis.", WebConstants.DOCS_URL + "Synapse-Docker-Registry.2011037752.html", EntityArea.DOCKER);
	...
	public void setTargetBundle(EntityBundle bundle) {
		if (bundle != null) {
			...
			if (isRepo) {
				...
				links.add(new LinkData(DOCKER, EntityTypeUtils.getEntityTypeForEntityClassName(DockerRepository.class.getName()), projectPlace));
