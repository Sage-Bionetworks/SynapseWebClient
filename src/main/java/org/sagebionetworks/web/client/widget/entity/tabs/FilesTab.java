...
public class FilesTab {
	...
	public FilesTab(Tab tab, PortalGinInjector ginInjector) {
		...
		tab.configure("Files",
				...
				WebConstants.DOCS_URL + "versioning.html");
	}
	...
	public void lazyInject() {
		if (view == null) {
			...
			discussionThreadListWidget
					.setThreadIdClickedCallback(new CallbackP<DiscussionThreadBundle>() {

						@Override
						public void invoke(DiscussionThreadBundle bundle) {
							globalApplicationState.getPlaceChanger()
									.goTo(TopicUtils.getThreadPlace(bundle.getProjectId(), bundle.getId()));
						}
					});
	...
	public void setTargetBundle(EntityBundle bundle, final Long versionNumber) {
		...
		if (isWikiPageVisible) {
			...
			wikiPageWidget.configure(new WikiPageKey(currentEntityId, ObjectType.ENTITY.toString(),
					bundle.getRootWikiId(), versionNumber), canEdit, wikiCallback);
			CallbackP<String> wikiReloadHandler = new CallbackP<String>() {
				@Override
				public void invoke(String wikiPageId) {
					wikiPageWidget.configure(new WikiPageKey(currentEntityId, ObjectType.ENTITY.toString(),
							wikiPageId, versionNumber), canEdit, wikiCallback);
				}
			};
