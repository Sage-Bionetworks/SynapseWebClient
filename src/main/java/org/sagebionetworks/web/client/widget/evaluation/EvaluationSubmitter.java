...
public class EvaluationSubmitter implements Presenter {
  ...
  @Inject
  public EvaluationSubmitter(
    EvaluationSubmitterView view,
  ...
  public void configureWithDockerCommit(Entity submissionEntity) {
    dockerDigest = null;
    dockerCommitList.setEmptyListCallback(
      new Callback() {
        @Override
        public void invoke() {
  ...
  private void configureWithEvaluations() {
    jsClient.getAvailableEvaluations(
      evaluationIds,
      true,
      Integer.MAX_VALUE,
      0,
      ...
      public void onSuccess(List<Evaluation> evaluations) {
        if (evaluations == null || evaluations.size() == 0) {
          ...
          view.showErrorMessage(
            DisplayConstants.NOT_PARTICIPATING_IN_ANY_EVALUATIONS
          );
        } else {
  ...
  public void onNextClicked(
    ...
    if (submissionEntity != null) {
      ...
      if (submissionEntity instanceof Versionable) submissionEntityVersion =
        ((Versionable) submissionEntity).getVersionNumber();
    } else {
  ...
  public void queryForChallenge() {
    ...
    challengeClient.getChallengeForProject(
      evaluation.getContentSource(),
  ...
  public void refreshRegisteredTeams() {
    challengeClient.getSubmissionTeams(
  ...
  public void onNewTeamClicked() {
    if (authenticationController.isLoggedIn()) globalApplicationState
      .getPlaceChanger()
      .goTo(
        new Profile(
          authenticationController.getCurrentUserPrincipalId() +
  ...
  public void getContributorList(
      ...
      public void onSuccess(TeamSubmissionEligibility teamEligibility) {
        ...
        if (!teamSubmissionEligibility.getIsEligible()) {
          ...
          if (!teamSubmissionEligibility.getIsRegistered()) {
            ...
            reason =
              selectedTeam.getName() + " has reached the submission quota.";
          }
          view.setTeamInEligibleError(reason);
        } else {
          selectedTeamMemberStateHash =
            teamEligibility.getEligibilityStateHash().toString();
          ...
          for (MemberSubmissionEligibility memberEligibility : teamEligibility.getMembersEligibility()) {
            if (memberEligibility.getIsEligible()) {
              selectedTeamEligibleMembers.add(
                memberEligibility.getPrincipalId()
              );
              view.addEligibleContributor(
                memberEligibility.getPrincipalId().toString()
              );
            } else {
              String reason = ""; // unknown reason
    ...
    challengeClient.getTeamSubmissionEligibility(
      evaluation.getId(),
      selectedTeam.getId(),
      callback
    );
    ...
    jsClient.getEntity(
      ...
      new AsyncCallback<Entity>() {
        public void onSuccess(Entity result) {
          ...
          entity = result;
          Long v = null;
          if (ver != null) v = ver; else if (entity instanceof Versionable) v =
            ((Versionable) entity).getVersionNumber(); else {
  ...
  public void submitToEvaluation(
    String entityId,
    Long versionNumber,
    final String etag
  ) {
  ...
  public void submitToEvaluation(
    ...
    try {
      ...
      if (isIndividualSubmission) {
        challengeClient.createIndividualSubmission(
          newSubmission,
          etag,
        ...
        challengeClient.createTeamSubmission(
          newSubmission,
          etag,
          memberStateHash,
          gwt.getHostPageBaseURL(),
    ...
    return new AsyncCallback<Submission>() {
      ...
      public void onSuccess(Submission result) {
        ...
        if (message == null || message.length() == 0) message =
          DisplayConstants.SUBMISSION_RECEIVED_TEXT;
        view.hideModal2();
