...
public class EvaluationSubmitterViewImpl implements EvaluationSubmitterView {
	...
	@Inject
	public EvaluationSubmitterViewImpl(Binder binder, EntityFinder entityFinder,
			EvaluationList evaluationList, PortalGinInjector ginInjector,
			AuthenticationController authController) {
	...
	public void showSubmissionAcceptedDialogs(String message) {
		DisplayUtils.showInfoDialog(DisplayConstants.THANK_YOU_FOR_SUBMISSION,
				SafeHtmlUtils.htmlEscape(message), null);
	}
	...
	public void showModal1(boolean isEntitySet, FormParams formParams, List<Evaluation> evaluations) {
		...
		if (isForm) {
			// here we go! add the SRC entityform component to the formDiv (and listen for detach for
			// cleanup)
			...
			_showForm(formDiv.getElement(), sessionToken, formParams.getContainerSynId(),
					formParams.getJsonSchemaSynId(), formParams.getUiSchemaSynId(), this);
		} else {
	...
	private static native void _showForm(Element el, String sessionToken, String parentContainerSynId,
			String jsonSchemaSynId, String uiSchemaSynId, EvaluationSubmitterViewImpl view) /*-{
		try {
