...
public class HeaderViewImpl extends Composite implements HeaderView {
	...
	@Inject
	public HeaderViewImpl(Binder binder, SageImageBundle sageImageBundle, SearchBox searchBox,
			CookieProvider cookies, UserBadge userBadge, GlobalApplicationState globalAppState) {
	...
	public void setUser(UserProfile profile) {
		...
		if (profile != null) {
			// has user data, update the user name and add user commands (and set to the current user
			// name)
		...
		if (visible) {
			try {
				if (json.has("backgroundColor")) {
					...
					portalAlert.getElement().setAttribute("style",
							oldStyle + ";background-color: " + color + ";");
				}
