...
public class LazyLoadWikiWidgetWrapper implements IsWidget {
	// TOC is broken, and LoadMoreWidgetContainer always puts widget into block element (div). Change
	// to use LazyLoadHelper, a small load icon, and put widget in a Span.
	...
	@Inject
	public LazyLoadWikiWidgetWrapper(LazyLoadWikiWidgetWrapperView view,
			LazyLoadHelper lazyLoadHelper, SynapseJSNIUtils jsniUtils, WidgetRegistrar widgetRegistrar) {
	...
	public void configure(String contentTypeKey, WikiPageKey wikiKey,
	...
	public void lazyLoad() {
		widgetRegistrar.getWidgetRendererForWidgetDescriptorAfterLazyLoad(contentTypeKey,
				new AsyncCallback<WidgetRendererPresenter>() {
					@Override
					public void onSuccess(WidgetRendererPresenter wikiWidget) {
						wikiWidget.configure(wikiKey, widgetDescriptor, widgetRefreshRequired,
								wikiVersionInView);
						// use the renderer class name as a css selector (for widget usage statistics, and
						// possibly automated UI testing)
