 ...
 *
 */
public class EmailAddressesWidget
  implements EmailAddressesWidgetView.Presenter, IsWidget {
      ...
      jsClient.getNotificationEmail(
        new AsyncCallback<NotificationEmail>() {
          ...
          public void onSuccess(NotificationEmail notificationEmail) {
            clear();
            view.setLoadingVisible(false);
            view.addPrimaryEmail(
              notificationEmail.getEmail(),
              AuthenticationControllerImpl.isQuarantined(
                notificationEmail.getQuarantineStatus()
              )
            );
