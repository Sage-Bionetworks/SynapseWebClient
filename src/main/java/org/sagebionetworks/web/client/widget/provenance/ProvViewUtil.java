...
public class ProvViewUtil {
  ...
  private static final String PROV_ACTIVITY_TIME_STYLE =
    "provActivityLabelTime";
  private static final String PROV_VERSION_DISPLAY_STYLE = "provVersionDisplay";
  private static final String PROV_ACTIVITY_UNDEFINED_STYLE =
    "provUndefinedChange";
  ...
  private static final String ACT_MARGIN_NAME =
    "margin-top-5 margin-right-4 margin-bottom-10 margin-left-10";

  public static ProvNodeContainer createActivityContainer(
    ...
    if (node.getType() == ActivityType.UNDEFINED) {
      ...
      if (node.getActivityName() != null) {
        label.add(
          new HTML(
            DisplayUtils.stubStrPartialWord(
              node.getActivityName(),
              MAX_ACT_CODE_NAME_CHAR
            )
          )
        );
      }
      ...
      HTML time = new HTML(
        ginInjector.getDateTimeUtils().getLongFriendlyDate(node.getModifiedOn())
      );
  ...
  public static ProvNodeContainer createEntityContainer(
    EntityGraphNode node,
    IconsImageBundle iconsImageBundle
  ) {
    ProvNodeContainer container = createEntityContainer(
      node.getId(),
      node.getEntityId(),
      node.getName(),
  ...
  public static ProvNodeContainer createExpandContainer(
    final ExpandGraphNode node,
    ...
    container.setStyleName(
      PROV_ENTTITY_NODE_STYLE + " " + PROV_EXPAND_NODE_STYLE
    );
    container.add(link);
    setPosition(node, container);
    return container;
  ...
  public static ProvNodeContainer createExternalUrlContainer(
    ...
    ImageResource icon;
    if (
      node.getUrl().contains("github.com") ||
      node.getUrl().contains("githubusercontent.com")
    ) icon = iconsImageBundle.github16(); else if (
      node.getUrl().contains("genomespace.org")
    ) icon = iconsImageBundle.genomespace16(); else icon =
      iconsImageBundle.documentExternal16();
    builder.appendHtmlConstant(AbstractImagePrototype.create(icon).getHTML());
    builder.appendHtmlConstant("<br/>");

    // name
    String stubName = DisplayUtils.stubStrPartialWord(
      node.getName(),
      ENTITY_LINE_NUMBER_CHARS
    );
  ...
  private static ProvNodeContainer createEntityContainer(
    String id,
    String entityId,
    String name,
    String versionLabel,
    ...
    if (versionNumber != null) {
      link.setHref(
        DisplayUtils.getSynapseHistoryToken(entityId, versionNumber)
      );
    } else {
    ...
    // icon
    IconType iconType = EntityTypeUtils.getIconTypeForEntityClassName(
      entityType
    );
    if (FileEntity.class.getName().equals(entityType)) {
      if (
        ContentTypeUtils.isRecognizedCodeFileName(name) ||
        org.sagebionetworks.web.client.ContentTypeUtils.isWebRecognizedCodeFileName(
          name
        )
      ) {
    ...
    if (versionNumber != null) {
      ...
      if (
        versionLabel == null || versionNumber.toString().equals(versionLabel)
      ) {
        versionDisplay = versionNumberStr;
      } else {
        versionDisplay =
          DisplayUtils.stubStrPartialWord(
            versionLabel + " (" + versionNumberStr + ")",
  ...
  public static SafeHtml createEntityPopoverHtml(
    KeyValueDisplay<String> kvDisplay
  ) {
    ...
    if (kvDisplay != null) {
      ...
      for (String key : kvDisplay.getKeyDisplayOrder()) {
        if (!DisplayConstants.DESCRIPTION.equalsIgnoreCase(key)) {
          ...
          if (val == null) val = "";
          val =
            val.length() > MAX_TOOL_TIP_VALUE_CHAR
              ? val.substring(0, MAX_TOOL_TIP_VALUE_CHAR - 3) + "..."
              : val;
          sb
            .appendHtmlConstant("<span class=\"boldText\">")
            .appendEscaped(key)
