...
public class ProvViewUtil {
	...
	public static ProvNodeContainer createActivityContainer(ActivityGraphNode node,
		...
		if (node.getType() == ActivityType.UNDEFINED) {
			...
			if (node.getActivityName() != null) {
				label.add(new HTML(
						DisplayUtils.stubStrPartialWord(node.getActivityName(), MAX_ACT_CODE_NAME_CHAR)));
			}
			UserBadge badge = ginInjector.getUserBadgeWidget();
			badge.configure(node.getModifiedBy());
			HTML time =
					new HTML(ginInjector.getDateTimeUtils().getLongFriendlyDate(node.getModifiedOn()));
	...
	public static ProvNodeContainer createEntityContainer(EntityGraphNode node,
			IconsImageBundle iconsImageBundle) {
		ProvNodeContainer container = createEntityContainer(node.getId(), node.getEntityId(),
				node.getName(), node.getVersionLabel(), node.getVersionNumber(), node.getEntityType(),
				iconsImageBundle);
		setPosition(node, container);
		return container;
	}

	public static ProvNodeContainer createExpandContainer(final ExpandGraphNode node,
			final Presenter presenter, final ProvenanceWidgetView view) {
	...
	private static ProvNodeContainer createEntityContainer(String id, String entityId, String name,
		...
		if (versionNumber != null) {
			...
			if (versionLabel == null || versionNumber.toString().equals(versionLabel)) {
				...
				versionDisplay =
						DisplayUtils.stubStrPartialWord(versionLabel + " (" + versionNumberStr + ")",
								ENTITY_LINE_NUMBER_CHARS - versionNumberStr.length());
			}
			versionHtml =
					new HTML(SafeHtmlUtils.fromString(DisplayUtils.stubStrPartialWord(versionDisplay,
							ENTITY_LINE_NUMBER_CHARS - versionNumberStr.length())));
		} else {
		...
		if (kvDisplay != null) {
			...
			for (String key : kvDisplay.getKeyDisplayOrder()) {
				if (!DisplayConstants.DESCRIPTION.equalsIgnoreCase(key)) {
					...
					if (val == null)
						val = "";
					val = val.length() > MAX_TOOL_TIP_VALUE_CHAR
							? val.substring(0, MAX_TOOL_TIP_VALUE_CHAR - 3) + "..."
							: val;
					sb.appendHtmlConstant("<span class=\"boldText\">").appendEscaped(key)
