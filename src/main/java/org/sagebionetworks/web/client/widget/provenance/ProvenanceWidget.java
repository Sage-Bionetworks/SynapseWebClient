...
public class ProvenanceWidget
  implements ProvenanceWidgetView.Presenter, WidgetRendererPresenter {
  ...
  @Inject
  public ProvenanceWidget(
    ProvenanceWidgetView view,
  ...
  public void configure(
    ...
    if (
      descriptor.containsKey(WidgetConstants.PROV_WIDGET_ENTITY_LIST_KEY)
    ) entityListStr =
      descriptor.get(WidgetConstants.PROV_WIDGET_ENTITY_LIST_KEY);
    ...
    if (descriptor.containsKey(WidgetConstants.PROV_WIDGET_ENTITY_ID_KEY)) {
      lookupVersion.add(
        descriptor.get(WidgetConstants.PROV_WIDGET_ENTITY_ID_KEY)
      );
    }

    // Set max depth (default 1), undefined (false) and expand (false)
    maxDepth =
      descriptor.containsKey(WidgetConstants.PROV_WIDGET_DEPTH_KEY)
        ? Integer.parseInt(
          descriptor.get(WidgetConstants.PROV_WIDGET_DEPTH_KEY)
        )
        : 1;
    showUndefinedAndErrorActivity =
      descriptor.get(WidgetConstants.PROV_WIDGET_UNDEFINED_KEY) != null
        ? Boolean.parseBoolean(
          descriptor.get(WidgetConstants.PROV_WIDGET_UNDEFINED_KEY)
        )
        : true;
    showExpand =
      descriptor.get(WidgetConstants.PROV_WIDGET_EXPAND_KEY) != null
        ? Boolean.parseBoolean(
          descriptor.get(WidgetConstants.PROV_WIDGET_EXPAND_KEY)
        )
        : false;
    if (
      descriptor.containsKey(WidgetConstants.PROV_WIDGET_DISPLAY_HEIGHT_KEY)
    ) {
      setHeight(
        Integer.parseInt(
          descriptor.get(WidgetConstants.PROV_WIDGET_DISPLAY_HEIGHT_KEY)
        )
      );
    } else {
      setHeight(330);
  ...
  private void processUsed(final ReferenceProcessItem item) {
    ...
    jsClient.getActivityForEntityVersion(
      item.getReference().getTargetId(),
      item.getReference().getTargetVersionNumber(),
      new AsyncCallback<Activity>() {
        ...
        public void onFailure(Throwable caught) {
          ...
          if (showUndefinedAndErrorActivity) activity =
            createErrorActivity(caught); // Display empty, fake provenance record
          addActivityToStack(activity);
        }
        ...
        private void addActivityToStack(Activity activity) {
          if (activity != null) {
            ...
            toProcess.push(
              new ActivityProcessItem(activity, item.getDepth() + 1)
            );
          }
          processNext();
        }
  ...
  private void processActivity(final ActivityProcessItem item) {
    ...
    jsClient.getEntitiesGeneratedBy(
            ...
            for (Used u : used) {
              if (u instanceof UsedEntity) { // ignore UsedUrl, nothing to process
                ...
                if (ue.getReference() != null) {
                  toProcess.push(
                    new ReferenceProcessItem(ue.getReference(), item.getDepth())
                  ); // same
                  // depth
                  // as
                  // activity
  ...
  public void cleanupCycles(
    ActivityProcessItem item,
  ...
  private void lookupReferencesThenBuildGraph() {
    ...
    jsClient.getEntityHeaderBatchFromReferences(
  ...
  private void buildGraphLayoutSendToView() {
    ...
    ProvGraph graph = ProvUtils.buildProvGraph(
      generatedByActivityId,
      processedActivities,
      idToNode,
      refToHeader,
      showExpand,
    ...
    NChartCharacters characters = NChartUtil.createNChartCharacters(
      jsoProvider,
      graph.getNodes()
    );
    NChartLayersArray layerArray = NChartUtil.createLayers(jsoProvider, graph);
    LayoutResult layoutResult = synapseJSNIUtils.nChartlayout(
      layerArray,
      characters
    );
  ...
  private Activity createErrorActivity(Throwable caught) {
    ...
    if (caught instanceof NotFoundException) {
      activity.setName(
        DisplayConstants.ACTIVITY + " " + DisplayConstants.NOT_FOUND
      );
    } else if (caught instanceof ForbiddenException) {
      activity.setName(
        DisplayConstants.ACTIVITY + " " + DisplayConstants.UNAUTHORIZED
      );
    } else {
  ...
  public void findOldVersions() {
    ...
    for (ProvGraphNode node : currentGraph.getNodes()) {
      if (node instanceof EntityGraphNode) {
        if (((EntityGraphNode) node).getVersionNumber() != null) {
          ...
          ref.setTargetVersionNumber(
            ((EntityGraphNode) node).getVersionNumber()
          );
    ...
    synAlert.clear();
    jsClient.getEntityHeaderBatchFromReferences(
      new ArrayList<Reference>(entityIds),
      new AsyncCallback<ArrayList<EntityHeader>>() {
        @Override
        public void onSuccess(ArrayList<EntityHeader> currentVersions) {
          ...
          for (Reference ref : refToNodeId.keySet()) {
            Long currrentVersion = entityToCurrentVersion.get(
              ref.getTargetId()
            );
            if (
              ref.getTargetVersionNumber() != null &&
              currrentVersion != null &&
  ...
  public static Map<String, String> getDefaultWidgetDescriptor() {
    ...
    configMap.put(
      WidgetConstants.PROV_WIDGET_DEPTH_KEY,
      Integer.toString(DEFAULT_DEPTH_KEY)
    );
    configMap.put(
