...
public class NChartUtil {
		...
		for (int i = 0; i <= maxLayer; i++) {
			...
			for (ProvGraphNode node : layerToNode.get(i)) {
				if (node instanceof ActivityGraphNode) {
					...
					layerNodes
							.add(createActivityLayerNode(jsoProvider, (ActivityGraphNode) node, connectedNodes));
	...
	private static void calculateNodeToLayer(ProvGraph graph, Map<ProvGraphNode, Integer> nodeToLayer,
	 ...
	 * @return
	 */
	public static NChartCharacters createNChartCharacters(JsoProvider jsoProvider,
	 ...
	 * @return
	 */
	public static NChartLayerNode createActivityLayerNode(JsoProvider jsoProvider,
			ActivityGraphNode activityNode, List<ProvGraphNode> connectedNodes) {
	 ...
	 * @return
	 */
	public static NChartLayerNode createEntityLayerNode(JsoProvider jsoProvider,
	 ...
	 * @return
	 */
	public static NChartLayerNode createExpandLayerNode(JsoProvider jsoProvider,
	 ...
	 * @return
	 */
	public static NChartLayerNode createExternalLayerNode(JsoProvider jsoProvider,
	...
	public static void fillPositions(LayoutResult layoutResult, ProvGraph graph) {
		...
		int range = minY + maxY;
		final double scaleY =
				graph.getNodes().size() >= LARGE_GRAPH_SIZE ? LARGE_GRAPH_SCALE_Y : SCALE_Y;
		for (ProvGraphNode node : graph.getNodes()) {
			List<XYPoint> xyPoints = layoutResult.getPointsForId(node.getId());
			if (xyPoints != null && xyPoints.size() > 0) {
	 ...
	 * Private Methods
	 */
	private static List<ProvGraphNode> getConnectedNodes(Set<ProvGraphEdge> edges,
			ProvGraphNode node) {
	...
	private static void processLayer(int layer, ProvGraphNode node,
			Map<ProvGraphNode, Integer> nodeToLayer, ProvGraph graph) {
