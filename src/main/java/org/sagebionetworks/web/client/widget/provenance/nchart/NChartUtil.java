...
public class NChartUtil {
  ...
  public static NChartLayersArray createLayers(
    ...
    for (ProvGraphNode node : nodeToLayer.keySet()) {
      ...
      if (!layerToNode.containsKey(layer)) layerToNode.put(
        layer,
        new ArrayList<ProvGraphNode>()
      );
    ...
    for (int i = 0; i <= maxLayer; i++) {
      ...
      for (ProvGraphNode node : layerToNode.get(i)) {
        if (node instanceof ActivityGraphNode) {
          List<ProvGraphNode> connectedNodes = getConnectedNodes(
            graph.getEdges(),
            node
          );
          layerNodes.add(
            createActivityLayerNode(
              jsoProvider,
          ...
          layerNodes.add(
            createEntityLayerNode(jsoProvider, (EntityGraphNode) node)
          );
        } else if (node instanceof ExpandGraphNode) {
          layerNodes.add(
            createExpandLayerNode(jsoProvider, (ExpandGraphNode) node)
          );
        } else if (node instanceof ExternalGraphNode) {
          layerNodes.add(
            createExternalLayerNode(jsoProvider, (ExternalGraphNode) node)
          );
        }
      }
      // build NChartLayer
  ...
  private static void calculateNodeToLayer(
    ProvGraph graph,
   ...
   * @return
   */
  public static NChartCharacters createNChartCharacters(
    JsoProvider jsoProvider,
    Set<ProvGraphNode> graphNodes
  ) {
   ...
   * @return
   */
  public static NChartLayerNode createActivityLayerNode(
    JsoProvider jsoProvider,
    ActivityGraphNode activityNode,
    List<ProvGraphNode> connectedNodes
  ) {
   ...
   * @return
   */
  public static NChartLayerNode createEntityLayerNode(
    JsoProvider jsoProvider,
   ...
   * @return
   */
  public static NChartLayerNode createExpandLayerNode(
    JsoProvider jsoProvider,
   ...
   * @return
   */
  public static NChartLayerNode createExternalLayerNode(
    JsoProvider jsoProvider,
  ...
  public static void fillPositions(LayoutResult layoutResult, ProvGraph graph) {
    ...
    int range = minY + maxY;
    final double scaleY = graph.getNodes().size() >= LARGE_GRAPH_SIZE
      ? LARGE_GRAPH_SCALE_Y
      : SCALE_Y;
    for (ProvGraphNode node : graph.getNodes()) {
      List<XYPoint> xyPoints = layoutResult.getPointsForId(node.getId());
   ...
   * Private Methods
   */
  private static List<ProvGraphNode> getConnectedNodes(
    Set<ProvGraphEdge> edges,
    ProvGraphNode node
  ) {
   ...
   * @return
   */
  private static EdgesForNode getEdgesForNode(
    Set<ProvGraphEdge> edges,
    ProvGraphNode node
  ) {
    ...
    for (ProvGraphEdge edge : edges) {
      if (edge.getSource().equals(node)) edgesForNode.out.add(edge);
