...
public class ReplyCountAlert implements RefreshAlertView.Presenter, SynapseWidgetPresenter {
	...
	public static final int DELAY = 70000; // check every 70 seconds (until detached, configuration
																					// cleared, or a change has been detected)

	@Inject
	public ReplyCountAlert(RefreshAlertView view, GWTWrapper gwt,
			GlobalApplicationState globalAppState, SynapseJSNIUtils utils,
			SynapseJavascriptClient jsClient) {
		this.view = view;
		this.gwt = gwt;
	...
	private void checkCount() {
		if (view.isAttached() && threadId != null) {
			jsClient.getReplyCountForThread(threadId, DiscussionFilter.NO_FILTER,
	...
	/**
	 * If you set this callback, it will be invoked when the user elects to refresh the data (instead
	 * of causing a page refresh)
	 * 
	 * @param c
	 */
