...
public class SynapseSuggestBoxViewImpl extends FlowPanel implements SynapseSuggestBoxView {
	...
	public void configure(SynapseSuggestOracle oracle) {
		...
		((SynapseSuggestionDisplay) suggestBox.getSuggestionDisplay()).getPrevButton().addClickHandler(event -> {
			presenter.getPrevSuggestions();
		});

		// Next suggestions button.
		((SynapseSuggestionDisplay) suggestBox.getSuggestionDisplay()).getNextButton().addClickHandler(event -> {
	...
	public void updateFieldStateForSuggestions(int numResults, int offset) {
		...
		String resultsLabel = "Displaying " + (offset + 1) + " - " + (moreResults ? offset + SynapseSuggestBox.PAGE_SIZE : numResults);
