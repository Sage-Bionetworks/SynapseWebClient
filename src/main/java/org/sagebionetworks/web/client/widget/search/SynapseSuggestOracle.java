...
public class SynapseSuggestOracle extends SuggestOracle {
	...
	public void configure(final SynapseSuggestBox suggestBox, int pageSize,
	...
	public void getSuggestions(final int offset) {
		if (!isLoading) {
			...
			provider.getSuggestions(type, offset, pageSize, suggestBox.getWidth(), request.getQuery(),
					new AsyncCallback<SynapseSuggestionBundle>() {
						@Override
						public void onSuccess(SynapseSuggestionBundle suggestionBundle) {
							...
							if (suggestBox != null) {
								suggestBox.updateFieldStateForSuggestions(
										(int) suggestionBundle.getTotalNumberOfResults(), offset);
							}
							SuggestOracle.Response response =
									new SuggestOracle.Response(suggestionBundle.getSuggestionBundle());
