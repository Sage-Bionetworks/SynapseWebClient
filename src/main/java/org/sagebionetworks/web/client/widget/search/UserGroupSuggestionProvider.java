...
public class UserGroupSuggestionProvider {
	...
	public void getSuggestions(TypeFilter type, final int offset, final int pageSize, final int width,
			final String prefix, final AsyncCallback<SynapseSuggestionBundle> callback) {
		jsClient.getUserGroupHeadersByPrefix(prefix, type, pageSize, offset,
				new AsyncCallback<UserGroupHeaderResponsePage>() {
					...
					public void onSuccess(UserGroupHeaderResponsePage result) {
						...
						SynapseSuggestionBundle suggestionBundle =
								new SynapseSuggestionBundle(suggestions, result.getTotalNumberOfResults());
