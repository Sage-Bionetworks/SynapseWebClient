 ...
 * @author bkng
 */
public class AccessControlListEditor
  implements AccessControlListEditorView.Presenter {

  private static final String ERROR_CANNOT_MODIFY_ACTIVE_USER_PERMISSIONS =
    "Current user permissions cannot be modified. Please select a different user.";
  private static final String NULL_UEP_MESSAGE =
    "User's entity permissions are missing.";
  ...
  public AccessControlListEditor(
    AccessControlListEditorView view,
  ...
  private void initViewPrincipalIds() {
    if (publicPrincipalIds != null) {
      view.setPublicAclPrincipalId(
        publicPrincipalIds.getPublicAclPrincipalId()
      );
      view.setAuthenticatedAclPrinciapalId(
        publicPrincipalIds.getAuthenticatedAclPrincipalId()
      );
    }
  }

  public static final PermissionLevel[] PERMISSIONS = new PermissionLevel[] {
    PermissionLevel.CAN_VIEW,
    PermissionLevel.CAN_DOWNLOAD,
    PermissionLevel.CAN_EDIT,
    PermissionLevel.CAN_EDIT_DELETE,
    PermissionLevel.CAN_ADMINISTER,
  };
  ...
  public HashMap<PermissionLevel, String> getPermissionsToDisplay() {
    ...
    permissionDisplay.put(
      PermissionLevel.CAN_DOWNLOAD,
      DisplayConstants.MENU_PERMISSION_LEVEL_CAN_DOWNLOAD
    );
    permissionDisplay.put(
      PermissionLevel.CAN_EDIT,
      DisplayConstants.MENU_PERMISSION_LEVEL_CAN_EDIT
    );
    permissionDisplay.put(
      PermissionLevel.CAN_EDIT_DELETE,
      DisplayConstants.MENU_PERMISSION_LEVEL_CAN_EDIT_DELETE
    );
    permissionDisplay.put(
      PermissionLevel.CAN_ADMINISTER,
      DisplayConstants.MENU_PERMISSION_LEVEL_CAN_ADMINISTER
    );
  ...
  private void refresh(final AsyncCallback<Void> callback) {
    if (this.entity.getId() == null) throw new IllegalStateException(
      NULL_ENTITY_MESSAGE
    );
    view.showLoading();
  ...
  private void updateIsPublicAccess() {
    view.setIsPubliclyVisible(
      PublicPrivateBadge.isPublic(acl, publicPrincipalIds)
    );
  }
  ...
  public void makePrivate() {
    ...
    for (ResourceAccess ra : acl.getResourceAccess()) {
      if (
        publicPrincipalIds
          .getAuthenticatedAclPrincipalId()
          .equals(ra.getPrincipalId())
      ) {
        toRemove.add(publicPrincipalIds.getAuthenticatedAclPrincipalId());
      } else if (
        publicPrincipalIds.getPublicAclPrincipalId().equals(ra.getPrincipalId())
      ) {
        toRemove.add(publicPrincipalIds.getPublicAclPrincipalId());
      } else if (
        publicPrincipalIds
          .getAnonymousUserPrincipalId()
          .equals(ra.getPrincipalId())
      ) {
  ...
  private void populateAclEntries() {
    for (final ResourceAccess ra : acl.getResourceAccess()) {
      ...
      if (header != null) {
        String title = header.getIsIndividual()
  ...
  private void fetchUserGroupHeaders(final AsyncCallback<Void> callback) {
    ...
    for (ResourceAccess ra : acl.getResourceAccess()) ids.add(
      ra.getPrincipalId().toString()
    );
  ...
  public void setAccess(Long principalId, PermissionLevel permissionLevel) {
    ...
    synAlert.clear();
    if (
      principalId.equals(publicPrincipalIds.getPublicAclPrincipalId())
    ) uep.setCanPublicRead(true);

    ResourceAccess toSet = null;
    for (ResourceAccess ra : acl.getResourceAccess()) {
      if (ra.getPrincipalId().equals(principalId)) toSet = ra;
    }
    if (toSet == null) {
  ...
  public void removeAccess(Long principalIdToRemove) {
    ...
    if (
      currentUserId != null &&
      principalIdToRemove.toString().equals(currentUserId)
    ) {
      synAlert.showError(ERROR_CANNOT_MODIFY_ACTIVE_USER_PERMISSIONS);
      return;
    }
    if (
      principalIdToRemove.equals(publicPrincipalIds.getPublicAclPrincipalId())
    ...
    if (foundUser) {
      ...
      // not found
      synAlert.showError(
        "ACL does not have a record for " + principalIdToRemove
      );
    }
  }
    ...
    AsyncCallback<AccessControlList> callback = new AsyncCallback<AccessControlList>() {
      ...
      public void onFailure(Throwable caught) {
        ...
        if (
          ...
          ErrorResponseCode.USER_CERTIFICATION_REQUIRED.equals(
            ((ForbiddenException) caught).getErrorResponseCode()
          )
        ) {
  ...
  public void notifyNewUsers() {
    if (view.isNotifyPeople()) {
      ...
      for (ResourceAccess ra : acl.getResourceAccess()) {
        ...
        UserGroupHeader header = userGroupHeaders.get(
          ra.getPrincipalId().toString()
        );
      ...
      if (newPrincipalIdSet.size() > 0) {
        ...
        String userDisplayName = DisplayUtils.getDisplayName(
          authenticationController.getCurrentUserProfile()
        );
        String message = DisplayUtils.getShareMessage(
          userDisplayName,
          entity.getId(),
          gwt.getHostPageBaseURL()
        );
        String subject =
          entity.getName() + DisplayConstants.SHARED_ON_SYNAPSE_SUBJECT;
        synAlert.clear();
  ...
  public void refresh() {
    ...
    refresh(
      new AsyncCallback<Void>() {
        ...
        public void onFailure(Throwable caught) {
          synAlert.showError(
            DisplayConstants.ERROR_ACL_RETRIEVAL_FAILED +
            ": " +
            caught.getMessage()
          );
        }
