...
public class AccessControlListEditor implements AccessControlListEditorView.Presenter {
	...
	public AccessControlListEditor(AccessControlListEditorView view,
			SynapseClientAsync synapseClientAsync, AuthenticationController authenticationController,
			SynapseProperties synapseProperties, GWTWrapper gwt, SynapseJavascriptClient jsClient,
			SynapseAlert synAlert, PortalGinInjector ginInjector) {
		this.view = view;
	...
	public static final PermissionLevel[] PERMISSIONS = new PermissionLevel[] {
			PermissionLevel.CAN_VIEW, PermissionLevel.CAN_DOWNLOAD, PermissionLevel.CAN_EDIT,
	...
	public HashMap<PermissionLevel, String> getPermissionsToDisplay() {
		...
		permissionDisplay.put(PermissionLevel.CAN_VIEW,
				DisplayConstants.MENU_PERMISSION_LEVEL_CAN_VIEW);
		permissionDisplay.put(PermissionLevel.CAN_DOWNLOAD,
				DisplayConstants.MENU_PERMISSION_LEVEL_CAN_DOWNLOAD);
		permissionDisplay.put(PermissionLevel.CAN_EDIT,
				DisplayConstants.MENU_PERMISSION_LEVEL_CAN_EDIT);
		permissionDisplay.put(PermissionLevel.CAN_EDIT_DELETE,
				DisplayConstants.MENU_PERMISSION_LEVEL_CAN_EDIT_DELETE);
		permissionDisplay.put(PermissionLevel.CAN_ADMINISTER,
	...
	private void setViewDetails() {
		...
		view.buildWindow(entity instanceof Project, isInherited, acl.getId(), canEnableInheritance,
				canChangePermission, PermissionLevel.CAN_DOWNLOAD, authenticationController.isLoggedIn());
		populateAclEntries();
			...
			if (header != null) {
				String title =
						header.getIsIndividual()
								? DisplayUtils.getDisplayName(header.getFirstName(), header.getLastName(),
										header.getUserName())
								: header.getUserName();
				view.addAclEntry(
						new AclEntry(principalId, ra.getAccessType(), title, "", header.getIsIndividual()));
			} else {
	...
	public void deleteAcl() {
		...
		synapseClient.getEntityBenefactorAcl(entity.getParentId(),
	...
	public void pushChangesToSynapse(final boolean recursive, final Callback changesPushedCallback) {
		...
		AsyncCallback<AccessControlList> callback = new AsyncCallback<AccessControlList>() {
			...
			public void onFailure(Throwable caught) {
				...
				if (caught instanceof ForbiddenException && ErrorResponseCode.USER_CERTIFICATION_REQUIRED
						.equals(((ForbiddenException) caught).getErrorResponseCode())) {
		...
		if (view.isNotifyPeople()) {
			...
			if (newPrincipalIdSet.size() > 0) {
				...
				String userDisplayName =
						DisplayUtils.getDisplayName(authenticationController.getCurrentUserProfile());
				String message =
						DisplayUtils.getShareMessage(userDisplayName, entity.getId(), gwt.getHostPageBaseURL());
				String subject = entity.getName() + DisplayConstants.SHARED_ON_SYNAPSE_SUBJECT;
				synAlert.clear();
				synapseClient.sendMessage(newPrincipalIdSet, subject, message, gwt.getHostPageBaseURL(),
	...
	public void refresh() {
		...
		refresh(new AsyncCallback<Void>() {
			...
			public void onFailure(Throwable caught) {
				synAlert
						.showError(DisplayConstants.ERROR_ACL_RETRIEVAL_FAILED + ": " + caught.getMessage());
			}
		});
	}
