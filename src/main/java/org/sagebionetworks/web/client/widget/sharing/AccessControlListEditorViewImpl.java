...
public class AccessControlListEditorViewImpl
  extends FlowPanel
  ...
  private Button deleteAclButton = new Button(
    DisplayConstants.BUTTON_PERMISSIONS_DELETE_ACL
  );
  ...
  @Inject
  public AccessControlListEditorViewImpl(
  ...
  public void setPermissionsToDisplay(
    PermissionLevel[] permList,
  ...
  public void addAclEntry(AclEntry aclEntry) {
    if (!aclEntry.isIndividual()) {
      getSharingPermissionsGrid()
        .insert(aclEntry, 0, permList, permissionDisplay, true); // insert groups first
    } else {
  ...
  public void buildWindow(
    ...
    if (!canChangePermission) {
      ...
      if (isInherited) {
        ...
        // 'Create ACL' button
        Button createAclButton = new Button(
          DisplayConstants.BUTTON_PERMISSIONS_CREATE_NEW_ACL,
          IconType.PLUS,
          event -> {
            presenter.createAcl();
          }
  ...
  public void setIsNotifyPeople(Boolean value) {
    if (value != null) getAclAddPeoplePanel()
      .getNotifyPeopleCheckBox()
      .setValue(value);
  }
