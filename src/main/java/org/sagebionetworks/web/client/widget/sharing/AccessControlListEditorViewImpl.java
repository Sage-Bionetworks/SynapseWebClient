...
public class AccessControlListEditorViewImpl extends FlowPanel
	...
	public AccessControlListEditorViewImpl(PortalGinInjector ginInjector) {
		...
		helpWidget
				.setHelpMarkdown("Learn more about managing access controls and permissions in Synapse.");
	...
	public void setPermissionsToDisplay(PermissionLevel[] permList,
			Map<PermissionLevel, String> permissionsDisplay) {
	...
	public void addAclEntry(AclEntry aclEntry) {
		if (!aclEntry.isIndividual()) {
			getSharingPermissionsGrid().insert(aclEntry, 0, permList, permissionDisplay, true); // insert
																																													// groups
																																													// first
		} else {
	...
	@Override
	public void buildWindow(boolean isProject, boolean isInherited, String aclEntityId,
			boolean canEnableInheritance, boolean canChangePermission,
			PermissionLevel defaultPermissionLevel, boolean isLoggedIn) {
		clear();
		...
		if (!isInherited) {
			if (isProject) {
				div.add(new Text(
						"The sharing settings shown below apply to this project and are inherited by all project contents unless local sharing settings have been set."));
				if (canChangePermission) {
					add(new Heading(HeadingSize.H5, "Manage Sharing Settings"));
				}
			} else {
				div.add(new HTML(
						"The local sharing settings shown below are <strong>not</strong> being inherited from a parent resource."));
				if (canChangePermission) {
			...
			// is inherited
			div.add(
					new Span("The sharing settings shown below are currently being inherited from&nbsp;"));
		...
		if (!canChangePermission) {
			...
			if (isInherited) {
				...
				// 'Create ACL' button
				Button createAclButton =
						new Button(DisplayConstants.BUTTON_PERMISSIONS_CREATE_NEW_ACL, IconType.PLUS, event -> {
							presenter.createAcl();
						});
				...
				getAclAddPeoplePanel().configure(permList, addPersonCallback, makePublicCallback,
						isPubliclyVisible);
				add(getAclAddPeoplePanel().asWidget());
				if (canEnableInheritance) {
					Label deleteInfoLabel = new Label(
							"The sharing settings will be inherited from the parent folder or project if local sharing settings are deleted.");
