...
/**
 * Editor dialog to view and modify the Access Control List of a given Evaluation. This class is the
 * Presenter in the MVP design pattern.
 *
 * @author jay
 */
public class EvaluationAccessControlListEditor
  ...
  private static final String NULL_UEP_MESSAGE =
    "User's evaluation permissions are missing.";
  private static final String NULL_ACL_MESSAGE = "ACL is missing.";
  private static final String NULL_EVALUATION_MESSAGE =
  ...
  @Inject
  public EvaluationAccessControlListEditor(
  ...
  public PermissionLevel[] getPermList() {
    return new PermissionLevel[] {
      ...
      PermissionLevel.CAN_SUBMIT_EVALUATION,
      PermissionLevel.CAN_SCORE_EVALUATION,
      PermissionLevel.CAN_ADMINISTER_EVALUATION,
    };
  }
  ...
  public HashMap<PermissionLevel, String> getPermissionsToDisplay() {
    ...
    permissionDisplay.put(
      PermissionLevel.CAN_SCORE_EVALUATION,
      DisplayConstants.MENU_PERMISSION_LEVEL_CAN_SCORE
    );
    permissionDisplay.put(
      PermissionLevel.CAN_SUBMIT_EVALUATION,
      DisplayConstants.MENU_PERMISSION_LEVEL_CAN_SUBMIT
    );
    permissionDisplay.put(
      PermissionLevel.CAN_ADMINISTER_EVALUATION,
  ...
  private void refresh(final AsyncCallback<Void> callback) {
    if (this.evaluation.getId() == null) throw new IllegalStateException(
      NULL_EVALUATION_MESSAGE
    );
    view.showLoading();
    hasChangesHandler.hasChanges(false);

    challengeClient.getEvaluationAcl(
      evaluation.getId(),
  ...
  public void getUserPermissions(final AsyncCallback<Void> callback) {
    challengeClient.getUserEvaluationPermissions(
      evaluation.getId(),
      new AsyncCallback<String>() {
        @Override
        public void onSuccess(String result) {
          ...
          try {
            uep =
              new UserEvaluationPermissions(
                jsonObjectAdapter.createNew(result)
              );
          } catch (JSONObjectAdapterException e) {
            onFailure(e);
            return;
