...
public class PublicPrivateBadge {
  ...
  private void setAcl(
    ...
    final AsyncCallback<Boolean> isPublicCallback
  ) {
    this.acl = acl;
    DisplayUtils.getPublicPrincipalIds(
      userAccountService,
      new AsyncCallback<PublicPrincipalIds>() {
        ...
        public void onSuccess(PublicPrincipalIds result) {
          ...
          if (isPublicCallback != null) isPublicCallback.onSuccess(isPublic);
        }

        @Override
        public void onFailure(Throwable caught) {
          if (isPublicCallback != null) isPublicCallback.onFailure(
            caught
          ); else {
            view.showErrorMessage(
              "Could not find the public group: " + caught.getMessage()
            );
          }
        }
      }
  ...
  public static boolean isPublic(
    ...
    for (final ResourceAccess ra : acl.getResourceAccess()) {
      ...
      if (publicPrincipalIds.isPublic(pricipalIdLong)) return true;
    }
    return false;
  }
