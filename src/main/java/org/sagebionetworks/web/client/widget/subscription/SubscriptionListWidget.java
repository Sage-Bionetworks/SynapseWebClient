...
public class SubscriptionListWidget
		implements SubscriptionListWidgetView.Presenter, SynapseWidgetPresenter, PageChangeListener {
	...
	@Inject
	public SubscriptionListWidget(SubscriptionListWidgetView view, SynapseJavascriptClient jsClient,
			PortalGinInjector ginInjector, SynapseAlert synAlert, AuthenticationController authController,
			BasicPaginationWidget paginationWidget) {
		this.view = view;
	...
	public void onPageChange(final Long newOffset) {
		...
		jsClient.getAllSubscriptions(filter, LIMIT, newOffset, SortByType.CREATED_ON, sortDirection,
				new AsyncCallback<SubscriptionPagedResults>() {
					@Override
					public void onSuccess(SubscriptionPagedResults results) {
						...
						paginationWidget.configure(LIMIT, newOffset, results.getTotalNumberOfResults(),
								SubscriptionListWidget.this);
