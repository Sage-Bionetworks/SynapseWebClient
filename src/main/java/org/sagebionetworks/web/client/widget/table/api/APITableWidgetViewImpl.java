...
public class APITableWidgetViewImpl implements APITableWidgetView {
  ...
  public void configurePager(int start, int end, int total) {
    ...
    if (pager.getParent() == null) div.add(pager);
  }
  ...
  public void showTableUnavailable() {
    ...
    unavailableContainer.add(
      new HTML(
        "<h2>" +
        DisplayConstants.TABLE_UNAVAILABLE +
        "</h2><p><strong>" +
        TableState.PROCESSING +
        "</strong>: " +
        DisplayConstants.TABLE_PROCESSING_DESCRIPTION +
        "</p>"
      )
    );
