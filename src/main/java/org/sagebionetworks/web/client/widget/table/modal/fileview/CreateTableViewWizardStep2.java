...
public class CreateTableViewWizardStep2 implements ModalPage, IsWidget {
	...
	public CreateTableViewWizardStep2(CreateTableViewWizardStep2View view,
			ColumnModelsEditorWidget editor, SynapseClientAsync synapseClient,
	...
	public void getDefaultColumnsForView() {
		...
		List<ColumnModel> defaultColumns =
				fileViewDefaultColumns.getDefaultViewColumns(tableType.isIncludeFiles(), clearIds);
		...
		jsClient.deleteEntityById(entityId, true, new AsyncCallback<Void>() {
			...
			public void onFailure(Throwable caught) {
				jsniUtils.consoleError(
						DELETE_PLACEHOLDER_FAILURE_MESSAGE + entityId + ": " + caught.getMessage());
			}
	...
	public void onPrimary() {
		...
		presenter.clearErrors();
		synapseClient.getTableUpdateTransactionRequest(entity.getId(), new ArrayList<ColumnModel>(),
				newSchema, new AsyncCallback<TableUpdateTransactionRequest>() {
					@Override
					public void onFailure(Throwable caught) {
						presenter.setError(caught);
					}
	...
	public void startTrackingJob(TableUpdateTransactionRequest request) {
		...
		presenter.setLoading(true);
		presenter.clearErrors();
		this.jobTrackingWidget.startAndTrackJob(ColumnModelsWidget.UPDATING_SCHEMA, false,
