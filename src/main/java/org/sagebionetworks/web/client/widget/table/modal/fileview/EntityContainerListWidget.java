...
public class EntityContainerListWidget
	...
	@Inject
	public EntityContainerListWidget(EntityContainerListWidgetView view, EntityFinder finder,
	...
	public void configure(List<String> entityContainerIds, boolean canEdit, TableType tableType) {
		...
		if (!entityContainerIds.isEmpty()) {
			jsClient.getEntityHeaderBatch(entityContainerIds,
					new AsyncCallback<ArrayList<EntityHeader>>() {
						...
						public void onSuccess(ArrayList<EntityHeader> entityHeaders) {
							for (EntityHeader header : entityHeaders) {
								...
								view.addEntity(header.getId(), header.getName(),
	...
	public void onAddProject(String id) {
		jsClient.getEntityHeaderBatch(Collections.singletonList(id),
				new AsyncCallback<ArrayList<EntityHeader>>() {
					@Override
					public void onFailure(Throwable caught) {
