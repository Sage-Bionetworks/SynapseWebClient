...
public class EntityContainerListWidget
  implements EntityContainerListWidgetView.Presenter, IsWidget {
  ...
  @Inject
  public EntityContainerListWidget(
    EntityContainerListWidgetView view,
    EntityFinderWidget.Builder entityFinderBuilder,
    SynapseJavascriptClient jsClient,
    SynapseAlert synAlert
    ...
    selectionHandler =
      new SelectedHandler<List<Reference>>() {
        @Override
  ...
  public void configure(
    List<Reference> entityContainerIds,
    boolean canEditScope,
    TableType tableType
  ) {
    view.clear();
    references.clear();
          ...
          public void onSuccess(ArrayList<EntityHeader> entityHeaders) {
            for (EntityHeader header : entityHeaders) {
              ...
              view.addEntity(
                header.getId(),
                header.getName(),
                EntityContainerListWidget.this.canEdit
              );
            }
          }
        }
    ...
    if (TableType.project_view.equals(tableType)) {
      ...
      entityFinderBuilder
        ...
        .setHelpMarkdown(
          ...
          "s containing items for this " +
          tableType.getDisplayName()
        )
        .setPromptCopy(
          "Find and select " +
          EntityTypeUtils.getDisplayName(EntityType.folder) +
          "s to add their contents"
        );
    }
    finder =
      entityFinderBuilder
      ...
      new AsyncCallback<ArrayList<EntityHeader>>() {
        ...
        public void onSuccess(ArrayList<EntityHeader> entityHeaders) {
          if (entityHeaders.size() == 1) {
            ...
            onFailure(
              new UnknownErrorException(DisplayConstants.ERROR_LOADING)
            );
          }
        }
      }
