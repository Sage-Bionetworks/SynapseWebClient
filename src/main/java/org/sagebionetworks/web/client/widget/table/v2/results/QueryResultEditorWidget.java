...
public class QueryResultEditorWidget
  ...
  public static final String VIEW_RECENTLY_CHANGED_KEY =
    "_view_recently_changed_etag";
  ...
  @Inject
  public QueryResultEditorWidget(
    QueryResultEditorView view,
    TablePageWidget pageWidget,
    JobTrackingWidget editJobTrackingWidget,
    GlobalApplicationState globalApplicationState,
  ...
  public void showEditor(QueryResultBundle bundle, TableType tableType) {
    ...
    pageWidget.configure(bundle, null, null, tableType, this, null, null);
    setJobRunning(false);
  ...
  private PartialRowSet extractDelta() {
    PartialRowSet prs = RowSetUtils.buildDelta(
      startingBundle.getQueryResult().getQueryResults(),
      pageWidget.extractRowSet(),
      pageWidget.extractHeaders()
    );
    return prs;
  }
  ...
  /**
   * @param response
   * @return Returns an EntityUpdateResults (if response contains one). Otherwise this method returns
   *         null.
   */
  public static EntityUpdateResults getEntityUpdateResults(
    AsynchronousResponseBody response
  ) {
    if (response instanceof TableUpdateTransactionResponse) {
      List<TableUpdateResponse> results =
        ((TableUpdateTransactionResponse) response).getResults();
      if (results != null) {
  ...
  public static String getEntityUpdateResultsFailures(
    AsynchronousResponseBody response
  ) {
   ...
   *         found
   */
  public static int getFirstIndexOfEntityUpdateResultSuccess(
    AsynchronousResponseBody response
  ) {
    EntityUpdateResults results = getEntityUpdateResults(response);
    if (results != null) {
  ...
  public void onSave() {
    ...
    editJobTrackingWidget.startAndTrackJob(
      "Applying changes...",
      false,
      ...
      new AsynchronousProgressHandler() {
        ...
        public void onComplete(AsynchronousResponseBody response) {
          ...
          if (!TableType.table.equals(tableType)) {
            int successIndex = getFirstIndexOfEntityUpdateResultSuccess(
              response
            );
            if (successIndex > -1) {
              ...
              Date now = new Date();
              clientCache.put(
                tableId + VIEW_RECENTLY_CHANGED_KEY,
                etag,
   ...
   * @return
   */
  public static boolean isQueryResultEditable(
