...
public class RowFormEditorWidget implements IsWidget, RowFormView.Presenter {
  ...
  public static final String SUBMISSION_TIMESTAMP_COLUMN_NAME =
    "submissiontimestamp";
  ...
  public void configure(String tableId, List<ColumnModel> types) {
    ...
    for (int i = 0; i < types.size(); i++) {
      ...
      this.cells.add(cell);
      if (
        type.getName().toLowerCase().equals(SUBMITTER_USER_ID_COLUMN_NAME) &&
        ColumnType.USERID.equals(type.getColumnType())
      ) {
        submitterUserIdColIndex = i;
      } else if (
        type.getName().toLowerCase().equals(SUBMISSION_TIMESTAMP_COLUMN_NAME) &&
        ColumnType.DATE.equals(type.getColumnType())
      ) {
