...
public class RowSetUtils {
	 ...
	 * @return
	 */
	public static PartialRowSet buildDelta(RowSet original, List<Row> updateRows,
			List<ColumnModel> selectData) {
		...
		for (Row toUpdate : updateRows) {
			...
			if (toUpdate.getRowId() != null) {
				...
				if (originalRow == null) {
					throw new IllegalArgumentException("An updated row has a RowID: " + toUpdate.getRowId()
	 ...
	 *        applied to the resulting PartialRow. If null a new PartialRow will be created with all
	 *        of the values from the updated.
