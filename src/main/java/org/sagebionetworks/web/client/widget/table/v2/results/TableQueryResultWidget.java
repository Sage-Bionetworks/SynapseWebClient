...
public class TableQueryResultWidget
  implements
    TableQueryResultView.Presenter, IsWidget, PagingAndSortingListener {
  ...
  private static final long CACHED_PARTS_MASK =
  ...
  public TableQueryResultWidget(
    ...
    facetChangedHandler =
      new CallbackP<FacetColumnRequest>() {
        @Override
        public void invoke(FacetColumnRequest request) {
          ...
          for (FacetColumnRequest facetColumnRequest : selectedFacets) {
            if (
              facetColumnRequest.getColumnName().equals(request.getColumnName())
            ) {
  ...
  private void runQuery(final int jobIndex) {
    ...
    String viewEtag = clientCache.get(
      entityId + QueryResultEditorWidget.VIEW_RECENTLY_CHANGED_KEY
    );
    if (viewEtag == null) {
      if (facetsRequireRefresh) {
        // no need to update facets if it's just a page change or sort
        facetsWidget.configure(
          startingQuery,
          facetChangedHandler,
          resetFacetsHandler
        );
      ...
      progressWidget.startAndTrackJob(
   ...
   * @param oldEtag
   */
  public void verifyOldEtagIsNotInView(
    final String fileViewEntityId,
    String oldEtag
  ) {
    ...
    query.setSql(
      "select * from " + fileViewEntityId + " where ROW_ETAG='" + oldEtag + "'"
    );
      ...
      new AsynchronousProgressHandler() {
        ...
        public void onComplete(AsynchronousResponseBody response) {
          ...
          if (resultBundle.getQueryCount() > 0) {
            ...
            clientCache.remove(
              fileViewEntityId +
              QueryResultEditorWidget.VIEW_RECENTLY_CHANGED_KEY
            );
            runQuery();
          }
        }
  ...
  private void setQueryResultsAndSort(
    QueryResultBundle bundle,
    ...
    this.pageViewerWidget.configure(
        bundle,
        this.startingQuery,
        sortItems,
        false,
    ...
    fireFinishEvent(
      true,
      QueryResultEditorWidget.isQueryResultEditable(this.bundle, tableType)
    );
  }
  ...
  private void showError(Throwable caught) {
    ...
    if (
      ...
      ErrorResponseCode.INVALID_TABLE_QUERY_FACET_COLUMN_REQUEST.equals(
        ((BadRequestException) caught).getErrorResponseCode()
      )
    ) {
