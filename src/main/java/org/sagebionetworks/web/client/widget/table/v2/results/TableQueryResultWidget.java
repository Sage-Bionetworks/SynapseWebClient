 ...
 *
 */
public class TableQueryResultWidget
		implements TableQueryResultView.Presenter, IsWidget, PagingAndSortingListener {
	public static final String SCHEMA_CHANGED_MESSAGE =
			"The underlying Table/View schema has been changed so this query must be reset.";
	public static final String FACET_COLUMNS_CHANGED_MESSAGE =
			"requested facet column names must all be in the set";
	public static final int ETAG_CHECK_DELAY_MS = 5000;
	public static final String VERIFYING_ETAG_MESSAGE =
			"Verifying that the recent changes have propagated through the system...";
	public static final String RUNNING_QUERY_MESSAGE = ""; // while running, just show loading spinner
																													// (and cancel)
	...
	public TableQueryResultWidget(TableQueryResultView view, SynapseClientAsync synapseClient,
			PortalGinInjector ginInjector, SynapseAlert synapseAlert, ClientCache clientCache,
			GWTWrapper gwt, FacetsWidget facetsWidget, PopupUtilsView popupUtils) {
	 ...
	 */
	public void configure(Query query, boolean isEditable, TableType tableType,
	...
	private void runQuery(final int jobIndex) {
		...
		if (viewEtag == null) {
			...
			progressWidget.startAndTrackJob(RUNNING_QUERY_MESSAGE, false, AsynchType.TableQuery, qbr,
	...
	public void retryConsistentQuery(String message) {
		if (!startingQuery.getIsConsistent()) {
			...
			synapseAlert.consoleError(
					"Unexpected results when isConsistent=false, retrying with isConsistent=true.  "
							+ message);
	...
	/**
	 * Look for the given etag in the given file view. If it is still there, wait a few seconds and
	 * try again. If the etag is not in the view, then remove the clientCache key and run the query
	 * (since this indicates that the user change was propagated to the replicated layer)
	 * 
	...
	public void verifyOldEtagIsNotInView(final String fileViewEntityId, String oldEtag) {
		...
		AsynchronousProgressWidget progressWidget = ginInjector.creatNewAsynchronousProgressWidget();
		this.view.setProgressWidget(progressWidget);
		progressWidget.startAndTrackJob(VERIFYING_ETAG_MESSAGE, false, AsynchType.TableQuery, qbr,
				new AsynchronousProgressHandler() {
					@Override
					public void onFailure(Throwable failure) {
						showError(failure);
					...
					public void onComplete(AsynchronousResponseBody response) {
						...
						if (resultBundle.getQueryCount() > 0) {
							...
							clientCache
									.remove(fileViewEntityId + QueryResultEditorWidget.VIEW_RECENTLY_CHANGED_KEY);
							runQuery();
						}
					}
	...
	private void setQueryResultsAndSort(QueryResultBundle bundle, List<SortItem> sortItems) {
		...
		this.pageViewerWidget.configure(bundle, this.startingQuery, sortItems, false, tableType, null,
				this, facetChangedHandler);
	...
	private void showError(Throwable caught) {
		...
		if (caught instanceof BadRequestException
				&& ErrorResponseCode.INVALID_TABLE_QUERY_FACET_COLUMN_REQUEST
						.equals(((BadRequestException) caught).getErrorResponseCode())) {
	...
	public void onToggleSort(String header) {
		...
		// transition through UNSORTED (not in sort list) -> DESC -> ASC -> UNSORTED (remove from sort
		// list)
