...
public class FacetColumnResultValuesWidget
		implements IsWidget, FacetColumnResultValuesView.Presenter {
	...
	public void configure(FacetColumnResultValues facet, ColumnType columnType,
			CallbackP<FacetColumnRequest> onFacetRequest) {
		...
		for (FacetColumnResultValueCount valueCount : facet.getFacetValues()) {
			...
			if (i < max) {
				view.addValue(valueCount.getIsSelected(), displayWidget, valueCount.getCount(),
						valueCount.getValue());
			} else {
				view.addValueToOverflow(valueCount.getIsSelected(), displayWidget, valueCount.getCount(),
						valueCount.getValue());
			}
			i++;
		}
