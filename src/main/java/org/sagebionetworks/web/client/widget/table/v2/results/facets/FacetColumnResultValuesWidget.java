...
public class FacetColumnResultValuesWidget
  ...
  @Inject
  public FacetColumnResultValuesWidget(
    FacetColumnResultValuesView view,
    PortalGinInjector ginInjector
  ) {
    this.view = view;
  ...
  public void configure(
    ...
    for (
      int j = MAX_VISIBLE_FACET_VALUES;
      j < facet.getFacetValues().size();
      j++
    ) {
    ...
    for (FacetColumnResultValueCount valueCount : facet.getFacetValues()) {
      ...
      if (
        valueCount.getValue() == null ||
        NULL_VALUE_KEYWORD.equals(valueCount.getValue())
      ) {
      ...
      if (i < max) {
        view.addValue(
          valueCount.getIsSelected(),
          displayWidget,
          valueCount.getCount(),
          valueCount.getValue()
        );
      } else {
        view.addValueToOverflow(
          valueCount.getIsSelected(),
          displayWidget,
          valueCount.getCount(),
          valueCount.getValue()
