...
public class FacetsWidget implements IsWidget {
	...
	public void configure(Query query, CallbackP<FacetColumnRequest> facetChangedHandler,
			Callback resetFacetsHandler) {
		view.clear();
		...
		long partMask = TableQueryResultWidget.BUNDLE_MASK_QUERY_FACETS
				| TableQueryResultWidget.BUNDLE_MASK_QUERY_COLUMN_MODELS;
		qbr.setPartMask(partMask);
		qbr.setQuery(query);
		qbr.setEntityId(QueryBundleUtils.getTableId(query));
		view.add(progress);
		progress.startAndTrackJob("", false, AsynchType.TableQuery, qbr,
	...
	public void configure(List<FacetColumnResult> facets,
			CallbackP<FacetColumnRequest> facetChangedHandler, List<ColumnModel> types,
			Callback resetFacetsHandler) {
		view.clear();
				...
				if (cm != null) {
					switch (facet.getFacetType()) {
						case enumeration:
							...
							if (facetResultValues.getFacetValues() != null
									&& facetResultValues.getFacetValues().size() > 0) {
								FacetColumnResultValuesWidget valuesWidget =
										ginInjector.getFacetColumnResultValuesWidget();
						...
						case range:
							...
							if (rangeFacet.getColumnMin() != null) {
								...
								FacetColumnResultRangeWidget rangeWidget =
										ginInjector.getFacetColumnResultRangeWidget();
