...
public class JoinTeamWidget
  implements JoinTeamWidgetView.Presenter, WidgetRendererPresenter {
  ...
  @Inject
  public JoinTeamWidget(
    JoinTeamWidgetView view,
  ...
  public void configure(
    String teamId,
    boolean isChallengeSignup,
    TeamMembershipStatus teamMembershipStatus,
    Callback teamUpdatedCallback,
    ...
    accessRequirementsUrl =
      ...
      "=" +
      RestrictableObjectType.TEAM.toString();
    ...
    if (isLoggedIn && teamMembershipStatus != null) {
      ...
      if (teamMembershipStatus.getIsMember()) {
        ...
        teamMembershipStatus.getMembershipApprovalRequired() &&
  ...
  public void configure(
    ...
    this.teamId = null;
    if (descriptor.containsKey(WidgetConstants.TEAM_ID_KEY)) this.teamId =
      descriptor.get(WidgetConstants.TEAM_ID_KEY);
    ...
    if (descriptor.containsKey(WebConstants.JOIN_WIDGET_IS_CHALLENGE_KEY)) {
      this.isChallengeSignup =
        Boolean.parseBoolean(
          descriptor.get(WebConstants.JOIN_WIDGET_IS_CHALLENGE_KEY)
        );
    } else {
      // check for old param
      this.isChallengeSignup =
        descriptor.containsKey(
            WidgetConstants.JOIN_WIDGET_SHOW_PROFILE_FORM_KEY
          )
          ? Boolean.parseBoolean(
            descriptor.get(WidgetConstants.JOIN_WIDGET_SHOW_PROFILE_FORM_KEY)
          )
          : false;
    }
    this.isSimpleRequestButton = false;
    if (
      descriptor.containsKey(WidgetConstants.JOIN_TEAM_IS_SIMPLE_REQUEST_BUTTON)
    ) {
      this.isSimpleRequestButton =
        Boolean.parseBoolean(
          descriptor.get(WidgetConstants.JOIN_TEAM_IS_SIMPLE_REQUEST_BUTTON)
        );
    }
    this.requestExpiresInXDays = null;
    if (
      descriptor.containsKey(
        WidgetConstants.JOIN_WIDGET_REQUEST_EXPIRES_IN_X_DAYS_KEY
      )
    ) {
      this.requestExpiresInXDays =
        Integer.parseInt(
          descriptor.get(
            WidgetConstants.JOIN_WIDGET_REQUEST_EXPIRES_IN_X_DAYS_KEY
          )
    ...
    this.requestOpenInfoText =
      descriptor.get(WidgetConstants.JOIN_TEAM_OPEN_REQUEST_TEXT);
    refresh();
  }
  ...
  public void sendJoinRequestStep0() {
    ...
    wizardSynAlert.clear();
    synapseClient.getTeamAccessRequirements(
      teamId,
  ...
  public void getChallengeParticipationInfoWikiKey(
    final CallbackP<WikiPageKey> callback
  ) {
    synapseClient.getPageNameToWikiKeyMap(
      new AsyncCallback<HashMap<String, WikiPageKey>>() {
        @Override
        public void onSuccess(HashMap<String, WikiPageKey> result) {
          callback.invoke(
            result.get(WebConstants.CHALLENGE_PARTICIPATION_INFO)
          );
        }
  ...
  public void sendJoinRequestStep2() {
    if (currentAccessRequirement >= accessRequirements.size()) {
      ...
      final AccessRequirement accessRequirement = accessRequirements.get(
      ...
      if (
        accessRequirement instanceof SelfSignAccessRequirementInterface ||
        accessRequirement instanceof ACTAccessRequirement ||
        accessRequirement instanceof ManagedACTAccessRequirement
      ) {
        String text = GovernanceServiceHelper.getAccessRequirementText(
          accessRequirement
        );
        ...
        boolean isACTAccessRequirement =
          accessRequirement instanceof ACTAccessRequirement ||
          accessRequirement instanceof ManagedACTAccessRequirement;
        String primaryButtonText = isACTAccessRequirement
          ? "Continue"
          : "Accept";
        ...
        synAlert.showError(
          "Unsupported access restriction type - " +
          accessRequirement.getClass().getName()
        );
      }
    }
  }

  public String getEncodedParamValue(
    String paramKey,
    String value,
    String suffix
  ) {
  ...
  public void setLicenseAccepted(AccessRequirement ar) {
    ...
    if (
      ar instanceof ACTAccessRequirement ||
      ar instanceof ManagedACTAccessRequirement
    ) {
    ...
    synapseClient.requestMembership(
      ...
      new AsyncCallback<TeamMembershipStatus>() {
        ...
        public void onSuccess(TeamMembershipStatus teamMembershipStatus) {
          ...
          if (teamUpdatedCallback != null) teamUpdatedCallback.invoke();
          if (widgetRefreshRequired != null) widgetRefreshRequired.invoke();
        }
