...
public class JoinTeamWidget implements JoinTeamWidgetView.Presenter, WidgetRendererPresenter {
	...
	@Inject
	public JoinTeamWidget(JoinTeamWidgetView view, SynapseClientAsync synapseClient,
			GlobalApplicationState globalApplicationState,
			AuthenticationController authenticationController, GWTWrapper gwt, MarkdownWidget wikiPage,
			WizardProgressWidget progressWidget, SynapseAlert synAlert, CookieProvider cookies) {
		this.view = view;
	...
	/**
	 * Simple join button configuration. Give a team to join, it will invoke the callback when the
	...
	public void configure(String teamId, boolean isChallengeSignup,
			TeamMembershipStatus teamMembershipStatus, Callback teamUpdatedCallback,
			String isMemberMessage, String successMessage, String buttonText, String requestOpenInfoText,
		...
		accessRequirementsUrl =
				"#!AccessRequirements:" + AccessRequirementsPlace.ID_PARAM + "=" + teamId + "&"
		...
		if (isLoggedIn) {
			...
			if (teamMembershipStatus.getIsMember()) {
			...
			} else if (teamMembershipStatus.getCanJoin()) { // not in team but can join with a single
																											// request
				...
				// display a message saying "your membership request is pending review by team
				// administration"
	...
	public void configure(WikiPageKey wikiKey, Map<String, String> descriptor,
		...
		if (descriptor.containsKey(WebConstants.JOIN_WIDGET_IS_CHALLENGE_KEY)) {
			this.isChallengeSignup =
					Boolean.parseBoolean(descriptor.get(WebConstants.JOIN_WIDGET_IS_CHALLENGE_KEY));
		} else {
			// check for old param
			this.isChallengeSignup =
					descriptor.containsKey(WidgetConstants.JOIN_WIDGET_SHOW_PROFILE_FORM_KEY)
							? Boolean
									.parseBoolean(descriptor.get(WidgetConstants.JOIN_WIDGET_SHOW_PROFILE_FORM_KEY))
							: false;
		}
		this.isSimpleRequestButton = false;
		if (descriptor.containsKey(WidgetConstants.JOIN_TEAM_IS_SIMPLE_REQUEST_BUTTON)) {
			this.isSimpleRequestButton =
					Boolean.parseBoolean(descriptor.get(WidgetConstants.JOIN_TEAM_IS_SIMPLE_REQUEST_BUTTON));
		}
		this.requestExpiresInXDays = null;
		if (descriptor.containsKey(WidgetConstants.JOIN_WIDGET_REQUEST_EXPIRES_IN_X_DAYS_KEY)) {
			this.requestExpiresInXDays = Integer
					.parseInt(descriptor.get(WidgetConstants.JOIN_WIDGET_REQUEST_EXPIRES_IN_X_DAYS_KEY));
		}
	...
	private void refresh() {
		...
		if (isLoggedIn) {
			...
			synapseClient.getTeamBundle(authenticationController.getCurrentUserPrincipalId(), teamId,
					isLoggedIn, new AsyncCallback<TeamBundle>() {
						@Override
						public void onSuccess(TeamBundle result) {
							...
							configure(team.getId(), isChallengeSignup, teamMembershipStatus, null,
									isMemberMessage, successMessage, buttonText, requestOpenInfoText,
									isSimpleRequestButton);
						}
			...
			configure(teamId, isChallengeSignup, null, null, isMemberMessage, successMessage, buttonText,
					requestOpenInfoText, isSimpleRequestButton);
		}
	}
	...
	public void sendJoinRequestStep0() {
		...
		synapseClient.getTeamAccessRequirements(teamId, new AsyncCallback<List<AccessRequirement>>() {
			...
			public void onSuccess(List<AccessRequirement> results) {
				...
				// access requirements initialized, show the join wizard if challenge signup, or if there
				// are AR to show
	...
	/**
	 * Check for unmet access restrictions. As long as more exist, it will keep calling itself until
	 * all restrictions are approved. Will not proceed to step3 (joining the team) until all have been
	 * approved.
	 * 
		...
		if (currentAccessRequirement >= accessRequirements.size()) {
			...
			if (accessRequirement instanceof SelfSignAccessRequirementInterface
				...
				if (text == null || text.trim().isEmpty()) {
					WikiPageKey wikiKey = new WikiPageKey(accessRequirement.getId().toString(),
				...
				synAlert.showError(
						"Unsupported access restriction type - " + accessRequirement.getClass().getName());
			}
		}
	}
	...
	public void setLicenseAccepted(AccessRequirement ar) {
		AsyncCallback<AccessApproval> callback = new AsyncCallback<AccessApproval>() {
			...
			public void onSuccess(AccessApproval result) {
				// ToU signed, now try to register for the challenge (will check for other access
				// restrictions before join)
				sendJoinRequestStep2();
			}
		...
		if (ar instanceof ACTAccessRequirement || ar instanceof ManagedACTAccessRequirement) {
			...
			GovernanceServiceHelper.signTermsOfUse(authenticationController.getCurrentUserPrincipalId(),
					ar, synapseClient, callback);
		}
	}
	...
	public void sendJoinRequestStep3() {
		...
		synapseClient.requestMembership(authenticationController.getCurrentUserPrincipalId(), teamId,
				message, gwt.getHostPageBaseURL(), expiresOn, new AsyncCallback<TeamMembershipStatus>() {
					@Override
