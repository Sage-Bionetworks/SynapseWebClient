...
public class MemberListWidget implements MemberListWidgetView.Presenter {
	...
	@Inject
	public MemberListWidget(MemberListWidgetView view, SynapseClientAsync synapseClient,
			SynapseJavascriptClient jsClient, AuthenticationController authenticationController,
			GlobalApplicationState globalApplicationState, LoadMoreWidgetContainer membersContainer) {
		this.view = view;
	...
	public void configure(String teamId, boolean isAdmin, TeamMemberTypeFilterOptions memberType,
	...
	public void loadMore() {
		jsClient.getTeamMembers(teamId, searchTerm, memberType, MEMBER_LIMIT, offset,
				new AsyncCallback<TeamMemberPagedResults>() {
					@Override
	...
	public void setIsAdmin(String principalId, boolean isAdmin) {
		synapseClient.setIsTeamAdmin(authenticationController.getCurrentUserPrincipalId(), principalId,
				teamId, isAdmin, new AsyncCallback<Void>() {
					@Override
					public void onSuccess(Void result) {
						// success, refresh the team
