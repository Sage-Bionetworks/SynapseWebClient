		...
		for (TeamMemberBundle teamMember : members) {
			...
			if (isAdmin) {
				...
				userBadge.addContextCommand("Remove", () -> {
					DisplayUtils.showConfirmDialog("Remove Member?",
							DisplayUtils.getDisplayName(member) + DisplayConstants.PROMPT_SURE_REMOVE_MEMBER,
							() -> {
			...
			if (teamMember.getIsTeamAdmin()) {
				...
				teamManagerBadge
						.addStyleName("lightGreyBackground border-left-1 border-bottom-1 border-right-1");
