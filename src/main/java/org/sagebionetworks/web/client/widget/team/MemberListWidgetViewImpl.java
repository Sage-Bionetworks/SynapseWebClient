...
public class MemberListWidgetViewImpl
  extends FlowPanel
  implements MemberListWidgetView {
  ...
  @Inject
  public MemberListWidgetViewImpl(
  ...
  public void addMembers(List<TeamMemberBundle> members, boolean isAdmin) {
    for (TeamMemberBundle teamMember : members) {
      ...
      if (isAdmin) {
        ...
        String commandName = isTeamAdmin
          ? "Make Team Member"
          : "Make Team Manager";
        userBadge.addContextCommand(
          commandName,
          () -> {
        ...
        userBadge.addContextCommand(
          ...
          () -> {
            DisplayUtils.showConfirmDialog(
              "Remove Member?",
              DisplayUtils.getDisplayName(member) +
              DisplayConstants.PROMPT_SURE_REMOVE_MEMBER,
              () -> {
