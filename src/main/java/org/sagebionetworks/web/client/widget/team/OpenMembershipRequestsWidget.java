...
public class OpenMembershipRequestsWidget
  implements OpenMembershipRequestsWidgetView.Presenter {
  ...
  @Inject
  public OpenMembershipRequestsWidget(
    OpenMembershipRequestsWidgetView view,
  ...
  public void configure(String teamId, Callback teamUpdatedCallback) {
    ...
    synapseClient.getOpenRequests(
      teamId,
      new AsyncCallback<List<MembershipRequestBundle>>() {
        @Override
        public void onSuccess(List<MembershipRequestBundle> result) {
          ...
          for (MembershipRequestBundle b : result) {
            ...
            if (request.getMessage() != null) requestMessage =
              request.getMessage();
            requestMessages.add(requestMessage);
            requestIds.add(request.getId());
            profiles.add(b.getUserProfile());
            createdOnDates.add(
              dateTimeUtils.getDateTimeString(request.getCreatedOn())
            );
          }
  ...
  public void acceptRequest(String userId) {
    ...
    synAlert.clear();
    // try to add user id to team (since we know there's an open membership request). then update open
