...
public class OpenMembershipRequestsWidget implements OpenMembershipRequestsWidgetView.Presenter {
	...
	public OpenMembershipRequestsWidget(OpenMembershipRequestsWidgetView view,
			...
			GWTWrapper gwt, SynapseAlert synAlert, PopupUtilsView popupUtils, DateTimeUtils dateTimeUtils,
	...
	public void acceptRequest(String userId) {
		...
		synAlert.clear();
		// try to add user id to team (since we know there's an open membership request). then update
		// open membership request list
		synapseClient.addTeamMember(userId, teamId, "", gwt.getHostPageBaseURL(),
