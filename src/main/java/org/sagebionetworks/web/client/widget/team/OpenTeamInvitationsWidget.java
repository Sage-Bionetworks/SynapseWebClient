...
public class OpenTeamInvitationsWidget
		implements OpenTeamInvitationsWidgetView.Presenter, IsWidget {
	...
	public OpenTeamInvitationsWidget(OpenTeamInvitationsWidgetView view,
			...
			AuthenticationController authenticationController, PortalGinInjector ginInjector,
			SynapseAlert synAlert, DateTimeUtils dateTimeUtils, PopupUtilsView popupUtils,
	...
	public void configure(final Callback teamUpdatedCallback,
			final CallbackP<List<OpenUserInvitationBundle>> openTeamInvitationsCallback) {
		...
		if (authenticationController.isLoggedIn()) {
			...
			synapseClient.getOpenInvitations(authenticationController.getCurrentUserPrincipalId(),
					new AsyncCallback<ArrayList<OpenUserInvitationBundle>>() {
						@Override
	...
	public void showTeamInvites(List<OpenUserInvitationBundle> invites) {
		...
		for (OpenUserInvitationBundle b : invites) {
			...
			view.addTeamInvite(b.getTeam(), invitationMessage, createdOnString, invitationId,
					joinButton.asWidget());
		}
	}
