...
public class OpenUserInvitationsWidget implements OpenUserInvitationsWidgetView.Presenter {
	...
	public OpenUserInvitationsWidget(OpenUserInvitationsWidgetView view,
			...
			SynapseAlert synAlert, GWTWrapper gwt, DateTimeUtils dateTimeUtils,
			SynapseJavascriptClient jsClient, PopupUtilsView popupUtils, PortalGinInjector ginInjector) {
		this.view = view;
	...
	public void getNextBatch() {
		...
		synapseClient.getOpenTeamInvitations(teamId, INVITATION_BATCH_LIMIT, currentOffset,
				new AsyncCallback<ArrayList<OpenTeamInvitationBundle>>() {
					@Override
	...
	private void addInvitations(List<OpenTeamInvitationBundle> bundles) {
		...
		for (OpenTeamInvitationBundle b : bundles) {
			...
			String createdOn =
					dateTimeUtils.getDateTimeString(b.getMembershipInvitation().getCreatedOn());
			if (b.getUserProfile() != null) {
				...
				view.addInvitation(userBadge, mis.getInviteeEmail(), mis.getId(), mis.getMessage(),
						createdOn);
				...
				synAlert.showError(
						"Membership invitation with ID " + mis.getId() + " is not in a valid state.");
			}
		}
	}
	...
	public void resendInvitation(String membershipInvitationId) {
		gwt.saveWindowPosition();
		synapseClient.resendTeamInvitation(membershipInvitationId, gwt.getHostPageBaseURL(),
