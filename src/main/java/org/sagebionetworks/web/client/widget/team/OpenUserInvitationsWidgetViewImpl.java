...
public class OpenUserInvitationsWidgetViewImpl
  implements OpenUserInvitationsWidgetView {

  public static final String MEMBERSHIP_INVITATION_ID =
    "data-membership-invitation-id";
  ...
  private ClickHandler removeInvitationClickHandler = event -> {
    ...
    String membershipInvitationId = btn
      .getElement()
      .getAttribute(MEMBERSHIP_INVITATION_ID);
  ...
  public void addInvitation(
    ...
    if (isInviteeEmail) {
      openUserInvitationWidget.inviteeEmailTableData.add(
        new Text(inviteeEmail)
      );
    } else {
      openUserInvitationWidget.badgeTableData.add(badge);
    }
    if (message != null) {
      openUserInvitationWidget.messageTableData.add(
        new HTML(jsniUtils.sanitizeHtml(message))
      );
    }
    openUserInvitationWidget.createdOnTableData.add(new Italic(createdOn));

    openUserInvitationWidget.cancelButton
      .getElement()
      .setAttribute(MEMBERSHIP_INVITATION_ID, misId);
    openUserInvitationWidget.cancelButton.addClickHandler(
      removeInvitationClickHandler
    );

    openUserInvitationWidget.resendButton
      .getElement()
      .setAttribute(MEMBERSHIP_INVITATION_ID, misId);
    openUserInvitationWidget.resendButton.addClickHandler(
      resendInvitationClickHandler
    );
