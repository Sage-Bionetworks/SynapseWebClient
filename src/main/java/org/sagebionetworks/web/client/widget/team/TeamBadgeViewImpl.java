...
public class TeamBadgeViewImpl extends FlowPanel implements TeamBadgeView {
	...
	@Inject
	public TeamBadgeViewImpl(SynapseJSNIUtils synapseJSNIUtils, SageImageBundle sageImageBundle,
			IconsImageBundle iconsImageBundle, GlobalApplicationState globalApplicationState) {
	...
	@Override
	public void setTeam(final Team team, Integer maxNameLength, String teamIconUrl,
			ClickHandler customClickHandler) {
		clear();
		teamId = team.getId();
		...
		if (team != null) {
			String name = maxNameLength == null ? team.getName()
					: DisplayUtils.stubStrPartialWord(team.getName(), maxNameLength);
			anchor.setText(name);
	...
	public void setTeamWithoutLink(String name, boolean isPublic) {
		...
		profilePicture
				.addStyleName("font-size-lg imageButton lightGreyText margin-right-4 margin-left-5");
		add(profilePicture);
	...
	public void setRequestCount(String count) {
		...
		notificationsPanel
				.setWidget(DisplayUtils.addTooltip(widget, DisplayConstants.PENDING_JOIN_REQUESTS_TOOLTIP));
	}
