...
public class UserTeamBadge implements WidgetRendererPresenter {
  ...
  public void configure(
    ...
    final boolean isToc = widgetDescriptor.containsKey(
        WidgetConstants.IS_TOC_KEY
      )
    ...
    if (
      ...
      alias =
        widgetDescriptor.get(
          WidgetConstants.USER_TEAM_BADGE_WIDGET_USERNAME_KEY
        );
    }
    if (alias != null) {
      // get user group header for this alias (using a new service)
      usgFromAliasAsyncHandler.getUserGroupHeader(
        alias,
        new AsyncCallback<UserGroupHeader>() {
          @Override
      ...
      String id = widgetDescriptor.get(
  ...
  public void configure(final String id) {
    ...
    usgFromIdAsyncHandler.getUserGroupHeader(
      id,
      new AsyncCallback<UserGroupHeader>() {
        @Override
        public void onFailure(Throwable caught) {
