...
public class TeamEditModalWidget implements IsWidget, TeamEditModalWidgetView.Presenter {
	...
	@Inject
	public TeamEditModalWidget(SynapseAlert synAlert, final TeamEditModalWidgetView view,
			SynapseClientAsync synapseClient, final ImageUploadWidget uploader,
			SynapseJSNIUtils jsniUtils, AuthenticationController authenticationController,
			SynapseProperties synapseProperties, PortalGinInjector ginInjector) {
		...
		authenticatedUserGroupId = Long.parseLong(
				synapseProperties.getSynapseProperty(WebConstants.AUTHENTICATED_ACL_PRINCIPAL_ID));
	...
	private void show() {
		...
		if (team.getIcon() != null)
			view.setImageURL(jsniUtils.getFileHandleAssociationUrl(team.getId(),
					FileHandleAssociateType.TeamAttachment, team.getIcon()));
		else
			view.setDefaultIconVisible();
	...
	public void updateViewFromACL() {
		...
		if (ra != null) {
			view.setAuthenticatedUsersCanSendMessageToTeam(
					ModelConstants.TEAM_MESSENGER_PERMISSIONS.equals(ra.getAccessType()));
		} else {
