	...
	public FileMetadata[] getSelectedFileMetadata() {
		...
		if (fileNames != null) {
			...
			for (int i = 0; i < fileNames.length; i++) {
				...
				String contentType = org.sagebionetworks.web.client.ContentTypeUtils
						.fixDefaultContentType(synapseJsniUtils.getContentType(fileList, i), name);
		...
		multipartUploader.uploadFile(fileMetadata.getFileName(), fileMetadata.getContentType(), blob,
				new ProgressingFileUploadHandler() {
					...
					public void updateProgress(double currentProgress, String progressText,
							String uploadSpeed) {
						int totalProgress = count * 100 / fileMetaArr.length
