...
public class MultipartUploaderImpl implements MultipartUploader {
	...
	@Inject
	public MultipartUploaderImpl(GWTWrapper gwt, SynapseJSNIUtils synapseJsniUtils,
	...
	@Override
	public void uploadFile(final String fileName, final String contentType,
			final JavaScriptObject blob, ProgressingFileUploadHandler handler,
			final Long storageLocationId, HasAttachHandlers view) {
	...
	private void retryUpload() {
		uploadFile(request.getFileName(), request.getContentType(), blob, handler,
				request.getStorageLocationId(), view);
	}
	...
	private void startMultipartUpload(String md5) {
		...
		String fileStats = "fileName=" + request.getFileName() + " MD5=" + request.getContentMD5Hex()
				+ " contentType=" + request.getContentType() + " fileSize=" + request.getFileSizeBytes()
				+ " partSizeBytes=" + request.getPartSizeBytes() + "\n";
		log(fileStats);
	...
	public void attemptUploadCurrentPart() {
		...
		batchPresignedUploadUrlRequest
				.setPartNumbers(Collections.singletonList(new Long(currentPartNumber)));
		batchPresignedUploadUrlRequest.setUploadId(currentStatus.getUploadId());
		jsClient.getMultipartPresignedUrlBatch(batchPresignedUploadUrlRequest,
				new AsyncCallback<BatchPresignedUploadUrlResponse>() {
					@Override
					...
					public void onSuccess(BatchPresignedUploadUrlResponse batchPresignedUploadUrlResponse) {
						if (isStillUploading()) {
							...
							if (xhr != null) {
								xhr.setOnReadyStateChange(new ReadyStateChangeHandler() {
									...
									public void onReadyStateChange(XMLHttpRequest xhr) {
										log("XMLHttpRequest.setOnReadyStateChange: readyState=" + xhr.getReadyState()
												+ " status=" + xhr.getStatus() + "\n");
										if (xhr.getReadyState() == 4) { // XMLHttpRequest.DONE=4, posts suggest this
																										// value is not resolved in some browsers
							...
							ByteRange range = new ByteRange(currentPartNumber, request.getFileSizeBytes(),
									request.getPartSizeBytes());
							log("attemptChunkUpload: uploading file chunk. ByteRange=" + range.getStart() + "-"
									+ range.getEnd() + " \n");
							ProgressCallback progressCallback = new ProgressCallback() {
								@Override
								public void updateProgress(double loaded, double total) {
									// 0 < currentPartProgress < 1. We need to add this to the chunks that have
									// already been uploaded. And divide by the total chunk count.
									if (!isStillUploading()) {
										if (xhr != null) {
											xhr.abort();
										}
										...
										double currentProgress = (((double) (completedPartCount)) + currentPartProgress)
												/ ((double) totalPartCount);
										...
										if (msElapsed > 0 && (msElapsed > nextProgressPoint)) {
											double totalBytesTransfered =
													(request.getPartSizeBytes() * completedPartCount) + loaded;
											uploadSpeed = "(" + DisplayUtils
													.getFriendlySize(totalBytesTransfered / (msElapsed / 1000), true) + "/s)";
											nextProgressPoint += 2000;
										}
							...
							synapseJsniUtils.uploadFileChunk(BINARY_CONTENT_TYPE, blob, range.getStart(),
									range.getEnd(), urlString, xhr, progressCallback);
						}
					}
				});
	}
	...
	private void uploadFailedDueToFileModification(String startMd5, String newMd5) {
		handler.uploadFailed("Unable to upload the file \"" + request.getFileName()
				+ "\" because it's been modified during the upload.  The starting md5 of the file ("
				+ startMd5 + ") differs from the current md5 (" + newMd5 + ").");
	}

	public void completeMultipartUploadAfterMd5Verification() {
		logFullUpload();
		// combine
		jsClient.completeMultipartUpload(currentStatus.getUploadId(),
				new AsyncCallback<MultipartUploadStatus>() {
					@Override
					public void onFailure(Throwable caught) {
	...
	public void addCurrentPartToMultipartUpload() {
		...
		if (isStillUploading()) {
			synapseJsniUtils.getFilePartMd5(blob, currentPartNumber - 1, request.getPartSizeBytes(),
					partMd5 -> {
						log("partNumber=" + currentPartNumber + " partNumberMd5=" + partMd5);
						jsClient.addPartToMultipartUpload(currentStatus.getUploadId(), currentPartNumber,
								partMd5, new AsyncCallback<AddPartResponse>() {
									@Override
									public void onFailure(Throwable caught) {
										partFailure(caught.getMessage());
									}
