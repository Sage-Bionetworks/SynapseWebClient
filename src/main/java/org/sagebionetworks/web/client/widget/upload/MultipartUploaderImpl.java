...
public class MultipartUploaderImpl implements MultipartUploader {
    ...
    if (isStillUploading()) {
      ...
      synapseJsniUtils.getFileMd5(
        ...
        md5 -> {
          ...
          if (
            request.getContentMD5Hex() != null &&
            !md5.equals(request.getContentMD5Hex())
          ) {
    ...
    jsClient.startMultipartUpload(
      ...
      new AsyncCallback<MultipartUploadStatus>() {
        ...
        public void onSuccess(MultipartUploadStatus status) {
          ...
          completedPartCount =
            getCompletedPartCount(currentStatus.getPartsState());
  ...
  public void attemptToUploadNextPart() {
    ...
    if (currentStatus.getPartsState().charAt(currentPartNumber - 1) == '0') {
      ...
      log(
        "attemptChunkUpload: skipping part number = " + currentPartNumber + "\n"
      );
      partSuccess();
    }
  }
  ...
  public void attemptUploadCurrentPart() {
    log(
      "attemptChunkUpload: attempting to upload part number = " +
      currentPartNumber +
      "\n"
    );
    ...
    jsClient.getMultipartPresignedUrlBatch(
      ...
      new AsyncCallback<BatchPresignedUploadUrlResponse>() {
        ...
        public void onSuccess(
          BatchPresignedUploadUrlResponse batchPresignedUploadUrlResponse
        ) {
          if (isStillUploading()) {
            PartPresignedUrl url = batchPresignedUploadUrlResponse
              ...
              xhr.setOnReadyStateChange(
                new ReadyStateChangeHandler() {
                  ...
                  public void onReadyStateChange(XMLHttpRequest xhr) {
                    log(
                      "XMLHttpRequest.setOnReadyStateChange: readyState=" +
                      xhr.getReadyState() +
                      " status=" +
                      xhr.getStatus() +
                      "\n"
                    ...
                    if (xhr.getReadyState() == 4) { // XMLHttpRequest.DONE=4, posts suggest this value is not resolved in some browsers
                      ...
                      if (xhr.getStatus() == 200) { // OK
                        ...
                        log(
                          "XMLHttpRequest.setOnReadyStateChange: Failure\n" +
                          xhr.getStatusText()
                        );
                        partFailure(
            ...
            log(
              "attemptChunkUpload: uploading file chunk. ByteRange=" +
              range.getStart() +
              "-" +
              range.getEnd() +
              " \n"
              ...
              public void updateProgress(double loaded, double total) {
                ...
                if (!isStillUploading()) {
                  ...
                  double currentProgress =
                    (((double) (completedPartCount)) + currentPartProgress) /
                  ...
                  if (msElapsed > 0 && (msElapsed > nextProgressPoint)) {
                    double totalBytesTransfered =
                      (request.getPartSizeBytes() * completedPartCount) +
                      loaded;
                    uploadSpeed =
                      "(" +
                      DisplayUtils.getFriendlySize(
                        totalBytesTransfered / (msElapsed / 1000),
                        true
                      ) +
                      "/s)";
                    nextProgressPoint += 2000;
            ...
            synapseJsniUtils.uploadFileChunk(
              BINARY_CONTENT_TYPE,
              blob,
              range.getStart(),
              range.getEnd(),
  ...
  private void uploadFailedDueToFileModification(
    String startMd5,
    String newMd5
  ) {
    handler.uploadFailed(
      ...
      request.getFileName() +
      "\" because it's been modified during the upload.  \n\nThe starting md5 of the file (" +
      startMd5 +
      ") differs from the current md5 (" +
      newMd5 +
      ")."
    );
  }
  ...
  private void uploadFailedDueToFileModification(Date lastModifiedDate) {
    handler.uploadFailed(
      ...
      dateTimeUtils.getRelativeTime(lastModifiedDate) +
      "."
    );
  }
  ...
  public void completeMultipartUploadAfterMd5Verification() {
    // combine
    jsClient.completeMultipartUpload(
      currentStatus.getUploadId(),
      new AsyncCallback<MultipartUploadStatus>() {
        @Override
        public void onFailure(Throwable caught) {
  ...
  public void addCurrentPartToMultipartUpload() {
    ...
    if (isStillUploading()) {
      synapseJsniUtils.getFilePartMd5(
        blob,
        currentPartNumber - 1,
        request.getPartSizeBytes(),
        partMd5 -> {
          log("partNumber=" + currentPartNumber + " partNumberMd5=" + partMd5);
          jsClient.addPartToMultipartUpload(
            currentStatus.getUploadId(),
            currentPartNumber,
            partMd5,
            new AsyncCallback<AddPartResponse>() {
              @Override
              public void onFailure(Throwable caught) {
                partFailure(caught.getMessage());
              }
              ...
              public void onSuccess(AddPartResponse addPartResponse) {
                if (
                  addPartResponse
                    .getAddPartState()
                    .equals(AddPartState.ADD_SUCCESS)
                ) {
                  completedPartCount++;
                  partSuccess();
                } else {
