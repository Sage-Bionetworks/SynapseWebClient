...
public class VerificationSubmissionWidget
	...
	// this could be Reject or Suspend. We store this state while the reason is being collected from
	// the ACT user
	...
	@Inject
	public VerificationSubmissionWidget(PortalGinInjector ginInjector,
			UserProfileClientAsync userProfileClient, SynapseAlert synAlert,
			FileHandleList fileHandleList, RejectReasonWidget promptModalView,
			GlobalApplicationState globalAppState, GWTWrapper gwt) {
	 ...
	 * @return
	 */
	public VerificationSubmissionWidget configure(VerificationSubmission verificationSubmission,
			boolean isACTMember, boolean isModal) {
		...
		this.profile = null;
		initView(isModal);
		view.setProfileLink(verificationSubmission.getCreatedBy(),
	 ...
	 * @return
	 */
	public VerificationSubmissionWidget configure(UserProfile userProfile, String orcId,
			boolean isModal, List<AttachmentMetadata> existingAttachments) {
	...
	public void showNewVerificationSubmission() {
		if (isPreconditionsForNewSubmissionMet(profile, orcId)) {
			...
			fileHandleList.configure().setUploadButtonText("Upload...").setCanDelete(true)
					.setCanUpload(true);
	...
	public void showExistingVerificationSubmission() {
		...
		// marking own submission as rejected currently forbidden, and we don't want to actually delete
		// the submission
		...
		} else if (VerificationStateEnum.SUSPENDED.equals(currentState.getState())
	...
	public void updateVerificationState(VerificationStateEnum state, String reason) {
		...
		userProfileClient.updateVerificationState(verificationId, newState, gwt.getHostPageBaseURL(),
	...
	public void submitVerification() {
		...
		sub.setOrcid(orcId);
		userProfileClient.createVerificationSubmission(sub, gwt.getHostPageBaseURL(),
