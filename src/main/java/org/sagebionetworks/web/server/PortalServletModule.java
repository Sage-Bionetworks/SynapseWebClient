...
public class PortalServletModule extends ServletModule {
	...
	protected void configureServlets() {
		...
		// Catch-all. Note that "/*" would override all other servlet binding, and "/" overrides the
		// default handler
			...
			try {
				...
				while (it.hasNext()) {
					...
					if (obKey instanceof String) {
						...
						if (previous != null) {
							logger.info("Overriding a ServerConstants.properties key: " + key
									+ " with a value from System.properties(). New value: " + newValue);
						}
					}
				}
				// Bind the properties
