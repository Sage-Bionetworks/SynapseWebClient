...
public class ChallengeClientImpl extends SynapseClientBase implements ChallengeClient {
	...
	public PaginatedResults<Evaluation> getAvailableEvaluations() throws RestServiceException {
		...
		try {
			return convertPaginated(synapseClient.getAvailableEvaluationsPaginated(
					EVALUATION_PAGINATION_OFFSET, EVALUATION_PAGINATION_LIMIT));
	...
	public PaginatedResults<Evaluation> getAvailableEvaluations(Set<String> targetEvaluationIds)
		...
		try {
			...
			return convertPaginated(synapseClient.getAvailableEvaluationsPaginated(
					EVALUATION_PAGINATION_OFFSET, EVALUATION_PAGINATION_LIMIT, targetEvaluationIdsList));
		} catch (SynapseException e) {
			throw ExceptionUtil.convertSynapseException(e);
		}
	}
	...
	public static List<Evaluation> getShareableEvaluations(String entityId,
			SynapseClient synapseClient) throws SynapseException {
		// look up the available evaluations
		org.sagebionetworks.reflection.model.PaginatedResults<Evaluation> allEvaluations =
				synapseClient.getEvaluationByContentSource(entityId, EVALUATION_PAGINATION_OFFSET,
	...
	public Submission createTeamSubmission(Submission submission, String etag, String memberStateHash,
		...
		try {
			...
			return synapseClient.createTeamSubmission(submission, etag, memberStateHash,
					challengeEndpoint, signedTokenEndpoint);
		} catch (SynapseException e) {
			throw ExceptionUtil.convertSynapseException(e);
		}
	}
	...
	public Submission createIndividualSubmission(Submission submission, String etag,
		...
		try {
			String signedTokenEndpoint = SynapseClientImpl.getSignedTokenEndpoint(hostPageBaseURL);
			String challengeEndpoint = SynapseClientImpl.getChallengeEndpoint(hostPageBaseURL);
			return synapseClient.createIndividualSubmission(submission, etag, challengeEndpoint,
					signedTokenEndpoint);
		} catch (SynapseException e) {
			throw ExceptionUtil.convertSynapseException(e);
		}
	...
	public List<Team> getSubmissionTeams(String userId, String challengeId)
		...
		org.sagebionetworks.client.SynapseClient synapseClient = createSynapseClient();
		try {
			PaginatedIds results = synapseClient.listSubmissionTeams(challengeId, userId,
					GROUPS_PAGINATION_LIMIT, GROUPS_PAGINATION_OFFSET);
	...
	public List<Team> getTeams(List<String> teamIds, boolean sortAlphabetically,
	...
	@Override
	public ChallengeTeamPagedResults getChallengeTeams(String currentUserId, String challengeId,
			Integer limit, Integer offset) throws RestServiceException {
		org.sagebionetworks.client.SynapseClient synapseClient = createSynapseClient();
		try {
			org.sagebionetworks.repo.model.ChallengeTeamPagedResults pagedResults =
					synapseClient.listChallengeTeams(challengeId, limit.longValue(), offset.longValue());
			...
			if (currentUserId != null) {
				for (int i = 0; i < teamIds.size(); i++) {
					try {
						TeamMember member =
								synapseClient.getTeamMember(teamIds.get(i).toString(), currentUserId);
			...
			ChallengeTeamPagedResults returnResults =
					new ChallengeTeamPagedResults(challengeTeamList, totalCount);
	...
	@Override
	public UserProfilePagedResults getChallengeParticipants(boolean affiliated, String challengeId,
			Integer limit, Integer offset) throws RestServiceException {
		try {
			org.sagebionetworks.client.SynapseClient synapseClient = createSynapseClient();
			PaginatedIds paginatedIds = synapseClient.listChallengeParticipants(challengeId, affiliated,
					limit.longValue(), offset.longValue());
	...
	public List<Team> getRegistratableTeams(String userId, String challengeId)
		...
		try {
			...
			TeamMembershipStatus status =
					synapseClient.getTeamMembershipStatus(challenge.getParticipantTeamId(), userId);
			if (!status.getIsMember()) {
				throw new NotFoundException("User is not registered for the Challenge.");
			}
			PaginatedIds results = synapseClient.listRegistratableTeams(challengeId,
					GROUPS_PAGINATION_LIMIT, GROUPS_PAGINATION_OFFSET);
			return getTeams(results.getResults(), false, synapseClient);
		} catch (SynapseException e) {
			throw ExceptionUtil.convertSynapseException(e);
	...
	public Set<String> getChallengeEvaluationIds(String challengeId) throws RestServiceException {
		...
		try {
			...
			org.sagebionetworks.reflection.model.PaginatedResults<Evaluation> allEvaluations =
					synapseClient.getEvaluationByContentSource(challenge.getProjectId(),
							EVALUATION_PAGINATION_OFFSET, EVALUATION_PAGINATION_LIMIT);
	...
	public Set<String> getProjectEvaluationIds(String projectId) throws RestServiceException {
		...
		try {
			org.sagebionetworks.reflection.model.PaginatedResults<Evaluation> allEvaluations =
					synapseClient.getEvaluationByContentSource(projectId, EVALUATION_PAGINATION_OFFSET,
							EVALUATION_PAGINATION_LIMIT);
