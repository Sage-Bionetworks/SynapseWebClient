...
public class FileHandleAssociationServlet extends HttpServlet {
  ...
  private TokenProvider tokenProvider = () ->
    UserDataProvider.getThreadLocalUserToken(
      FileHandleAssociationServlet.perThreadRequest.get()
    );
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    response.setHeader(
      ...
      "max-age=" + CACHE_TIME_SECONDS
    );
    String token = getToken(request);
    SynapseClient client = createNewClient(token);

    String objectId = request.getParameter(
      WebConstants.ASSOCIATED_OBJECT_ID_PARAM_KEY
    );
    String objectType = request.getParameter(
      WebConstants.ASSOCIATED_OBJECT_TYPE_PARAM_KEY
    );
    String fileHandleId = request.getParameter(
    ...
    try {
      if (fileHandleId != null && (objectId == null || objectType == null)) {
        ...
        if (
          FileHandleAssociateType.UserProfileAttachment.equals(
            fha.getAssociateObjectType()
          ) ||
          FileHandleAssociateType.TeamAttachment.equals(
            fha.getAssociateObjectType()
          )
        ) {
