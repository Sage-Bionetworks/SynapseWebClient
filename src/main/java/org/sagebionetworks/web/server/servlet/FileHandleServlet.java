...
public class FileHandleServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		response.setHeader(WebConstants.CACHE_CONTROL_KEY, WebConstants.CACHE_CONTROL_VALUE_NO_CACHE); // Set
																																																		// standard
																																																		// HTTP/1.1
																																																		// no-cache
																																																		// headers.
		response.setHeader(WebConstants.PRAGMA_KEY, WebConstants.NO_CACHE_VALUE); // Set standard
																																							// HTTP/1.0 no-cache
																																							// header.
		...
		Boolean isPreview =
				Boolean.parseBoolean(request.getParameter(WebConstants.FILE_HANDLE_PREVIEW_PARAM_KEY));
		...
		try {
			resolveUrlAndRedirect(request, response, client, isProxy, teamId, entityId, entityVersion,
					tableColumnId, tableRowId, tableRowVersionNumbrer, ownerId, ownerType, fileName,
			...
			if (isPreview) {
				try {
					...
					// show generic image
					doRedirect(request, response, isProxy,
			...
			response.sendRedirect(FileHandleAssociationServlet.getBaseUrl(request)
					+ FileHandleAssociationServlet.ERROR_PLACE + URLEncoder.encode(e.getMessage()));
		}
	}

	private void resolveUrlAndRedirect(HttpServletRequest request, HttpServletResponse response,
			SynapseClient client, boolean isProxy, String teamId, String entityId, String entityVersion,
			String tableColumnId, String tableRowId, String tableRowVersionNumbrer, String ownerId,
			String ownerType, String fileName, Boolean isPreview, String redirectUrlString,
			URL resolvedUrl, String rawFileHandleId)
			throws MalformedURLException, UnsupportedEncodingException, ClientProtocolException,
			IOException, SynapseException, ServletException {
		...
		if (rawFileHandleId != null) {
			...
			if (wikiVersion != null) {
				if (isPreview) {
					resolvedUrl = client.getVersionOfV2WikiAttachmentPreviewTemporaryUrl(properKey, fileName,
							new Long(wikiVersion));
				} else {
					resolvedUrl = client.getVersionOfV2WikiAttachmentTemporaryUrl(properKey, fileName,
							new Long(wikiVersion));
				}
			} else {
				if (isPreview) {
			...
			if (tableColumnId != null || tableRowId != null || tableRowVersionNumbrer != null) {
				if (tableColumnId != null && tableRowId != null && tableRowVersionNumbrer != null) {
					try {
						...
						if (isPreview)
							resolvedUrl =
									client.getTableFileHandlePreviewTemporaryUrl(entityId, row, tableColumnId);
						else
							resolvedUrl = client.getTableFileHandleTemporaryUrl(entityId, row, tableColumnId);
					} catch (NumberFormatException e) {
						throw new ServletException(WebConstants.TABLE_ROW_ID + " and "
								+ WebConstants.TABLE_ROW_VERSION_NUMBER + " must be Long values. Actual values: "
								+ tableRowId + ", " + tableRowVersionNumbrer);
					}

				} else {
					throw new ServletException("All table fields must be defined, if any are defined: "
							+ WebConstants.TABLE_COLUMN_ID + ", " + WebConstants.TABLE_ROW_ID + ", "
							+ WebConstants.TABLE_ROW_VERSION_NUMBER);
				}
			} else {
				// Entity
				if (entityVersion == null) {
					...
					if (isPreview)
						resolvedUrl =
								client.getFileEntityPreviewTemporaryUrlForVersion(entityId, versionNumber);
					else
	...
	private void doRedirect(HttpServletRequest request, HttpServletResponse response, boolean isProxy,
			URL resolvedUrl) throws ClientProtocolException, IOException {
		if (resolvedUrl != null) {
			if (isProxy) {
				...
				try {
					...
					HttpEntity responseEntity =
							(null != newResponse.getEntity()) ? newResponse.getEntity() : null;
	...
	public static void fillResponseWithSuccess(HttpServletResponse response, String id)
			throws JSONObjectAdapterException, UnsupportedEncodingException, IOException {
	...
	public static void fillResponseWithFailure(HttpServletResponse response, Exception e)
	...
	public static FileEntity getNewFileEntity(String parentEntityId, String fileHandleId, String name,
			SynapseClient client) throws SynapseException {
	...
	public static void fixName(FileEntity fileEntity, FileHandle newFileHandle, SynapseClient client)
