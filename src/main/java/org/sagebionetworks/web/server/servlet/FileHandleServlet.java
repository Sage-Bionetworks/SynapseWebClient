...
public class FileHandleServlet extends HttpServlet {
  ...
  private static Logger logger = Logger.getLogger(
    FileHandleServlet.class.getName()
  );
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    Boolean isPreview = Boolean.parseBoolean(
      request.getParameter(WebConstants.FILE_HANDLE_PREVIEW_PARAM_KEY)
    );
    String redirectUrlString = request.getParameter(
      WebConstants.REDIRECT_URL_KEY
    );
    URL resolvedUrl = null;
    String rawFileHandleId = request.getParameter(
      WebConstants.RAW_FILE_HANDLE_PARAM
    );
  ...
  private void resolveUrlAndRedirect(
    ...
    URL resolvedUrl,
    String rawFileHandleId
  )
    throws MalformedURLException, UnsupportedEncodingException, ClientProtocolException, IOException, SynapseException, ServletException {
    ...
    if (rawFileHandleId != null) {
      ...
      String wikiVersion = request.getParameter(
        WebConstants.WIKI_VERSION_PARAM_KEY
      );
      ...
      if (wikiVersion != null) {
        if (isPreview) {
          resolvedUrl =
            client.getVersionOfV2WikiAttachmentPreviewTemporaryUrl(
              properKey,
              fileName,
          ...
          resolvedUrl =
            client.getVersionOfV2WikiAttachmentTemporaryUrl(
              properKey,
              fileName,
              new Long(wikiVersion)
            );
        ...
        if (isPreview) {
          resolvedUrl =
            client.getV2WikiAttachmentPreviewTemporaryUrl(properKey, fileName);
        } else {
          resolvedUrl =
            client.getV2WikiAttachmentTemporaryUrl(properKey, fileName);
        }
      }
      // Done
      ...
      if (
        ...
        if (
          ...
          try {
            ...
            if (isPreview) resolvedUrl =
              client.getTableFileHandlePreviewTemporaryUrl(
                entityId,
                row,
                tableColumnId
              ); else resolvedUrl =
              client.getTableFileHandleTemporaryUrl(
                entityId,
                row,
                tableColumnId
              );
            ...
            throw new ServletException(
              WebConstants.TABLE_ROW_ID +
              " and " +
              WebConstants.TABLE_ROW_VERSION_NUMBER +
              " must be Long values. Actual values: " +
              tableRowId +
              ", " +
              tableRowVersionNumbrer
            );
          ...
          throw new ServletException(
            "All table fields must be defined, if any are defined: " +
            WebConstants.TABLE_COLUMN_ID +
            ", " +
            WebConstants.TABLE_ROW_ID +
            ", " +
            WebConstants.TABLE_ROW_VERSION_NUMBER
        ...
        if (entityVersion == null) {
          if (isPreview) resolvedUrl =
            client.getFileEntityPreviewTemporaryUrlForCurrentVersion(
              entityId
            ); else resolvedUrl =
          ...
          if (isPreview) resolvedUrl =
            client.getFileEntityPreviewTemporaryUrlForVersion(
              entityId,
              versionNumber
            ); else resolvedUrl =
  ...
  public static void fillResponseWithSuccess(
    HttpServletResponse response,
    String id
  )
    throws JSONObjectAdapterException, UnsupportedEncodingException, IOException {
    UploadResult result = new UploadResult();
    if (id != null) result.setMessage(id); else result.setMessage(
  ...
  public static void fillResponseWithFailure(
    HttpServletResponse response,
    Exception e
  ...
  public static FileEntity getNewFileEntity(
    ...
    if (name != null) fileEntity.setName(name);
  ...
  public static void setForbiddenMessage(HttpServletResponse response)
    ...
    response
      .getOutputStream()
      .write("No session token found".getBytes("UTF-8"));
    response.getOutputStream().flush();
  }
