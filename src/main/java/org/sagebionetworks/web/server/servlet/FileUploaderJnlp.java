...
public class FileUploaderJnlp extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		Boolean isUpdate =
				Boolean.parseBoolean(request.getParameter(WebConstants.FILE_UPLOADER_IS_UPDATE_PARAM));
		...
		try {
			...
			out.println(
					buildJnlp(getFullURL(request), jarUrl, getSessionToken(request), entityId, isUpdate));
		} finally {
			if (out != null)
				out.close();
		}
	...
	private void setJarUrl() throws SynapseException, JSONObjectAdapterException {
		...
		jarUrl =
				"http://sagebionetworks.artifactoryonline.com/sagebionetworks/simple/libs-releases-local/org/sagebionetworks/fileUploadClient/"
						+ version.getVersion() + "/fileUploadClient-" + version.getVersion()
	...
	private static String buildJnlp(String jnlpServletUrl, String fileUploaderJarUrl,
			String sessionToken, String entityId, boolean isUpdate) {
		String jnlp = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" + "<jnlp spec=\"1.0+\" href=\""
				+ jnlpServletUrl
				+ "\" codebase=\"https://s3.amazonaws.com/versions.synapse.sagebase.org/\">\n"
