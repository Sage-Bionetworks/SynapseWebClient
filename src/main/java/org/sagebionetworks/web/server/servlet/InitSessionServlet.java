...
public class InitSessionServlet extends HttpServlet {
  ...
  private static Log log = LogFactory.getLog(InitSessionServlet.class);
  ...
  public static Cookie getNewAccessTokenCookie(
    String accessToken,
  ...
  public void doPost(
    ...
    try {
      ...
      Cookie cookie = getNewAccessTokenCookie(
        accessToken,
        request.getScheme(),
      ...
      response
        .getOutputStream()
        .write(("Invalid session token - " + e.getMessage()).getBytes("UTF-8"));
  ...
  private TokenProvider tokenProvider = new TokenProvider() {
    ...
    public String getToken() {
      return UserDataProvider.getThreadLocalUserToken(
        InitSessionServlet.perThreadRequest.get()
      );
    }
  };
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (token != null) {
      ...
      response.setHeader(
        WebConstants.CONTENT_TYPE_OPTIONS,
        WebConstants.NOSNIFF
      );
