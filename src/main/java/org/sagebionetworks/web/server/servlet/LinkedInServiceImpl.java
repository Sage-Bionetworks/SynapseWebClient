...
public class LinkedInServiceImpl
  ...
  @Override
  public UserProfile getCurrentUserInfo(
    String requestToken,
    String secret,
    String verifier,
    String callbackUrl
  ...
  private String saveImageToFileHandle(String imageUrl) {
    ...
    try {
      ...
      temp =
        ServiceUtils.writeToTempFile(
          conn.getInputStream(),
          MAX_ATTACHMENT_SIZE_IN_BYTES
        );
      // Now upload the file
      CloudProviderFileHandleInterface s3fh = client.multipartUpload(
        temp,
        null,
        false,
        false
  ...
  public static UserProfile parseLinkedInResponse(String response) {
    ...
    try {
      ...
      throw new RestClientException(
        "Unable to obtain LinkedIn profile information.",
        e
      );
    }
    return linkedInProfile;
  ...
  public static String parseLinkedInPictureResponse(String response) {
    ...
    try {
      ...
      throw new RestClientException(
        "Unable to obtain LinkedIn profile picture information.",
        e
      );
    }
  }
  ...
  private static String getLinkedInProfileElementValue(
    ...
    if (elements.getLength() > 0) {
      ...
      if (n.hasChildNodes()) val = n.getFirstChild().getNodeValue();
    }
    return val;
  }
