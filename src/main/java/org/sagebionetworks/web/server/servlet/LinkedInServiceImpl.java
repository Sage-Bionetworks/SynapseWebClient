...
public class LinkedInServiceImpl extends RemoteServiceServlet
	...
	public static final long MAX_ATTACHMENT_SIZE_IN_BYTES =
			MAX_ATTACHMENT_MEGABYTES * BYTES_PER_MEGABYTE; // 4 MB
	...
	@Override
	public UserProfile getCurrentUserInfo(String requestToken, String secret, String verifier,
			String callbackUrl) {
		validateService(callbackUrl);
		...
		request =
				new OAuthRequest(Verb.GET, "http://api.linkedin.com/v1/people/~/picture-urls::(original)");
		oAuthService.signRequest(accessToken, request);
	...
	public static UserProfile parseLinkedInResponse(String response) {
		...
		try {
			...
			try {
				location = ((Element) doc.getElementsByTagName("location").item(0))
						.getElementsByTagName("name").item(0).getFirstChild().getNodeValue();
				Element threeCurrentPositionsElement =
						(Element) doc.getElementsByTagName("positions").item(0);
				Element positionElement =
						(Element) threeCurrentPositionsElement.getElementsByTagName("position").item(0);
				position.append(
						positionElement.getElementsByTagName("title").item(0).getFirstChild().getNodeValue());
				Element companyElement = (Element) positionElement.getElementsByTagName("company").item(0);
				company.append(
						companyElement.getElementsByTagName("name").item(0).getFirstChild().getNodeValue());
			} catch (Throwable t) {
	...
	private static String getLinkedInProfileElementValue(Document linkedInProfile,
			String elementName) {
		String val = "";
