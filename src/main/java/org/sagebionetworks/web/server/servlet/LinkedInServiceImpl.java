...
public class LinkedInServiceImpl extends RemoteServiceServlet implements
	...
	private static Logger logger = Logger.getLogger(LinkedInServiceImpl.class
			.getName());

	public static final long BYTES_PER_MEGABYTE = 1048576;
	public static final long MAX_ATTACHMENT_MEGABYTES = 4;
	public static final long MAX_ATTACHMENT_SIZE_IN_BYTES = MAX_ATTACHMENT_MEGABYTES
			* BYTES_PER_MEGABYTE; // 4 MB
	...
	public LinkedInInfo returnAuthUrl(String callbackUrl) {
		...
		LinkedInInfo linkedInInfo = new LinkedInInfo(authUrl,
				requestToken.getSecret(), null);
		return linkedInInfo;
	}

	@Override
	public UserProfile getCurrentUserInfo(String requestToken, String secret,
			String verifier, String callbackUrl) {
		validateService(callbackUrl);
		...
		linkedInProfile
				.setProfilePicureFileHandleId(saveImageToFileHandle(picUrl));
	...
	/**
	 * Given the linkedin response, parse out profile information and return
	 * json representing a UserProfile object (with LinkedIn information filled
	 * in).
	 * 
	 * @param response
	 * @return
	...
	public static UserProfile parseLinkedInResponse(String response) {
		...
		try {
			...
			try {
				location = ((Element) doc.getElementsByTagName("location")
				...
				position.append(positionElement.getElementsByTagName("title")
						.item(0).getFirstChild().getNodeValue());
				Element companyElement = (Element) positionElement
						.getElementsByTagName("company").item(0);
				company.append(companyElement.getElementsByTagName("name")
						.item(0).getFirstChild().getNodeValue());
			} catch (Throwable t) {
			...
			throw new RestClientException(
					"Unable to obtain LinkedIn profile information.", e);
		}
		return linkedInProfile;
	}
	...
	public static String parseLinkedInPictureResponse(String response) {
		...
		try {
			...
			throw new RestClientException(
					"Unable to obtain LinkedIn profile picture information.", e);
		}
	}

	private static String getLinkedInProfileElementValue(
			Document linkedInProfile, String elementName) {
		String val = "";
