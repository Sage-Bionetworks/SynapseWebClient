...
public class SynapseRetryProxy {
	...
	/**
	 * Handler that will retry the call if it encounters an error corresponding to the TOO MANY
					...
					public Object call() throws Exception {
						try {
							...
							if (cause instanceof SynapseTooManyRequestsException) {
								// if 429, we can retry. send full exception trace to logger.debug to inform
								// developer that we have a problem.
								Calendar now = Calendar.getInstance();
								log.debug(
										"THROTTLED! Attempted to call " + method.getName() + " at " + now.getTime(),
										cause);
