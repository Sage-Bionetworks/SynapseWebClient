...
public class CrawlFilter implements Filter {
				...
				if (fixedQueryString.contains("#!Home")) {
					...
					if (fixedQueryString.contains(DISCUSSION_THREAD_ID)) {
						String threadId = fixedQueryString.substring(
								fixedQueryString.indexOf(DISCUSSION_THREAD_ID) + DISCUSSION_THREAD_ID.length());
						html = getThreadHtml(threadId);
					} else {
						// index information about the synapse entity
						String entityId = fixedQueryString
								.substring(fixedQueryString.indexOf(":", fixedQueryString.indexOf("#!")) + 1);
						html = getEntityHtml(entityId);
					}
				} else if (fixedQueryString.contains("#!Search")) {
					// index all projects
					String searchQueryJson = fixedQueryString
							.substring(fixedQueryString.indexOf(":", fixedQueryString.indexOf("#!")) + 1);
					...
					String startIndex = fixedQueryString.substring(
							fixedQueryString.indexOf(TeamSearch.START_DELIMITER, fixedQueryString.indexOf("#!"))
					...
					String teamId = fixedQueryString
							.substring(fixedQueryString.indexOf(":", fixedQueryString.indexOf("#!")) + 1);
					html = getTeamHtml(teamId);
				} else if (fixedQueryString.contains("#!Profile")) {
					// index team (including members)
					String profileId = fixedQueryString
							.substring(fixedQueryString.indexOf(":", fixedQueryString.indexOf("#!")) + 1);
					html = getProfileHtml(profileId);
				}

				URL url = new URL(scheme, domain, port, fixedQueryString);
				// replace all relative links with full links due to this Google AJAX crawler support
				// chicken-dance
	...
	private String getHomePageHtml() throws JSONObjectAdapterException, RestServiceException {
		...
		html.append("<!DOCTYPE html><html><head><title>" + DisplayConstants.DEFAULT_PAGE_TITLE
				+ "</title><meta name=\"description\" content=\""
				+ DisplayConstants.DEFAULT_PAGE_DESCRIPTION + "\" /></head><body>");
		// add direct links to all public projects in the system
		SearchQuery query = SearchQueryUtils.getDefaultSearchQuery();
		html.append("<h1>" + DisplayConstants.DEFAULT_PAGE_TITLE + "</h1>"
				+ DisplayConstants.DEFAULT_PAGE_DESCRIPTION + "<br />");
		// add link to team search
		html.append("<h3><a href=\"https://www.synapse.org/#!TeamSearch:" + TeamSearch.START_DELIMITER
				+ "0\">Teams</a></h3><br />");
		...
		while (results.getHits().size() > 0) {
			for (Hit hit : results.getHits()) {
				// add links
				html.append("<a href=\"https://www.synapse.org/#!Synapse:" + hit.getId() + "\">"
						+ hit.getName() + "</a><br />");
			}
	...
	private String getUserProfileString(UserProfile profile) {
		...
		createdByBuilder
				.append("<a href=\"https://www.synapse.org/#!Profile:" + profile.getOwnerId() + "\">");
	...
	private String getEntityHtml(String entityId)
		...
		try {
			WikiPage rootPage = synapseClient
					.getV2WikiPageAsV1(new WikiPageKey(entity.getId(), ObjectType.ENTITY.toString(), null));
		...
		html.append("<!DOCTYPE html><html><head><title>" + name + " - " + entity.getId()
				+ "</title></head><body>");
		...
		if (entity instanceof Project) {
			...
			if (forum != null) {
				...
				do {
					paginatedThreads = discussionForumClient.getThreadsForForum(forumId, 20L, currentOffset,
							DiscussionThreadOrder.PINNED_AND_LAST_ACTIVITY, false,
					...
					for (DiscussionThreadBundle thread : threadList) {
						html.append("<a href=\"https://www.synapse.org/#!Synapse:" + entity.getId()
								+ DISCUSSION_THREAD_ID + thread.getId() + "\">" + thread.getTitle() + "</a><br />");
					}
					currentOffset += 20;
		...
		do {
			...
			for (EntityHeader childId : childList.getPage()) {
				html.append("<a href=\"https://www.synapse.org/#!Synapse:" + childId.getId() + "\">"
						+ childId.getId() + "</a><br />");
			}
	...
	private String getThreadHtml(String threadId)
		...
		PaginatedResults<DiscussionReplyBundle> replies =
				discussionForumClient.getRepliesForThread(thread.getId(), 100L, 0L,
						DiscussionReplyOrder.CREATED_ON, false, DiscussionFilter.EXCLUDE_DELETED);
	...
	private String getTeamHtml(String teamId)
		...
		TeamMemberPagedResults teamMembers =
				synapseClient.getTeamMembers(team.getId(), "", TeamMemberTypeFilterOptions.ALL, 20, 0);
	...
	private String getAllProjectsHtml(String searchQueryJson)
		...
		SearchQuery inputQuery =
				EntityFactory.createEntityFromJSONString(searchQueryJson, SearchQuery.class);
		SearchResults results = synapseClient.search(inputQuery);

		// append this set to the list
		StringBuilder html = new StringBuilder();
		html.append("<!DOCTYPE html><html><head><title>Sage Synapse: All Projects - starting from "
				+ inputQuery.getStart()
		...
		html.append("<a href=\"https://www.synapse.org/#!Search:" + URLEncoder.encode(newJson)
				+ "\">Next Page</a><br />");
	...
	private String getAllTeamsHtml(String startIndex) throws RestServiceException {
		...
		// append this set to the list
		StringBuilder html = new StringBuilder();
		html.append("<!DOCTYPE html><html><head><title>Sage Synapse: All Teams - from " + startIndex
				+ "</title><meta name=\"description\" content=\"\" /></head><body>");
		for (Team team : teams.getResults()) {
			// add links
			html.append("<a href=\"https://www.synapse.org/#!Team:" + team.getId() + "\">"
					+ team.getName() + "</a><br />");
		}
		// add another link for the next page of results
		long newStart = start + teams.getResults().size();
		html.append("<h4><a href=\"https://www.synapse.org/#!TeamSearch:" + TeamSearch.START_DELIMITER
				+ newStart + "\">Next Page</a></h4><br />");
