...
public abstract class RedirectFilter implements Filter {
	...
	@Override
	public void doFilter(ServletRequest rqst, ServletResponse rspn, FilterChain chain)
			throws IOException, ServletException {

		HttpServletRequest httpRqst = (HttpServletRequest) rqst;
		URL requestURL = new URL(httpRqst.getRequestURL().toString());
		URL redirectURL = new URL(requestURL.getProtocol(), requestURL.getHost(), requestURL.getPort(),
				requestURL.getPath().substring(0, requestURL.getPath().length() - getUrlPath().length())
						+ getTargetPage());
