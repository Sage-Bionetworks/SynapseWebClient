...
public abstract class OAuth2Servlet extends HttpServlet {
	...
	public String createRedirectUrl(HttpServletRequest request, OAuthProvider provider) {
		return request.getRequestURL().toString() + "?" + WebConstants.OAUTH2_PROVIDER + "="
				+ provider.name();
	}
	 ...
	 * @throws IOException
	 */
	public void redirectToProvider(HttpServletRequest req, HttpServletResponse resp,
			OAuthProvider provider, String redirectUrl, String state) throws IOException {
		...
		try {
			...
			if (e instanceof SynapseServiceUnavailable) {
				resp.sendRedirect(new URL(requestURL.getProtocol(), requestURL.getHost(),
						requestURL.getPort(), "/#!Down:0").toString());
			} else {
	...
	private void sendRedirectToError(HttpServletRequest request, Exception e,
			HttpServletResponse resp) throws MalformedURLException, IOException {
		resp.sendRedirect(FileHandleAssociationServlet.getBaseUrl(request)
