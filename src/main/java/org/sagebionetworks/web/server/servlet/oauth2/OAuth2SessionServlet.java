...
public class OAuth2SessionServlet extends OAuth2Servlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (athenticationCode == null) {
			...
			createAccountViaOauth(req, resp, URLDecoder.decode(state), provider, athenticationCode,
					redirectUrl);
		} else {
	...
	public void validateUser(HttpServletRequest req, HttpServletResponse resp, OAuthProvider provider,
		...
		try {
			...
			resp.sendRedirect(FileHandleAssociationServlet.getBaseUrl(req)
					+ FileHandleAssociationServlet.ERROR_PLACE + URLEncoder.encode(e.getMessage()));
		}
	}

	/**
	 * Step 2b, use the resulting authentication code and username to create a new Synapse user
	 * account.
	 * 
	 * @param resp
	 * @param provider
	 ...
	 * @throws IOException
	 */
	public void createAccountViaOauth(HttpServletRequest req, HttpServletResponse resp,
			String username, OAuthProvider provider, String athenticationCode, String redirectUrl)
			throws IOException {
		try {
			// use new service to validate code and create a new account using the given username (and
			// info from Google)
			SynapseClient client = createSynapseClient();
