...
public class SynapseJavascriptClientTest {
	...
	public void before() throws RequestException {
		...
		when(mockSynapseProperties.getSynapseProperty(AUTH_PUBLIC_SERVICE_URL_KEY))
				.thenReturn(AUTH_ENDPOINT);
		...
		client = new SynapseJavascriptClient(jsonObjectAdapter, mockSynapseProperties, mockGwt,
				synapseJsFactory, mockGinInjector, mockJsniUtils, entityId2BundleCache);
	}
	...
	public void testGetException() {
		...
		assertTrue(
				getException(SC_UNAUTHORIZED, reason, errorResponseCode) instanceof UnauthorizedException);
		...
		assertTrue(
				getException(SC_BAD_REQUEST, reason, errorResponseCode) instanceof BadRequestException);
		assertTrue(getException(SC_LOCKED, reason, errorResponseCode) instanceof LockedException);
		assertTrue(getException(SC_PRECONDITION_FAILED, reason,
				errorResponseCode) instanceof ConflictingUpdateException);
		assertTrue(
				getException(SC_GONE, reason, errorResponseCode) instanceof DeprecatedServiceException);
		assertTrue(getException(SynapseTooManyRequestsException.TOO_MANY_REQUESTS_STATUS_CODE, reason,
				errorResponseCode) instanceof TooManyRequestsException);
		assertTrue(getException(SC_SERVICE_UNAVAILABLE, reason,
				errorResponseCode) instanceof SynapseDownException);
		...
		assertEquals(ErrorResponseCode.PASSWORD_RESET_VIA_EMAIL_REQUIRED,
				getException(SC_FORBIDDEN, reason, errorResponseCode).getErrorResponseCode());
	}

	@Test
	public void testGetEntityBundleAnonymousSuccess()
	...
	@Test
	public void testGetEntityBundleFromCacheHitOldVersion()
			throws RequestException, JSONObjectAdapterException {
		String entityId = "syn291";
		EntityBundle testBundle = new EntityBundle();
	...
	@Test
	public void testGetEntityBundleFromCacheMissFailureToGet()
			throws RequestException, JSONObjectAdapterException {
		String entityId = "syn291";
	...
	@Test
	public void testGetEntityBundleForVersionLoggedInFailure()
			throws RequestException, JSONObjectAdapterException {
		String entityId = "syn291";
		EntityBundleRequest request = new EntityBundleRequest();
	...
	public void testGetReasonFromFailure() throws RequestException, JSONObjectAdapterException {
		...
		String reason =
				"The results of this query exceeded the maximum number of allowable bytes: 512000.";
	...
	public void testPostRestrictionInformation() throws RequestException, JSONObjectAdapterException {
		...
		RestrictionInformationRequest request =
				new RestrictionInformationRequest(jsonObjectAdapter.createNew(json));
		assertEquals(subjectId, request.getObjectId());
		assertEquals(type, request.getRestrictableObjectType());
	}

	@Test
	public void testPostEntityChildrenLoggedInWithRetry()
			throws RequestException, JSONObjectAdapterException {
		...
		// simulate retry
		callbackCaptor.getValue().invoke();
		verify(mockRequestBuilder, times(2)).sendRequest(stringCaptor.capture(),
		...
		verify(mockGwt).scheduleExecution(callbackCaptor.capture(),
				eq(INITIAL_RETRY_REQUEST_DELAY_MS * 2));
	}
	...
	public void testGetVersionOfV2WikiPageAsV1() throws RequestException, JSONObjectAdapterException {
		...
		// verify url and method
		String url = REPO_ENDPOINT + "/" + ownerObjectType + "/" + ownerObjectId + WIKI + pageId
				+ WIKI_VERSION_PARAMETER + versionNumber;
		verify(mockRequestBuilder).configure(GET, url);
	}

	@Test
	public void testGetUserGroupHeadersByPrefix()
			throws RequestException, JSONObjectAdapterException {
		String prefix = "hello";
		...
		// verify url and method
		String url = REPO_ENDPOINT + USER_GROUP_HEADER_PREFIX_PATH + prefix + "&" + LIMIT_PARAMETER
				+ limit + "&" + OFFSET_PARAMETER + offset + TYPE_FILTER_PARAMETER + typeFilter.name();
		verify(mockRequestBuilder).configure(GET, url);
	}
	...
	public void testGetFavorites() throws RequestException, JSONObjectAdapterException {
		...
		// verify url and method
		String url =
				REPO_ENDPOINT + FAVORITE_URI_PATH + "?" + OFFSET_PARAMETER + "0&" + LIMIT_PARAMETER + "200";
		verify(mockRequestBuilder).configure(GET, url);
	}
	...
	@Test
	public void testGetOpenMembershipInvitationCount()
			throws RequestException, JSONObjectAdapterException {
	...
	@Test
	public void testGetOpenMembershipRequestCount()
			throws RequestException, JSONObjectAdapterException {
	...
	public void testGetForEntity() throws RequestException, JSONObjectAdapterException {
		...
		// verify url and method
		String url =
				REPO_ENDPOINT + ENTITY + "/" + entityId + REPO_SUFFIX_VERSION + "/" + versionNumber;
		verify(mockRequestBuilder).configure(GET, url);
	}

	@Test
	public void testGetNewEntityInstance()
			throws RequestException, JSONObjectAdapterException, ResultNotReadyException {
		JSONObjectAdapter adapter = jsonObjectAdapter.createNew();
		...
		assertTrue(
				synapseJsFactory.newInstance(OBJECT_TYPE.Entity, adapter) instanceof DockerRepository);

	}

	@Test(expected = IllegalArgumentException.class)
	public void testGetInvalidEntityInstance()
			throws RequestException, JSONObjectAdapterException, ResultNotReadyException {
	...
	public void testUpdateV2WikiPage() throws RequestException, JSONObjectAdapterException {
		...
		// verify url and method
		String url = REPO_ENDPOINT + "/" + pageKey.getOwnerObjectType().toLowerCase() + "/"
				+ ownerObjectId + WIKI2 + wikiPageId;
	...
	@Test
	public void testAsyncGetTableQueryJobResults()
			throws RequestException, JSONObjectAdapterException {
		...
		// response status code is OK, but if we request a query result but it responds with a job
		// status, then a ResultNotReadyException should be thrown
	...
	public void testGetTableQueryJobResultsReady()
			throws RequestException, JSONObjectAdapterException, ResultNotReadyException {
		...
		QueryResultBundle newResultBundleInstance =
				(QueryResultBundle) synapseJsFactory.newInstance(OBJECT_TYPE.AsyncResponse, adapter);
	...
	public void testGetV2WikiHeaderTree() throws Exception {
		...
		// verify url and method
		String url = REPO_ENDPOINT + ENTITY + "/" + ownerId + WIKI_HEADER_TREE + "?" + LIMIT_PARAMETER
				+ LIMIT_50 + "&" + OFFSET_PARAMETER + "0";
		verify(mockRequestBuilder).configure(GET, url);
	}
	...
	public void testRefreshSessionToken() throws RequestException, JSONObjectAdapterException {
		...
		verify(mockRequestBuilder, times(2)).setHeader(WebConstants.CONTENT_TYPE,
	...
	@Test
	public void testRefreshSessionTokenAnonymous()
			throws RequestException, JSONObjectAdapterException {
	...
	public void testGetTeamMembersFirstStep() throws RequestException, JSONObjectAdapterException {
		...
		// verify url and method
		String url = REPO_ENDPOINT + TEAM_MEMBERS + teamId + "?" + OFFSET_PARAMETER + offset + "&"
				+ LIMIT_PARAMETER + limit + "&" + NAME_FRAGMENT_FILTER + fragment + "&"
				+ NAME_MEMBERTYPE_FILTER + memberType.toString();
		verify(mockRequestBuilder).configure(GET, url);
	}
