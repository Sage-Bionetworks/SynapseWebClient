...
public class AuthenticationControllerImplTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).initSession(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(SESSION_TOKEN).when(mockUserAccountService)
				.getCurrentSessionToken(any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(usd).when(mockUserAccountService)
				.getCurrentUserSessionData(any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockNotificationEmail).when(mockJsClient)
				.getNotificationEmail(any(AsyncCallback.class));
		when(mockGinInjector.getSynapseJavascriptClient()).thenReturn(mockJsClient);
		authenticationController = new AuthenticationControllerImpl(mockUserAccountService,
				mockClientCache, mockCookieProvider, mockGinInjector, mockSynapseJSNIUtils);
	...
	public void testLogout() {
		...
		verify(mockClientCache).put(eq(USER_AUTHENTICATION_RECEIPT),
				eq(USER_AUTHENTICATION_RECEIPT_VALUE), anyLong());
	...
	public void testLoginUser() {
		...
		AsyncMockStubber.callSuccessWith(loginResponse).when(mockJsClient)
	...
	public void testLoginUserNotAcceptedTermsOfUse() {
		...
		assertEquals(usd.getSession().getSessionToken(),
	...
	public void testLoginUserFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).login(any(LoginRequest.class),
				any(AsyncCallback.class));
		String username = "testusername";
		String password = "pw";
		AsyncCallback loginCallback = mock(AsyncCallback.class);
