...
public class EntityRefreshAlertTest {
	...
	public void before() {
		...
		entityRefreshAlert = new EntityRefreshAlert(mockView, mockJsClient, mockGWTWrapper,
				mockGlobalApplicationState, mockSynapseJSNIUtils);
		when(mockView.isAttached()).thenAnswer((InvocationOnMock invocation) -> true);
		AsyncMockStubber.callSuccessWith(entity).when(mockJsClient).getEntity(anyString(),
	...
	public void testCheckEtag_EtagMatch() {
		...
		verify(mockGWTWrapper).scheduleExecution(entityRefreshAlert.invokeCheckEtag,
