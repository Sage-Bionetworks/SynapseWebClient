...
public class DateTimeUtilsImplTest {
  ...
  @Before
  public void setup() {
    when(mockGWT.getFormat(DATE_ONLY_FORMAT_STRING))
      .thenReturn(mockDateOnlyFormat);
    when(mockGWT.getFormat(DATE_ONLY_FORMAT_STRING + UTC))
      .thenReturn(mockDateOnlyFormatUTC);
    when(mockGWT.getFormat(SMALL_DATE_FORMAT_STRING))
      .thenReturn(mockSmallDateFormat);
    when(mockGWT.getFormat(SMALL_DATE_FORMAT_STRING + UTC))
      .thenReturn(mockSmallDateFormatUTC);
    when(mockGWT.getFormat(LONG_DATE_FORMAT_STRING))
      .thenReturn(mockLongDateFormat);
    when(mockGWT.getFormat(LONG_DATE_FORMAT_STRING + UTC))
      .thenReturn(mockLongDateFormatUTC);
    when(mockGWT.getFormat(PredefinedFormat.ISO_8601))
      .thenReturn(mockISO8601Format);
  ...
  public void testFriendlyTimeEstimate() {
    ...
    assertEquals(
      "1 h 1 min",
      dateTimeUtils.getFriendlyTimeEstimate(60 * 60 + 60)
    );
    // ignore seconds if greater than an hour
    assertEquals(
      "1 h 1 min",
      dateTimeUtils.getFriendlyTimeEstimate(60 * 60 + 60 + 9)
    );
  }
