...
public class DisplayUtilsTest {
  ...
  private String textWithoutMarkdown =
    "This is the test markdown\nthat will be used.";
  private String textWithMarkdown =
    "This is the **test** markdown\nthat will be used.";
  ...
  public void testWidgetNotSelectedState() {
    ...
    String fullSynapseWidgetText =
      WidgetConstants.WIDGET_START_MARKDOWN +
      synapseWidgetInnerText +
    ...
    DisplayUtils.updateWidgetSelectionState(
      state,
      fullSynapseWidgetText + markdownText,
    ...
    DisplayUtils.updateWidgetSelectionState(
      state,
      markdownText + fullSynapseWidgetText,
      5
    );
    assertFalse(state.isWidgetSelected());
  }
  ...
  public void testWidgetSelectedState() {
    ...
    DisplayUtils.updateWidgetSelectionState(
      state,
      testMarkdown,
      testMarkdown.length() - 1
    );
    assertTrue(state.isWidgetSelected());
    assertEquals(synapseWidgetInnerText, state.getInnerWidgetText());
    assertEquals(
      testMarkdown.length() - fullSynapseWidgetText.length(),
    ...
    testMarkdown =
      markdownText.substring(0, insertPoint) +
      fullSynapseWidgetText +
    ...
    assertEquals(
      insertPoint + fullSynapseWidgetText.length(),
      state.getWidgetEndIndex()
    );
  }
  ...
  public void testGetDisplayName() {
    ...
    assertEquals(
      "Strong Bad",
      DisplayUtils.getDisplayName(fName, lName, tempUserName)
    );
    ...
    assertEquals(
      "Strong Bad (SBEmail)",
      DisplayUtils.getDisplayName(fName, lName, userName)
    );
  }
  ...
  public void testSurroundText() {
    ...
    String result = DisplayUtils.surroundText(
      textWithoutMarkdown,
      markdownDelimiter,
      markdownDelimiter,
      false,
  ...
  public void testSurroundTextMarkdownEndOfText() {
    ...
    assertEquals(
      textWithMarkdown + markdownDelimiter2 + markdownDelimiter2,
      result
    );
  }
  ...
  public void testParseEntityVersionString() {
    ...
    testRef =
      DisplayUtils.parseEntityVersionString(validSynId + "." + validVersion);
    assertEquals(expectedRef, testRef);
    // verify ref with version defined using "/version/" notation
    testRef =
      DisplayUtils.parseEntityVersionString(
        validSynId + WebConstants.ENTITY_VERSION_STRING + validVersion
      );
    assertEquals(expectedRef, testRef);
  }
  ...
  @Test
  public void testReplaceWithNullIfEmptyString() {
    assertEquals(null, replaceWithNullIfEmptyTrimmedString(""));
    assertEquals(null, replaceWithNullIfEmptyTrimmedString(null));
    assertEquals(null, replaceWithNullIfEmptyTrimmedString(" \t"));
    assertEquals("test", replaceWithNullIfEmptyTrimmedString("  test     \t"));
  }
}
