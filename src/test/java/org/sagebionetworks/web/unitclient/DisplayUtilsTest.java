...
public class DisplayUtilsTest {
	...
	public void testWidgetNotSelectedState() {
		...
		String fullSynapseWidgetText = WidgetConstants.WIDGET_START_MARKDOWN + synapseWidgetInnerText
				+ WidgetConstants.WIDGET_END_MARKDOWN;

		// verify that when selecting outside of the widget text, it will report that widget is not
		// selected
		DisplayUtils.updateWidgetSelectionState(state, fullSynapseWidgetText + markdownText,
				fullSynapseWidgetText.length() + 1);
		assertFalse(state.isWidgetSelected());
	...
	public void testWidgetSelectedState() {
		...
		assertTrue(state.isWidgetSelected());
		assertEquals(synapseWidgetInnerText, state.getInnerWidgetText());
		assertEquals(testMarkdown.length() - fullSynapseWidgetText.length(),
				state.getWidgetStartIndex());
		...
		fullSynapseWidgetText = WidgetConstants.WIDGET_START_MARKDOWN + synapseWidgetInnerText
				+ WidgetConstants.WIDGET_END_MARKDOWN;
		testMarkdown = markdownText + fullSynapseWidgetText;
		DisplayUtils.updateWidgetSelectionState(state, testMarkdown,
				testMarkdown.length() - fullSynapseWidgetText.length() + 4);
		assertTrue(state.isWidgetSelected());
		assertEquals(synapseWidgetInnerText, state.getInnerWidgetText());
		assertEquals(testMarkdown.length() - fullSynapseWidgetText.length(),
				state.getWidgetStartIndex());
		...
		testMarkdown = markdownText.substring(0, insertPoint) + fullSynapseWidgetText
				+ markdownText.substring(insertPoint);
	...
	public void testInvalidWidget() {
		String markdownText = WidgetConstants.WIDGET_START_MARKDOWN
				+ "onlyinvalid?because=it&does=not&end so \nwill the entire thing be overwritten?";
		WidgetSelectionState state = new WidgetSelectionState();
		// verify that when selecting outside of the widget text, it will report that widget is not
		// selected
	...
	public void testSurroundText() {
		...
		String result = DisplayUtils.surroundText(textWithoutMarkdown, markdownDelimiter,
				markdownDelimiter, false, startPos, selectionLength);
	...
	public void testSurroundTextNoSelection() {
		...
		String result = DisplayUtils.surroundText(textWithoutMarkdown, markdownDelimiter,
				markdownDelimiter, true, startPos, selectionLength);
		assertEquals("This is the **test markdown\nthat will be used.", result);
	}
	...
	public void testSurroundTextPastNewline() {
		...
		DisplayUtils.surroundText(textWithoutMarkdown, markdownDelimiter, markdownDelimiter, false,
	...
	public void testSurroundTextPastNewlineSupportMultiline() {
		int startPos = textWithoutMarkdown.indexOf("test");
		int selectionLength = "test markdown\nthat".length();
		DisplayUtils.surroundText(textWithoutMarkdown, markdownDelimiter, markdownDelimiter, true,
				startPos, selectionLength);
	}
	...
	public void testSurroundTextStripMarkdownTest1() {
		...
		int selectionLength = "test".length();
		String result = DisplayUtils.surroundText(textWithMarkdown, markdownDelimiter2,
				markdownDelimiter2, false, startPos, selectionLength);
	...
	public void testHeading() {
		...
		String result =
				DisplayUtils.surroundText(textWithoutMarkdown, "\n#", "", false, startPos, selectionLength);
	...
	public void testCodeBlock() {
		...
		String result = DisplayUtils.surroundText(text, markdownDelimiter, markdownDelimiter, true,
				startPos, selectionLength);
	...
	public void testSurroundTextMarkdownEndOfText() {
		...
		int selectionLength = 0;
		String result = DisplayUtils.surroundText(textWithMarkdown, markdownDelimiter2,
				markdownDelimiter2, true, startPos, selectionLength);
	...
	public void testSurroundTextNoText() {
		int startPos = 0;
		int selectionLength = 0;
		String result = DisplayUtils.surroundText("", markdownDelimiter, markdownDelimiter, false,
				startPos, selectionLength);
	...
	public void testParseEntityVersionString() {
		...
		testRef = DisplayUtils
				.parseEntityVersionString(validSynId + WebConstants.ENTITY_VERSION_STRING + validVersion);
		assertEquals(expectedRef, testRef);
	}
