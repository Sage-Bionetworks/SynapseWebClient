...
public class GlobalApplicationStateImplTest {
  ...
  public void testGoToNewPlaceError() {
    ...
    doThrow(new RuntimeException(errorMessage))
      .when(mockPlaceController)
      .goTo(any(Place.class));
    changer.goTo(newPlace);
  ...
  public void testIgnoredJSExceptions() {
    ...
    assertFalse(
      isIgnoredErrorMessage("Blocked an iframe from loading on this page.")
    );
  }
  ...
  public void testUnwrap() {
    ...
    UmbrellaException umbrellaUmbrellaException = new UmbrellaException(
      umbrellaUmbrellaCauses
    );

    // single exception being wrapped twice with umbrella exceptions, unwrap it!
    assertEquals(
      actualException,
      globalApplicationState.unwrap(umbrellaUmbrellaException)
    );
    // pass through for non-umbrella exceptions
    assertEquals(
      actualException,
      globalApplicationState.unwrap(actualException)
    );
    ...
    assertEquals(
      actualException,
      globalApplicationState.unwrap(umbrellaException)
    );
  }
  ...
  public void testCheckVersionCompatibility() {
    ...
    ArgumentCaptor<VersionState> captor = ArgumentCaptor.forClass(
      VersionState.class
    );
  ...
  public void testCheckVersionCompatibilityCheckedRecently() {
    when(
      mockLocalStorage.get(
        GlobalApplicationStateImpl.RECENTLY_CHECKED_SYNAPSE_VERSION
      )
    )
  ...
  public void testClearLastPlace() {
    ...
    verify(mockSessionStorage)
      .removeItem(GlobalApplicationStateImpl.LAST_PLACE);
  }
  ...
  public void testReplaceCurrentPlaceFailure() {
    ...
    doThrow(new RuntimeException(error))
      .when(mockGWT)
      .replaceItem(anyString(), anyBoolean());
    String newToken = "/some/new/token";
    Place mockPlace = mock(Place.class);
    when(mockAppPlaceHistoryMapper.getToken(mockPlace)).thenReturn(newToken);
    globalApplicationState.replaceCurrentPlace(mockPlace);
  ...
  public void testRefreshPage() {
    ...
    assertEquals(
      GlobalApplicationStateImpl.DEFAULT_REFRESH_PLACE,
      captor.getValue()
    );
  }
  ...
  public void testSetShowLocalTime() {
    ...
    verify(mockCookieProvider)
      .setCookie(
        eq(CookieKeys.SHOW_DATETIME_IN_UTC),
        eq(Boolean.FALSE.toString()),
        any(Date.class)
      );
  }
