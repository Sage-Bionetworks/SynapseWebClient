...
public class GlobalApplicationStateImplTest {
	...
	public void before() {
		...
		AsyncMockStubber.callSuccessWith("v1").when(mockStackConfigService)
				.getSynapseVersions(any(AsyncCallback.class));

		globalApplicationState = new GlobalApplicationStateImpl(mockView, mockCookieProvider,
				mockEventBus, mockStackConfigService, mockSynapseJSNIUtils, mockLocalStorage, mockGWT,
				mockDateTimeUtils, mockJsClient, mockSynapseProperties, mockSessionStorage);
	...
	public void testIgnoredJSExceptions() {
		...
		assertTrue(isIgnoredErrorMessage(
	...
	private void verifyAndInvokeSynapsePropertiesInitCallback() {
		verify(mockSynapseProperties)
				.initSynapseProperties(synapsePropertiesInitCallbackCaptor.capture());
	...
	public void testCheckVersionCompatibility() {
		...
		AsyncMockStubber.callSuccessWith("v2").when(mockStackConfigService)
				.getSynapseVersions(any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith("v3").when(mockStackConfigService)
				.getSynapseVersions(any(AsyncCallback.class));
		globalApplicationState.checkVersionCompatibility(callback);
		// showVersionOutOfDateGlobalMessage() has still only been called once, despite detecting
	...
	public void testCheckVersionCompatibilityFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockStackConfigService)
				.getSynapseVersions(any(AsyncCallback.class));
	...
	public void testCheckVersionCompatibilityCheckedRecently() {
		when(mockLocalStorage.get(GlobalApplicationStateImpl.RECENTLY_CHECKED_SYNAPSE_VERSION))
	...
	public void testGetLastPlaceWhenSet() {
		...
		when(mockSessionStorage.getItem(GlobalApplicationStateImpl.LAST_PLACE))
				.thenReturn("a history value");
	...
	public void testSetShowLocalTime() {
		...
		verify(mockCookieProvider).setCookie(eq(CookieKeys.SHOW_DATETIME_IN_UTC),
				eq(Boolean.FALSE.toString()), any(Date.class));
	}
	...
	public void testSetShowUTCTime() {
		...
		verify(mockCookieProvider).setCookie(eq(CookieKeys.SHOW_DATETIME_IN_UTC),
				eq(Boolean.TRUE.toString()), any(Date.class));
	}
}
