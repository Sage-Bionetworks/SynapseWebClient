...
public class AppActivityMapperTest {
  ...
  @Before
  public void before() {
    when(mockInjector.getBulkPresenterProxy())
      .thenReturn(mockBulkPresenterProxy);
    when(mockAuthenticationController.isLoggedIn()).thenReturn(true);
    when(mockSynapseJSNIUtils.getCurrentHistoryToken())
      .thenReturn(historyToken);
    when(mockInjector.getCookieProvider()).thenReturn(mockCookies);
    when(mockInjector.getGlobalApplicationState())
      .thenReturn(mockGlobalApplicationState);
    when(mockInjector.getAuthenticationController())
      .thenReturn(mockAuthenticationController);
    ...
    appActivityMapper =
      new AppActivityMapper(mockInjector, mockSynapseJSNIUtils, null);
  }
