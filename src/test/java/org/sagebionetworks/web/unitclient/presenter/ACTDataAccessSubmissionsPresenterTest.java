...
public class ACTDataAccessSubmissionsPresenterTest {
	...
	public void setup() {
		...
		presenter = new ACTDataAccessSubmissionsPresenter(mockView, mockSynAlert, mockGinInjector,
				mockLoadMoreContainer, mockACTAccessRequirementWidget, mockButton,
				mockDucTemplateFileHandleWidget, mockDataAccessClient, mockSubjectsWidget, mockGWT);
		AsyncMockStubber.callSuccessWith(mockACTAccessRequirement).when(mockDataAccessClient)
				.getAccessRequirement(anyLong(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockDataAccessSubmissionPage).when(mockDataAccessClient)
				.getDataAccessSubmissions(anyLong(), anyString(), any(SubmissionState.class),
						any(SubmissionOrder.class), anyBoolean(), any(AsyncCallback.class));
	...
	public void testLoadData() {
		...
		// verify duc template file handle widget is configured properly (basd on act duc file handle
		// id)
		...
		verify(mockACTAccessRequirementWidget).setRequirement(eq(mockACTAccessRequirement),
				any(Callback.class));
		verify(mockLoadMoreContainer).setIsProcessing(true);
		verify(mockDataAccessClient).getDataAccessSubmissions(anyLong(), eq((String) null),
				any(SubmissionState.class), any(SubmissionOrder.class), anyBoolean(),
		...
		presenter.loadMore();
		verify(mockDataAccessClient).getDataAccessSubmissions(anyLong(), eq(NEXT_PAGE_TOKEN),
				any(SubmissionState.class), any(SubmissionOrder.class), anyBoolean(),
				any(AsyncCallback.class));
	...
	public void testLoadDataFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockDataAccessClient).getDataAccessSubmissions(
				anyLong(), anyString(), any(SubmissionState.class), any(SubmissionOrder.class),
