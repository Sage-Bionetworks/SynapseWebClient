...
public class AccessRequirementsPresenterTest {
	...
	public void setup() {
		...
		presenter = new AccessRequirementsPresenter(mockView, mockDataAccessClient, mockSynAlert,
				mockGinInjector, mockEntityIdCellRenderer, mockTeamBadge, mockCreateARButton,
				mockEmptyResultsDiv, mockUnmetAccessRequirementsDiv, mockMetAccessRequirementsDiv,
				mockAuthController);
		...
		accessRequirementApprovalStatus.add(status);
		AsyncMockStubber.callSuccessWith(accessRequirements).when(mockDataAccessClient)
				.getAccessRequirements(any(RestrictableObjectDescriptor.class), anyLong(), anyLong(),
						any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockBatchAccessApprovalInfoResponse).when(mockDataAccessClient)
				.getAccessRequirementStatus(any(BatchAccessApprovalInfoRequest.class),
						any(AsyncCallback.class));
	...
	public void testLoadDataEntity() {
		...
		verify(mockDataAccessClient).getAccessRequirements(subjectCaptor.capture(),
				eq(AccessRequirementsPresenter.LIMIT), eq(0L), any(AsyncCallback.class));
		verify(mockDataAccessClient).getAccessRequirementStatus(
				any(BatchAccessApprovalInfoRequest.class), any(AsyncCallback.class));
		...
		verify(mockAccessRequirementWidget, times(4)).configure(any(AccessRequirement.class),
				any(RestrictableObjectDescriptor.class), any(Callback.class));
		...
		// load the next page
		verify(mockDataAccessClient).getAccessRequirements(any(RestrictableObjectDescriptor.class),
				eq(AccessRequirementsPresenter.LIMIT), eq(AccessRequirementsPresenter.LIMIT),
				any(AsyncCallback.class));
	}
	...
	public void testLoadDataEntityFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockDataAccessClient).getAccessRequirements(
				any(RestrictableObjectDescriptor.class), anyLong(), anyLong(), any(AsyncCallback.class));
		when(mockPlace.getParam(AccessRequirementsPlace.ID_PARAM)).thenReturn(ENTITY_ID);
		when(mockPlace.getParam(AccessRequirementsPlace.TYPE_PARAM))
				.thenReturn(RestrictableObjectType.ENTITY.toString());
	...
	public void testAnonymous() {
		...
		verify(mockDataAccessClient).getAccessRequirements(subjectCaptor.capture(),
				eq(AccessRequirementsPresenter.LIMIT), eq(0L), any(AsyncCallback.class));
		verify(mockDataAccessClient, never()).getAccessRequirementStatus(
				any(BatchAccessApprovalInfoRequest.class), any(AsyncCallback.class));
