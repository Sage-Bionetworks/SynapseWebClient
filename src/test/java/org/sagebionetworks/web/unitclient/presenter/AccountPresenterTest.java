...
public class AccountPresenterTest {
	...
	public void setup() {
		...
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient)
				.addEmail(any(EmailValidationSignedToken.class), any(AsyncCallback.class));
	}

	@Test
	public void testAddEmail() {
		presenter.addEmail(new EmailValidationSignedToken());
		verify(mockSynapseClient).addEmail(any(EmailValidationSignedToken.class),
	...
	public void testAddEmailFailure() {
		AsyncMockStubber.callFailureWith(new Exception("unhandled")).when(mockSynapseClient)
				.addEmail(any(EmailValidationSignedToken.class), any(AsyncCallback.class));
		presenter.addEmail(new EmailValidationSignedToken());
		verify(mockSynapseClient).addEmail(any(EmailValidationSignedToken.class),
				any(AsyncCallback.class));
