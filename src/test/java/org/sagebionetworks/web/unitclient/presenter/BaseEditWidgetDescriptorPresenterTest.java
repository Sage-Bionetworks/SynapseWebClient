...
public class BaseEditWidgetDescriptorPresenterTest {
	...
	public void setup() throws Exception {
		...
		when(mockWidgetRegistrar.getFriendlyTypeName(eq(WidgetConstants.YOUTUBE_CONTENT_TYPE)))
	...
	public void testEditNew() {
		descriptor1.clear(); // should be no arguments passed to the view, since this is editing a new
													// widget

		presenter.editNew(new WikiPageKey(entity.getId(), ObjectType.ENTITY.toString(), null),
				WidgetConstants.YOUTUBE_CONTENT_TYPE);
		verify(mockView).setWidgetDescriptor(any(WikiPageKey.class),
				eq(WidgetConstants.YOUTUBE_CONTENT_TYPE), eq(descriptor1));
	...
	public void testEditNewFailedPreconditions3() {
		presenter.editNew(new WikiPageKey(entity.getId(), null, null),
				WidgetConstants.YOUTUBE_CONTENT_TYPE);
	}
