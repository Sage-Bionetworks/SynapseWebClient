...
public class BaseEditWidgetDescriptorPresenterTest {
  ...
  public void setup() throws Exception {
    ...
    presenter =
      new BaseEditWidgetDescriptorPresenter(mockView, mockWidgetRegistrar);
    ...
    when(
      mockWidgetRegistrar.getFriendlyTypeName(
        eq(WidgetConstants.YOUTUBE_CONTENT_TYPE)
      )
    )
      .thenReturn(WidgetConstants.YOUTUBE_FRIENDLY_NAME);
    ...
    descriptorUpdateEventCaptor =
      ArgumentCaptor.forClass(WidgetDescriptorUpdatedEvent.class);
  }
  ...
  public void testEditNewFailedPreconditions3() {
    presenter.editNew(
      new WikiPageKey(entity.getId(), null, null),
      WidgetConstants.YOUTUBE_CONTENT_TYPE
    );
  }
  ...
  public void testApplySimpleInsertAndAddFileHandles() throws Exception {
    ...
    verify(mockDescriptorUpdatedHandler)
      .onUpdate(descriptorUpdateEventCaptor.capture());
    assertEquals(
      myInsertText,
      descriptorUpdateEventCaptor.getValue().getInsertValue()
    );
    assertEquals(
      fileHandleIds,
      descriptorUpdateEventCaptor.getValue().getNewFileHandleIds()
    );
  }
