...
public class DownPresenterTest {
	...
	public void setUp() {
		...
		AsyncMockStubber.callSuccessWith(mockStackStatus).when(mockStackConfigService)
				.getCurrentStatus(any(AsyncCallback.class));
	...
	public void testUpdateTimer() {
		...
		// now that we've verified the repeating scheduled execution, eat up the rest of the seconds to
		// get down to zero
	...
	public void testCheckFailure() {
		...
		AsyncMockStubber.callFailureWith(new Exception(error)).when(mockStackConfigService)
				.getCurrentStatus(any(AsyncCallback.class));
		presenter.checkForRepoDown();
