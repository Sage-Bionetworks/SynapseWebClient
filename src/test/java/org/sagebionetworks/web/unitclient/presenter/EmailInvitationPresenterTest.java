...
public class EmailInvitationPresenterTest {
	...
	public void before() {
		...
		presenter = new EmailInvitationPresenter(mockView, mockJsClient, futureClient, mockSynapseAlert,
	...
	public void beforeSetPlace(boolean loggedIn) {
		...
		when(futureClient.hexDecodeAndDeserialize(eq(encodedMISignedToken)))
				.thenReturn(getDoneFuture(decodedToken));
		when(mockJsClient.getMembershipInvitation(decodedToken))
				.thenReturn(getDoneFuture(mockMembershipInvitation));
	...
	public void testLoggedInEmailOwnerAddToTeamSuccess() {
		...
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).addTeamMember(anyString(),
				anyString(), any(AsyncCallback.class));
		beforeSetPlace(true);
		when(mockJsClient.getInviteeVerificationSignedToken(mockMembershipInvitation.getId()))
				.thenReturn(getDoneFuture(mockInviteeVerificationSignedToken));
		when(mockJsClient.updateInviteeId(mockInviteeVerificationSignedToken))
	...
	public void testLoggedInEmailOwnerAddToTeamFailure() {
		...
		// membership invitation). This is the case when the team has access requirements, for example.
		// See SWC-4759
		AsyncMockStubber.callFailureWith(new Exception("failed to add the team member after binding"))
				.when(mockJsClient).addTeamMember(anyString(), anyString(), any(AsyncCallback.class));
		beforeSetPlace(true);
		when(mockJsClient.getInviteeVerificationSignedToken(mockMembershipInvitation.getId()))
				.thenReturn(getDoneFuture(mockInviteeVerificationSignedToken));
		when(mockJsClient.updateInviteeId(mockInviteeVerificationSignedToken))
	...
	public void testLoggedInNotEmailOwner() {
		// currently logged in user does currently own the given email address, binding Synapse account
		// to invitation will fail
		beforeSetPlace(true);
		when(mockJsClient.getInviteeVerificationSignedToken(mockMembershipInvitation.getId()))
				.thenReturn(getFailedFuture(new ForbiddenException()));
		when(mockJsClient.updateInviteeId(mockInviteeVerificationSignedToken))
				.thenReturn(getDoneFuture(null));
		...
		// instead, in this edge case we send the currently logged in user their settings place to add
	...
	public void testNotLoggedIn() {
		...
		when(mockJsClient.getTeam(mockMembershipInvitation.getTeamId()))
				.thenReturn(getDoneFuture(mockTeam));
		when(mockTeam.getName()).thenReturn("teamName");
		when(mockJsClient.getUserProfile(mockMembershipInvitation.getCreatedBy()))
				.thenReturn(getDoneFuture(mockInviterProfile));
