...
public class EmailInvitationPresenterTest {
  ...
  public void beforeSetPlace(boolean loggedIn) {
    ...
    AsyncMockStubber
      .callSuccessWith(decodedToken)
      .when(mockSynapseClient)
      .hexDecodeAndDeserialize(
        eq(encodedMISignedToken),
    ...
    when(mockAuthController.getCurrentUserProfile())
      .thenReturn(mockCurrentUserProfile);
    when(mockCurrentUserProfile.getEmails())
  ...
  public void testLoggedInEmailOwnerAddToTeamSuccess() {
    ...
    beforeSetPlace(true);
    when(
      mockJsClient.getInviteeVerificationSignedToken(
        mockMembershipInvitation.getId()
      )
    )
  ...
  public void testLoggedInEmailOwnerAddToTeamFailure() {
    ...
    AsyncMockStubber
      .callFailureWith(
        new Exception("failed to add the team member after binding")
      )
      .when(mockJsClient)
      .addTeamMember(anyString(), anyString(), any(AsyncCallback.class));
    beforeSetPlace(true);
  ...
  public void testLoggedInNotEmailOwner() {
    ...
    verify(mockJsClient, never())
      .updateInviteeId(mockInviteeVerificationSignedToken);
  ...
  public void testLoggedInInviteHasUserID() {
    ...
    verify(mockJsClient, never())
      .getInviteeVerificationSignedToken(anyString());
    verify(mockJsClient, never())
      .updateInviteeId(mockInviteeVerificationSignedToken);
  ...
  public void testNotLoggedIn() {
    ...
    ArgumentCaptor<RegisterAccount> captor = ArgumentCaptor.forClass(
  ...
  public void testOnLoginClick() {
    ...
    ArgumentCaptor<LoginPlace> captor = ArgumentCaptor.forClass(
      LoginPlace.class
    );
    verify(mockPlaceChanger).goTo(captor.capture());
