...
public class HomePresenterTest {
	...
	public void setup() throws RestServiceException, JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(openInvitations).when(mockSynapseClient)
				.getOpenInvitations(anyString(), any(AsyncCallback.class));

		mockAuthenticationController = Mockito.mock(AuthenticationController.class);
		when(mockAuthenticationController.isLoggedIn()).thenReturn(true);
		when(mockGlobalApplicationState.getPlaceChanger()).thenReturn(mockPlaceChanger);
		homePresenter = new HomePresenter(mockView, mockAuthenticationController,
				mockGlobalApplicationState, mockCookies);

		when(mockAuthenticationController.isLoggedIn()).thenReturn(true);
		testProfile = new UserProfile();
		when(mockAuthenticationController.getCurrentUserProfile()).thenReturn(testProfile);

		AsyncMockStubber.callSuccessWith("").when(mockSynapseClient)
