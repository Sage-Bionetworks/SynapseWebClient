...
public class PasswordResetPresenterTest {
	...
	public void setup() {
		...
		presenter = new PasswordResetPresenter(mockView, mockAuthenticationController,
				mockGlobalApplicationState, mockJsClient, mockSynAlert);
	...
	private void resetAll() {
		...
		AsyncMockStubber.callSuccessWith(profile).when(mockAuthenticationController)
				.loginUser(anyString(), anyString(), any(AsyncCallback.class));
	}
	...
	public void testSetPasswordLoad() {
		AsyncMockStubber.callSuccessWith(profile).when(mockAuthenticationController)
				.setNewSessionToken(anyString(), any(AsyncCallback.class));
	...
	public void testResetPassword2() {
		// if there is a failure to re-login using the available credentials, send to the login page
		// instead
		resetAll();
		AsyncMockStubber.callFailureWith(new Exception()).when(mockAuthenticationController)
				.loginUser(anyString(), anyString(), any(AsyncCallback.class));
