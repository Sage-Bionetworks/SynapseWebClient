...
public class PasswordResetSignedTokenPresenterTest {
	...
	@Before
	public void setup() {
		presenter = new PasswordResetSignedTokenPresenter(mockView, mockSynapseClient, mockJsClient,
				mockSynapseAlert, mockAuthenticationController, mockGlobalApplicationState);

		when(testPlace.getToken()).thenReturn(TEST_TOKEN);

		// by default, decode into a PasswordResetSignedToken
		AsyncMockStubber.callSuccessWith(mockPasswordResetSignedToken).when(mockSynapseClient)
		...
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient)
				.changePassword(any(ChangePasswordWithToken.class), any(AsyncCallback.class));
	}
	...
	public void testSetPlaceInvalidSignedTokenType() {
		AsyncMockStubber.callSuccessWith(mockJoinTeamSignedToken).when(mockSynapseClient)
				.hexDecodeAndDeserialize(anyString(), any(AsyncCallback.class));
		...
		verify(mockSynapseAlert)
				.showError(PasswordResetSignedTokenPresenter.INVALID_PASSWORD_RESET_SIGNED_TOKEN);
	}
	...
	public void testChangePasswordSuccess() {
		...
		verify(mockJsClient).changePassword(changePasswordWithTokenCaptor.capture(),
		...
		assertEquals(mockPasswordResetSignedToken,
