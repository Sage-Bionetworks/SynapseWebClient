...
public class ProfilePresenterTest {
	...
	public void setup() throws JSONObjectAdapterException {
		...
		projectList = new PagedResults();
		projectsJson = new ArrayList<String>();
		projectsJson.add(projectHeader1.writeToJSONObject(adapterFactory.createNew()).toJSONString());
		projectsJson.add(projectHeader2.writeToJSONObject(adapterFactory.createNew()).toJSONString());
		myProjects = new ArrayList<ProjectHeader>();
		myProjects.add(projectHeader1);
		myProjects.add(projectHeader2);
		projectList.setResults(projectsJson);
		projectList.setTotalNumberOfResults(2);
		
		AsyncMockStubber.callSuccessWith(projectList).when(mockSynapseClient).getMyProjects(anyInt(), anyInt(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(projectList).when(mockSynapseClient).getUserProjects(anyString(), anyInt(), anyInt(), any(AsyncCallback.class));
