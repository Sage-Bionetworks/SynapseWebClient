...
public class QuestionContainerWidgetTest {
	...
	public void setup() {
		...
		mockMultichoiceQuestion =
				mock(MultichoiceQuestion.class, withSettings().extraInterfaces(Question.class));
	...
	public void testConfigureRadioButtons() {
		...
		verify(mockView, times(2)).addRadioButton(eq(mockMultichoiceQuestion.getQuestionIndex()),
				anyString(), any(ClickHandler.class), eq(false));
	...
	public void testConfigureCheckBoxes() {
		setExclusive(false);
		String docLink =
				"docs.synapse.org/articles/accounts_certified_users_and_qualified_researchers.html";
		...
		questionContainerWidget.configure(1L, mockMultichoiceQuestion, null);
		verify(mockView, times(2)).addCheckBox(eq(mockMultichoiceQuestion.getQuestionIndex()),
				anyString(), any(ClickHandler.class), eq(false));
