...
public class QuizPresenterTest {
	...
	public void setup() throws JSONObjectAdapterException {
		...
		presenter =
				new QuizPresenter(mockView, mockAuthenticationController, mockGlobalApplicationState,
						mockSynapseClient, adapterFactory, adapter, mockInjector, mockSynAlert);
		...
		AsyncMockStubber.callSuccessWith(questionnaireJson).when(mockSynapseClient)
	...
	public static Quiz mockQuiz() {
		...
		answers.add(
				getAnswer(answerIndex++, "only people working at not-for-profit research organizations"));
		q1.setAnswers(answers);
		questionOptions.add(q1);
		q1 = new MultichoiceQuestion();
		...
		answers.add(getAnswer(answerIndex++,
				"Synapse has a set of programmatic clients in addition to the web client"));
		answers.add(
				getAnswer(answerIndex++, "I can <b>only</b> upload or download data via the web client"));
		q1.setAnswers(answers);
		questionOptions.add(q1);
	...
	private void setPassingRecordResponse(PassingRecord pr) throws JSONObjectAdapterException {
		AsyncMockStubber.callSuccessWith(pr).when(mockSynapseClient)
				.submitCertificationQuizResponse(any(QuizResponse.class), any(AsyncCallback.class));
	}
	...
	public void testSubmitAnswersPass() throws JSONObjectAdapterException {
		...
		verify(mockSynapseClient).submitCertificationQuizResponse(arg.capture(),
				any(AsyncCallback.class));
		// reconstruct the QuestionnaireResponse, and sanity check that it should have 2 question
		// responses
