...
public class SearchPresenterTest {
	...
	public void setup() throws Exception {
		...
		searchPresenter = new SearchPresenter(mockView, mockGlobalApplicationState, mockJsClient,
				new JSONObjectAdapterImpl(), mockSynAlert, mockLoadMoreWidgetContainer);
		...
		exampleTermSearchQueryJson =
				exampleTermSearchQuery.writeToJSONObject(jsonObjectAdapter.createNew()).toJSONString();
	}
	...
	public void testTimeFacets() {
		...
		facetValue = "2";
		searchPresenter.addTimeFacet(facetName, facetValue, "Yesterday");
		verify(mockJsClient, times(2)).getSearchResults(any(SearchQuery.class),
				any(AsyncCallback.class)); // 4
		facetValues = getTimeFacet(facetName);
		assertEquals(1, facetValues.size());
		assertEquals(facetValue, facetValues.get(0).getMin());

		searchPresenter.removeTimeFacetAndRefresh(facetName);
		// optimization. if search is empty (no search term and no facets) then a search is not
		// performed (empty results are shown).
		verify(mockJsClient, times(2)).getSearchResults(any(SearchQuery.class),
				any(AsyncCallback.class)); // 4
