...
public class SignedTokenPresenterTest {
	...
	@Before
	public void setup() {
		presenter = new SignedTokenPresenter(mockView, mockSynapseClient, mockGWTWrapper,
				mockSynapseAlert, mockGlobalApplicationState, mockUserBadge, mockAuthenticationController,
				mockPopupUtils);
		...
		AsyncMockStubber.callSuccessWith(responseMessage).when(mockSynapseClient)
				.handleSignedToken(any(SignedTokenInterface.class), anyString(), any(AsyncCallback.class));

		// by default, decode into a JoinTeamSignedToken
		AsyncMockStubber.callSuccessWith(defaultToken).when(mockSynapseClient)
		...
		// by default, the team has no access requirements (so it should just handle the signed token
		// like any other signed token request).
		accessRequirements = new ArrayList<AccessRequirement>();
		AsyncMockStubber.callSuccessWith(accessRequirements).when(mockSynapseClient)
	...
	public void testSetPlaceJoinTeamSignedToken() {
		...
		verify(mockSynapseClient).handleSignedToken(any(SignedTokenInterface.class), anyString(),
	...
	public void testSetPlaceEmailValidationSignedToken() {
		AsyncMockStubber.callSuccessWith(new EmailValidationSignedToken()).when(mockSynapseClient)
				.hexDecodeAndDeserialize(anyString(), any(AsyncCallback.class));
	...
	public void testSetPlaceWithAccessRequirements() {
		...
		verify(mockSynapseClient).hexDecodeAndDeserialize(anyString(), any(AsyncCallback.class));
		verify(mockSynapseClient, never()).handleSignedToken(any(SignedTokenInterface.class),
	...
	public void testJoinTeamExpiredSessionToken() {
		...
		AsyncMockStubber.callSuccessWith(new JoinTeamSignedToken()).when(mockSynapseClient)
				.hexDecodeAndDeserialize(anyString(), any(AsyncCallback.class));
		presenter.setPlace(testPlace);
		...
		verify(mockSynapseClient, times(2)).getTeamAccessRequirements(anyString(),
				asyncCaptor.capture());
	...
	public void testJoinTeamExpiredSessionTokenMultipleErrors() {
		...
		verify(mockSynapseClient, times(2)).getTeamAccessRequirements(anyString(),
				asyncCaptor.capture());
		// if it runs into another error (even if it's an UnauthorizedException) it should not try
		// again.
	...
	public void testSetPlaceUnsubscribe() {
		...
		AsyncMockStubber.callSuccessWith(token).when(mockSynapseClient)
				.hexDecodeAndDeserialize(anyString(), any(AsyncCallback.class));
		presenter.setPlace(testPlace);
		...
		verify(mockSynapseClient).handleSignedToken(any(NotificationSettingsSignedToken.class),
				anyString(), any(AsyncCallback.class));
	}
