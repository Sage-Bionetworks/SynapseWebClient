...
public class SubscriptionPresenterTest {
  ...
  private static final SubscriptionObjectType TEST_OBJECT_TYPE =
  ...
  public void testSubscribed() {
    ...
    AsyncMockStubber
      .callSuccessWith(mockSubscription)
      .when(mockJsClient)
      .getSubscription(anyString(), any(AsyncCallback.class));
    when(mockPlace.getParam(SubscriptionPlace.SUBSCRIPTION_ID_FILTER_PARAM))
      .thenReturn(TEST_SUBSCRIPTION_ID);
    presenter.setPlace(mockPlace);
    verify(mockSynAlert).clear();
    verify(mockView).selectSubscribedButton();
    verify(mockTopicWidget).configure(TEST_OBJECT_TYPE, TEST_OBJECT_ID);
    verify(mockJsClient)
      .getSubscription(eq(TEST_SUBSCRIPTION_ID), any(AsyncCallback.class));
    ...
    verify(mockPlace)
      .putParam(SubscriptionPlace.OBJECT_ID_PARAM, TEST_OBJECT_ID);
    verify(mockPlace)
      .putParam(SubscriptionPlace.OBJECT_TYPE_PARAM, TEST_OBJECT_TYPE.name());
  ...
  public void testSubscribedFailure() {
    ...
    when(mockPlace.getParam(SubscriptionPlace.SUBSCRIPTION_ID_FILTER_PARAM))
      .thenReturn(subscriptionId.toString());
    presenter.setPlace(mockPlace);
    verify(mockSynAlert).clear();
    verify(mockView).selectSubscribedButton();
  ...
  public void testUnsubscribed() {
    ...
    when(mockPlace.getParam(SubscriptionPlace.OBJECT_ID_PARAM))
      .thenReturn(TEST_OBJECT_ID);
    when(mockPlace.getParam(SubscriptionPlace.OBJECT_TYPE_PARAM))
      .thenReturn(TEST_OBJECT_TYPE.name());
    presenter.setPlace(mockPlace);
    verify(mockSynAlert).clear();
    ...
    verify(mockPlace)
      .putParam(
        SubscriptionPlace.SUBSCRIPTION_ID_FILTER_PARAM,
  ...
  public void testNoParams() {
    ...
    verify(mockSynAlert).clear();
    verify(mockSynAlert)
      .showError(SubscriptionPresenter.MISSING_PARAMS_MESSAGE);
  }
}
