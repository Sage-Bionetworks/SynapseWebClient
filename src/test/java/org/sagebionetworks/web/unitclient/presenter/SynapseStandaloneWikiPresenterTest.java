...
public class SynapseStandaloneWikiPresenterTest {
	...
	public void setup() {
		...
		AsyncMockStubber.callSuccessWith(targetWikiPage).when(mockSynapseJavascriptClient)
				.getV2WikiPageAsV1(any(WikiPageKey.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(pageName2WikiKeyMap).when(mockSynapseClient)
	...
	public void testSetPlace() {
		...
		// from the page place token, it has a valid wiki page alias that matches a result from the
		// portal properties
	...
	public void testSetPlaceFailedToGetPageNameToWikiKeyMap() {
		...
		AsyncMockStubber.callFailureWith(error).when(mockSynapseClient)
				.getPageNameToWikiKeyMap(any(AsyncCallback.class));
		presenter.setPlace(testPlace);
