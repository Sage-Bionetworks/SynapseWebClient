...
public class TeamPresenterTest {
	...
	public void setup() {
		...
		presenter = new TeamPresenter(mockView, mockAuthenticationController, mockGlobalAppState,
				mockSynClient, mockSynAlert, mockInviteModal, mockJoinWidget, mockManagerListWidget,
				mockMemberListWidget, mockOpenMembershipRequestsWidget, mockOpenUserInvitationsWidget,
				mockGoogleMap, mockCookies, mockIsACTMemberAsyncHandler, mockGinInjector);
		when(mockTeam.getName()).thenReturn(teamName);
		AsyncMockStubber.callSuccessWith(mockTeamBundle).when(mockSynClient).getTeamBundle(anyString(),
	...
	public void testRefreshFailure() {
		AsyncMockStubber.callFailureWith(caught).when(mockSynClient).getTeamBundle(anyString(),
				anyString(), anyBoolean(), any(AsyncCallback.class));
	...
	public void testRefreshAdmin() {
		...
		verify(mockManagerListWidget).configure(eq(teamId), eq(isAdmin),
				eq(TeamMemberTypeFilterOptions.ADMIN), any(Callback.class));
		verify(mockMemberListWidget).configure(eq(teamId), eq(isAdmin),
				eq(TeamMemberTypeFilterOptions.MEMBER), any(Callback.class));
		...
		// never
		verify(mockJoinWidget, never()).configure(eq(teamId), anyBoolean(),
				eq(mockTeamMembershipStatus), any(Callback.class), anyString(), anyString(), anyString(),
	...
	public void testRefreshOpenMembershipRequests() {
		...
		verify(mockSynClient).getTeamBundle(anyString(), anyString(), anyBoolean(),
