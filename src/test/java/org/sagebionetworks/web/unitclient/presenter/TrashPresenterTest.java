...
public class TrashPresenterTest {
	...
	public void testGetTrash() {
		AsyncMockStubber.callSuccessWith(trashList).when(mockSynapse).viewTrashForUser(anyInt(),
	...
	public void testGetTrashFailure() {
		...
		AsyncMockStubber.callFailureWith(caught).when(mockSynapse).viewTrashForUser(anyLong(),
	...
	public void testPurgeEntities() {
		AsyncMockStubber.callSuccessWith(null).when(mockSynapse)
				.purgeMultipleTrashedEntitiesForUser(anySet(), any(AsyncCallback.class));
	...
	public void testPurgeEntitiesFailure() {
		Exception caught = new Exception("this is an exception");
		AsyncMockStubber.callFailureWith(caught).when(mockSynapse)
				.purgeMultipleTrashedEntitiesForUser(anySet(), any(AsyncCallback.class));
		presenter.purgeEntities(new HashSet<TrashedEntity>(trashList.getResults()));
		verify(mockSynAlert).showError(anyString());
	}
	...
	public void testRestore() {
		AsyncMockStubber.callSuccessWith(null).when(mockSynapse).restoreFromTrash(anyString(),
	...
	public void testRestoreFailure() {
		Exception caught = new Exception("this is an exception");
		AsyncMockStubber.callFailureWith(caught).when(mockSynapse).restoreFromTrash(anyString(),
				anyString(), any(AsyncCallback.class));
		presenter.restoreEntity(trashList.getResults().get(0));
		verify(mockSynAlert).showError(anyString());
	}
	...
	public void testRestoreParentNotFoundFailureRestoreCall() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapse).restoreFromTrash(anyString(),
				anyString(), any(AsyncCallback.class));
