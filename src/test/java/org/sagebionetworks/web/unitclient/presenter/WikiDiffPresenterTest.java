...
public class WikiDiffPresenterTest {
	...
	public void setUp() throws Exception {
		...
		presenter = new WikiDiffPresenter(mockView, mockSynapseClient, mockJsClient, mockSynAlert,
				mockGlobalAppState, mockResourceLoader, mockJsniUtils);
	}
	...
	public void testConstructor() {
		...
		AsyncMockStubber.callFailureWith(loadingErrorEx).when(mockResourceLoader).requires(anyList(),
	...
	public void testGetWikiHistory() {
		...
		AsyncMockStubber
				.callSuccessWith(mockSingleItemHistoryPaginatedResults,
						mockEmptyItemHistoryPaginatedResults)
				.when(mockSynapseClient)
				.getV2WikiHistory(any(WikiPageKey.class), anyLong(), anyLong(), any(AsyncCallback.class));
		...
		verify(mockSynapseClient).getV2WikiHistory(wikiPageKeyCaptor.capture(),
				eq(WikiDiffPresenter.LIMIT), eq(0L), any(AsyncCallback.class));
		...
		verify(mockSynapseClient).getV2WikiHistory(any(WikiPageKey.class), eq(WikiDiffPresenter.LIMIT),
				eq(WikiDiffPresenter.LIMIT), any(AsyncCallback.class));
	...
	public void testGetWikiMarkdown() {
		AsyncMockStubber.callSuccessWith(mockWikiPageV1).when(mockJsClient).getVersionOfV2WikiPageAsV1(
				any(WikiPageKey.class), eq(VERSION_1_VALUE), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockWikiPageV2).when(mockJsClient).getVersionOfV2WikiPageAsV1(
				any(WikiPageKey.class), eq(VERSION_2_VALUE), any(AsyncCallback.class));

		presenter.setPlace(mockPlace);

		verify(mockJsClient, never()).getVersionOfV2WikiPageAsV1(any(WikiPageKey.class), anyLong(),
				any(AsyncCallback.class));
		...
		verify(mockJsClient).getVersionOfV2WikiPageAsV1(any(WikiPageKey.class), eq(VERSION_1_VALUE),
				any(AsyncCallback.class));
		verify(mockJsClient).getVersionOfV2WikiPageAsV1(any(WikiPageKey.class), eq(VERSION_2_VALUE),
				any(AsyncCallback.class));
