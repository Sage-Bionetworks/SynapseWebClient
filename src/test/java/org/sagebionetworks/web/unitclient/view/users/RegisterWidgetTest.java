...
public class RegisterWidgetTest {
	...
	public void setup() {
		...
		AsyncMockStubber.callSuccessWith(null).when(mockUserService).createUserStep1(any(NewUser.class),
	...
	public void testRegisterUserUserExists() {
		AsyncMockStubber.callFailureWith(new ConflictException("user exists")).when(mockUserService)
				.createUserStep1(any(NewUser.class), anyString(), any(AsyncCallback.class));
		widget.registerUser(email);
	...
	public void testRegisterUserServiceFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockUserService).createUserStep1(any(NewUser.class),
				anyString(), any(AsyncCallback.class));
		widget.registerUser(email);
