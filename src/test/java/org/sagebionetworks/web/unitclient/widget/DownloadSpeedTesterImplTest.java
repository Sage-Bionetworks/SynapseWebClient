...
public class DownloadSpeedTesterImplTest {
	...
	public void setUp() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(mockFileEntity).when(mockJsClient).getEntity(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockFileResult).when(mockFileHandleAsyncHandler)
		...
		downloadSpeedTester = new DownloadSpeedTesterImpl(mockAuthController,
				mockFileHandleAsyncHandler, mockRequestBuilder, mockJsClient, mockClientCache);
		when(mockClientCache.get(DownloadSpeedTesterImpl.ESTIMATED_DOWNLOAD_SPEED_CACHE_KEY))
				.thenReturn(null);
		when(mockClientCache.contains(DownloadSpeedTesterImpl.ESTIMATED_DOWNLOAD_SPEED_CACHE_KEY))
				.thenReturn(false);
	}
	...
	public void testFailToGetEntity() {
		...
		AsyncMockStubber.callFailureWith(error).when(mockJsClient).getEntity(anyString(),
				any(AsyncCallback.class));
	...
	public void testFailToGetFileResult() {
		...
		verify(mockCallback).onFailure(throwableCaptor.capture());
		assertTrue(throwableCaptor.getValue().getMessage()
