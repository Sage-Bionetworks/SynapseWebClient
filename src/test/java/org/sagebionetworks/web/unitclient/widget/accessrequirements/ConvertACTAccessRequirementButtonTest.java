...
public class ConvertACTAccessRequirementButtonTest {
	...
	public void setUp() throws Exception {
		...
		widget = new ConvertACTAccessRequirementButton(mockButton, mockIsACTMemberAsyncHandler,
				mockPopupUtilsView, mockDataAccessClientAsync);
	...
	public void testOnClick() {
		...
		verify(mockPopupUtilsView).showConfirmDialog(eq(CONVERT_AR_CONFIRM_TITLE),
				eq(CONVERT_AR_CONFIRM_MESSAGE), any(Callback.class));
	}

	@Test
	public void testConvertAccessRequirement() {
		AsyncMockStubber.callSuccessWith(mockAccessRequirement).when(mockDataAccessClientAsync)
				.convertAccessRequirement(any(AccessRequirementConversionRequest.class),
						any(AsyncCallback.class));

		widget.convertAccessRequirement();

		verify(mockDataAccessClientAsync).convertAccessRequirement(conversionRequestCaptor.capture(),
				any(AsyncCallback.class));
	...
	public void testConvertAccessRequirementFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockDataAccessClientAsync).convertAccessRequirement(
				any(AccessRequirementConversionRequest.class), any(AsyncCallback.class));
