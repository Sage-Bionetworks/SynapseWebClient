...
public class DeleteAccessRequirementButtonTest {
	...
	public void setUp() throws Exception {
		...
		widget = new DeleteAccessRequirementButton(mockButton, mockIsACTMemberAsyncHandler,
				mockSynapseClient, mockPopupUtilsView, mockCookies);
		...
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient)
				.deleteAccessRequirement(anyLong(), any(AsyncCallback.class));
	...
	private Callback verifyConfirmDelete() {
		verify(mockPopupUtilsView).showConfirmDialog(eq(DELETE_ACCESS_REQUIREMENT_TITLE),
				eq(DELETE_ACCESS_REQUIREMENT_MESSAGE), callbackCaptor.capture());
	...
	public void testFailureToDelete() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).deleteAccessRequirement(anyLong(),
