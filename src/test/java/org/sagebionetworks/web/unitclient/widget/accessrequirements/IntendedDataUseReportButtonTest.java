...
public class IntendedDataUseReportButtonTest {
	...
	public void setUp() throws Exception {
		...
		when(mockSubmissionPage2.getResults())
				.thenReturn(Collections.singletonList(mockSubmissionInPage2));
		widget = new IntendedDataUseReportButton(mockButton, mockIsACTMemberAsyncHandler,
				mockDataAccessClient, mockPopupUtils, mockBigPromptModal, mockDateTimeUtils);
		verify(mockButton).addClickHandler(clickHandlerCaptor.capture());
		onButtonClickHandler = clickHandlerCaptor.getValue();
		AsyncMockStubber.callSuccessWith(mockSubmissionPage1, mockSubmissionPage2)
				.when(mockDataAccessClient).getDataAccessSubmissions(anyLong(), anyString(),
	...
	public void testConfigureWithAR() {
		...
		verify(mockDataAccessClient).getDataAccessSubmissions(eq(AR_ID), eq(null),
				eq(SubmissionState.APPROVED), eq(SubmissionOrder.CREATED_ON), eq(true),
				any(AsyncCallback.class));
		verify(mockDataAccessClient).getDataAccessSubmissions(eq(AR_ID), eq(NEXT_PAGE_TOKEN),
				eq(SubmissionState.APPROVED), eq(SubmissionOrder.CREATED_ON), eq(true),
				any(AsyncCallback.class));

		verify(mockBigPromptModal).configure(eq(IntendedDataUseReportButton.IDU_MODAL_TITLE),
				eq(IntendedDataUseReportButton.IDU_MODAL_FIELD_NAME), stringCaptor.capture());
