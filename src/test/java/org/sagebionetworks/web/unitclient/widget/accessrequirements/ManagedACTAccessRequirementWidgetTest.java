...
public class ManagedACTAccessRequirementWidgetTest {
	...
	public void setUp() throws Exception {
		...
		widget = new ManagedACTAccessRequirementWidget(mockView, mockJsClient, mockWikiPageWidget,
				mockSynAlert, mockGinInjector, mockSubjectsWidget, mockCreateAccessRequirementButton,
				mockDeleteAccessRequirementButton, mockReviewAccessRequestsButton, mockIduReportButton,
				mockDataAccessClient, mockLazyLoadHelper, mockAuthController, mockSubmitterUserBadge,
				mockDateTimeUtils, mockManageAccessButton);
		...
		AsyncMockStubber.callSuccessWith(ROOT_WIKI_ID).when(mockJsClient)
		...
		AsyncMockStubber.callSuccessWith(mockDataAccessSubmissionStatus).when(mockDataAccessClient)
	...
	public void testSetRequirement() {
		...
		verify(mockCreateAccessRequirementButton).configure(eq(mockACTAccessRequirement),
				any(Callback.class));
		verify(mockDeleteAccessRequirementButton).configure(eq(mockACTAccessRequirement),
				any(Callback.class));
	...
	public void testSetRequirementWithWikiTerms() {
		widget.setRequirement(mockACTAccessRequirement, mockRefreshCallback);
		verify(mockWikiPageWidget).configure(any(WikiPageKey.class), eq(false),
	...
	public void testGetSubmissionStatusError() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockDataAccessClient)
				.getAccessRequirementStatus(anyString(), any(AsyncCallback.class));
		widget.setRequirement(mockACTAccessRequirement, mockRefreshCallback);
		lazyLoadDataCallback.invoke();
	...
	public void testCancel() {
		AsyncMockStubber.callSuccessWith(null).when(mockDataAccessClient)
		...
		verify(mockDataAccessClient).cancelDataAccessSubmission(eq(SUBMISSION_ID),
				any(AsyncCallback.class));
		// refreshes status after cancel
		verify(mockDataAccessClient, times(2)).getAccessRequirementStatus(anyString(),
				any(AsyncCallback.class));
	}
	...
	public void testNoWiki() {
		AsyncMockStubber.callFailureWith(new NotFoundException()).when(mockJsClient)
				.getRootWikiPageKey(anyString(), anyString(), any(AsyncCallback.class));
		widget.setRequirement(mockACTAccessRequirement, mockRefreshCallback);
