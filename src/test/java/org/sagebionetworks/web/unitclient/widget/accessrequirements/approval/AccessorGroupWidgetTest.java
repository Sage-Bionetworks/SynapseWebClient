...
public class AccessorGroupWidgetTest {
	...
	public void setUp() throws Exception {
		widget = new AccessorGroupWidget(mockView, mockSynAlert, mockGinInjector, mockPopupUtils,
				mockAccessRequirementWidget, mockDataAccessClient, mockDateTimeUtils,
	...
	public void testOnRevoke() {
		widget.onRevoke();
		verify(mockPopupUtils).showConfirmDialog(eq(REVOKE_ACCESS_TO_GROUP), eq(ARE_YOU_SURE),
				any(Callback.class));
	...
	public void testOnRevokeAfterConfirm() {
		AsyncMockStubber.callSuccessWith(null).when(mockDataAccessClient).revokeGroup(anyString(),
				anyString(), any(AsyncCallback.class));
		widget.configure(mockAccessorGroup);
		widget.setOnRevokeCallback(onRevokeCallback);
		widget.onRevokeAfterConfirm();

		verify(mockDataAccessClient).revokeGroup(eq(ACCESS_REQUIREMENT_ID), eq(SUBMITTER_USER_ID),
