...
public class CreateACTAccessRequirementStep2Test {
	...
	public void setUp() throws Exception {
		...
		widget = new CreateManagedACTAccessRequirementStep2(mockView, mockSynapseClient,
				mockWikiMarkdownEditor, mockWikiPageRenderer, mockDucTemplateUploader,
	...
	public void testConfigureWithWiki() {
		...
		widget.configure(mockACTAccessRequirement);
		verify(mockWikiPageRenderer).configure(wikiPageKeyCaptor.capture(), eq(false),
		...
		// verify access requirement was updated from the view (view value responses configured in the
		// the test setUp()
		verify(mockACTAccessRequirement).setAreOtherAttachmentsRequired(false);
		verify(mockACTAccessRequirement).setExpirationPeriod(
				newExpirationPeriodDays * CreateManagedACTAccessRequirementStep2.DAY_IN_MS);
