...
public class CreateDataAccessSubmissionStep2Test {
  ...
  public static final RestrictableObjectType TARGET_SUBJECT_TYPE =
  ...
  public void setUp() throws Exception {
    ...
    when(mockOtherDocuments.setUploadButtonText(anyString()))
      .thenReturn(mockOtherDocuments);
    when(mockOtherDocuments.setCanDelete(anyBoolean()))
      .thenReturn(mockOtherDocuments);
    when(mockOtherDocuments.setCanUpload(anyBoolean()))
      .thenReturn(mockOtherDocuments);

    widget =
      new CreateDataAccessSubmissionStep2(
        mockView,
        mockClient,
    ...
    AsyncMockStubber
      .callSuccessWith(mockDataAccessRequest)
      .when(mockClient)
    ...
    when(mockGinInjector.getFileHandleWidget())
      .thenReturn(mockFileHandleWidget);
    ...
    AsyncMockStubber
      ...
      .submitDataAccessRequest(
        any(CreateSubmissionRequest.class),
        anyLong(),
        any(AsyncCallback.class)
      );
  }
  ...
  public void testUserSynapseSuggestion() {
    ...
    verify(mockPeopleSuggestBox)
      .addItemSelectedHandler(callbackPUserSuggestionCaptor.capture());
  ...
  public void testConfigureWithRenewal() {
    AsyncMockStubber
      .callSuccessWith(mockDataAccessRenewal)
      .when(mockClient)
      .getDataAccessRequest(anyLong(), any(AsyncCallback.class));
    widget.configure(
      mockResearchProject,
  ...
  public void testConfigureWithAccessorChanges() {
    ...
    when(mockDataAccessRequest.getAccessorChanges())
      .thenReturn(accessorUserIds);
    widget.configure(
      mockResearchProject,
      mockACTAccessRequirement,
      mockSubject
  ...
  public void testSubmit() {
    ...
    when(mockAccessorsList.getAccessorChanges())
      .thenReturn(mockAccessorChanges);
    when(mockOtherDocuments.getFileHandleIds())
      .thenReturn(mockOtherFileHandleIds);
    widget.onPrimary();
  ...
  public void testSave() {
    ...
    verify(mockPopupUtils)
      .showConfirmDialog(
        anyString(),
        eq(CreateDataAccessSubmissionStep2.SAVE_CHANGES_MESSAGE),
    ...
    verify(mockPopupUtils)
      .showInfo(CreateDataAccessSubmissionStep2.SAVED_PROGRESS_MESSAGE);
    InOrder order = inOrder(mockModalPresenter);
    order.verify(mockModalPresenter).setLoading(true);
    order.verify(mockModalPresenter).setLoading(false);
    verify(mockModalPresenter).onFinished();
