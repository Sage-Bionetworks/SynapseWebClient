...
public class CreateDataAccessSubmissionStep2Test {
	...
	public void setUp() throws Exception {
		...
		widget = new CreateDataAccessSubmissionStep2(mockView, mockClient, mockTemplateFileRenderer,
				mockDucUploader, mockIrbUploader, mockJsniUtils, mockAuthController, mockGinInjector,
				mockAccessorsList, mockPeopleSuggestBox, mockProvider, mockOtherDocuments, mockPopupUtils);
		widget.setModalPresenter(mockModalPresenter);
		AsyncMockStubber.callSuccessWith(mockDataAccessRequest).when(mockClient)
				.getDataAccessRequest(anyLong(), any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(mockDataAccessRequest).when(mockClient)
				.updateDataAccessRequest(any(RequestInterface.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(null).when(mockClient).submitDataAccessRequest(
				any(CreateSubmissionRequest.class), anyLong(), any(AsyncCallback.class));
	}
	...
	public void testConfigureFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockClient).getDataAccessRequest(anyLong(),
	...
	public void testConfigureWithRenewal() {
		AsyncMockStubber.callSuccessWith(mockDataAccessRenewal).when(mockClient)
				.getDataAccessRequest(anyLong(), any(AsyncCallback.class));
		widget.configure(mockResearchProject, mockACTAccessRequirement, mockSubject);
	...
	public void testConfigureWithAttachments() {
		...
		assertEquals(FileHandleAssociateType.DataAccessRequestAttachment,
				fhas.get(0).getAssociateObjectType());
		...
		assertEquals(FileHandleAssociateType.DataAccessRequestAttachment,
				fhas.get(1).getAssociateObjectType());
	...
	public void testSubmit() {
		...
		verify(mockClient).updateDataAccessRequest(any(RequestInterface.class),
				any(AsyncCallback.class));

		// submitted (primary button was clicked)
		verify(mockClient).submitDataAccessRequest(submissionRequestCaptor.capture(),
				eq(ACCESS_REQUIREMENT_ID), any(AsyncCallback.class));
		...
		verify(mockPopupUtils).showInfoDialog(SUCCESSFULLY_SUBMITTED_TITLE,
				SUCCESSFULLY_SUBMITTED_MESSAGE, null);
	...
	public void testSubmitFailure() {
		...
		Exception ex = new Exception(error);
		AsyncMockStubber.callFailureWith(ex).when(mockClient)
				.updateDataAccessRequest(any(RequestInterface.class), any(AsyncCallback.class));
		widget.onPrimary();
		verify(mockClient).updateDataAccessRequest(any(RequestInterface.class),
				any(AsyncCallback.class));
	...
	public void testSave() {
		...
		verify(mockPopupUtils).showConfirmDialog(anyString(),
				eq(CreateDataAccessSubmissionStep2.SAVE_CHANGES_MESSAGE), callbackCaptor.capture());
		...
		verify(mockClient).updateDataAccessRequest(any(RequestInterface.class),
				any(AsyncCallback.class));
		verify(mockClient, never()).submitDataAccessRequest(any(CreateSubmissionRequest.class),
