...
public class RejectReasonWidgetTest {
	...
	public void setUp() throws Exception {
		...
		when(mockSynapseProperties.getSynapseProperty(WebConstants.ACT_PROFILE_VALIDATION_REJECTION_REASONS_PROPERTY_KEY)).thenReturn(REASONS_FILE_SYN_ID);
		when(mockGinInjector.getSynapseJavascriptClient()).thenReturn(mockJsClient);
		when(mockGinInjector.getPresignedURLAsyncHandler()).thenReturn(mockPresignedURLAsyncHandler);
		...
		when(mockResponse.getText()).thenReturn("{\"reasons\":[\"test1\", \"test2\"]}");
		...
		widget = new RejectReasonWidget(mockView, mockSynapseProperties, mockGinInjector);		
	}
	...
	public void testShowOnFailure() {
		...
		when(mockResponse.getText()).thenReturn("{\"unexpectedkey\":\"not-and-array\"}");
		widget.show(getReasonCallback);
