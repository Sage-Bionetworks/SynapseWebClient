...
public class AsynchronousJobTrackerTest {
	...
	public void testMultipleStatesThenSuccess() throws JSONObjectAdapterException {
		// Simulate start
		AsyncMockStubber.callSuccessWith(jobId).when(mockJsClient).startAsynchJob(any(AsynchType.class),
				any(AsynchronousRequestBody.class), any(AsyncCallback.class));
		// simulate three calls
		AsyncMockStubber.callMixedWith(startNotReady, middleNotReady, doneNotReady, responseBody)
				.when(mockJsClient).getAsynchJobResults(any(AsynchType.class), anyString(),
						any(AsynchronousRequestBody.class), any(AsyncCallback.class));
	...
	public void testWithFailure() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callFailureWith(error).when(mockJsClient).getAsynchJobResults(
				any(AsynchType.class), anyString(), any(AsynchronousRequestBody.class),
	...
	public void testMultipleStatesThenFailure() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callMixedWith(startNotReady, middleNotReady, doneNotReady, exception)
				.when(mockJsClient).getAsynchJobResults(any(AsynchType.class), anyString(),
						any(AsynchronousRequestBody.class), any(AsyncCallback.class));
		tracker.startAndTrack(type, requestBody, waitTimeMS, mockHandler);
		// Update should occur for all three phases
	...
	public void testCancel() throws JSONObjectAdapterException {
		...
		// The handler should not get the onCancle() because the onComplete() would have already been
		// sent.
		verify(mockHandler, never()).onCancel();
	}
