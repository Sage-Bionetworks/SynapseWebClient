...
public class IsACTMemberAsyncHandlerTest {
	...
	public void setUp() throws Exception {
		...
		widget = new IsACTMemberAsyncHandlerImpl(mockUserProfileClient, mockSessionStorage,
				mockAuthController, mockJsniUtils);
	...
	public void testCache() {
		when(mockSessionStorage.getItem(SESSION_KEY_PREFIX + CURRENT_USER_ID))
	...
	public void testRpcSuccess() {
		...
		AsyncMockStubber.callSuccessWith(mockUserBundle).when(mockUserProfileClient)
				.getMyOwnUserBundle(anyInt(), any(AsyncCallback.class));
		widget.isACTActionAvailable(mockCallback);
		verify(mockUserProfileClient).getMyOwnUserBundle(anyInt(), any(AsyncCallback.class));
		verify(mockSessionStorage).setItem(SESSION_KEY_PREFIX + CURRENT_USER_ID,
				isACTMember.toString());
	...
	public void testRpcFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockUserProfileClient).getMyOwnUserBundle(anyInt(),
