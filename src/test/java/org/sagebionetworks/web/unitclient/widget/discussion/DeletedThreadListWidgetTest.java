...
import org.sagebionetworks.web.client.widget.discussion.DeletedThreadListWidget;
...
public class DeletedThreadListWidgetTest {
	...
	DeletedThreadListWidget discussionThreadListWidget;

	@Before
	public void before() {
		MockitoAnnotations.initMocks(this);
		when(mockGinInjector.createThreadWidget()).thenReturn(mockDiscussionThreadWidget);
		discussionThreadListWidget = new DeletedThreadListWidget(mockView, mockGinInjector, mockDiscussionForumClient, mockSynAlert);
	}
	...
	public void testLoadMoreSuccess() {
		...
		AsyncMockStubber.callSuccessWith(mockThreadBundlePage)
				.when(mockDiscussionForumClient).getDeletedThreadsForForum(anyString(), anyLong(),
		...
		verify(mockDiscussionForumClient).getDeletedThreadsForForum(anyString(), anyLong(),
				anyLong(), any(DiscussionThreadOrder.class), anyBoolean(), any(AsyncCallback.class));
