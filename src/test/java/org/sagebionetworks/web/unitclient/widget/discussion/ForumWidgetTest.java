...
public class ForumWidgetTest {
  ...
  public void setUp() {
    ...
    when(
      mockSynapseProperties.getSynapseProperty(
        ForumWidget.DEFAULT_THREAD_ID_KEY
      )
    )
      .thenReturn(DEFAULT_THREAD_ID);
    AsyncMockStubber
      .callSuccessWith(mockDefaultDiscussionThreadBundle)
      .when(mockSynapseJavascriptClient)
      .getThread(eq(DEFAULT_THREAD_ID), any(AsyncCallback.class));
    when(mockDefaultDiscussionThreadBundle.getId())
      .thenReturn(DEFAULT_THREAD_ID);
    when(mockDefaultDiscussionThreadBundle.getMessageKey())
      .thenReturn(DEFAULT_THREAD_MESSAGE_KEY);
    ...
    forumWidget =
      new ForumWidget(
        ...
        mockAvailableThreadListWidget,
        mockDeletedThreadListWidget,
        mockNewDiscussionThreadModal,
        mockAuthController,
        mockGlobalApplicationState,
        mockDiscussionThreadWidget,
        mockSubscribeButtonWidget,
        mockDefaultThreadWidget,
        mockSubscribersWidget,
  ...
  public void testConstruction() {
    ...
    verify(mockSubscribeButtonWidget)
      .setOnUnsubscribeCallback(captor.capture());
    ...
    ArgumentCaptor<CallbackP> captorP = ArgumentCaptor.forClass(
      CallbackP.class
    );
    ...
    assertEquals(
      DEFAULT_THREAD_MESSAGE_KEY,
      defaultThreadBundle.getMessageKey()
    );
    ...
    verify(mockAvailableThreadListWidget)
      .setThreadIdClickedCallback(any(CallbackP.class));
    verify(mockDeletedThreadListWidget)
      .setThreadIdClickedCallback(any(CallbackP.class));
  }
  ...
  public void testSubscribeCallback() {
    verify(mockSubscribeButtonWidget)
      .setOnSubscribeCallback(callbackCaptor.capture());
  ...
  public void testUnsubscribeCallback() {
    verify(mockSubscribeButtonWidget)
      .setOnUnsubscribeCallback(callbackCaptor.capture());
    callbackCaptor.getValue().invoke();
    verify(mockAvailableThreadListWidget)
      .configure(
        any(),
  ...
  public void testDefaultBundleInit() {
    ...
    verify(mockSynapseJavascriptClient, atLeastOnce())
      .getThread(eq(DEFAULT_THREAD_ID), any(AsyncCallback.class));
    ...
    verify(mockDefaultThreadWidget, atLeastOnce())
      .setNewReplyContainerVisible(false);
  ...
  public void testGoBackToForum() {
    ...
    forumWidget.configure(
      ...
      mockParamChangeCallback,
      mockCallback
    );
    verify(mockSubscribeButtonWidget).clear();
    verify(mockSubscribeButtonWidget)
      .configure(SubscriptionObjectType.FORUM, forumId, mockActionMenuWidget);
    ...
    inOrder.verify(mockView).setDiscussionThreadVisible(false);
    ...
    verify(mockSynapseJavascriptClient)
      .getForumByProjectId(anyString(), any(AsyncCallback.class));
    verify(mockNewDiscussionThreadModal)
      .configure(anyString(), any(Callback.class));
    ...
    assertEquals(
      ...
      topicCaptor.getValue().getObjectType()
    );

    ArgumentCaptor<ParameterizedToken> captorToken = ArgumentCaptor.forClass(
      ParameterizedToken.class
    );
    verify(mockParamChangeCallback).invoke(captorToken.capture());
    assertEquals(
      ParameterizedToken.DEFAULT_TOKEN,
      captorToken.getValue().toString()
    );
    ...
    verify(mockAvailableThreadListWidget, never())
      .configure(
        anyString(),
        eq(canModerate),
        eq(moderatorIds),
  ...
  public void testGoBackToForumOnDelete() {
    ...
    ArgumentCaptor<Callback> onShowAllThreadsCallback = ArgumentCaptor.forClass(
      Callback.class
    );
  ...
  public void testConfigureForumSuccessWithModerator() {
    ...
    verify(mockActionMenuWidget)
      .setActionVisible(Action.SHOW_DELETED_THREADS, true);
  ...
  public void testConfigureSingleThreadSuccess() {
    ...
    verify(mockActionMenuWidget)
      .setActionVisible(Action.SHOW_DELETED_THREADS, false);
    ...
    verify(mockActionMenuWidget, never())
      .setActionVisible(eq(Action.DELETE_THREAD), anyBoolean());
    verify(mockActionMenuWidget, never())
      .setActionVisible(eq(Action.RESTORE_THREAD), anyBoolean());
    ...
    verify(mockView).configureDiscussionThread(eq(threadId), eq(limit));
    verify(mockSynapseJavascriptClient)
      .getForumByProjectId(anyString(), any(AsyncCallback.class));
    verify(mockSynapseJavascriptClient)
      .getThread(eq(threadId), any(AsyncCallback.class));
  ...
  public void testConfigureSingleThreadSingleReplySuccess() {
    ...
    int limit = 20;
    String areaToken =
      ForumWidget.THREAD_ID_KEY +
      "=" +
      threadId +
      "&" +
      ForumWidget.REPLY_ID_KEY +
      "=" +
      replyId;
    ParameterizedToken param = new ParameterizedToken(areaToken);
    forumWidget.configure(
    ...
    ArgumentCaptor<CallbackP> onReplyIdCallbackCaptor = ArgumentCaptor.forClass(
      CallbackP.class
    );
    verify(mockDiscussionThreadWidget)
      .setReplyIdCallback(onReplyIdCallbackCaptor.capture());
  ...
  public void testOnClickDeletedThreadButtonNotShown() {
    ...
    verify(mockDeletedThreadListWidget)
      .configure(
        ...
        eq(moderatorIds),
        eq((CallbackP) null),
        eq(DiscussionFilter.DELETED_ONLY)
      );
  }
  ...
  public void testCreateDefaultTest() {
    ...
    DiscussionThreadBundle defaultThread = forumWidget.createDefaultThread(
      bundle
    );
