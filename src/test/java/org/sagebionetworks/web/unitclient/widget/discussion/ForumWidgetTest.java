...
public class ForumWidgetTest {
	...
	public void setUp() {
		...
		when(mockSynapseProperties.getSynapseProperty(ForumWidget.DEFAULT_THREAD_ID_KEY))
				.thenReturn(DEFAULT_THREAD_ID);
		AsyncMockStubber.callSuccessWith(mockDefaultDiscussionThreadBundle)
	...
	public void testConstruction() {
		...
		verify(mockAvailableThreadListWidget).configure(anyString(), anyBoolean(), anySet(),
				captorP.capture(), any(DiscussionFilter.class));
		...
		verify(mockDefaultThreadWidget).configure(threadCaptor.capture(), eq(replyId),
				eq(isCurrentUserModerator), eq(moderatorIds), eq(actionMenu), eq(deleteCallback));
	...
	public void testSubscribeCallback() {
		...
		verify(mockAvailableThreadListWidget).configure(anyString(), anyBoolean(), anySet(),
				any(CallbackP.class), eq(DiscussionFilter.EXCLUDE_DELETED));
	...
	public void testDefaultBundleInit() {
		...
		verify(mockSynapseJavascriptClient, atLeastOnce()).getThread(eq(DEFAULT_THREAD_ID),
				any(AsyncCallback.class));
		assertNotNull(ForumWidget.defaultThreadBundle);
		verify(mockDefaultThreadWidget).configure(defaultThreadBundle, null, false,
	...
	public void testDefaultThreadBundleCached() {
		...
		forumWidget = new ForumWidget(mockView, mockStuAlert, mockDiscussionForumClient,
				...
				mockSubscribeButtonWidget, mockDefaultThreadWidget, mockSubscribersWidget,
				mockSynapseJavascriptClient, mockSynapseProperties);
		verify(mockSynapseJavascriptClient, never()).getThread(eq(DEFAULT_THREAD_ID),
				any(AsyncCallback.class));
	}
	...
	public void testGoBackToForum() {
		// configure the widget to show the full forum, click on a thread, and then go back to all
		// threads.
		String entityId = "syn1";
		String areaToken = "a=b&c=d";
		ParameterizedToken param = new ParameterizedToken(areaToken);
		forumWidget.configure(entityId, param, canModerate, mockActionMenuWidget,
				mockParamChangeCallback, mockCallback);
		...
		verify(mockAvailableThreadListWidget).configure(anyString(), eq(canModerate), eq(moderatorIds),
				any(CallbackP.class), eq(DiscussionFilter.EXCLUDE_DELETED));
		...
		ArgumentCaptor<ParameterizedToken> captorToken =
				ArgumentCaptor.forClass(ParameterizedToken.class);
		...
		verify(mockStuAlert, atLeastOnce()).clear();
		verify(mockSynapseJavascriptClient).getModerators(eq(mockForum.getId()), eq(MODERATOR_LIMIT),
				eq(0L), any(AsyncCallback.class));
		...
		verify(mockAvailableThreadListWidget, never()).configure(anyString(), eq(canModerate),
				eq(moderatorIds), any(CallbackP.class), eq(DiscussionFilter.EXCLUDE_DELETED));
	...
	public void testLoadForumSuccess() {
		...
		verify(mockStuAlert, atLeastOnce()).clear();
		verify(mockSynapseJavascriptClient).getForumByProjectId(anyString(), any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient).getModerators(anyString(), anyLong(), anyLong(),
				any(AsyncCallback.class));
	...
	public void testLoadForumFailure() {
		...
		verify(mockSynapseJavascriptClient).getForumByProjectId(anyString(), any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient, never()).getModerators(anyString(), anyLong(), anyLong(),
				any(AsyncCallback.class));
	...
	public void testConfigureForumSuccessWithModerator() {
		...
		verify(mockAvailableThreadListWidget).configure(anyString(), eq(canModerate), eq(moderatorIds),
				any(CallbackP.class), any(DiscussionFilter.class));

		// verify action menu configuration
		verify(mockActionMenuWidget).setActionListener(eq(Action.CREATE_THREAD),
		...
		verify(mockActionMenuWidget).setActionListener(eq(Action.SHOW_DELETED_THREADS),
				actionListenerCaptor.capture());
	...
	public void testConfigureSingleThreadSuccess() {
		...
		verify(mockDiscussionThreadWidget).configure(eq(mockDiscussionThreadBundle), eq(replyId),
				eq(canModerate), eq(moderatorIds), eq(mockActionMenuWidget),
				onShowAllThreadsCallback.capture());
		verify(mockAvailableThreadListWidget, never()).configure(anyString(), anyBoolean(), anySet(),
				any(CallbackP.class), any(DiscussionFilter.class));
	...
	public void testConfigureSingleThreadSingleReplySuccess() {
		...
		String replyId = "008";
		String areaToken =
				ForumWidget.THREAD_ID_KEY + "=" + threadId + "&" + ForumWidget.REPLY_ID_KEY + "=" + replyId;
		ParameterizedToken param = new ParameterizedToken(areaToken);
		forumWidget.configure(entityId, param, canModerate, mockActionMenuWidget,
				mockParamChangeCallback, mockCallback);
	...
	public void testConfigureSingleThreadFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseJavascriptClient).getThread(anyString(),
				any(AsyncCallback.class));
	...
	public void testOnClickDeletedThreadButtonNotShown() {
		...
		forumWidget.onClickDeletedThreadCommand();
		verify(mockView).setDeletedThreadListVisible(true);
		verify(mockDeletedThreadListWidget).configure(anyString(), eq(canModerate), eq(moderatorIds),
				eq((CallbackP) null), eq(DiscussionFilter.DELETED_ONLY));
	}

	@Test
	public void testCreateDefaultTest() {
		DiscussionThreadBundle bundle =
				DiscussionTestUtils.createThreadBundle("1", "2", "3", "title", "messageKey", false, false,
						false, "4", new Date(System.currentTimeMillis()), new Date(System.currentTimeMillis()),
						new Date(System.currentTimeMillis()), "etag", Arrays.asList("5"), 2L, 8L);
	...
	public void testLoadModeratorsFail() {
		...
		verify(mockStuAlert).clear();
		verify(mockSynapseJavascriptClient).getModerators(eq(forumId), eq(MODERATOR_LIMIT), eq(0L),
				any(AsyncCallback.class));
	...
	public void testLoadModeratorsTwoPage() {
		...
		verify(mockSynapseJavascriptClient, times(2)).getModerators(eq(forumId), eq(MODERATOR_LIMIT),
	...
	public void testOnSortReplies() {
		...
		forumWidget.configure("syn123", param, canModerate, mockActionMenuWidget,
				mockParamChangeCallback, mockCallback);
