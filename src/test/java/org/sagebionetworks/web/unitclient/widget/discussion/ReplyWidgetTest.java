...
public class ReplyWidgetTest {
	...
	public void before() {
		...
		replyWidget = new ReplyWidget(mockView, mockAuthorWidget, mockDateTimeUtils, mockSynAlert,
				mockRequestBuilder, mockDiscussionForumClientAsync, mockAuthController, mockEditReplyModal,
				mockMarkdownWidget, mockGwt, mockCopyTextModal, mockSynapseJavascriptClient,
				mockClientCache, mockPopupUtils);
	...
	public void testConfigureMessageFailToGetMessageURL() throws RequestException {
		...
		DiscussionReplyBundle bundle =
				createReplyBundle("123", "1", "messageKey", new Date(), isDeleted, CREATED_BY, isEdited);
	...
	public void testConfigureMessageFailToGetMessage() throws RequestException {
		...
		String url = "url";
		AsyncMockStubber.callSuccessWith(url).when(mockSynapseJavascriptClient).getReplyUrl(anyString(),
	...
	public void testDeleteReplySuccess() throws RequestException {
		...
		AsyncMockStubber.callSuccessWith((Void) null).when(mockDiscussionForumClientAsync)
				.markReplyAsDeleted(anyString(), any(AsyncCallback.class));
	...
	public void testReconfigureSuccess() throws RequestException {
		...
		RequestBuilderMockStubber.callOnResponseReceived(null, mockResponse).when(mockRequestBuilder)
				.sendRequest(anyString(), any(RequestCallback.class));
		replyWidget.configure(bundle, canModerate, moderatorIds, mockDeleteCallback, isThreadDeleted);
		AsyncMockStubber.callSuccessWith(bundle).when(mockSynapseJavascriptClient).getReply(anyString(),
				any(AsyncCallback.class));
	...
	public void testReplyClicked() {
		...
		verify(mockCopyTextModal)
				.setText("https://www.synapse.org/#!Synapse:syn007/discussion/threadId=321&replyId=123");
		verify(mockCopyTextModal).show();
	}

	private DiscussionReplyBundle createReplyBundle(String replyId, String author, String messageKey,
			Date createdOn, Boolean isDeleted, String createdBy, Boolean isEdited) {
