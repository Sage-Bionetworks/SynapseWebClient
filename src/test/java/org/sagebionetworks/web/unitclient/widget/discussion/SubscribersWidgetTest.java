...
public class SubscribersWidgetTest {
	...
	public void before() {
		...
		widget = new SubscribersWidget(mockView, mockGinInjector, mockSynAlert,
				mockLoadMoreWidgetContainer, mockSynapseJavascriptClient);
		AsyncMockStubber.callSuccessWith(mockSubscriberPagedResults).when(mockSynapseJavascriptClient)
				.getSubscribers(any(Topic.class), anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(TEST_SUBSCRIBER_COUNT).when(mockSynapseJavascriptClient)
