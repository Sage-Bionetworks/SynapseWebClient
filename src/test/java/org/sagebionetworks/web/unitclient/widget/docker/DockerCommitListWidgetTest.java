...
public class DockerCommitListWidgetTest {
	...
	public void before() {
		...
		dockerCommitListWidget = new DockerCommitListWidget(mockView, mockJsClient, mockSynAlert,
				mockCommitsContainer, mockGinInjector, mockGwtWrapper);

		entityId = "syn123";
		dockerCommitList = new ArrayList<DockerCommit>();
		AsyncMockStubber.callSuccessWith(dockerCommitList).when(mockJsClient).getDockerTaggedCommits(
				anyString(), anyLong(), anyLong(), any(DockerCommitSortBy.class), anyBoolean(),
	...
	public void testConstructor() {
		...
		verify(mockJsClient).getDockerTaggedCommits(anyString(), anyLong(), anyLong(),
				any(DockerCommitSortBy.class), anyBoolean(), any(AsyncCallback.class));
	}
	...
	public void testConfigure() {
		...
		verify(mockSynAlert).clear();
		verify(mockJsClient).getDockerTaggedCommits(eq(entityId), anyLong(), anyLong(),
				any(DockerCommitSortBy.class), anyBoolean(), any(AsyncCallback.class));
	}
	...
	public void testLoadMoreFailure() {
		...
		AsyncMockStubber.callFailureWith(exception).when(mockJsClient).getDockerTaggedCommits(
				anyString(), anyLong(), anyLong(), any(DockerCommitSortBy.class), anyBoolean(),
				any(AsyncCallback.class));
		dockerCommitListWidget.configure(entityId, withRadio);
		verify(mockCommitsContainer).clear();
