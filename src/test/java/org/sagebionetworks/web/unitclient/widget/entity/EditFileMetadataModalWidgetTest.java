...
public class EditFileMetadataModalWidgetTest {
  ...
  public void before() {
    ...
    when(mockFileEntity.getDataFileHandleId())
      .thenReturn(OLD_DATA_FILE_HANDLE_ID);
    ...
    widget =
      new EditFileMetadataModalWidgetImpl(
        mockView,
        mockSynapseClient,
        mockJsClient
      );
  ...
  public void testNullContentType() {
    ...
    verify(mockSynapseClient)
      .updateFileEntity(
        ...
        any(FileHandleCopyRequest.class),
  ...
  public void testUpdateFileEntityHandle() {
    ...
    verify(mockView).hide();
    verify(mockCallback).invoke();
    ArgumentCaptor<FileHandleCopyRequest> captor = ArgumentCaptor.forClass(
      FileHandleCopyRequest.class
    );
    verify(mockSynapseClient)
      .updateFileEntity(
        any(FileEntity.class),
  ...
  public void testUpdateFileEntityHandleFailed() {
    Exception error = new Exception(
      "something wrong with file name, content type, or entity name"
    );
    widget.configure(mockFileEntity, mockFileHandle, mockCallback);
    AsyncMockStubber
      .callFailureWith(error)
