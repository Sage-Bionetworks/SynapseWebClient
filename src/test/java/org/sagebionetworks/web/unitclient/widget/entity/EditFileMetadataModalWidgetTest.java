...
public class EditFileMetadataModalWidgetTest {
	...
	public void testNullFileName() {
		...
		widget.onPrimary();
		verify(mockView)
				.showError(EditFileMetadataModalWidgetImpl.FILE_NAME_MUST_INCLUDE_AT_LEAST_ONE_CHARACTER);
		verifyZeroInteractions(mockSynapseClient);
		// should only be called on success
		verify(mockCallback, never()).invoke();
	}
	...
	public void testNullContentType() {
		...
		verify(mockSynapseClient).updateFileEntity(any(FileEntity.class),
				any(FileHandleCopyRequest.class), any(AsyncCallback.class));
	...
	public void testUpdateEntity() {
		...
		widget.configure(mockFileEntity, mockFileHandle, mockCallback);
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).updateEntity(any(Entity.class),
	...
	public void testUpdateFileEntityHandle() {
		...
		widget.configure(mockFileEntity, mockFileHandle, mockCallback);
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).updateFileEntity(
				any(FileEntity.class), any(FileHandleCopyRequest.class), any(AsyncCallback.class));
		// save button
		widget.onPrimary();
		...
		verify(mockView).hide();
		verify(mockCallback).invoke();
		ArgumentCaptor<FileHandleCopyRequest> captor =
				ArgumentCaptor.forClass(FileHandleCopyRequest.class);
		verify(mockSynapseClient).updateFileEntity(any(FileEntity.class), captor.capture(),
				any(AsyncCallback.class));
	...
	public void testUpdateEntityFailed() {
		...
		widget.configure(mockFileEntity, mockFileHandle, mockCallback);
		AsyncMockStubber.callFailureWith(error).when(mockJsClient).updateEntity(any(Entity.class),
				anyString(), anyBoolean(), any(AsyncCallback.class));
		// save button
		widget.onPrimary();
		verify(mockView).setLoading(true);
	...
	public void testUpdateFileEntityHandleFailed() {
		...
		widget.configure(mockFileEntity, mockFileHandle, mockCallback);
		AsyncMockStubber.callFailureWith(error).when(mockSynapseClient).updateFileEntity(
				any(FileEntity.class), any(FileHandleCopyRequest.class), any(AsyncCallback.class));
		// save button
		widget.onPrimary();
		verify(mockView).setLoading(true);
