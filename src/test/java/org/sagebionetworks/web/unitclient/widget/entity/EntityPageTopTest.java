...
public class EntityPageTopTest {
	...
	public void setUp() {
		...
		pageTop = new EntityPageTop(mockView, mockSynapseClientAsync, mockTabs, mockProjectMetadata,
				mockWikiTab, mockFilesTab, mockTablesTab, mockChallengeTab, mockDiscussionTab,
				mockDockerTab, mockProjectActionController, mockProjectActionMenuWidget,
				mockEntityActionController, mockActionMenuWidget, mockCookies, mockSynapseJavascriptClient,
				mockGlobalApplicationState, entityId2BundleCache, mockEventBus);
		AsyncMockStubber.callSuccessWith(mockProjectBundle).when(mockSynapseJavascriptClient)
				.getEntityBundleFromCache(anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockEntityBundle).when(mockSynapseJavascriptClient)
				.getEntityBundleForVersion(anyString(), anyLong(), any(EntityBundleRequest.class),
		...
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseJavascriptClient).isWiki(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseJavascriptClient)
				.isFileOrFolder(anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseJavascriptClient).isTable(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseClientAsync).isChallenge(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseJavascriptClient).isDocker(anyString(),
				any(AsyncCallback.class));
	...
	public void testConfigureWithProject() {
		...
		// Once to show the active tab, and once after configuration so that the place is pushed into
		// the history.
		...
		verify(mockWikiTab).configure(eq(projectEntityId), eq(projectName), eq(projectWikiId),
				eq(canEdit), any(WikiPageWidget.Callback.class), eq(mockActionMenuWidget));

		// verify it never asks for the project bundle (SWC-4462)
		verify(mockSynapseJavascriptClient, never()).getEntityBundleFromCache(eq(projectEntityId),
				any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient, never()).getEntityBundleForVersion(anyString(), anyLong(),
				any(EntityBundleRequest.class), any(AsyncCallback.class));

		// entity area for the project settings doesn't apply (so it's set to null).
		EntityArea projectSettingsEntityArea = null;
		verify(mockProjectActionController).configure(mockProjectActionMenuWidget, mockProjectBundle,
				true, projectWikiId, projectSettingsEntityArea);
		verify(mockProjectActionMenuWidget).setToolsButtonIcon(EntityPageTop.PROJECT_SETTINGS,
				IconType.GEAR);
		...
		verify(mockDiscussionTab, never()).configure(projectEntityId, projectName, areaToken,
				canModerate, mockActionMenuWidget);
	...
	public void testConfigureWithProjectWikiToken() {
		...
		Long versionNumber = null;
		pageTop.configure(mockProjectBundle, versionNumber, mockProjectHeader, area, areaToken);
		verify(mockEntityActionController).configure(eq(mockActionMenuWidget), eq(mockProjectBundle),
				eq(true), eq(areaToken), eq(area));
	}
	...
	public void testConfigureWithFile() {
		...
		verify(mockWikiTab, never()).configure(eq(projectEntityId), eq(projectName), eq(projectWikiId),
				eq(canEdit), any(WikiPageWidget.Callback.class), any(ActionMenuWidget.class));
		verify(mockTablesTab, never()).setProject(projectEntityId, mockProjectBundle, null);
		...
		Synapse expectedPlace =
				new Synapse(projectEntityId, projectVersionNumber, EntityArea.FILES, areaToken);
	...
	public void testConfigureWithFileAndFailureToLoadProject() {
		...
		AsyncMockStubber.callFailureWith(projectLoadError).when(mockSynapseJavascriptClient)
				.getEntityBundleFromCache(anyString(), any(AsyncCallback.class));
		Synapse.EntityArea area = null;
		String areaToken = null;
		Long versionNumber = 5L;
		...
		String wikiId = null;
		canEdit = false;
		verify(mockWikiTab).configure(eq(projectEntityId), eq(projectName), eq(wikiId), eq(canEdit),
				any(WikiPageWidget.Callback.class), any(ActionMenuWidget.class));
	...
	public void testConfigureWithTable() {
		...
		verify(mockWikiTab).configure(eq(projectEntityId), eq(projectName), eq(projectWikiId),
				eq(canEdit), any(WikiPageWidget.Callback.class), eq(mockActionMenuWidget));
		verify(mockFilesTab).setProject(projectEntityId, mockProjectBundle, null);
		verify(mockFilesTab).configure(mockProjectBundle, null, mockActionMenuWidget);
		verify(mockChallengeTab).configure(projectEntityId, projectName);
		verify(mockDiscussionTab).configure(projectEntityId, projectName, null, canModerate,
				mockActionMenuWidget);
	...
	public void testConfigureWithDocker() {
		...
		verify(mockFilesTab).configure(mockProjectBundle, versionNumber, mockActionMenuWidget);
		verify(mockTablesTab).setProject(projectEntityId, mockProjectBundle, null);
		verify(mockTablesTab).configure(mockProjectBundle, versionNumber, null, mockActionMenuWidget);
		verify(mockChallengeTab).configure(projectEntityId, projectName);
		verify(mockDiscussionTab).configure(projectEntityId, projectName, null, canModerate,
				mockActionMenuWidget);
	}
	...
	public void testConfigureProjectInvalidWikiId() {
		...
		verify(mockWikiTab).configure(eq(projectEntityId), eq(projectName), eq(invalidWikiId),
				eq(canEdit), wikiCallbackCaptor.capture(), eq(mockActionMenuWidget));
	...
	public void testConfigureProjectNoWiki() {
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseJavascriptClient).isWiki(anyString(),
				any(AsyncCallback.class));
		...
		pageTop.configure(mockProjectBundle, versionNumber, mockProjectHeader, area, invalidWikiId);
		verify(mockWikiTab, never()).configure(anyString(), anyString(), anyString(), anyBoolean(),
				any(WikiPageWidget.Callback.class), any(ActionMenuWidget.class));
	...
	public void testContentMixtureOfTabsShownCanEdit() {
		...
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseJavascriptClient).isTable(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseClientAsync).isChallenge(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseJavascriptClient).isDocker(anyString(),
				any(AsyncCallback.class));

		pageTop.configure(mockProjectBundle, versionNumber, mockProjectHeader, area, areaToken);

		verify(mockSynapseJavascriptClient, never()).isWiki(anyString(), any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient, never()).isFileOrFolder(anyString(),
				any(AsyncCallback.class));
	...
	public void testContentOneTabShown() {
		...
		String areaToken = null;
		Long versionNumber = null;
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseJavascriptClient)
				.isFileOrFolder(anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseJavascriptClient).isTable(anyString(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseClientAsync).isChallenge(anyString(),
	...
	public void testOnChangeSynapsePlaceSameEntityId() {
		...
		verify(mockEntityActionController).configure(eq(mockActionMenuWidget), eq(mockProjectBundle),
				eq(true), eq(areaToken), eq(EntityArea.DISCUSSION));
	...
	public void testOnChangeSynapsePlaceDifferentEntityIdSameProject() {
		...
		// verify we did not change place, but instead reconfigured for target entity under the same
		// project
		verify(mockPlaceChanger, never()).goTo(newPlace);
		verify(mockEntityActionController, times(2)).configure(eq(mockActionMenuWidget),
				eq(mockProjectBundle), eq(true), eq(areaToken), eq(EntityArea.WIKI));
	}
}
