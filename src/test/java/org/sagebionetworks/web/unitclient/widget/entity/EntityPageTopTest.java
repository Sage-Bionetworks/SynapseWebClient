...
public class EntityPageTopTest {
  ...
  @Mock
  LinkWithIconImpl mockLinkWithIcon;
  ...
  public void testSelectEntity() {
    ...
    ArgumentCaptor<CallbackP> selectEntityCallback = ArgumentCaptor.forClass(
      CallbackP.class
    );

    verify(mockFilesTab)
      .setEntitySelectedCallback(selectEntityCallback.capture());
  ...
  public void testConfigureWithProject() {
    ...
    verify(mockSynapseJavascriptClient, never())
      .getEntityBundleFromCache(eq(projectEntityId), any(AsyncCallback.class));
    verify(mockSynapseJavascriptClient, never())
    ...
    verify(mockProjectActionController)
      .configure(
        mockProjectActionMenuWidget,
        mockProjectBundle,
        true,
    ...
    verify(mockTablesTab, never())
      .setProject(projectEntityId, mockProjectBundle, null);
    verify(mockTablesTab, never())
      .configure(mockProjectBundle, versionNumber, areaToken);
    verify(mockDatasetsTab, never())
      .setProject(projectEntityId, mockProjectBundle, null);
    verify(mockDatasetsTab, never())
      .configure(mockProjectBundle, versionNumber, areaToken);
    verify(mockChallengeTab, never())
      .configure(projectEntityId, projectName, mockProjectBundle);
    verify(mockDiscussionTab, never())
      .configure(
        projectEntityId,
        projectName,
        mockProjectBundle,
  ...
  private void clickAllTabs() {
    ...
    ArgumentCaptor<CallbackP> tabCaptor = ArgumentCaptor.forClass(
      CallbackP.class
    );
  ...
  public void testConfigureWithFile() {
    ...
    verify(mockTablesTab, never())
      .setProject(projectEntityId, mockProjectBundle, null);
    verify(mockTablesTab, never())
      .configure(any(EntityBundle.class), anyLong(), eq(areaToken));
    verify(mockDatasetsTab, never())
      .setProject(projectEntityId, mockProjectBundle, null);
    verify(mockDatasetsTab, never())
      .configure(any(EntityBundle.class), anyLong(), eq(areaToken));
    ...
    verify(mockTablesInnerTab)
      .setEntityNameAndPlace(projectName, expectedPlace);
    expectedPlace.setArea(EntityArea.DATASETS);
    verify(mockDatasetsInnerTab)
      .setEntityNameAndPlace(projectName, expectedPlace);
    expectedPlace.setArea(EntityArea.CHALLENGE);
    verify(mockChallengeInnerTab)
      .setEntityNameAndPlace(projectName, expectedPlace);
    expectedPlace.setArea(EntityArea.DISCUSSION);
    verify(mockDiscussionInnerTab)
      .setEntityNameAndPlace(projectName, expectedPlace);
    expectedPlace.setArea(EntityArea.DOCKER);
    verify(mockDockerInnerTab)
      .setEntityNameAndPlace(projectName, expectedPlace);
  ...
  public void testConfigureWithFileAndFailureToLoadProject() {
    ...
    verify(mockProjectMetadata, Mockito.never())
      .configure(mockProjectBundle, null, null);
    EntityBundle expectedProjectEntityBundle = null;
    verify(mockFilesTab)
      .setProject(
        projectEntityId,
        expectedProjectEntityBundle,
        projectLoadError
      );
    verify(mockFilesTab).configure(mockEntityBundle, versionNumber);
  ...
  public void testConfigureWithDataset() {
    ...
    verify(mockDatasetsTab)
      .setProject(projectEntityId, mockProjectBundle, null);
    verify(mockDatasetsTab)
      .configure(mockEntityBundle, versionNumber, areaToken);
