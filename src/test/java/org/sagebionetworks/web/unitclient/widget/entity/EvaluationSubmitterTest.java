...
public class EvaluationSubmitterTest {
  ...
  public void setupTeamSubmissionEligibility() {
    ...
    testTeamSubmissionEligibility.setEligibilityStateHash(
      ELIGIBILITY_STATE_HASH
    );
  ...
  @Test
  public void testSubmitToEvaluation()
    ...
    assertTrue(
      receiptMessage.contains(EVALUATION_1_SUBMISSION_RECEIPT_MESSAGE)
    );
  }
  ...
  public void testSubmitToEvaluationsWithSubmissionName()
    ...
    // should invoke submission directly without terms of use
    ArgumentCaptor<Submission> captor = ArgumentCaptor.forClass(
      Submission.class
    );
    verify(mockChallengeClient)
      .createIndividualSubmission(
  ...
  @Test
  public void testContributorsListMemberEligibility()
    ...
    verify(mockView)
      .addInEligibleContributor(eq(inEligibleMemberId.toString()), anyString());
    assertEquals(
      ELIGIBILITY_STATE_HASH.toString(),
      submitter.getSelectedTeamMemberStateHash()
    );
  }
  ...
  public void testConfigureWithDockerEntity() {
    ...
    ArgumentCaptor<Callback> emptyCallbackCaptor = ArgumentCaptor.forClass(
      Callback.class
    );
    verify(mockDockerCommitListWidget)
      .setEmptyListCallback(emptyCallbackCaptor.capture());
