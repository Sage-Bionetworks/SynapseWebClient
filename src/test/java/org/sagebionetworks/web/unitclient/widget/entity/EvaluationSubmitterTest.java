...
public class EvaluationSubmitterTest {
	...
	private static final String EVALUATION_2_SUBMISSION_RECEIPT_MESSAGE =
			"Evaluation 2 Submission Receipt Message";
	private static final String EVALUATION_1_SUBMISSION_RECEIPT_MESSAGE =
			"Evaluation 1 Submission Receipt Message";
	...
	public void setup() throws RestServiceException, JSONObjectAdapterException {
		...
		submitter = new EvaluationSubmitter(mockView, mockSynapseJavascriptClient,
				mockGlobalApplicationState, mockAuthenticationController, mockChallengeClient,
				mockGWTWrapper, mockInjector, mockDockerCommitListWidget);
		...
		AsyncMockStubber.callSuccessWith(returnSubmission).when(mockChallengeClient)
				.createIndividualSubmission(any(Submission.class), anyString(), anyString(),
						any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(returnSubmission).when(mockChallengeClient)
				.createTeamSubmission(any(Submission.class), anyString(), anyString(), anyString(),
						any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(availableEvaluations).when(mockChallengeClient)
				.getAvailableEvaluations(any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(entity).when(mockSynapseJavascriptClient)
	...
	public void setupTeamSubmissionEligibility() {
		...
		AsyncMockStubber.callSuccessWith(testTeamSubmissionEligibility).when(mockChallengeClient)
				.getTeamSubmissionEligibility(anyString(), anyString(), any(AsyncCallback.class));
	}
	...
	public void testSubmitToEvaluation() throws RestServiceException, JSONObjectAdapterException {
		...
		verify(mockChallengeClient).createIndividualSubmission(any(Submission.class), anyString(),
				eq(HOST_PAGE_URL), any(AsyncCallback.class));
	...
	@Test
	public void testSubmitToEvaluationsWithSubmissionName()
			throws RestServiceException, JSONObjectAdapterException {
		String submissionName = "my custom submission name";
		requirements.setTotalNumberOfResults(0);
		submitter.configure(entity, null, null);

		// also set contributors, and verify on individual submission that this is not set in the
		// submission
		// add eligible member
		...
		verify(mockChallengeClient).createIndividualSubmission(captor.capture(), anyString(),
				eq(HOST_PAGE_URL), any(AsyncCallback.class));
	...
	@Test
	public void testSubmitToEvaluationsFailure()
			throws RestServiceException, JSONObjectAdapterException {
		submitter.configure(entity, null, null);
		reset(mockView);

		AsyncMockStubber.callFailureWith(new Exception("unhandled exception")).when(mockChallengeClient)
				.createIndividualSubmission(any(Submission.class), anyString(), anyString(),
						any(AsyncCallback.class));
	...
	@Test
	public void testShowSingleAvailableEvaluation()
			throws RestServiceException, JSONObjectAdapterException {
		PaginatedResults<Evaluation> availableEvaluations = new PaginatedResults<Evaluation>();
	...
	@Test
	public void testShowAvailableEvaluations()
			throws RestServiceException, JSONObjectAdapterException {
		PaginatedResults<Evaluation> availableEvaluations = new PaginatedResults<Evaluation>();
		availableEvaluations.setTotalNumberOfResults(2);
		List<Evaluation> evaluationList = new ArrayList<Evaluation>();
	...
	@Test
	public void testShowAvailableEvaluationsNoResults()
			throws RestServiceException, JSONObjectAdapterException {
	...
	@Test
	public void testShowAvailableEvaluationsFailure1()
			throws RestServiceException, JSONObjectAdapterException {
		Exception caught = new ForbiddenException("this is forbidden");
		AsyncMockStubber.callFailureWith(caught).when(mockChallengeClient)
				.getAvailableEvaluations(any(AsyncCallback.class));
		submitter.configure(entity, null, null);
		verify(mockChallengeClient).getAvailableEvaluations(any(AsyncCallback.class));
		// no evaluations to join error message
	...
	public void testQueryForChallengeAndTeams() throws RestServiceException {
		...
		AsyncMockStubber.callSuccessWith(testChallenge).when(mockChallengeClient)
				.getChallengeForProject(anyString(), any(AsyncCallback.class));
		Team testTeam = new Team();
		testTeam.setId("80");
		testTeam.setName("test team");
		List<Team> submissionTeams = Collections.singletonList(testTeam);
		AsyncMockStubber.callSuccessWith(submissionTeams).when(mockChallengeClient)
		...
		verify(mockChallengeClient).createTeamSubmission(any(Submission.class), anyString(),
				anyString(), eq(HOST_PAGE_URL), any(AsyncCallback.class));
	}
	...
	public void testQueryForChallengeForbidden() throws RestServiceException {
		...
		AsyncMockStubber.callFailureWith(new ForbiddenException()).when(mockChallengeClient)
				.getChallengeForProject(anyString(), any(AsyncCallback.class));
	...
	public void testSubmitDockerRepo() throws RestServiceException {
		...
		submitter.onDoneClicked();
		verify(mockView).setSubmitButtonLoading();
		verify(mockChallengeClient).createTeamSubmission(captor.capture(), anyString(), anyString(),
				eq(HOST_PAGE_URL), any(AsyncCallback.class));
