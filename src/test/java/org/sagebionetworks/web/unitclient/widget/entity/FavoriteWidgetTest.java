...
public class FavoriteWidgetTest {
  ...
  public void testSetIsFavorite() throws Exception {
    ...
    AsyncMockStubber
      .callSuccessWith(new EntityHeader())
    ...
    verify(mockSynapseClient)
      .addFavorite(eq(entityId), any(AsyncCallback.class));
  ...
  public void testSetIsFavoriteUnset() throws Exception {
    ...
    AsyncMockStubber
      ...
      .when(mockSynapseClient)
      .removeFavorite(anyString(), any(AsyncCallback.class));
    ...
    verify(mockSynapseClient)
      .removeFavorite(eq(entityId), any(AsyncCallback.class));
    verify(mockSynapseJavascriptClient).getFavorites(any(AsyncCallback.class));
    verify(mockGlobalApplicationState).setFavorites(results);
    verify(mockView).setLoadingVisible(false);
  }
  ...
  public void testUpdateIsFavoriteViewNotAFavorite() {
    ...
    when(mockGlobalApplicationState.getFavorites())
      .thenReturn(new ArrayList<EntityHeader>());
  ...
  public void testFavoriteAnynomous() {
    ...
    verify(mockView, Mockito.never())
      .setNotFavoriteVisible(Mockito.anyBoolean());
