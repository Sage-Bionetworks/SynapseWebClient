...
public class HtmlPreviewWidgetTest {
	...
	public static final String HTML =
			"<html><body><img src=a onerror=\"javascript:alert('running my js')\" /><p>hello</p></body></html>";
	...
	public void before() throws Exception {
		...
		previewWidget =
				new HtmlPreviewWidget(mockView, mockPresignedURLAsyncHandler, mockSynapseJSNIUtils,
						mockRequestBuilder, mockSynapseAlert, mockSynapseClient, mockPopupUtils, mockGWT);
		...
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseClient)
	...
	public void testGetFileResultFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockPresignedURLAsyncHandler)
				.getFileResult(any(FileHandleAssociation.class), any(AsyncCallback.class));
	...
	public void testRenderHtmlUntrustedButSafe() {
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseClient)
				.isUserAllowedToRenderHTML(anyString(), any(AsyncCallback.class));
	...
	public void testOnShowFullContent() {
		...
		verify(mockPopupUtils).showConfirmDialog(eq(""),
				eq(HtmlPreviewWidget.CONFIRM_OPEN_HTML_MESSAGE), callbackCaptor.capture());
	...
	public void testMaxFileSizeExceeded() {
		when(mockFileHandle.getContentSize())
				.thenReturn(new Double(HtmlPreviewWidget.MAX_HTML_FILE_SIZE + 10).longValue());
