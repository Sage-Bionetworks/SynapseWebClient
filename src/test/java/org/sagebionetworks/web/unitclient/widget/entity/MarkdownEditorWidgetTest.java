...
public class MarkdownEditorWidgetTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		presenter = new MarkdownEditorWidget(mockView, mockSynapseClient, mockCookies, mockGwt,
				mockEditDescriptor, mockWidgetRegistrar, mockUserSelector, mockGinInjector);
	...
	public void testUserSelectorModalShownHandler() {
		ArgumentCaptor<ModalShownHandler> callbackCaptor =
				ArgumentCaptor.forClass(ModalShownHandler.class);
	...
	public void testGetFormattingGuide() throws Exception {
		...
		WikiPageKey formattingGuideWikiKey =
				new WikiPageKey("syn1234", ObjectType.ENTITY.toString(), null);
		testHelpPagesMap.put(WebConstants.FORMATTING_GUIDE, formattingGuideWikiKey);
		AsyncMockStubber.callSuccessWith(testHelpPagesMap).when(mockSynapseClient)
	...
	public void testGetFormattingGuideFailure() throws Exception {
		reset(mockSynapseClient);
		AsyncMockStubber.callFailureWith(new Exception()).when(mockSynapseClient)
				.getPageNameToWikiKeyMap(any(AsyncCallback.class));
		CallbackP<WikiPageKey> mockCallback = mock(CallbackP.class);
		presenter.getFormattingGuideWikiKey(mockCallback);
		// service was called
	...
	public void testResizeMarkdown() {
		...
		presenter.resizeMarkdownTextArea();
		verify(mockView)
				.setMarkdownTextAreaHeight(1000 - MarkdownEditorWidget.OTHER_EDITOR_COMPONENTS_HEIGHT);
	}
	...
	public void testEditWidgetNoneSelected() {
		...
		verify(mockEditDescriptor, Mockito.never()).editExisting(any(WikiPageKey.class), anyString(),
				any(Map.class));
	}
	...
	public void testEditWidget() {
		...
		when(mockWidgetRegistrar.getWidgetDescriptor(widgetDefinition))
				.thenReturn(mockWidgetDescriptor);
		...
		// verify that a widget descriptor update handler is added, and when fired it removes the old
		// widget markdown (to replace it with the updated value)
		ArgumentCaptor<WidgetDescriptorUpdatedHandler> captor =
				ArgumentCaptor.forClass(WidgetDescriptorUpdatedHandler.class);
		verify(mockEditDescriptor).addWidgetDescriptorUpdatedHandler(captor.capture());
		WidgetDescriptorUpdatedEvent event = new WidgetDescriptorUpdatedEvent();
	...
	public void testHandleCommandTeamMemberCount() {
		...
		verify(mockEditDescriptor).editNew(eq(wikiPageKey),
				eq(WidgetConstants.TEAM_MEMBER_COUNT_CONTENT_TYPE));
	}
	...
	public void testHandleCommandTeamMembers() {
		...
		verify(mockEditDescriptor).editNew(eq(wikiPageKey),
				eq(WidgetConstants.TEAM_MEMBERS_CONTENT_TYPE));
	}
}
