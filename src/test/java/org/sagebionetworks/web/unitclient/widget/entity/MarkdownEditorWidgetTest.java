...
public class MarkdownEditorWidgetTest {
  ...
  public void testUserSelectorConfigure() {
    ...
    ArgumentCaptor<CallbackP> callbackCaptor = ArgumentCaptor.forClass(
      CallbackP.class
    );
  ...
  public void testUserSelectorModalShownHandler() {
    ArgumentCaptor<ModalShownHandler> callbackCaptor = ArgumentCaptor.forClass(
      ModalShownHandler.class
    );
  ...
  public void testGetFormattingGuide() throws Exception {
    ...
    ArgumentCaptor<WikiPageKey> wikiKeyCaptor = ArgumentCaptor.forClass(
      WikiPageKey.class
    );
  ...
  public void testEditWidgetNoneSelected() {
    ...
    verify(mockEditDescriptor, Mockito.never())
      .editExisting(any(WikiPageKey.class), anyString(), any(Map.class));
  }
  ...
  public void testEditWidget() {
    ...
    when(mockWidgetRegistrar.getWidgetContentType(widgetDefinition))
    ...
    verify(mockView)
      .setSelectionRange(startWidgetIndex, widgetDefinition.length());

    verify(mockEditDescriptor)
      .editExisting(wikiPageKey, contentType, mockWidgetDescriptor);
  ...
  private void setupSurroundText() {
    when(mockView.getSelectionLength())
      .thenReturn(surroundTextMarkdown.length());
  ...
  public void testHandleCommandWikiFilesPreview() {
    ...
    assertTrue(
      getNewMarkdown().contains(WidgetConstants.WIKI_FILES_PREVIEW_CONTENT_TYPE)
    );
  }
  ...
  public void testHandleCommandTutorialWizard() {
    ...
    assertTrue(
      getNewMarkdown().contains(WidgetConstants.TUTORIAL_WIZARD_CONTENT_TYPE)
    );
  }
