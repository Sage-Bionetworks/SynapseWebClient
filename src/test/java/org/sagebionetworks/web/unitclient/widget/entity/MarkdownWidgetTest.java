...
public class MarkdownWidgetTest {
	...
	public void setup() {
		...
		when(mockWidgetRegistrar.getWidgetRendererForWidgetDescriptor(any(WikiPageKey.class),
				anyString(), anyMap(), any(Callback.class), any(Long.class)))
		...
		presenter = new MarkdownWidget(mockSynapseJavascriptClient, mockSynapseJSNIUtils,
				mockWidgetRegistrar, mockCookies, mockResourceLoader, mockGwt, mockInjector, mockView,
				mockSynAlert, mockMarkdownIt);
	}
	...
	public void testConfigureSuccess() {
		...
		// only the first getElementById called by each getElementById finds its target so it doesn't
		// look forever but still can be verified
		when(mockView.getElementById(WidgetConstants.MARKDOWN_TABLE_ID_PREFIX + "0"))
				.thenReturn(mockElementWrapper);
		when(mockView.getElementById(Mockito.contains(WidgetConstants.DIV_ID_MATHJAX_PREFIX + "0")))
				.thenReturn(mockElementWrapper);
		when(mockView.getElementById(Mockito.contains(
				org.sagebionetworks.markdown.constants.WidgetConstants.DIV_ID_WIDGET_PREFIX + "0")))
						.thenReturn(mockElementWrapper);
		...
		verify(mockWidgetRegistrar).getWidgetRendererForWidgetDescriptor(Mockito.eq(mockWikiPageKey),
				anyString(), anyMap(), any(Callback.class), any(Long.class));
		verify(mockView).addWidget(any(Widget.class), Mockito.contains(
				org.sagebionetworks.markdown.constants.WidgetConstants.DIV_ID_WIDGET_PREFIX + "0"));
	...
	public void testLoadMarkdownFromWikiPageSuccess() {
		String sampleHTML = "<h1>heading</h1><p>foo baz bar</p>";
		AsyncMockStubber.callSuccessWith(mockWikiPage).when(mockSynapseJavascriptClient)
		...
		verify(mockWidgetRegistrar).getWidgetContentType(elementContentType);
		verify(mockWidgetRegistrar).getWidgetDescriptor(elementContentType);
		verify(mockWidgetRegistrar).getWidgetRendererForWidgetDescriptor(any(WikiPageKey.class),
				anyString(), anyMap(), any(Callback.class), any(Long.class));
		verify(mockView).addWidget(any(Widget.class), Mockito.contains(
				org.sagebionetworks.markdown.constants.WidgetConstants.DIV_ID_WIDGET_PREFIX + "0"));
	}
	...
	public void testLoadMarkdownFromWikiPageFailure() {
		AsyncMockStubber.callFailureWith(caught).when(mockSynapseJavascriptClient)
				.getV2WikiPageAsV1(any(WikiPageKey.class), any(AsyncCallback.class));
