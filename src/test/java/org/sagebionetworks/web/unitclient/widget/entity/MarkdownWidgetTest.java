...
public class MarkdownWidgetTest {
  ...
  public void setup() {
    ...
    when(mockElementWrapper.getAttribute("data-widgetParams"))
      .thenReturn(elementContentType);
    presenter =
      new MarkdownWidget(
  ...
  public void testConfigureSuccess() {
    ...
    when(
      mockView.getElementById(WidgetConstants.MARKDOWN_TABLE_ID_PREFIX + "0")
    )
      .thenReturn(mockElementWrapper);
    when(
      mockView.getElementById(
        Mockito.contains(WidgetConstants.DIV_ID_MATHJAX_PREFIX + "0")
      )
    )
      .thenReturn(mockElementWrapper);
    when(
