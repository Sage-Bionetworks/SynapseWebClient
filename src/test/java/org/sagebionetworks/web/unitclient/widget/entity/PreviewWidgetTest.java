...
public class PreviewWidgetTest {
	...
	public void before() throws Exception {
		...
		previewWidget = new PreviewWidget(mockView, mockRequestBuilder, mockSynapseJSNIUtils,
				mockSynapseAlert, mockSynapseClient, mockAuthController, mockSynapseJavascriptClient,
				mockPortalGinInjector);
		testEntity = new FileEntity();
		testEntity.setId(TEST_ENTITY_ID);
		// when asking for the FileEntity EntityBundle, the version is always set (even if the latest
		// version). so we should set it here.
		...
		AsyncMockStubber.callSuccessWith(testBundle).when(mockSynapseJavascriptClient)
		...
		AsyncMockStubber.callSuccessWith(TARGET_FILE_PRESIGNED_URL).when(mockSynapseJavascriptClient)
				.getFileEntityTemporaryUrlForVersion(anyString(), anyLong(), anyBoolean(),
						any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockParseResults).when(mockSynapseClient).parseCsv(anyString(),
				anyChar(), any(AsyncCallback.class));
	...
	public void testFollowLink() {
		...
		verify(mockSynapseJavascriptClient).getEntityBundle(eq(targetEntityId),
	...
	public void testFollowLinkWithVersion() {
		...
		verify(mockSynapseJavascriptClient).getEntityBundleForVersion(eq(targetEntityId),
				eq(targetVersion), any(EntityBundleRequest.class), any(AsyncCallback.class));
	}
	...
	public void testWikiConfigureWithVersion() {
		...
		// verify that it tries to get the entity bundle (without version)
		verify(mockSynapseJavascriptClient).getEntityBundleForVersion(anyString(), anyLong(),
				any(EntityBundleRequest.class), any(AsyncCallback.class));
	}
	...
	public void testWikiConfigureWithDotVersion() {
		...
		verify(mockSynapseJavascriptClient).getEntityBundleForVersion(eq(entityId), eq(versionNumber),
				any(EntityBundleRequest.class), any(AsyncCallback.class));
	}
	...
	public void testWikiConfigureFailure() {
		...
		AsyncMockStubber.callFailureWith(new Exception(exceptionMessage))
	...
	public void testGetPreviewFileContents() {
		...
		verify(mockSynapseJavascriptClient).getFileEntityTemporaryUrlForVersion(eq(TEST_ENTITY_ID),
				eq(TEST_ENTITY_VERSION_NUMBER), eq(isPreview), any(AsyncCallback.class));
	...
	public void testParseCsvFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).parseCsv(anyString(), anyChar(),
				any(AsyncCallback.class));
