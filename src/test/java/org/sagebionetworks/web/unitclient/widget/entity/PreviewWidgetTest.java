...
public class PreviewWidgetTest {
  ...
  public static final String TARGET_FILE_PRESIGNED_URL =
    "https://target.presigned.url/test.txt";
  ...
  public void before() throws Exception {
    ...
    when(mockPortalGinInjector.getPDFPreviewWidget())
      .thenReturn(mockPDFPreviewWidget);
    when(mockPortalGinInjector.getHtmlPreviewWidget())
      .thenReturn(mockHtmlPreviewWidget);
    when(mockPortalGinInjector.getNbConvertPreviewWidget())
      .thenReturn(mockNbConvertPreviewWidget);
    when(mockPortalGinInjector.getMarkdownWidget())
      .thenReturn(mockMarkdownWidget);
    when(mockSynapseJSNIUtils.getBaseFileHandleUrl())
      .thenReturn("http://fakebaseurl/");
  ...
  public void testNoPreviewFileHandleAvailable() {
    ...
    verify(mockView)
      .showNoPreviewAvailable(TEST_ENTITY_ID, TEST_ENTITY_VERSION_NUMBER);
  }
  ...
  public void testPreviewHtmlContentType() {
    ...
    assertEquals(
      ...
      previewWidget.getOriginalFileType(mainFileHandle)
    );
  }
  ...
  public void testHtmlContentType() {
    ...
    assertEquals(
      ...
      previewWidget.getPreviewFileType(null, mainFileHandle)
    );
    assertEquals(
      PreviewFileType.HTML,
      previewWidget.getOriginalFileType(mainFileHandle)
  ...
  public void testLongContent() {
    ...
    assertEquals(
      PreviewWidget.MAX_LENGTH + "...".length(),
      textSentToView.length()
    );
  }
  ...
  public void testWikiConfigureFailure() {
    ...
    AsyncMockStubber
      .callFailureWith(new Exception(exceptionMessage))
      .when(mockSynapseJavascriptClient)
      .getEntityBundle(
        anyString(),
        any(EntityBundleRequest.class),
  ...
  public void testGetPreviewFileContents() {
    ...
    verify(mockSynapseJavascriptClient)
      .getFileEntityTemporaryUrlForVersion(
        eq(TEST_ENTITY_ID),
        eq(TEST_ENTITY_VERSION_NUMBER),
        eq(isPreview),
        any(AsyncCallback.class)
      );
    verify(mockRequestBuilder)
      .configure(eq(RequestBuilder.GET), stringCaptor.capture());
  ...
  public void testGetJupyterNotebook() {
    ...
    verify(mockNbConvertPreviewWidget)
      .configure(TEST_ENTITY_ID, mainFileHandle);
  ...
  public void testParseTab() {
    ...
    verify(mockSynapseClient)
      .parseCsv(eq(tabPreviewText), eq(delimiter), any(AsyncCallback.class));
    verify(mockView).setTablePreview(mockParseResults);
  }
