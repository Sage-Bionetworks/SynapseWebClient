...
public class ProjectBadgeTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(userProfile).when(mockUserProfileAsyncHandler)
				.getUserProfile(anyString(), any(AsyncCallback.class));
	}

	private void setupEntity(Project entity, Date lastActivityDate)
	...
	public void testGetProjectTooltipNoUserProfile() {
		AsyncMockStubber.callFailureWith(new NotFoundException()).when(mockUserProfileAsyncHandler)
				.getUserProfile(anyString(), any(AsyncCallback.class));
		ProjectHeader header = new ProjectHeader();
		String id = "syn37373";
		String name = "a name";
		...
		String expected =
				ProvViewUtil.createEntityPopoverHtml(new KeyValueDisplay<String>(map, order)).asString();
