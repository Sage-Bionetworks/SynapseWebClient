...
public class RegisterTeamDialogTest {
  ...
  public void before() {
    ...
    AsyncMockStubber
      .callSuccessWith(registratableTeams)
    ...
    when(mockAuthenticationController.getCurrentUserPrincipalId())
      .thenReturn(LOGGED_IN_USER_ID);
    ChallengeTeam newChallengeTeam = new ChallengeTeam();
    AsyncMockStubber
      .callSuccessWith(newChallengeTeam)
      .when(mockChallengeClient)
  ...
  public void testRegisterChallengeTeam() {
    widget.configure(CHALLENGE_ID, mockCallback);
    widget.onOk();
    ArgumentCaptor<ChallengeTeam> challengeTeamCaptor = ArgumentCaptor.forClass(
      ChallengeTeam.class
    );
    verify(mockChallengeClient)
      .registerChallengeTeam(
        challengeTeamCaptor.capture(),
        any(AsyncCallback.class)
      );
