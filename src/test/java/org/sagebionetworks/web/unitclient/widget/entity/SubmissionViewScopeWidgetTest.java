...
public class SubmissionViewScopeWidgetTest {
	...
	@Before
	public void before() {
		widget = new SubmissionViewScopeWidget(mockView, mockJsClient, mockViewScopeWidget, mockEditScopeWidget, mockSynAlert, mockEventBus);
	...
	public void testConfigure() {
		...
		verify(mockViewScopeWidget).configure(evaluationListCaptor.capture(), eq(expectedIsSelectable));
		verifyEvaluationListContainsMockEvaluations(evaluationListCaptor.getValue());
	}

	private void verifyEvaluationListContainsMockEvaluations(List<Evaluation> evaluationList) {
	...
	public void testOnSaveEmptyScope() {
		boolean isEditable = true;
		when(mockEditScopeWidget.getEvaluationIds()).thenReturn(new ArrayList<String>());
		
		widget.configure(mockEntityBundle, isEditable);
		widget.onSave();
		
		verify(mockSynAlert).showError(CreateTableViewWizardStep1.EMPTY_SCOPE_MESSAGE);
	}
