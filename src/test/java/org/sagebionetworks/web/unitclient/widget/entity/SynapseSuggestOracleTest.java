...
public class SynapseSuggestOracleTest {
  ...
  public void testGetSuggestionsWithoutExactMatch() {
    ...
    AsyncMockStubber
      .callSuccessWith(suggBundle)
      .when(mockSuggestionProvider)
      .getSuggestions(
    ...
    verify(mockSuggestBox)
      .setSelectedSuggestion((UserGroupSuggestion) isNull());
    verify(mockSuggestionProvider)
      .getSuggestions(
        eq(TypeFilter.ALL),
        eq(offset),
        eq(pageSize),
    ...
    verify(mockSuggestBox)
      .updateFieldStateForSuggestions(
        (int) suggBundle.getTotalNumberOfResults(),
        offset
      );
    verify(mockCallback)
      .onSuggestionsReady(eq(mockRequest), any(Response.class));
  }
