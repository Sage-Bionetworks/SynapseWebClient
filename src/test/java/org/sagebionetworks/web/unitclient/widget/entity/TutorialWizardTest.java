...
public class TutorialWizardTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(wikiHeadersList).when(mockJsClient)
	...
	public void testConfigure() {
		...
		// and verify the order (page1 has a larger ID (added later) but it should be first because of
		// it's title.
	...
	public void testConfigureFail() {
		AsyncMockStubber.callFailureWith(new Exception()).when(mockJsClient)
				.getV2WikiHeaderTree(anyString(), anyString(), any(AsyncCallback.class));
