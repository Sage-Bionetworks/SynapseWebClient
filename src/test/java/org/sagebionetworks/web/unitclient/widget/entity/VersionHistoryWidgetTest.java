...
public class VersionHistoryWidgetTest {
  ...
  public void testOnMore() throws Exception {
    ...
    verify(mockJsClient)
      .getEntityVersions(
        anyString(),
        eq(WebConstants.ZERO_OFFSET.intValue()),
        anyInt(),
        any(AsyncCallback.class)
      );
  }
  ...
  public void testDeleteVersion() {
    ...
    verify(mockSynapseClient)
      .deleteEntityVersionById(
        matches(vb.getId()),
        eq(vb.getVersionNumber()),
        (AsyncCallback<Void>) any()
      );
  ...
  public void testGetMore() {
    ...
    verify(mockJsClient)
      .getEntityVersions(
        eq(entityId),
        eq(0),
        eq(VersionHistoryWidget.VERSION_LIMIT),
        any(AsyncCallback.class)
      );
    verify(mockView).clearVersions();
    ...
    verify(mockView, times(VersionHistoryWidget.VERSION_LIMIT - 1))
      .addVersion(
        eq(entityId),
