...
public class VersionHistoryWidgetTest {
	...
	public void before() throws JSONObjectAdapterException {
		versionHistoryWidget = new VersionHistoryWidget(mockView, mockSynapseClient, mockJsClient,
				mockGlobalApplicationState, mockPreflightController, mockSynAlert);
		...
		AsyncMockStubber.callSuccessWith(vb).when(mockSynapseJavascriptClient).getEntity(anyString(),
		...
		versions.add(v2);
		AsyncMockStubber.callSuccessWith(versions).when(mockJsClient).getEntityVersions(anyString(),
				anyInt(), anyInt(), any(AsyncCallback.class));
		when(mockGlobalApplicationState.getPlaceChanger()).thenReturn(mockPlaceChanger);
		AsyncMockStubber.callSuccessWith(vb).when(mockJsClient).updateEntity(any(Entity.class),
				anyString(), anyBoolean(), any(AsyncCallback.class));
	}
	...
	public void testOnMore() throws Exception {
		...
		verify(mockJsClient).getEntityVersions(anyString(), eq(WebConstants.ZERO_OFFSET.intValue()),
				anyInt(), any(AsyncCallback.class));
	}

	@Test
	public void testLoadVersionsFail() throws Exception {
		AsyncMockStubber.callFailureWith(new IllegalArgumentException()).when(mockJsClient)
				.getEntityVersions(anyString(), anyInt(), anyInt(), any(AsyncCallback.class));
	...
	public void testUpdateVersionInfo() throws Exception {
		...
		verify(mockJsClient).updateEntity(entityCaptor.capture(), anyString(), anyBoolean(),
	...
	public void testUpdateVersionInfoNoOp() {
		...
		versionHistoryWidget.setEntityBundle(bundle, null);
		versionHistoryWidget.updateVersionInfo(testLabel, testComment);
		verify(mockJsClient, never()).updateEntity(any(Entity.class), anyString(), anyBoolean(),
				(AsyncCallback<Entity>) any());
	...
	public void testUpdateVersionInfoFailure() {
		...
		String testComment = "testComment";
		versionHistoryWidget.setEntityBundle(bundle, null);
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).updateEntity(any(Entity.class),
				anyString(), anyBoolean(), any(AsyncCallback.class));
		versionHistoryWidget.updateVersionInfo(testLabel, testComment);
		verify(mockJsClient).updateEntity(any(Entity.class), anyString(), anyBoolean(),
				(AsyncCallback<Entity>) any());
	...
	public void testDeleteVersion() {
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient)
				.deleteEntityVersionById(anyString(), anyLong(), any(AsyncCallback.class));
		...
		verify(mockSynapseClient).deleteEntityVersionById(matches(vb.getId()),
				eq(vb.getVersionNumber()), (AsyncCallback<Void>) any());
		// deleting a different version, verify file history widget is refreshed
		verify(mockJsClient, times(2)).getEntityVersions(anyString(),
				eq(WebConstants.ZERO_OFFSET.intValue()), anyInt(), any(AsyncCallback.class));
	}
	...
	public void testDeleteCurrentlyViewedVersion() {
		...
		verify(mockSynapseClient).deleteEntityVersionById(matches(vb.getId()),
				eq(vb.getVersionNumber()), (AsyncCallback<Void>) any());
		// deleting a different version, verify file history widget is not simply refreshed (still
		// called only once during setEntityBundle())
		verify(mockJsClient).getEntityVersions(anyString(), eq(WebConstants.ZERO_OFFSET.intValue()),
				anyInt(), any(AsyncCallback.class));
	...
	public void testGetMore() {
		...
		verify(mockJsClient).getEntityVersions(eq(entityId), eq(0),
				eq(VersionHistoryWidget.VERSION_LIMIT), any(AsyncCallback.class));
		verify(mockView).clearVersions();
		...
		verify(mockView, times(VersionHistoryWidget.VERSION_LIMIT - 1)).addVersion(eq(entityId),
				any(VersionInfo.class), eq(canEdit), eq(isVersionSelected));
		isVersionSelected = true;
		verify(mockView).addVersion(eq(entityId), any(VersionInfo.class), eq(canEdit),
				eq(isVersionSelected));
		...
		verify(mockJsClient).getEntityVersions(eq(entityId), eq(VersionHistoryWidget.VERSION_LIMIT),
				eq(VersionHistoryWidget.VERSION_LIMIT), any(AsyncCallback.class));
		isVersionSelected = false;
		verify(mockView, times(2)).addVersion(eq(entityId), any(VersionInfo.class), eq(canEdit),
				eq(isVersionSelected));
	...
	public void testTableOnMore() throws Exception {
		...
		// verify current version is not set when offset is 0, because (unlike Files) the first row of
		// the version history does not represent the latest version for Tables
