...
public class WikiHistoryWidgetTest {
	...
	public void before() throws Exception {
		...
		presenter = new WikiHistoryWidget(mockGlobalApplicationState, mockView, mockSynapseClient,
				mockAuthenticationController, mockSynapseJavascriptClient, mockSynAlert);
		...
		AsyncMockStubber.callSuccessWith(paginatedHistory).when(mockSynapseClient).getV2WikiHistory(
				any(WikiPageKey.class), any(Long.class), any(Long.class), any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(responsePage).when(mockSynapseJavascriptClient)
	...
	public void testConfigure() {
		presenter.configure(new WikiPageKey("ownerId", ObjectType.ENTITY.toString(), null, null), true,
				new ActionHandler() {
					@Override
	...
	public void testConfigureNextPageFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).getV2WikiHistory(
				any(WikiPageKey.class), any(Long.class), any(Long.class), any(AsyncCallback.class));
	...
	public void testConfigureNextPageUnavailableFailure() {
		AsyncMockStubber
				.callFailureWith(
						new Exception(WikiHistoryWidget.NO_HISTORY_IS_FOUND_FOR_A_WIKI + " page id: 1298923"))
				.when(mockSynapseClient).getV2WikiHistory(any(WikiPageKey.class), any(Long.class),
						any(Long.class), any(AsyncCallback.class));
		presenter.configureNextPage(new Long(0), new Long(10));
