...
public class WikiMarkdownEditorTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		presenter = new WikiMarkdownEditor(mockView, mockMarkdownEditorWidget, mockSynapseClient,
				mockGlobalApplicationState, mockSynapseJavascriptClient, mockPortalGinInjector,
				mockPopupUtilsView);
		...
		AsyncMockStubber.callSuccessWith(mockWikiPage).when(mockSynapseClient).createV2WikiPageWithV1(
				anyString(), anyString(), any(WikiPage.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockWikiPage).when(mockSynapseClient).updateV2WikiPageWithV1(
				anyString(), anyString(), any(WikiPage.class), any(AsyncCallback.class));
	...
	public void testConfigure() {
		...
		// SWC-4121: order is important - we must set the cursor position to the start index before
		// focus so that the browser will auto-scroll to the top
	...
	public void testSave() {
		...
		verify(mockSynapseClient).updateV2WikiPageWithV1(eq(wikiPageKey.getOwnerObjectId()),
				eq(wikiPageKey.getOwnerObjectType()), eq(testPage), any(AsyncCallback.class));
	}
	...
	public void testAddAttachments()
			throws IOException, RestServiceException, JSONObjectAdapterException {
		presenter.configure(testPage);
	...
	public void testCreateNewWiki() {
		...
		AsyncMockStubber.callFailureWith(notFoundEx).when(mockSynapseJavascriptClient)
				.getV2WikiPageAsV1(any(WikiPageKey.class), any(AsyncCallback.class));
