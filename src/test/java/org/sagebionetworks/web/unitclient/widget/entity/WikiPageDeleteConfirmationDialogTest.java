...
public class WikiPageDeleteConfirmationDialogTest {
	...
	@Before
	public void before() {
		AsyncMockStubber.callSuccessWith(getWikiHeaderTree()).when(mockJsClient)
		...
		dialog = new WikiPageDeleteConfirmationDialog(mockView, mockSynapseClient, mockJsClient,
				mockPopupUtils);
	}

	/**
	 * Sets up the wiki header tree to have a root page (id=root) that has one child page (id=A),
	 * which has 2 children (id=A1 and id=A2).
	 * 
	 * @return
	 */
	...
	public void testGetWikiChildrenMap() {
		...
		assertTrue(children.get(0).getId().equals(WIKI_TREE_PAGE_A1_ID)
	...
	public void testOnDeleteWiki() {
		...
		verify(mockSynapseClient, never()).deleteV2WikiPage(any(WikiPageKey.class),
	...
	public void testOnDeleteWikiPageConfirmedDeleteSuccess() {
		...
		verify(mockView).showModal(eq(WIKI_TREE_PAGE_A2_ID), any(Map.class), any(Map.class));
		verify(mockSynapseClient).deleteV2WikiPage(any(WikiPageKey.class), any(AsyncCallback.class));
	...
	public void testOnDeleteWikiPageConfirmedDeleteSuccessSinglePage() {
		...
		AsyncMockStubber.callSuccessWith(headers).when(mockJsClient).getV2WikiHeaderTree(anyString(),
		...
		verify(mockSynapseClient, never()).deleteV2WikiPage(any(WikiPageKey.class),
				any(AsyncCallback.class));
		verify(mockPopupUtils).showConfirmDelete(
				eq(WikiPageDeleteConfirmationDialog.DELETE_WIKI_PAGE_CONFIRMATION_MESSAGE),
