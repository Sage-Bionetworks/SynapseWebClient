...
public class ApproveUserAccessModalTest {
	...
	public void before() {
		...
		dialog =
				new ApproveUserAccessModal(mockView, mockSynAlert, mockPeopleSuggestWidget, mockProvider,
						mockSynapseClient, mockSynapseProperties, mockProgressWidget, mockDataAccessClient);
	...
	public void testLoadEmailMessageOnFailure() {
		dialog.configure(actList, mockEntityBundle);
		verify(mockProgressWidget).startAndTrackJob(anyString(), anyBoolean(), any(AsynchType.class),
				any(QueryBundleRequest.class), phCaptor.capture());
	...
	public void testOnSubmitOnSuccess() {
		...
		verify(mockSynapseClient).sendMessage(anySetOf(String.class), anyString(), anyString(),
				anyString(), sCaptor.capture());
	}
