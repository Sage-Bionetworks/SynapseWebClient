...
public class UserBadgeListTest {
	...
	public void testDeleteSelected() {
		...
		list.deleteSelected();
		verify(mockView).clearUserBadges();
		verify(mockView, times(1)).addUserBadge(any(Widget.class)); // only called when user added
																																// initially

		// if we add it again, it should add a new user badge
		list.addAccessorChange(mockChange1);
		verify(mockView, times(2)).addUserBadge(any(Widget.class)); // only called when user added
																																// initially
	}
