...
public class EntityFinderTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		entityFinder = new EntityFinder(mockView, mockGlobalApplicationState,
				mockAuthenticationController, mockClientCache, mockSynAlert, mockJsClient);
	...
	public void testLoadEntity() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(entityHeaderResults).when(mockJsClient)
	...
	public void testLookupEntityEmptyResults() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(new ArrayList<>()).when(mockJsClient)
				.getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
		CallbackP<List<EntityHeader>> mockCallback = mock(CallbackP.class);
	...
	public void testLoadVersions() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(results).when(mockJsClient).getEntityVersions(eq(id), anyInt(),
				anyInt(), any(AsyncCallback.class));

		entityFinder.loadVersions(id);
		verify(mockJsClient).getEntityVersions(eq(id), eq(WebConstants.ZERO_OFFSET.intValue()),
				anyInt(), any(AsyncCallback.class));
	...
	public void testLoadVersionsFail() throws Exception {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).getEntityVersions(eq(id), anyInt(),
				anyInt(), any(AsyncCallback.class));
	...
	private void verifyWrongEntityTypeSelected(ArrayList<EntityHeader> entitySelected,
			SelectedHandler mockSelectionHandler) {
		reset(mockSynAlert, mockSelectionHandler);
		AsyncMockStubber.callSuccessWith(entitySelected).when(mockJsClient)
				.getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
		entityFinder.okClicked();
	...
	private void verifyCorrectEntityTypeSelected(ArrayList<EntityHeader> results,
			SelectedHandler mockSelectionHandler) {
		reset(mockSynAlert, mockSelectionHandler);
		AsyncMockStubber.callSuccessWith(results).when(mockJsClient)
				.getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
		entityFinder.okClicked();
	...
	private void verifySelectedWithoutTypeCheck(SelectedHandler mockSelectionHandler) {
		reset(mockSynAlert, mockSelectionHandler);
		entityFinder.okClicked();
		verify(mockJsClient, never()).getEntityHeaderBatchFromReferences(anyList(),
	...
	public void testShowBrowseArea() {
		when(mockClientCache.get(EntityFinder.ENTITY_FINDER_AREA_KEY))
				.thenReturn(EntityFinderArea.BROWSE.toString());
		entityFinder.show();
		verify(mockView).clear();
		verify(mockView).setBrowseAreaVisible();
		verify(mockView).show();
	...
	public void testShowSearchArea() {
		when(mockClientCache.get(EntityFinder.ENTITY_FINDER_AREA_KEY))
				.thenReturn(EntityFinderArea.SEARCH.toString());
		entityFinder.show();
		verify(mockView).clear();
	...
	public void testShowSynIdArea() {
		when(mockClientCache.get(EntityFinder.ENTITY_FINDER_AREA_KEY))
				.thenReturn(EntityFinderArea.SYNAPSE_ID.toString());
		entityFinder.show();
		verify(mockView).clear();
	...
	public void testShowMultiSynIdArea() {
		when(mockClientCache.get(EntityFinder.ENTITY_FINDER_AREA_KEY))
				.thenReturn(EntityFinderArea.SYNAPSE_MULTI_ID.toString());
		entityFinder.show();
		verify(mockView).clear();
	...
	public void testHideSearchArea() {
		...
		entityFinder.hide();
		verify(mockClientCache).put(EntityFinder.ENTITY_FINDER_AREA_KEY,
				EntityFinderArea.SEARCH.toString());
	}
