...
public class EntityTreeBrowserTest {
  ...
  public void testGetChildren() {
    ...
    verify(mockSynapseJavascriptClient)
      .getEntityChildren(
        entityChildrenRequestCaptor.capture(),
        any(AsyncCallback.class)
      );
    ...
    assertEquals(
      EntityTreeBrowser.DEFAULT_DIRECTION,
  ...
  public void testGetFolderChildrenRaceCondition() {
    ...
    ArgumentCaptor<AsyncCallback> captor = ArgumentCaptor.forClass(
      AsyncCallback.class
    );
    verify(mockSynapseJavascriptClient)
      .getEntityChildren(any(EntityChildrenRequest.class), captor.capture());
  ...
  public void testMoreButtonRootLevel() {
    ...
    verify(mockView)
      .placeRootMoreTreeItem(
        any(MoreTreeItem.class),
        eq(parentId),
        eq(nextPageToken)
      );
  }
  ...
  public void testMoreButtonChildLevel() {
    ...
    // Creates the limited number of entity items
    verify(mockView)
      .appendChildEntityTreeItem(
        any(EntityTreeItem.class),
        eq(mockEntityTreeItem)
      );
    // Calls once for folders, once for files.
    verify(mockView)
      .placeChildMoreTreeItem(
        any(MoreTreeItem.class),
