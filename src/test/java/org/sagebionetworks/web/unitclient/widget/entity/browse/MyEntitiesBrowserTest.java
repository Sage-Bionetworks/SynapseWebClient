...
public class MyEntitiesBrowserTest {
	...
	public void before() {
		...
		widget = new MyEntitiesBrowser(mockView, mockAuthenticationController,
				mockGlobalApplicationState, jsonObjectAdapter, mockSynapseJavascriptClient);
	...
	public void testLoadUserUpdateable() {
		...
		verify(mockSynapseJavascriptClient).getMyProjects(any(ProjectListType.class), anyInt(),
		...
		entities.clear();
		widget.loadMoreUserUpdateable();
		verify(mockSynapseJavascriptClient, times(2)).getMyProjects(any(ProjectListType.class),
				anyInt(), anyString(), any(ProjectListSortColumn.class), any(SortDirection.class),
				any(AsyncCallback.class));
	...
	public void testContextChange() {
		...
		when(mockGlobalApplicationState.getCurrentPlace())
				.thenReturn(new Synapse("yet another different place"));
		widget.clearState();
	...
	public void testLoadContextNotSynapsePlace() {
		// in case the entity finder is used outside of a Synapse place, we don't know how to figure out
		// the current project
	...
	public void testLoadContextSynapsePlaceSuccess() {
		...
		eb.setPath(path);
		AsyncMockStubber.callSuccessWith(eb).when(mockSynapseJavascriptClient)
