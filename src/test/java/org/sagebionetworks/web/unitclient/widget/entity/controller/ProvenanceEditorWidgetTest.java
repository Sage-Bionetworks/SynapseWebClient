...
public class ProvenanceEditorWidgetTest {
	...
	public void before() {
		...
		presenter = new ProvenanceEditorWidget(mockView, mockJsClient, mockSynAlert, mockInjector,
				mockEntityFinder, mockUrlDialog, mockEventBus);
	...
	public void testConfigureSuccessProvenanceCreated() {
		...
		AsyncMockStubber.callSuccessWith(mockActivity).when(mockJsClient)
		...
		verify(mockInjector.getEntityRefEntry(), Mockito.never()).configure(mockRef.getTargetId(),
				mockRef.getTargetVersionNumber().toString());
	...
	public void testConfigureSuccess() {
		...
		verify(mockInjector.getEntityRefEntry()).configure(mockRef.getTargetId(),
				mockRef.getTargetVersionNumber().toString());
	...
	public void testConfigureFailure() {
		AsyncMockStubber.callFailureWith(caught).when(mockJsClient)
				.getActivityForEntityVersion(anyString(), anyLong(), any(AsyncCallback.class));
		presenter.configure(mockEntityBundle);
	...
	public void onSaveSuccess() {
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).updateActivity(eq(mockActivity),
	...
	public void onSaveNewActivitySuccess() {
		AsyncMockStubber.callFailureWith(new NotFoundException()).when(mockJsClient)
				.getActivityForEntityVersion(anyString(), anyLong(), any(AsyncCallback.class));
		presenter.configure(mockEntityBundle);
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).createActivityAndLinkToEntity(
				any(Activity.class), eq(mockEntity), any(AsyncCallback.class));
		presenter.onSave();
		verify(mockJsClient).createActivityAndLinkToEntity(any(Activity.class), eq(mockEntity),
				any(AsyncCallback.class));
		verify(mockView).hide();
		verify(mockEventBus).fireEvent(any(EntityUpdatedEvent.class));
	}
	...
	public void onSaveFailure() {
		AsyncMockStubber.callFailureWith(caught).when(mockJsClient).updateActivity(eq(mockActivity),
				any(AsyncCallback.class));
		presenter.setActivity(mockActivity);
		presenter.onSave();
		verify(mockJsClient).updateActivity(eq(mockActivity), any(AsyncCallback.class));
