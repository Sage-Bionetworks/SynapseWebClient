...
public class SynapseAlertImplTest {
	...
	public void before() {
		...
		widget =
				new SynapseAlertImpl(mockView, mockGlobalApplicationState, mockAuthenticationController,
						mockGWT, mockPortalGinInjector, mockJsniUtils, jsonObjectAdapter);
	...
	public void testHandleServiceUnknownErrorExceptionLoggedIn() {
		...
		verify(mockJsniUtils).showJiraIssueCollector(eq(""), anyString(), eq(SWC_ISSUE_COLLECTOR_URL),
				eq(USER_ID), eq(DisplayUtils.getDisplayName(FIRST_NAME, LAST_NAME, USERNAME)),
