...
public class S3DirectUploaderTest {
	...
	public void setUp() throws Exception {
		...
		when(mockSynapseJsniUtils.getFileBlob(anyInt(), any(JavaScriptObject.class)))
				.thenReturn(mockBlob);
	...
	public void testUpload() {
		uploader.uploadFile(FILE_NAME, CONTENT_TYPE, mockBlob, mockProgressHandler, KEY_PREFIX_UUID,
				STORAGE_LOCATION_ID, mockView);
		...
		verify(mockAwsSdk).getS3(eq(ACCESS_KEY_ID), eq(SECRET_KEY_ID), eq(BUCKET_NAME), eq(ENDPOINT),
				callbackPCaptor.capture());
		callbackPCaptor.getValue().invoke(mockS3);
		verify(mockAwsSdk).upload(eq(KEY_PREFIX_UUID + "/" + FILE_NAME), any(JavaScriptObject.class),
				eq(CONTENT_TYPE), any(JavaScriptObject.class), eq(uploader));
	}
	...
	public void testUploadFolder() {
		...
		verify(mockAwsSdk, never()).getS3(anyString(), anyString(), anyString(), anyString(),
				any(CallbackP.class));
	}
