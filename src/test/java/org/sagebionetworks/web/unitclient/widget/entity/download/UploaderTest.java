...
public class UploaderTest {
	...
	public void before() throws Exception {
		...
		when(mockSynapseJsniUtils.getContentType(any(JavaScriptObject.class), anyInt()))
		...
		destinations.add(d);
		AsyncMockStubber.callSuccessWith(destinations).when(mockSynapseJavascriptClient)
		...
		String completedUploadDaemonStatusJson =
				status.writeToJSONObject(adapterFactory.createNew()).toJSONString();

		AsyncMockStubber.callSuccessWith("entityID").when(mockSynapseClient)
				.setFileEntityFileHandle(anyString(), anyString(), anyString(), any(AsyncCallback.class));

		when(mockGwt.createXMLHttpRequest()).thenReturn(null);

		String[] fileNames = {"newFile.txt"};
		when(mockSynapseJsniUtils.getMultipleUploadFileNames(any(JavaScriptObject.class)))
				.thenReturn(fileNames);
		AsyncMockStubber.callSuccessWith(testEntity).when(mockSynapseClient).updateExternalFile(
				anyString(), anyString(), anyString(), anyString(), anyLong(), anyString(), anyLong(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(testEntity).when(mockSynapseClient).createExternalFile(
				anyString(), anyString(), anyString(), anyString(), anyLong(), anyString(), anyLong(),
				any(AsyncCallback.class));
		// by default, there is no name conflict
		AsyncMockStubber.callFailureWith(new NotFoundException()).when(mockSynapseClient)
				.getFileEntityIdWithSameName(anyString(), anyString(), any(AsyncCallback.class));
		uploader = new Uploader(mockView, mockSynapseClient, mockSynapseJsniUtils, mockGwt,
				mockAuthenticationController, multipartUploader, mockGlobalApplicationState,
				mockS3DirectUploader, mockSynapseJavascriptClient, mockSynapseProperties, mockEventBus);
	...
	public void testSetNewExternalPath() throws Exception {
		...
		// if entity is null, it should call synapseClient.createExternalFile() to create the FileEntity
		// and associate the path.
		uploader.setExternalFilePath("http://fakepath.url/blah.xml", "", storageLocationId);
		verify(mockSynapseClient).createExternalFile(anyString(), anyString(), anyString(), anyString(),
				eq((Long) null), eq((String) null), eq(storageLocationId), any(AsyncCallback.class));
	...
	public void testSetExternalPathFailedCreate() throws Exception {
		AsyncMockStubber.callFailureWith(new Exception("failed to create")).when(mockSynapseClient)
				.createExternalFile(anyString(), anyString(), anyString(), anyString(), eq((Long) null),
						eq((String) null), anyLong(), any(AsyncCallback.class));
		uploader.setExternalFilePath("http://fakepath.url/blah.xml", "", storageLocationId);
	...
	public void testSetExternalPathFailedUpdateFile() throws Exception {
		AsyncMockStubber.callFailureWith(new Exception("failed to update path")).when(mockSynapseClient)
				.createExternalFile(anyString(), anyString(), anyString(), anyString(), eq((Long) null),
						eq((String) null), anyLong(), any(AsyncCallback.class));
		uploader.setExternalFilePath("http://fakepath.url/blah.xml", "", storageLocationId);
		verify(mockView).showErrorMessage(anyString());
	...
	public void testSetNewExternalPathEncoding() throws Exception {
		...
		verify(mockSynapseClient).createExternalFile(anyString(), eq(encodedUrl), anyString(),
				anyString(), eq((Long) null), eq((String) null), eq(storageLocationId),
				any(AsyncCallback.class));
		verify(mockView).showInfo(anyString());
	}
	...
	public void testSetExternalFileEntityPathWithFileEntity() throws Exception {
		...
		uploader.setExternalFilePath("http://fakepath.url/blah.xml", "", storageLocationId);
		verify(mockSynapseClient).updateExternalFile(anyString(), anyString(), anyString(), anyString(),
				eq((Long) null), eq((String) null), eq(storageLocationId), any(AsyncCallback.class));
		verify(mockView).showInfo(anyString());
	}
	...
	public void testSetNewSftpExternalPath() throws Exception {
		...
		verify(mockSynapseClient).createExternalFile(anyString(), anyString(), anyString(), anyString(),
				anyLong(), eq(md5), eq(storageLocationId), any(AsyncCallback.class));
		verify(mockView).showInfo(anyString());
	}
	...
	public void testSetSftpExternalPathToFolder() throws Exception {
		...
		uploader.setSftpExternalFilePath("http://fakepath.url/blah.xml", storageLocationId);
		verify(mockSynapseClient, never()).createExternalFile(anyString(), anyString(), anyString(),
				anyString(), anyLong(), eq(md5), eq(storageLocationId), any(AsyncCallback.class));
	...
	public void testSetSftpExternalPathFailedCreate() throws Exception {
		uploader.setFileNames(new String[] {"test.txt"});
		AsyncMockStubber.callFailureWith(new Exception("failed to create")).when(mockSynapseClient)
				.createExternalFile(anyString(), anyString(), anyString(), anyString(), anyLong(), eq(md5),
	...
	public void testSetSftpExternalFileEntityPathWithFileEntity() throws Exception {
		...
		uploader.setSftpExternalFilePath("http://fakepath.url/blah.xml", storageLocationId);
		verify(mockSynapseClient).updateExternalFile(anyString(), anyString(), eq(fileName),
				anyString(), anyLong(), eq(md5), eq(storageLocationId), any(AsyncCallback.class));
		verify(mockView).showInfo(anyString());
	}
	...
	public void testDirectUploadHappyCase() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(testEntity).when(mockSynapseJavascriptClient)
				.getEntity(anyString(), any(OBJECT_TYPE.class), any(AsyncCallback.class));
		uploader.handleUploads();
		verify(mockSynapseClient).setFileEntityFileHandle(anyString(), anyString(), anyString(),
				any(AsyncCallback.class));
	...
	public void testDirectUploadStep1Failure() throws Exception {
		AsyncMockStubber.callFailureWith(new IllegalArgumentException()).when(mockSynapseClient)
				.getFileEntityIdWithSameName(anyString(), anyString(), any(AsyncCallback.class));
	...
	public void testDirectUploadStep1SameNameFound() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(duplicateNameEntityId).when(mockSynapseClient)
				.getFileEntityIdWithSameName(anyString(), anyString(), any(AsyncCallback.class));
		uploader.checkForExistingFileName("newFile.txt", mockCallback);
	...
	public void testDirectUploadSameNameFoundMultipleFiles() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(duplicateNameEntityId).when(mockSynapseClient)
				.getFileEntityIdWithSameName(eq(file1), anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callFailureWith(new NotFoundException()).when(mockSynapseClient)
				.getFileEntityIdWithSameName(eq(file2), anyString(), any(AsyncCallback.class));
		uploader.handleUploads();
		...
		// SWC-4274: Verify the 2 rpcs. The first adds a new file version to duplicateNameEntityId, and
		// the second creates a new file entity.
		verify(mockSynapseClient).setFileEntityFileHandle(anyString(), eq(duplicateNameEntityId),
				eq(parentEntityId), any(AsyncCallback.class));
		verify(mockSynapseClient).setFileEntityFileHandle(anyString(), eq(null), eq(parentEntityId),
				any(AsyncCallback.class));
	}
	...
	public void testDirectUploadStep1NoParentEntityId() throws Exception {
		...
		uploader.checkForExistingFileName("newFile.txt", mockCallback);
		verify(mockSynapseClient, Mockito.never()).getFileEntityIdWithSameName(anyString(), anyString(),
				any(AsyncCallback.class));
	...
	public void testMultipleFileUploads() throws Exception {
		...
		verify(mockSynapseClient).getFileEntityIdWithSameName(eq(file1), eq(parentEntityId),
				any(AsyncCallback.class));

		// triggers file2 to upload.
		verify(mockSynapseClient).getFileEntityIdWithSameName(eq(file2), eq(parentEntityId),
				any(AsyncCallback.class));

		// triggers file3 to upload
		verify(mockSynapseClient).getFileEntityIdWithSameName(eq(file3), eq(parentEntityId),
				any(AsyncCallback.class));
	}
	...
	public void testCalculatePercentOverAllFiles() {
		...
		int numberOfFiles = 3;
		int currentIndex = 0;
		assertEquals(0.166, Uploader.calculatePercentOverAllFiles(numberOfFiles, currentIndex, 0.50),
				tollerance);
		currentIndex = 1;
		assertEquals(0.50, Uploader.calculatePercentOverAllFiles(numberOfFiles, currentIndex, 0.50),
				tollerance);
		currentIndex = 2;
		assertEquals(0.833, Uploader.calculatePercentOverAllFiles(numberOfFiles, currentIndex, 0.50),
				tollerance);
	}
	...
	public void testUploadToExternal() {
		...
		when(mockSynapseProperties.getSynapseProperty(WebConstants.SFTP_PROXY_ENDPOINT))
				.thenReturn(sftpProxy);
		...
		AsyncMockStubber.callSuccessWith(destinations).when(mockSynapseJavascriptClient)
				.getUploadDestinations(anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith("ok.net").when(mockSynapseClient).getHost(anyString(),
				any(AsyncCallback.class));
		...
		verify(mockSynapseClient).getFileEntityIdWithSameName(anyString(), anyString(),
				any(AsyncCallback.class));
	...
	public void testQueryForUploadDestinationsWithUploadToExternalObjectStore() {
		...
		AsyncMockStubber
				.callSuccessWith(Collections.singletonList(mockExternalObjectStoreUploadDestination))
		...
		verify(mockS3DirectUploader).uploadFile(anyString(), anyString(), any(JavaScriptObject.class),
				eq(uploader), eq(keyPrefixUUID), eq(storageLocationId), eq(mockView));
	}
	...
	public void testDragAndDrop() throws RestServiceException {
		...
		verify(mockSynapseClient, never()).setFileEntityFileHandle(anyString(), anyString(),
	...
	public void testQueryForUploadDestinationsWithoutParentEntityId() {
		...
		verify(mockSynapseJavascriptClient).getUploadDestinations(stringCaptor.capture(),
				any(AsyncCallback.class));
	...
	public void testGetSftpProxyLink() throws UnsupportedEncodingException {
		...
		when(mockSynapseProperties.getSynapseProperty(WebConstants.SFTP_PROXY_ENDPOINT))
				.thenReturn(sftpProxy + "?gwt.codesvr=localhost:9999");
		...
		String sftpProxyLink =
				Uploader.getSftpProxyLink(filename, sftpLink, mockSynapseProperties, mockGwt);
	...
	public void testHandleSftpSubmitResult() throws RestServiceException {
		...
		verify(mockSynapseClient).createExternalFile(anyString(), anyString(), anyString(), anyString(),
				anyLong(), anyString(), anyLong(), any(AsyncCallback.class));
	}
	...
	public void testUploadIllegalName() {
		...
		verify(mockView).showErrorMessage(DisplayConstants.ERROR_UPLOAD_TITLE,
	...
	public void testMkdirs() {
		// upload f1/f1a/test.txt. simulate first folder exists, and second folder must first be
		// created.
		String folder1SynId = "syn232";
		AsyncMockStubber.callMixedWith(folder1SynId, new NotFoundException())
				.when(mockSynapseJavascriptClient)
		...
		verify(mockSynapseJavascriptClient).lookupChild(eq(folder1), eq(parentEntityId),
				any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient).lookupChild(eq(folder2), eq(folder1SynId),
				any(AsyncCallback.class));
	...
	public void testMkdirsFailureToCreateFolder() {
		AsyncMockStubber.callFailureWith(new NotFoundException()).when(mockSynapseJavascriptClient)
				.lookupChild(anyString(), anyString(), any(AsyncCallback.class));
		when(mockSynapseJavascriptClient.createEntity(any(Entity.class)))
