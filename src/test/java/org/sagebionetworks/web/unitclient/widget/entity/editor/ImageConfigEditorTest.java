...
public class ImageConfigEditorTest {
  ...
  public void setup() {
    ...
    descriptor.put(
      WidgetConstants.IMAGE_WIDGET_SYNAPSE_ID_KEY,
      WidgetConstants.FLOAT_LEFT
    );
    editor =
      new ImageConfigEditor(mockView, mockFileInputWidget, mockAttachments);
    // leave map as null?
    ...
    when(mockFileInputWidget.getSelectedFileMetadata())
      .thenReturn(new FileMetadata(testFileName, "image/png", fileSize));
  ...
  public void testSynapseIdWithVersionBased() {
    ...
    verify(mockView).showSynapseTab();
    verify(mockView)
      .setSynapseId(synId + WebConstants.ENTITY_VERSION_STRING + version);
  }
