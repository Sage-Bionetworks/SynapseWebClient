...
public class PlotlyConfigEditorTest {
  ...
  public void setup() {
    ...
    editor =
      new PlotlyConfigEditor(
        mockView,
        mockEntityFinderBuilder,
        mockSynAlert,
        mockShowHideAdvancedButton,
        mockSynapseJavascriptClient
      );
    columnModels = new ArrayList<ColumnModel>();
    AsyncMockStubber
      .callSuccessWith(columnModels)
    ...
    AsyncMockStubber
      .callSuccessWith(mockTableEntity)
      .when(mockSynapseJavascriptClient)
  ...
  public void testConstruction() {
    ...
    verify(mockShowHideAdvancedButton)
  ...
  public void testGetXColumnFromSql() {
    ...
    assertEquals(
      "MY_x_column",
      getXColumnFromSql("SeLeCt MY_x_column, y1, y2 FrOM syn2")
    );
    assertEquals(
      "x column",
      getXColumnFromSql("SELECT \'x column\', y1, y2 from syn1")
    );
    assertEquals(
      "x column",
      getXColumnFromSql("SELECT \"x column\", y1, y2 from syn1")
  ...
  public void testGetYColumnsFromSql() {
    ...
    assertArrayEquals(
      new String[] { "y1" },
      getYColumnsFromSql("SeLeCt MY_x_column, y1 FrOM syn2")
    );
    assertArrayEquals(
      new String[] { "y1 Column", "y2 COLUMN" },
      getYColumnsFromSql(
        "SELECT \'x column\', \'y1 Column\',  \"   y2 COLUMN\" from syn1"
      )
    );
  }
  ...
  public void testGetAdvancedClauseFromQuery() {
    ...
    assertEquals(
      "where x>2",
      getAdvancedClauseFromQuery("select x, y from syn1 where x>2")
    );
    assertEquals(
      "GrOUP BY x",
      getAdvancedClauseFromQuery("select x from syn1 GrOUP BY x")
    );
    assertEquals(
      "where x>2 group by x",
  ...
  public void testUpdateDescriptorFromView() {
    ...
    // verify
    assertEquals(
      "select \"new X\", \"new Y1\" from syn999999 " + advancedClause,
  ...
  public void testUpdateDescriptorFromViewBarChart() {
    ...
    when(mockView.isBarOrientationHorizontal())
      .thenReturn(isHorizontalBarChart);
    ...
    assertEquals(
      Boolean.toString(isHorizontalBarChart),
      params.get(IS_HORIZONTAL)
    );
  ...
  public void testOnFindTable() {
    ...
    verify(mockSynapseJavascriptClient)
      .getEntity(eq(newTableId), any(AsyncCallback.class));
  }
}
