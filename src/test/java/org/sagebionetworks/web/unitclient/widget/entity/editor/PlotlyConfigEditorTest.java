...
public class PlotlyConfigEditorTest {
	...
	public void setup() {
		...
		editor = new PlotlyConfigEditor(mockView, mockFinder, mockSynAlert, mockShowHideAdvancedButton,
				mockSynapseJavascriptClient);
		columnModels = new ArrayList<ColumnModel>();
		AsyncMockStubber.callSuccessWith(columnModels).when(mockSynapseJavascriptClient)
				.getColumnModelsForTableEntity(anyString(), any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(mockTableEntity).when(mockSynapseJavascriptClient)
	...
	public void testConfigure() {
		...
		String sql = "select \"" + X_COLUMN_NAME + "\", \'" + Y_COLUMN_NAME + "\' from " + tableSynId
				+ " " + advancedClause;
	...
	public void testGetYColumnsFromSql() {
		...
		assertArrayEquals(new String[] {"y1 Column", "y2 COLUMN"},
				getYColumnsFromSql("SELECT \'x column\', \'y1 Column\',  \"   y2 COLUMN\" from syn1"));
	}
	...
	public void testGetAdvancedClauseFromQuery() {
		...
		assertEquals("where x>2 group by x",
				getAdvancedClauseFromQuery("select x from syn1 where x>2 group by x"));
	}
	...
	public void testUpdateDescriptorFromView() {
		...
		// verify
		assertEquals("select \"new X\", \"new Y1\" from syn999999 " + advancedClause,
				params.get(TABLE_QUERY_KEY));
	...
	public void testAvailableColumns() {
		...
		DialogCallback callback = null;
		editor.configure(wikiKey, params, callback);
		verify(mockSynapseJavascriptClient, never()).getColumnModelsForTableEntity(anyString(),
				any(AsyncCallback.class));

		// no columns defined, so it will pick the first column from the column models as the x column,
		// and leave the rest for the available columns
		editor.setTableId("syn2222");
		verify(mockSynapseJavascriptClient).getColumnModelsForTableEntity(anyString(),
				any(AsyncCallback.class));
	...
	public void testOnFindTable() {
		editor.onFindTable();
		verify(mockFinder).configure(eq(EntityFilter.PROJECT_OR_TABLE), eq(false),
				finderCallbackCaptor.capture());
