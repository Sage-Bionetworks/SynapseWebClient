...
public class AddToDownloadListTest {
	...
	public void setUp() throws Exception {
		widget = new AddToDownloadList(mockView, mockAsynchronousProgressWidget,
				mockInlineAsynchronousProgressView, mockPopupUtils, mockEventBus, mockSynAlert,
				mockPackageSizeSummary, mockJsClient, mockSynapseJSNIUtils, mockAuthController);
	...
	public void testAddQueryToDownloadList() {
		...
		verify(mockAsynchronousProgressWidget).startAndTrackJob(anyString(), eq(false),
				eq(AsynchType.TableQuery), queryBundleRequestCaptor.capture(),
		...
		// simulate success
		AsynchronousProgressHandler queryBundleRequestProgressHandler =
		...
		verify(mockAsynchronousProgressWidget).startAndTrackJob(anyString(), eq(false),
				eq(AsynchType.AddFileToDownloadList), requestCaptor.capture(),
				asyncProgressHandlerCaptor.capture());
		...
		AsynchronousProgressHandler addFileToDownloadListProgressHandler =
				asyncProgressHandlerCaptor.getValue();
		...
		verify(mockSynapseJSNIUtils).sendAnalyticsEvent(DOWNLOAD_ACTION_EVENT_NAME,
				FILES_ADDED_TO_DOWNLOAD_LIST_EVENT_NAME, queryCount.toString());
	...
	public void testAddFolderToDownloadList() {
		AsyncMockStubber.callSuccessWith(mockEntityChildrenResponse).when(mockJsClient)
				.getEntityChildren(any(EntityChildrenRequest.class), any(AsyncCallback.class));
		widget.addToDownloadList(folderId);

		verify(mockJsClient).getEntityChildren(entityChildrenRequestCaptor.capture(),
		...
		verify(mockPackageSizeSummary).addFiles(testFolderChildCount.intValue(),
				testFolderSumFileSizesBytes.doubleValue());
		verify(mockView).showConfirmAdd();
