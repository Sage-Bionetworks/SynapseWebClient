...
public class FileDownloadMenuItemTest {
	...
	public void setUp() throws Exception {
		...
		widget = new FileDownloadMenuItem(mockView, mockSynapseClient, mockLoginModalWidget,
				mockGinInjector, mockSynapseJavascriptClient, mockAuthController, mockJsniUtils, mockGwt,
				mockCookies, mockAwsSdk, mockPopupUtilsView);
		...
		AsyncMockStubber.callSuccessWith(SFTP_HOST).when(mockSynapseClient).getHost(anyString(),
				any(AsyncCallback.class));
		when(mockJsniUtils.getFileHandleAssociationUrl(anyString(), any(FileHandleAssociateType.class),
				anyString())).thenReturn(fileHandleAssociationUrl);
	...
	public void testConfigureSftpLink() {
		...
		verify(mockLoginModalWidget).configure(fileUrl, FormPanel.METHOD_POST,
				FormPanel.ENCODING_MULTIPART);
		verify(mockSynapseClient).getHost(anyString(), any(AsyncCallback.class));
		verify(mockLoginModalWidget)
				.setInstructionMessage(DisplayConstants.DOWNLOAD_CREDENTIALS_REQUIRED + SFTP_HOST);
	}
	...
	public void testConfigureExternalObjectStoreFileHandle() {
		...
		verify(mockAwsSdk).getS3(eq(accessKeyId), eq(secretAccessKey), eq(bucket), eq(endpointUrl),
		...
		when(mockAwsSdk.getPresignedURL(anyString(), anyString(), anyString(),
				any(JavaScriptObject.class))).thenReturn(presignedUrl);
	...
	public void testLicensedDownloadLink() {
		...
		verify(mockView)
				.setIsDirectDownloadLink(FileDownloadMenuItem.ACCESS_REQUIREMENTS_LINK + ENTITY_ID + "&"
