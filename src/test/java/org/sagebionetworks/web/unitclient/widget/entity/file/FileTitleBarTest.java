...
public class FileTitleBarTest {
	...
	@Before
	public void setup() {
		fileTitleBar = new FileTitleBar(mockView, mockSynapseProperties, mockFileDownloadButton,
				mockJsClient, mockFileClientsHelp, mockEventBus, mockSynapseJSNIUtils);
		...
		AsyncMockStubber.callSuccessWith(versions).when(mockJsClient).getEntityVersions(anyString(),
				anyInt(), anyInt(), any());
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).addFileToDownloadList(anyString(),
	...
	public void testSetS3DescriptionForExternalS3() {
		...
		fileTitleBar.configure(mockBundle);
		verify(mockView)
				.setFileLocation("| s3://" + s3FileHandle.getBucketName() + "/" + s3FileHandle.getKey());
	}
	...
	public void testSetGoogleCloudDescriptionForExternalGoogleCloud() {
		...
		fileTitleBar.configure(mockBundle);
		verify(mockView).setFileLocation(
				"| gs://" + googleCloudFileHandle.getBucketName() + "/" + googleCloudFileHandle.getKey());
	}
	...
	public void testAddToDownloadList() {
		...
		verify(mockJsClient).addFileToDownloadList(eq(DATA_FILE_HANDLE_ID), eq(ENTITY_ID),
		...
		verify(mockSynapseJSNIUtils).sendAnalyticsEvent(AddToDownloadList.DOWNLOAD_ACTION_EVENT_NAME,
				AddToDownloadList.FILES_ADDED_TO_DOWNLOAD_LIST_EVENT_NAME, Integer.toString(1));
	}
	...
	public void testAddToDownloadListFailure() {
		...
		AsyncMockStubber.callFailureWith(new Exception(errorMessage)).when(mockJsClient)
				.addFileToDownloadList(anyString(), anyString(), any(AsyncCallback.class));
	...
	public void testExternalObjectStoreFileHandle() {
		...
		Mockito.when(mockBundle.getFileHandles())
				.thenReturn(Collections.singletonList((FileHandle) mockExternalObjectStoreFileHandle));
		fileTitleBar.configure(mockBundle);
		verify(mockFileDownloadButton).configure(mockBundle);
	...
	public void testGetEntityVersionsFailure() {
		...
		AsyncMockStubber.callFailureWith(new Exception(error)).when(mockJsClient)
