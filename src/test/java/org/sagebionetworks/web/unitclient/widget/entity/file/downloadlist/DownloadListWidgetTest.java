...
public class DownloadListWidgetTest {
	...
	public void setUp() throws Exception {
		widget = new DownloadListWidget(mockView, mockSynAlert, mockJsClient, mockEventBus,
				mockFhaTable, mockPackageSizeSummary, mockProgressWidget,
				mockInlineAsynchronousProgressView, mockJsniUtils);
		when(mockDownloadList.getFilesToDownload()).thenReturn(mockFhas);
		when(mockDownloadOrder.getFiles()).thenReturn(mockFhas);
		AsyncMockStubber.callSuccessWith(mockDownloadList).when(mockJsClient)
				.getDownloadList(any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockDownloadOrder).when(mockJsClient)
				.createDownloadOrderFromUsersDownloadList(anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient)
				.clearDownloadList(any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockDownloadList).when(mockJsClient)
				.removeFileFromDownloadList(any(FileHandleAssociation.class), any(AsyncCallback.class));
	}
	...
	public void testRefresh() {
		...
		verify(mockFhaTable).configure(eq(mockFhas), addToPackageSizeCallbackCaptor.capture(),
				onRemoveFileHandleAssociationCaptor.capture());
	...
	public void testOnCreatePackageEmptyZipfileName() {
		...
		verify(mockJsClient, never()).createDownloadOrderFromUsersDownloadList(anyString(),
	...
	public void testOnCreatePackage() {
		...
		when(mockDownloadOrder.getZipFileName())
				.thenReturn(fileName + DownloadListWidget.ZIP_EXTENSION);

		widget.onCreatePackage(fileName);

		verify(mockJsClient).createDownloadOrderFromUsersDownloadList(
				eq(fileName + DownloadListWidget.ZIP_EXTENSION), any(AsyncCallback.class));
		verify(mockView).setProgressTrackingWidgetVisible(true);
		verify(mockView).setCreatePackageUIVisible(false);
		verify(mockJsniUtils).sendAnalyticsEvent(AddToDownloadList.DOWNLOAD_ACTION_EVENT_NAME,
				DownloadListWidget.DOWNLOAD_LIST_PACKAGE_CREATED_EVENT_NAME);
		verify(mockProgressWidget).startAndTrackJob(anyString(), eq(false),
				eq(AsynchType.BulkFileDownload), requestCaptor.capture(), progressHandlerCaptor.capture());
	...
	public void testOnClearDownloadList() {
		...
		verify(mockJsniUtils).sendAnalyticsEvent(AddToDownloadList.DOWNLOAD_ACTION_EVENT_NAME,
				DownloadListWidget.DOWNLOAD_LIST_CLEARED_EVENT_NAME);
	}
