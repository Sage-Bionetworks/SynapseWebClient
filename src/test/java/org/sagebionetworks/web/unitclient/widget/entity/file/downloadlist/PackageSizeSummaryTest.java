...
public class PackageSizeSummaryTest {
  ...
  public void setUp() throws Exception {
    widget =
      new PackageSizeSummary(
        mockView,
        mockGwt,
        mockDownloadSpeedTester,
        mockJsniUtils,
        mockDateTimeUtils
      );
    when(mockGwt.getFriendlySize(anyDouble(), anyBoolean()))
      .thenReturn(FRIENDLY_SIZE);
    when(mockDateTimeUtils.getFriendlyTimeEstimate(anyLong()))
      .thenReturn(FRIENDLY_TIME_ESTIMATE);
    widget.clear();
  }
  ...
  public void testClear() {
    ...
    verify(mockDownloadSpeedTester, never())
      .testDownloadSpeed(any(AsyncCallback.class));
  }
  ...
  public void testSpeedTestSuccess() {
    ...
    verify(mockDownloadSpeedTester)
      .testDownloadSpeed(asyncCallbackCaptor.capture());
