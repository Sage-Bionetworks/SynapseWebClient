...
public class WidgetEncodingUtilTest {
	...
	public void testRoundTripEncodeDecode() {
		...
		// SWC-4685: verify that if these were previously encoded in the markdown, they're still
		// properly decoded
		encoded = "%2F %23 %3A %21";
		out = WidgetEncodingUtil.decodeValue(encoded);
		assertEquals("/ # : !", out);
		...
		String oldParamName = "oldNonEscapedDot.png"; // now periods are escaped, but decoding one of
																									// these old values should not fail
		...
		assertEquals("$1$2$3",
				WidgetEncodingUtil.decodeValue(WidgetEncodingUtil.encodeValue("$1$2$3")));
		assertEquals("$1$2$34",
				WidgetEncodingUtil.decodeValue(WidgetEncodingUtil.encodeValue("$1$2$34")));
		assertEquals("$1$2$345",
				WidgetEncodingUtil.decodeValue(WidgetEncodingUtil.encodeValue("$1$2$345")));
		assertEquals("$1$2$3456",
				WidgetEncodingUtil.decodeValue(WidgetEncodingUtil.encodeValue("$1$2$3456")));
		assertEquals("syn1234/version/5",
				WidgetEncodingUtil.decodeValue(WidgetEncodingUtil.encodeValue("syn1234/version/5"))); // regression
																																															// test
	}
