...
public class ButtonLinkWidgetTest {
	...
	public void setup() {
		...
		widget = new ButtonLinkWidget(mockView, mockGwt, mockAuthController, mockAppPlaceHistoryMapper,
				mockJsClient);
	...
	public void testConfigureHighlightNull() {
		...
		verify(mockView).configure(eq(wikiKey), eq(buttonText), eq(validExternalUrl), eq(false),
				eq(true));
	}
	...
	public void testConfigureHighlightTrue() {
		...
		verify(mockView).configure(eq(wikiKey), eq(buttonText), eq(validExternalUrl), eq(true),
				eq(true));
	}
	...
	public void testConfigureLoggedInIncludePrincipalIdTrueWithoutOtherParams() {
		...
		widget.configure(wikiKey, descriptor, null, null);
		verify(mockView).configure(eq(wikiKey), eq(buttonText),
				eq(validExternalUrl + "?" + SYNAPSE_USER_ID_QUERY_PARAM + currentUserId), eq(false),
				eq(true));
	}
	...
	public void testConfigureLoggedInIncludePrincipalIdTrueWithOtherParams() {
		...
		widget.configure(wikiKey, descriptor, null, null);
		verify(mockView).configure(eq(wikiKey), eq(buttonText),
				eq(originalUrl + "&" + SYNAPSE_USER_ID_QUERY_PARAM + currentUserId), eq(false), eq(true));
	}
