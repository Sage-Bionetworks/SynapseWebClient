...
public class ImageWidgetTest {
	...
	public void setup() throws JSONObjectAdapterException {
		...
		widget = new ImageWidget(mockView, mockAuthenticationController, mockPresignedURLAsyncHandler,
				mockSynapseJavascriptClient, mockSynAlert);
		...
		AsyncMockStubber.callSuccessWith(mockFileResult).when(mockPresignedURLAsyncHandler)
	...
	public void testConfigureFromSynapseId() {
		AsyncMockStubber.callSuccessWith(mockFileEntity).when(mockSynapseJavascriptClient)
		...
		verify(mockSynapseJavascriptClient).getEntityForVersion(eq(synId), eq((Long) null),
				any(AsyncCallback.class));
		verify(mockPresignedURLAsyncHandler).getFileResult(any(FileHandleAssociation.class),
				any(AsyncCallback.class));
		boolean isLoggedIn = true;
		verify(mockView).configure(eq(PRESIGNED_URL), eq(FILE_NAME), anyString(), anyString(),
				eq(ALT_TEXT), eq(synId), eq(isLoggedIn));
	}
	...
	public void testConfigureFromSynapseIdWithVersion() {
		...
		verify(mockSynAlert).clear();
		verify(mockSynapseJavascriptClient).getEntityForVersion(eq(synId), eq(version),
				any(AsyncCallback.class));
	...
	public void testConfigureFromWikiAttachment() {
		...
		AsyncMockStubber.callSuccessWith(fileHandles).when(mockSynapseJavascriptClient)
				.getWikiAttachmentFileHandles(any(WikiPageKey.class), anyLong(), any(AsyncCallback.class));
		...
		verify(mockSynAlert).clear();
		verify(mockSynapseJavascriptClient).getWikiAttachmentFileHandles(any(WikiPageKey.class),
				eq(wikiVersion), any(AsyncCallback.class));
		verify(mockPresignedURLAsyncHandler).getFileResult(fhaCaptor.capture(),
				any(AsyncCallback.class));
		FileHandleAssociation fha = fhaCaptor.getValue();
		...
		boolean isLoggedIn = true;
		verify(mockView).configure(eq(PRESIGNED_URL), eq(FILE_NAME), anyString(), anyString(),
				eq(ALT_TEXT), eq((String) null), eq(isLoggedIn));
	}
