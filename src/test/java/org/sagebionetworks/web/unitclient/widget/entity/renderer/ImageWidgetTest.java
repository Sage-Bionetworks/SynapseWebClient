...
public class ImageWidgetTest {
  ...
  public void testConfigureFromSynapseIdWithVersion() {
    ...
    verify(mockSynAlert).clear();
    verify(mockSynapseJavascriptClient)
      .getEntityForVersion(eq(synId), eq(version), any(AsyncCallback.class));
    verify(mockPresignedURLAsyncHandler)
  ...
  public void testAlignmentStyles() {
    ...
    assertEquals(
      FLOAT_LEFT_STYLES,
      getAlignmentStyleNames(WidgetConstants.FLOAT_LEFT)
    );
    assertEquals(
      FLOAT_RIGHT_STYLES,
      getAlignmentStyleNames(WidgetConstants.FLOAT_RIGHT)
    );
    assertEquals(
      ALIGN_CENTER_STYLES,
      getAlignmentStyleNames(WidgetConstants.FLOAT_CENTER)
    );
  }
}
