...
public class PlotlyWidgetTest {
  ...
  public void setup() {
    ...
    when(mockQueryResultBundle.getMaxRowsPerPage())
      .thenReturn(MAX_ROWS_PER_PAGE);
  ...
  public void testConfigure() {
    ...
    assertEquals(
      ...
      ((PlotlyTraceWrapper) traces.get(0)).getType()
    );
    assertEquals(
      isHorizontal,
      ((PlotlyTraceWrapper) traces.get(0)).isHorizontal()
    );
  ...
  public void testTransformWithFill() {
    ...
    List<PlotlyTraceWrapper> traces = PlotlyWidget.transform(
      xAxisColumnName,
      fillColumnName,
      graphType,
      graphData
    ...
    PlotlyTraceWrapper a = traces.get(0).getName().equals("a")
      ? traces.get(0)
      : traces.get(1);
  ...
  public void testTransformWithXColumnFill() {
    ...
    assertEquals(2, traces.size());
    PlotlyTraceWrapper a = traces.get(0).getName().equals("1")
      ? traces.get(0)
      : traces.get(1);
    assertEquals(2, a.getX().length);
    assertEquals("40", a.getY()[0]);
  ...
  public void testXColumnIsAlias() {
    ...
    rows.add(mockRow);
    sql =
      "select hour(FROM_UNIXTIME(\"createdOn\"/1000)) as \"x\", y1, y2 from " +
      TABLE_ID +
      " where x>10";
    params.put(TABLE_QUERY_KEY, sql);
