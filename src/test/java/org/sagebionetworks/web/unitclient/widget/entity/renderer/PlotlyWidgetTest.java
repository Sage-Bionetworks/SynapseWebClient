...
public class PlotlyWidgetTest {
	...
	public void testConfigure() {
		...
		rowValues.add("1.1");
		rowValues.add("2.2");
		// add DEFAULT_LIMIT rows to the result (the max, so the widget will attempt to look for a
		// second page of data).
		...
		verify(mockJobTracker).startAndTrack(eq(AsynchType.TableQuery),
				queryBundleRequestCaptor.capture(), eq(AsynchronousProgressWidget.WAIT_MS),
		...
		verify(mockView, never()).showChart(eq(plotTitle), anyString(), anyString(), anyList(),
				anyString(), any(), any(), anyBoolean());

		// test final page
		rows.clear();
		verify(mockJobTracker, times(2)).startAndTrack(eq(AsynchType.TableQuery),
				queryBundleRequestCaptor.capture(), eq(AsynchronousProgressWidget.WAIT_MS),
				jobTrackerCallbackCaptor.capture());
		...
		// only sets the loading message on the third page (to avoid always showing "Loaded
		// DEFAULT_LIMIT rows" message).
		verify(mockView).setLoadingMessage("Loading...");

		jobTrackerCallbackCaptor.getValue().onComplete(mockQueryResultBundle);
		verify(mockView).showChart(eq(plotTitle), eq(xAxisLabel), eq(yAxisLabel),
				plotlyTraceArrayCaptor.capture(), eq(mode.toString().toLowerCase()), eq(xAxisType),
				eq(yAxisType), eq(showLegend));
	...
	public void testTransformWithFill() {
		 ...
		 * | | 2 | a | 60 |
		 * 
		 * For each y, there should be a new series for each fill value. So this should result in 2
		 * series, one for 'a' and one for 'b'.
		 */
		...
		List<PlotlyTraceWrapper> traces =
				PlotlyWidget.transform(xAxisColumnName, fillColumnName, graphType, graphData);
	...
	public void testInitializeHorizontalOrientation() {
		...
		verify(mockView).showChart(anyString(), eq(yAxisTitle), eq(xAxisTitle),
				plotlyTraceArrayCaptor.capture(), anyString(), any(), any(), anyBoolean());
		List traces = plotlyTraceArrayCaptor.getValue();
		assertTrue(traces.size() > 0);
		assertEquals(isHorizontal, ((PlotlyTraceWrapper) traces.get(0)).isHorizontal());
	...
	public void testXColumnIsAlias() {
		...
		rowValues.add("2.2");
		rows.add(mockRow);
		sql = "select hour(FROM_UNIXTIME(\"createdOn\"/1000)) as \"x\", y1, y2 from " + TABLE_ID
				+ " where x>10";
		params.put(TABLE_QUERY_KEY, sql);
