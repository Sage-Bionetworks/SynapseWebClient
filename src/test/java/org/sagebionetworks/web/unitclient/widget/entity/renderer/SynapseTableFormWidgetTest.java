...
public class SynapseTableFormWidgetTest {
  ...
  public void testConfigure() {
    ...
    verify(mockSynapseJavascriptClient)
      .getColumnModelsForTableEntity(eq(TABLE_ID), any(AsyncCallback.class));
  ...
  public void testOnSubmitInvalid() throws RequestException {
    ...
    verify(mockSynAlert).clear();
    verify(mockSynAlert)
      .showError(QueryResultEditorWidget.SEE_THE_ERRORS_ABOVE);
    verify(mockAsynchronousJobTracker, never())
      .startAndTrack(
        any(AsynchType.class),
        any(AsynchronousRequestBody.class),
        anyInt(),
        any(UpdatingAsynchProgressHandler.class)
      );
  }
