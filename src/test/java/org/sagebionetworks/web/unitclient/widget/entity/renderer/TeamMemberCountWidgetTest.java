...
public class TeamMemberCountWidgetTest {
	...
	public void before() throws RestServiceException, JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(TOTAL_COUNT).when(mockSynapseClient)
				.getTeamMemberCount(anyString(), any(AsyncCallback.class));
	}

	@Test
	public void testHappyCaseConfigure() throws Exception {
		widget.configure(new WikiPageKey("syn123", ObjectType.ENTITY.toString(), null), descriptor,
				null, null);
	...
	public void testHappyCaseNoParticipants() throws Exception {
		AsyncMockStubber.callSuccessWith(0L).when(mockSynapseClient).getTeamMemberCount(anyString(),
	...
	public void testGetChallengeTeamsFailure() throws Exception {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).getTeamMemberCount(anyString(),
				any(AsyncCallback.class));
		widget.configure(new WikiPageKey("syn123", ObjectType.ENTITY.toString(), null), descriptor,
				null, null);
		verify(mockSynAlert).clear();
