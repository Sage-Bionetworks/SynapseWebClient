...
public class TeamMembersWidgetTest {
	...
	public void before() throws RestServiceException, JSONObjectAdapterException {
		...
		widget = new TeamMembersWidget(mockView, mockPaginationWidget, mockJsClient, mockSynAlert,
				mockGinInjector);
		descriptor = new HashMap<String, String>();
		descriptor.put(WidgetConstants.TEAM_ID_KEY, TEAM_ID);
		when(mockGinInjector.getTeamMemberRowWidget()).thenReturn(mockRow);
		AsyncMockStubber.callSuccessWith(getTestUserProfilePagedResults()).when(mockJsClient)
				.getTeamMembers(anyString(), anyString(), any(TeamMemberTypeFilterOptions.class), anyInt(),
	...
	public void testHappyCaseConfigure() throws Exception {
		...
		verify(mockJsClient).getTeamMembers(anyString(), anyString(),
				eq(TeamMemberTypeFilterOptions.ALL), anyInt(), anyInt(), any(AsyncCallback.class));
	...
	public void testHappyCaseNoParticipants() throws Exception {
		AsyncMockStubber.callSuccessWith(getEmptyUserProfilePagedResults()).when(mockJsClient)
				.getTeamMembers(anyString(), anyString(), any(TeamMemberTypeFilterOptions.class), anyInt(),
						anyInt(), any(AsyncCallback.class));
		widget.configure(new WikiPageKey(entityId, ObjectType.ENTITY.toString(), null), descriptor,
				null, null);
