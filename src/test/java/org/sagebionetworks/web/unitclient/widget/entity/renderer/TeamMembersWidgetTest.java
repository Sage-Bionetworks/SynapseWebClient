...
public class TeamMembersWidgetTest {
	...
	public void before() throws RestServiceException, JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(getTestUserProfilePagedResults()).when(mockSynapseClient).getTeamMembers(anyString(), anyString(), any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(), any(AsyncCallback.class));
	}
	...
	public void testHappyCaseConfigure() throws Exception {
		...
		verify(mockSynapseClient).getTeamMembers(anyString(), anyString(), eq(TeamMemberTypeFilterOptions.ALL), anyInt(), anyInt(), any(AsyncCallback.class));
	...
	public void testHappyCaseNoParticipants() throws Exception {
		AsyncMockStubber.callSuccessWith(getEmptyUserProfilePagedResults()).when(mockSynapseClient).getTeamMembers(anyString(), anyString(), any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(), any(AsyncCallback.class));
		widget.configure(new WikiPageKey(entityId, ObjectType.ENTITY.toString(), null), descriptor, null, null);
