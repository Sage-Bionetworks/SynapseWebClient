...
public class WikiSubpageOrderEditorTreeTest {
	...
	public void before() {
		...
		tree.configure(selectWikiPageId, mockWikiPageKey, wikiHeaders, ownerObjectName,
				mockV2WikiOrderHint, mockCallback);

		AsyncMockStubber.callSuccessWith(mockV2WikiOrderHint).when(mockSynapseJavascriptClient)
				.updateV2WikiOrderHint(any(WikiPageKey.class), any(V2WikiOrderHint.class),
						any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockV2WikiPage).when(mockSynapseJavascriptClient)
				.updateV2WikiPage(any(WikiPageKey.class), any(V2WikiPage.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockV2WikiPage).when(mockSynapseJavascriptClient)
				.getV2WikiPage(any(WikiPageKey.class), any(AsyncCallback.class));
	}
	...
	public void testMoveSelectedItemUp() {
		...
		verify(mockSynapseJavascriptClient).updateV2WikiOrderHint(any(WikiPageKey.class),
				any(V2WikiOrderHint.class), any(AsyncCallback.class));
	...
	public void testMoveSelectedItemLeft() {
		...
		verify(mockSynapseJavascriptClient).getV2WikiPage(wikiPageKeyCaptor.capture(),
				any(AsyncCallback.class));
		assertEquals(c.getId(), wikiPageKeyCaptor.getValue().getWikiPageId());
		verify(mockV2WikiPage).setParentWikiId(a.getId());
		verify(mockSynapseJavascriptClient).updateV2WikiPage(any(WikiPageKey.class),
				any(V2WikiPage.class), any(AsyncCallback.class));
