...
public class WikiSubpagesOrderEditorTest {
	...
	public void before() {
		...
		AsyncMockStubber.callSuccessWith(mockWikiHeaders).when(mockJsClient)
				.getV2WikiHeaderTree(anyString(), anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockHint).when(mockJsClient)
	...
	public void testConfigure() {
		...
		verify(mockEditorTree).configure(eq((String) null), eq(mockPageKey), eq(mockWikiHeaders),
				eq(OWNER_OBJECT_NAME), eq(mockHint), any(CallbackP.class));
	...
	public void testConfigureGetHeaderTreeFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).getV2WikiHeaderTree(anyString(),
				anyString(), any(AsyncCallback.class));
		editor.configure(mockPageKey, OWNER_OBJECT_NAME);
		verify(mockJsClient).getV2WikiHeaderTree(anyString(), anyString(), any(AsyncCallback.class));
		verify(mockSynAlert).handleException(ex);
		verify(mockJsClient, never()).getV2WikiOrderHint(any(WikiPageKey.class),
