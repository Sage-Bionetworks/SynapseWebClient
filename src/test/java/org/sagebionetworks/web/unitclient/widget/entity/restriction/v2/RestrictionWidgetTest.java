...
public class RestrictionWidgetTest {
  ...
  public static final String CURRENT_URL =
    "https://www.synapse.org/review-my-data";

  @Before
  public void before() {
    widget =
      new RestrictionWidget(
        mockView,
    ...
    AsyncMockStubber
      .callSuccessWith(mockRestrictionInformation)
      .when(mockSynapseJavascriptClient)
      .getRestrictionInformation(
        anyString(),
        any(RestrictableObjectType.class),
  ...
  public void testImposeRestrictionClickedYesIsSelected() {
    ...
    verify(mockDataAccessClient)
      .createLockAccessRequirement(eq(ENTITY_ID), any(AsyncCallback.class));
  }
  ...
  public void testShowFolderRestrictionUI() {
    ...
    verify(mockView, never())
      .configureCurrentAccessComponent(anyString(), anyLong());
    verify(mockView).showFolderRestrictionsLink(ENTITY_ID);
  }

  @Test
  public void testShowChangeLinkNotCalledForDatasets() {
    // SWC-5909
    widget.setShowChangeLink(true);
    when(mockRestrictionInformation.getRestrictionLevel())
      .thenReturn(RestrictionLevel.OPEN);
