...
public class RestrictionWidgetTest {
	...
	@Before
	public void before() {
		widget = new RestrictionWidget(mockView, mockAuthenticationController, mockDataAccessClient,
				mockSynAlert, mockIsACTMemberAsyncHandler, mockSynapseJavascriptClient, mockGwt,
				mockJsniUtils);
		when(mockAuthenticationController.getCurrentUserProfile()).thenReturn(mockUserProfile);
		when(mockAuthenticationController.isLoggedIn()).thenReturn(true);
		AsyncMockStubber.callSuccessWith(mockRestrictionInformation).when(mockSynapseJavascriptClient)
				.getRestrictionInformation(anyString(), any(RestrictableObjectType.class),
	...
	public void testHideChangeLink2() {
		...
		when(mockRestrictionInformation.getRestrictionLevel())
				.thenReturn(RestrictionLevel.RESTRICTED_BY_TERMS_OF_USE);
		boolean canChangePermissions = true;
		widget.configure(mockEntity, canChangePermissions);
	...
	public void testImposeRestrictionClickedYesIsSelected() {
		...
		verify(mockView).setImposeRestrictionModalVisible(false);
		verify(mockDataAccessClient).createLockAccessRequirement(eq(ENTITY_ID),
				any(AsyncCallback.class));
	}
	...
	public void testImposeRestrictionCancelButtonClicked() {
		...
		verify(mockDataAccessClient, never()).createLockAccessRequirement(anyString(),
				any(AsyncCallback.class));
		verify(mockView).setImposeRestrictionModalVisible(false);
	}
	...
	public void testReportIssueClicked() {
		...
		verify(mockJsniUtils).showJiraIssueCollector("", // summary
				...
				FLAG_ISSUE_COLLECTOR_URL, OWNER_ID,
				DisplayUtils.getDisplayName(FIRST_NAME, LAST_NAME, USERNAME), EMAIL, ENTITY_ID, // Synapse
																																												// data
																																												// object ID
