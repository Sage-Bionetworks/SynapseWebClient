...
public class TablesTabTest {
	...
	public void testClickTable() {
		// verify that clicking on an item in the tables list sends the event back to the entity page
		// top (to get the new target entity)
	...
	private void verifyTableConfiguration(Long version) {
		...
		verify(mockTableEntityWidget).configure(mockTableEntityBundle, version, true, tab,
				mockActionMenuWidget);
		verify(mockView).setTableEntityWidget(any(Widget.class));
		verify(mockModifiedCreatedBy).configure(any(Date.class), anyString(), any(Date.class),
				anyString());
		...
		String expectedProvEntityList =
				DisplayUtils.createEntityVersionString(mockTableEntityBundle.getEntity().getId(), version);
	...
	public void testConfigureUsingProject() {
		...
		verify(mockModifiedCreatedBy, Mockito.never()).configure(any(Date.class), anyString(),
