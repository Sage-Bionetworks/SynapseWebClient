...
public class InviteWidgetTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		inviteWidget = new InviteWidget(mockView, mockSynapseClient, mockGWTWrapper, mockSynAlert,
				mockSuggestBox, mockSuggestionProvider);
	...
	public void testSendInvitation() throws Exception {
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).inviteMember(anyString(),
				anyString(), anyString(), anyString(), any(AsyncCallback.class));
		...
		verify(mockSynapseClient).inviteMember(eq(userId), anyString(), anyString(),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), any(AsyncCallback.class));
	...
	public void testSendToEmailAddress() {
		...
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).inviteNewMember(anyString(),
				anyString(), anyString(), anyString(), any(AsyncCallback.class));
		inviteWidget.doSendInvites(invitationMessage);
		verify(mockSynapseClient).inviteNewMember(eq(email), anyString(), anyString(),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), any(AsyncCallback.class));
		verify(mockRefreshCallback).invoke();
		verify(mockView).hide();
	}
	...
	public void testSendToEmailAddressInvalidEmail() {
		...
		verify(mockSynAlert).showError(InviteWidget.INVALID_EMAIL_ERROR_MESSAGE);
		verify(mockSynapseClient, never()).inviteNewMember(anyString(), anyString(), anyString(),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), any(AsyncCallback.class));
	...
	public void testSendInvitationFailure() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(false).when(mockSynapseClient).isTeamMember(anyString(),
				anyLong(), any(AsyncCallback.class));
		AsyncMockStubber.callFailureWith(caught).when(mockSynapseClient).inviteMember(anyString(),
				anyString(), anyString(), anyString(), any(AsyncCallback.class));
		when(mockSuggestBox.getSelectedSuggestion()).thenReturn(mockSuggestion);
		when(mockSuggestion.getHeader()).thenReturn(mockHeader);
	...
	public void testInviteMultipleUsers() {
		...
		verify(mockSynapseClient).inviteNewMember(eq(email1), eq(teamId), eq(invitationMessage),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), any(AsyncCallback.class));
		verify(mockSynapseClient).inviteNewMember(eq(email2), eq(teamId), eq(invitationMessage),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), any(AsyncCallback.class));
		verify(mockSynapseClient).inviteMember(eq(userId), eq(teamId), eq(invitationMessage),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), any(AsyncCallback.class));
		verify(mockRefreshCallback).invoke();
		verify(mockView).hide();
	}
