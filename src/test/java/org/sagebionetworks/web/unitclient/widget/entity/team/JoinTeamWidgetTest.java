...
public class JoinTeamWidgetTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callSuccessWith(true).when(mockSynapseClient).hasAccess(anyString(),
				anyString(), anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(ars).when(mockSynapseClient)
				.getTeamAccessRequirements(anyString(), any(AsyncCallback.class));


		joinWidget = new JoinTeamWidget(mockView, mockSynapseClient, mockGlobalApplicationState,
				mockAuthenticationController, mockGwt, mockWikiPageWidget, mockWizardProgress, mockSynAlert,
				mockCookies);
		...
		joinWidget.configure(teamId, false, status, mockTeamUpdatedCallback, null, null, null, null,
				false);

		AsyncMockStubber.callSuccessWith(status).when(mockSynapseClient).requestMembership(anyString(),
				anyString(), anyString(), anyString(), any(Date.class), any(AsyncCallback.class));

		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient)
				.createAccessApproval(any(AccessApproval.class), any(AsyncCallback.class));
	...
	public void testSimpleRequestButton() throws Exception {
		...
		// joinWidget.configure(teamId, isChallenge, status, mockTeamUpdatedCallback, isMemberMessage,
		// successMessage, buttonText, openRequestText, isSimpleRequestButton);
		// verify(mockView).configure(true, status, isMemberMessage, buttonText, openRequestText,
	...
	public void testInvitedToTeamWithAccessRequirements() throws Exception {
		// set up the team membership status so that the current user has been invited to a team (that
		// requires membership approval) that has unmet access requirements.
		reset(mockView);
		...
		boolean isChallenge = false;
		boolean isSimpleRequestButton = true;
		joinWidget.configure(teamId, isChallenge, mockStatus, mockTeamUpdatedCallback, isMemberMessage,
				successMessage, buttonText, openRequestText, isSimpleRequestButton);
	...
	public void testJoinRequestStep2NoRestriction() throws Exception {
		...
		verify(mockSynapseClient).requestMembership(anyString(), anyString(), anyString(),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), eq((Date) null), any(AsyncCallback.class));
	}
	...
	public void testGetTotalPageCount() throws Exception {
		...
		joinWidget.configure(teamId, isChallengeSignup, status, mockTeamUpdatedCallback, null, null,
				null, null, false);
		joinWidget.sendJoinRequestStep0();
		// one page for the AR
	...
	public void testJoinRequestStep3Failure() throws Exception {
		Exception ex = new Exception("unhandled exception");
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).requestMembership(anyString(),
				anyString(), anyString(), anyString(), any(Date.class), any(AsyncCallback.class));
	...
	public void testJoinRequestStep3WikiRefresh() throws Exception {
		...
		descriptor.put(WidgetConstants.JOIN_WIDGET_REQUEST_EXPIRES_IN_X_DAYS_KEY,
				requestExpiresInXDays.toString());
		...
		verify(mockSynapseClient).requestMembership(anyString(), anyString(), anyString(),
				eq(EvaluationSubmitterTest.HOST_PAGE_URL), dateCaptor.capture(), any(AsyncCallback.class));
		verify(mockView).showInfo(anyString());
	...
	public void testSetLicenseAccepted() throws Exception {
		...
		verify(mockSynapseClient).createAccessApproval(any(AccessApproval.class),
	...
	public void testSetLicenseAcceptedACT() throws Exception {
		...
		verify(mockSynapseClient, never()).createAccessApproval(any(AccessApproval.class),
				any(AsyncCallback.class));
		verify(mockView).hideJoinWizard();
	}
	...
	public void testIsRecognizedSite() {
		if (JoinTeamWidget.EXTRA_INFO_URL_WHITELIST.length > 0) {
			...
			// but not other sites
			assertFalse(JoinTeamWidget.isRecognizedSite(
					"http://mpmdev.ondemand.sas.com/projectdatasphere/html/registration/challenge")); // not
																																														// https
	...
	public void testGetEncodedParamValueIfDefined() {
		...
		assertEquals(WebConstants.USER_ID_PARAM + "=&",
				joinWidget.getEncodedParamValue(WebConstants.USER_ID_PARAM, null, "&"));
		// or empty string value
		assertEquals(WebConstants.USER_ID_PARAM + "=&",
				joinWidget.getEncodedParamValue(WebConstants.USER_ID_PARAM, "", "&"));

		// if defined, return <paramkey>=<url-encoded-value><suffix>
		when(mockGwt.encodeQueryString("bar")).thenReturn("encodedbar");
		assertEquals(WebConstants.USER_ID_PARAM + "=encodedbar&",
				joinWidget.getEncodedParamValue(WebConstants.USER_ID_PARAM, "bar", "&"));
	}
