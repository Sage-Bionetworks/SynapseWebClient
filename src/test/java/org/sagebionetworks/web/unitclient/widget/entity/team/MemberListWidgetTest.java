...
public class MemberListWidgetTest {
	...
	public void before() throws JSONObjectAdapterException {
		widget = new MemberListWidget(mockView, mockSynapseClient, mockJsClient,
				mockAuthenticationController, mockGlobalApplicationState, mockMembersContainer);
		isAdmin = true;

		AsyncMockStubber.callSuccessWith(getTestTeamMembers()).when(mockJsClient).getTeamMembers(
				anyString(), anyString(), any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).deleteTeamMember(anyString(),
				anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).setIsTeamAdmin(anyString(),
				anyString(), anyString(), anyBoolean(), any(AsyncCallback.class));
	}
	...
	public void testConfigure() {
		...
		verify(mockJsClient).getTeamMembers(anyString(), anyString(), eq(memberType), anyInt(),
	...
	public void testConfigureFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).getTeamMembers(anyString(), anyString(),
				any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(), any(AsyncCallback.class));
		widget.configure(teamId, isAdmin, memberType, mockTeamUpdatedCallback);
		verify(mockJsClient).getTeamMembers(anyString(), anyString(), eq(memberType), anyInt(),
				anyInt(), any(AsyncCallback.class));
	...
	public void testRemoveMemberFailure() {
		...
		String error = "unhandled exception";
		Exception ex = new Exception(error);
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).deleteTeamMember(anyString(),
				anyString(), any(AsyncCallback.class));
		widget.removeMember("a user id");
		verify(mockJsClient).deleteTeamMember(anyString(), anyString(), any(AsyncCallback.class));
		verify(mockView).showErrorMessage(error);
	...
	public void testSetIsAdmin() {
		...
		verify(mockSynapseClient).setIsTeamAdmin(anyString(), anyString(), anyString(), anyBoolean(),
				any(AsyncCallback.class));
		verify(mockView).showInfo(anyString());
		verify(mockTeamUpdatedCallback).invoke();
	}
	...
	public void testSetIsAdminFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).setIsTeamAdmin(anyString(),
				anyString(), anyString(), anyBoolean(), any(AsyncCallback.class));
		widget.setIsAdmin("a user id", true);
		verify(mockSynapseClient).setIsTeamAdmin(anyString(), anyString(), anyString(), anyBoolean(),
				any(AsyncCallback.class));
		verify(mockView).showErrorMessage(message);;
		// called twice. once during configuration, and once to refresh members to get correct admin
		// state
		verify(mockJsClient, times(2)).getTeamMembers(anyString(), anyString(),
				any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(), any(AsyncCallback.class));
	}
	...
	public void testSearch() {
		...
		AsyncMockStubber.callSuccessWith(searchResults).when(mockJsClient).getTeamMembers(anyString(),
				anyString(), any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(),
				any(AsyncCallback.class));

		widget.search(searchTerm);

		verify(mockJsClient).getTeamMembers(anyString(), eq(searchTerm),
				any(TeamMemberTypeFilterOptions.class), anyInt(), anyInt(), any(AsyncCallback.class));
	}
}
