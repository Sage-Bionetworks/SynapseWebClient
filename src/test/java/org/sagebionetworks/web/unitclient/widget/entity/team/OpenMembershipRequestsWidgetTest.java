...
public class OpenMembershipRequestsWidgetTest {
	...
	public void before() {
		...
		widget =
				new OpenMembershipRequestsWidget(mockView, mockSynapseClient, mockGlobalApplicationState,
						mockGwt, mockSynAlert, mockPopupUtils, mockDateTimeUtils, mockJsClient);
		membershipRequests = new ArrayList<>();
		AsyncMockStubber.callSuccessWith(membershipRequests).when(mockSynapseClient)
				.getOpenRequests(anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).addTeamMember(anyString(),
	...
	public void testConfigureFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).getOpenRequests(anyString(),
	...
	public void testAcceptRequest() {
		...
		verify(mockSynapseClient).addTeamMember(eq(USER_ID), eq(TEAM_ID), anyString(),
	...
	public void testAcceptRequestFailure() {
		Exception ex = new Exception();
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).addTeamMember(anyString(),
				anyString(), anyString(), anyString(), any(AsyncCallback.class));
		widget.configure(TEAM_ID, mockCallback);
		widget.acceptRequest(USER_ID);
		verify(mockSynapseClient).addTeamMember(eq(USER_ID), eq(TEAM_ID), anyString(),
