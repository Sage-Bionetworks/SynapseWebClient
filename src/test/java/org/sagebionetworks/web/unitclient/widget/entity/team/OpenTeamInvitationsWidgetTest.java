...
public class OpenTeamInvitationsWidgetTest {
	...
	public void before() throws JSONObjectAdapterException {
		...
		widget = new OpenTeamInvitationsWidget(mockView, mockSynapseClient, mockGlobalApplicationState,
				mockAuthenticationController, mockPortalGinInjector, mockSynapseAlert, mockDateTimeUtils,
				mockPopupUtils, mockJsClient);
		testTeam = new Team();
	...
	public void testConfigure() throws Exception {
		...
		verify(mockView).addTeamInvite(any(Team.class), any(String.class),
				eq(RECEIVED + FORMATTED_DATE), anyString(), any(Widget.class));
		...
		verify(mockView, times(2)).addTeamInvite(any(Team.class), anyString(),
				eq(RECEIVED + FORMATTED_DATE), anyString(), any(Widget.class));
	...
	public void testConfigureFailureGetOpenInvites() throws Exception {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).getOpenInvitations(anyString(),
