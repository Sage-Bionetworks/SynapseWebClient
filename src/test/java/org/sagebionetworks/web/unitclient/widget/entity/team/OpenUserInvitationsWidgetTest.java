...
public class OpenUserInvitationsWidgetTest {
  ...
  public void before() throws JSONObjectAdapterException {
    ...
    when(mockGinInjector.getEmailInvitationBadgeWidget())
      .thenReturn(mockEmailInvitationBadge);
  }

  private void setupGetOpenTeamInvitations(
    int userInvitationCount,
    int emailInvitationCount
  ) {
    List<OpenTeamInvitationBundle> testReturn = new ArrayList<>();
    for (int i = 0; i < userInvitationCount; i++) {
      OpenTeamInvitationBundle mockBundle = mock(
        OpenTeamInvitationBundle.class
      );
  ...
  public void testConfigureMultipleInvitations() {
    ...
    verify(mockView, times(invitationCount))
      .addInvitation(
        eq(mockUserBadge),
        eq(null),
        eq(testInvite.getId()),
  ...
  public void testConfigureMixedInvitations() {
    ...
    verify(mockView, times(userInvitationCount))
      .addInvitation(
        eq(mockUserBadge),
  ...
  public void testDeleteOpenInvite() throws Exception {
    ...
    verify(mockJsClient)
      .deleteMembershipInvitation(eq(invitationId), any(AsyncCallback.class));
  ...
  public void testMoreResultsUnavailable() throws Exception {
    setupGetOpenTeamInvitations(
      OpenUserInvitationsWidget.INVITATION_BATCH_LIMIT - 1,
      0
    );
    widget.configure(teamId, mockTeamUpdatedCallback);
  ...
  public void testMoreResultsAvailableGetNextBatch() throws Exception {
    setupGetOpenTeamInvitations(
      OpenUserInvitationsWidget.INVITATION_BATCH_LIMIT,
      0
    );
    widget.configure(teamId, mockTeamUpdatedCallback);
