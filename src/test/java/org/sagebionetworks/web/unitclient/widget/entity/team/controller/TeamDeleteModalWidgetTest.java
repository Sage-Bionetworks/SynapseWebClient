...
public class TeamDeleteModalWidgetTest {
	...
	public void setup() {
		...
		presenter = new TeamDeleteModalWidget(mockSynAlert, mockSynapseClient,
				mockGlobalApplicationState, mockView, mockAuthController);
	...
	public void testOnConfirmSuccess() {
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).deleteTeam(eq(teamId),
	...
	public void testOnConfirmFailure() {
		AsyncMockStubber.callFailureWith(caught).when(mockSynapseClient).deleteTeam(eq(teamId),
				any(AsyncCallback.class));
		presenter.onConfirm();
		verify(mockSynAlert).clear();
		verify(mockSynapseClient).deleteTeam(eq(teamId), any(AsyncCallback.class));
