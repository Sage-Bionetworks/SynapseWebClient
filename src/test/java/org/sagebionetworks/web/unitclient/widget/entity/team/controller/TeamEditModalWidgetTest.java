...
public class TeamEditModalWidgetTest {
	...
	public void setup() {
		...
		when(mockSynapseProperties.getSynapseProperty(WebConstants.AUTHENTICATED_ACL_PRINCIPAL_ID))
				.thenReturn(AUTHENTICATED_USERS_GROUP_ID.toString());
		presenter = new TeamEditModalWidget(mockSynAlert, mockView, mockSynapseClient, mockUploader,
				mockJSNIUtils, mockAuthenticationController, mockSynapseProperties, mockPortalGinInjector);
		AsyncMockStubber.callSuccessWith(acl).when(mockSynapseClient).getTeamAcl(eq(TEAM_ID.toString()),
		...
		AsyncMockStubber.callSuccessWith(mockTeam).when(mockSynapseClient).updateTeam(eq(mockTeam),
				eq(acl), any(AsyncCallback.class));
		...
		when(mockJSNIUtils.getFileHandleAssociationUrl(anyString(), any(FileHandleAssociateType.class),
				anyString())).thenReturn(FILE_HANDLE_ASSOCIATION_URL);
	}
	...
	public void testErrorOnTeamACLLoad() {
		...
		AsyncMockStubber.callFailureWith(new Exception(error)).when(mockSynapseClient)
				.getTeamAcl(eq(TEAM_ID.toString()), any(AsyncCallback.class));
		presenter.configureAndShow(mockTeam);
	...
	public void testConfirmFailedChanges() {
		presenter.configureAndShow(mockTeam);
		AsyncMockStubber.callFailureWith(caught).when(mockSynapseClient).updateTeam(eq(mockTeam),
				eq(acl), any(AsyncCallback.class));
	...
	public void testUpdateACLFromViewAuthenticatedUsersCanSend() {
		presenter.configureAndShow(mockTeam);
		// set up team can message, should not be touched in the acl. Authenticated users should be
		// added.
		...
		assertEquals(ModelConstants.TEAM_MESSENGER_PERMISSIONS,
				authenticatedUsersGroupRA.getAccessType());
	}
