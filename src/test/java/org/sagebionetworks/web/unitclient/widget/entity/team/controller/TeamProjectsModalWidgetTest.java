...
public class TeamProjectsModalWidgetTest {
	...
	public void setup() {
		...
		when(mockGinInjector.getLoadMoreProjectsWidgetContainer())
				.thenReturn(mockLoadMoreWidgetContainer);
		...
		AsyncMockStubber.callSuccessWith(mockProjectHeaderList).when(mockJsClient).getProjectsForTeam(
				anyString(), anyInt(), anyString(), any(ProjectListSortColumn.class),
	...
	public void testConfigureAndShow() {
		...
		verify(mockJsClient).getProjectsForTeam(eq(teamId), eq(ProfilePresenter.PROJECT_PAGE_SIZE),
				eq(expectedNextPageToken), eq(ProjectListSortColumn.LAST_ACTIVITY), eq(SortDirection.DESC),
	...
	public void testSort() {
		...
		verify(mockJsClient).getProjectsForTeam(eq(teamId), eq(ProfilePresenter.PROJECT_PAGE_SIZE),
				eq(null), eq(ProjectListSortColumn.LAST_ACTIVITY), eq(SortDirection.ASC),
				any(AsyncCallback.class));

		// simulate clicking sort on PROJECT_NAME
		presenter.sort(ProjectListSortColumn.PROJECT_NAME);
		verify(mockJsClient).getProjectsForTeam(eq(teamId), eq(ProfilePresenter.PROJECT_PAGE_SIZE),
				eq(null), eq(ProjectListSortColumn.PROJECT_NAME), eq(SortDirection.DESC),
				any(AsyncCallback.class));

		// simulate clicking sort on PROJECT_NAME again
		presenter.sort(ProjectListSortColumn.PROJECT_NAME);
		verify(mockJsClient).getProjectsForTeam(eq(teamId), eq(ProfilePresenter.PROJECT_PAGE_SIZE),
				eq(null), eq(ProjectListSortColumn.PROJECT_NAME), eq(SortDirection.ASC),
				any(AsyncCallback.class));
	}
}
