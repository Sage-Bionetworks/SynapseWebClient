...
public class AdministerEvaluationsListTest {
	...
	public void setup() throws Exception {
		...
		evalList = new AdministerEvaluationsList(mockView, mockChallengeClient, mockAclEditor,
				mockEvalEditor, mockSynAlert);
		...
		AsyncMockStubber.callSuccessWith(evaluationResults).when(mockChallengeClient)
	...
	public void testConfigureZeroResults() {
		AsyncMockStubber.callSuccessWith(new ArrayList<Evaluation>()).when(mockChallengeClient)
				.getSharableEvaluations(anyString(), any(AsyncCallback.class));
		evalList.configure("syn100");
		verify(mockChallengeClient).getSharableEvaluations(anyString(), any(AsyncCallback.class));
	...
	public void testConfigureFailure() throws Exception {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockChallengeClient)
				.getSharableEvaluations(anyString(), any(AsyncCallback.class));
		evalList.configure("syn100");
		verify(mockChallengeClient).getSharableEvaluations(anyString(), any(AsyncCallback.class));
	...
	public void testOnDeleteEvaluationClicked() {
		AsyncMockStubber.callSuccessWith(null).when(mockChallengeClient).deleteEvaluation(anyString(),
	...
	public void testOnDeleteEvaluationClickedFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockChallengeClient).deleteEvaluation(anyString(),
				any(AsyncCallback.class));
		evalList.onDeleteClicked(e1);
		verify(mockChallengeClient).deleteEvaluation(eq(e1.getId()), any(AsyncCallback.class));
		verify(mockSynAlert).handleException(ex);
		// no refresh
		verify(mockChallengeClient, never()).getSharableEvaluations(anyString(),
				any(AsyncCallback.class));
	}
