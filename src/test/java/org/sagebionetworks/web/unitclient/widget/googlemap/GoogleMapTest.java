...
public class GoogleMapTest {
	...
	public void setUp() throws Exception {
		...
		map = new GoogleMap(mockView, mockSynapseJSNIUtils, mockJsClient, mockSynapseAlert,
				mockPortalGinInjector, mockLazyLoadHelper);
	}
	...
	public void testConfigureNotLoaded() throws RequestException {
		...
		verify(mockJsClient).doGetString(eq(GoogleMap.GOOGLE_MAP_URL), eq(true),
	...
	public void testConfigure() throws RequestException {
		...
		AsyncMockStubber.callSuccessWith(data).when(mockJsClient).doGetString(anyString(), eq(true),
				any(AsyncCallback.class));
	...
	public void testFailedRequest() throws RequestException {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).doGetString(anyString(), eq(true),
				any(AsyncCallback.class));
		map.configure();
		simulateInView();
