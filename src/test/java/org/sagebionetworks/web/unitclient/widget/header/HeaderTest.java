...
public class HeaderTest {
	...
	public void setup() {
		...
		header = new Header(mockView, mockAuthenticationController, mockGlobalApplicationState,
				mockSynapseJavascriptClient, mockFavWidget, mockSynapseJSNIUtils, mockPendoSdk,
				mockPortalGinInjector, mockEventBus, mockCookies, jsonObjectAdapter);
		...
		AsyncMockStubber.callSuccessWith(mockDownloadList).when(mockSynapseJavascriptClient)
	...
	public void testFavoriteAnonymous() {
		...
		// shown.
		// in this case, do not even try to update the favorites, will show whatever we had (possibly
		// stale, like the rest of the page).
	...
	public void testOnCookieNotificationDismissed() {
		...
		verify(mockCookies).setCookie(eq(CookieKeys.COOKIES_ACCEPTED), eq(Boolean.TRUE.toString()),
				any(Date.class));
	}
