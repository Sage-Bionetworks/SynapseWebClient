...
public class LazyLoadWikiWidgetWrapperTest {
	...
	public void testHappyCase() {
		// configure
		widget.configure(WIDGET_CONTENT_TYPE, mockWikiKey, mockWidgetDescriptor,
				mockWidgetRefreshRequired, wikiVersionInView);
		verify(mockLazyLoadHelper).setIsConfigured();
		verify(mockView).showLoading();

		simulateLazyLoadEvent();
		verify(mockWidgetRegistrar).getWidgetRendererForWidgetDescriptorAfterLazyLoad(
				eq(WIDGET_CONTENT_TYPE), callbackCaptor.capture());
		callbackCaptor.getValue().onSuccess(mockWikiWidget);

		verify(mockWikiWidget).configure(mockWikiKey, mockWidgetDescriptor, mockWidgetRefreshRequired,
				wikiVersionInView);
		String className = mockWikiWidget.getClass().getSimpleName();
		verify(mockView).showWidget(any(Widget.class), eq(className));
		verify(mockSynapseJSNIUtils).sendAnalyticsEvent(className,
				LazyLoadWikiWidgetWrapper.LOADED_EVENT_NAME);
	}
