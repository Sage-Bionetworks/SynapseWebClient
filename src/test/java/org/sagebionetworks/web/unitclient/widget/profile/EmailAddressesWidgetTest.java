...
public class EmailAddressesWidgetTest {
	...
	public void setUp() {
		...
		widget = new EmailAddressesWidget(mockView, mockSynapseClient, mockSynapseJavascriptClient,
				mockSynAlert, mockAuthenticationController, mockPopupUtils, mockGwt);
		...
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).removeEmail(anyString(),
				any(AsyncCallback.class));
		...
		AsyncMockStubber.callSuccessWith(mockUserProfile).when(mockSynapseJavascriptClient)
	...
	public void testAdditionalEmailValidation() throws JSONObjectAdapterException {
		...
		verify(mockSynapseJavascriptClient).additionalEmailValidation(eq(USER_ID), eq(EMAIL3),
	...
	public void testAdditionalEmailValidationInvalidEmail() throws JSONObjectAdapterException {
		...
		verify(mockSynapseJavascriptClient, never()).additionalEmailValidation(anyString(), anyString(),
				anyString(), any(AsyncCallback.class));
	}
	...
	public void testOnRemoveEmailFailure() {
		Exception ex = new Exception("unexpected exception");
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).removeEmail(anyString(),
				any(AsyncCallback.class));
