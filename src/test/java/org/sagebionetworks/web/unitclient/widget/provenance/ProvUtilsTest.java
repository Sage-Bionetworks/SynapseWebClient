...
public class ProvUtilsTest {
	...
	public void testBuildProvGraph() throws Exception {
		...
		ProvGraph graph = ProvUtils.buildProvGraph(generatedByActivityId, processedActivities, idToNode,
				refToHeader, true, startRefs, noExpandNodes);
		...
		// find and verify expand nodes are not created for entities with no name (forbidden or not
		// found entities)
	...
	public void testMapReferencesToHeaders() throws Exception {
		...
		ArrayList<EntityHeader> referenceHeaders =
				new ArrayList<EntityHeader>(Arrays.asList(new EntityHeader[] {header}));
