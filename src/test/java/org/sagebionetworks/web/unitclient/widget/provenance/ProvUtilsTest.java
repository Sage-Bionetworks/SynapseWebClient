...
public class ProvUtilsTest {
  ...
  public void testBuildProvGraph() throws Exception {
    ...
    for (ProvGraphNode node : nodes) {
      if (node instanceof EntityGraphNode) {
        if (
          ((EntityGraphNode) node).getEntityId().equals(entity1.getId())
        ) entity1Node = (EntityGraphNode) node;
        if (
          ((EntityGraphNode) node).getEntityId().equals(entity2.getId())
        ) entity2Node = (EntityGraphNode) node;
        if (
          ((EntityGraphNode) node).getEntityId().equals(entity3.getId())
        ...
        if (
          ((ExpandGraphNode) node).getEntityId().equals(entity3.getId())
    ...
    for (ProvGraphEdge edge : edges) {
      if (
        edge.getSource().equals(entity2Node) &&
        edge.getSink() instanceof ExpandGraphNode
      ) {
        foundExpand2 = true;
      }
    }
  ...
  public void testMapReferencesToHeaders() throws Exception {
    ...
    UsedEntity ue = new UsedEntity();
    ue.setReference(ref);
    ArrayList<EntityHeader> referenceHeaders = new ArrayList<EntityHeader>(
      Arrays.asList(new EntityHeader[] { header })
    );

    Map<Reference, EntityHeader> refToHeader = ProvUtils.mapReferencesToHeaders(
      referenceHeaders
    );
