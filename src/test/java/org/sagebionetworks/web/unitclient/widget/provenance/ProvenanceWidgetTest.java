...
public class ProvenanceWidgetTest {
  ...
  public void setup() throws Exception {
    ...
    when(
      mockSynapseJSNIUtils.nChartlayout(
        any(NChartLayersArray.class),
        any(NChartCharacters.class)
      )
    )
      .thenReturn(new LayoutResultTestImpl());
    mockClientCache = mock(ClientCache.class);
    provenanceWidget =
      new ProvenanceWidget(
        mockView,
    ...
    referenceList =
      new ArrayList<Reference>(
        Arrays.asList(new Reference[] { ref123, ref456 })
      );
    referenceHeaders =
      new ArrayList<EntityHeader>(
        Arrays.asList(new EntityHeader[] { header456, header123 })
      );

    ArrayList<Reference> generatedBy = new ArrayList<Reference>(
      Arrays.asList(new Reference[] { ref123 })
    );
    ArrayList<EntityHeader> outputEntityList = new ArrayList<EntityHeader>();
    outputEntityList.add(header123);
    AsyncMockStubber
      .callSuccessWith(outputEntity)
  ...
  private void verifySuccessGraphStructure(ProvGraph graph) {
    ...
    for (ProvGraphNode node : graph.getNodes()) {
      if (node instanceof EntityGraphNode) {
        if (
          ((EntityGraphNode) node).getEntityId().equals(outputEntity.getId())
        ) node123 = node; else if (
          ((EntityGraphNode) node).getEntityId().equals(entity456Id)
        ) node456 = node;
      } else if (node instanceof ActivityGraphNode) {
        if (
          ((ActivityGraphNode) node).getActivityId().equals(act.getId())
        ) nodeAct = node;
      }
    }
  ...
  public void testBuildTreeFailGetActivity() throws Exception {
    AsyncMockStubber
      .callFailureWith(someException)
      .when(mockSynapseJavascriptClient)
      .getActivityForEntityVersion(
        eq(outputEntity.getId()),
        eq(outputEntity.getVersionNumber()),
        any(AsyncCallback.class)
      );
    ...
    assertNotNull(nodeAct);
    assertEquals(
      DisplayConstants.ERROR_PROVENANCE_RELOAD,
      nodeAct.getActivityName()
    );
  }
  ...
  public void testBuildTreeFailHeaderBatch() throws Exception {
    ...
    verify(mockSynapseJavascriptClient)
      .getEntityHeaderBatchFromReferences(
        eq(referenceList),
        any(AsyncCallback.class)
      );
    ProvGraph graph = captureGraph();
  ...
  public void testFindOldVersionsNotFoundException() throws Exception {
    ...
    AsyncMockStubber
      .callFailureWith(new NotFoundException(message))
      .when(mockSynapseJavascriptClient)
      .getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
    // create graph
    provenanceWidget.configure(null, descriptor, null, null);
  ...
  public void testFindOldVersions() throws Exception {
    ...
    ArrayList<EntityHeader> currentVersionBatch = new ArrayList<EntityHeader>(
      Arrays.asList(new EntityHeader[] { header456, header123 })
    );
    // new mocks for batch call
    AsyncMockStubber
      .callSuccessWith(currentVersionBatch)
      .when(mockSynapseJavascriptClient)
      .getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
  ...
  private ProvGraph captureGraph() {
    ArgumentCaptor<ProvGraph> argument = ArgumentCaptor.forClass(
      ProvGraph.class
    );
