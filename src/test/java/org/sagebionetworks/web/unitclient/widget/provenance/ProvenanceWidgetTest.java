...
public class ProvenanceWidgetTest {
	...
	public void setup() throws Exception {
		...
		when(mockSynapseJSNIUtils.nChartlayout(any(NChartLayersArray.class),
				any(NChartCharacters.class))).thenReturn(new LayoutResultTestImpl());
		mockClientCache = mock(ClientCache.class);
		provenanceWidget = new ProvenanceWidget(mockView, mockSynapseJSNIUtils, jsoProvider,
				mockClientCache, mockDateTimeUtils, mockSynapseJavascriptClient, mockSynAlert);
		...
		referenceHeaders =
				new ArrayList<EntityHeader>(Arrays.asList(new EntityHeader[] {header456, header123}));

		ArrayList<Reference> generatedBy =
				new ArrayList<Reference>(Arrays.asList(new Reference[] {ref123}));
		ArrayList<EntityHeader> outputEntityList = new ArrayList<EntityHeader>();
		outputEntityList.add(header123);
		AsyncMockStubber.callSuccessWith(outputEntity).when(mockSynapseJavascriptClient)
				.getEntity(eq(outputEntity.getId()), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(act).when(mockSynapseJavascriptClient)
				.getActivityForEntityVersion(anyString(), anyLong(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(outputEntityList).when(mockSynapseJavascriptClient)
				.getEntityHeaderBatch(anyList(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(referenceHeaders).when(mockSynapseJavascriptClient)
				.getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(generatedBy).when(mockSynapseJavascriptClient)
				.getEntitiesGeneratedBy(eq(act.getId()), anyInt(), anyInt(), any(AsyncCallback.class));
	...
	public void testBuildTreeFailGetActivity() throws Exception {
		AsyncMockStubber.callFailureWith(someException).when(mockSynapseJavascriptClient)
				.getActivityForEntityVersion(eq(outputEntity.getId()), eq(outputEntity.getVersionNumber()),
						any(AsyncCallback.class));

		provenanceWidget.configure(null, descriptor, null, null);
		verify(mockSynapseJavascriptClient).getActivityForEntityVersion(eq(outputEntity.getId()),
				eq(outputEntity.getVersionNumber()), any(AsyncCallback.class));
	...
	public void testBuildTreeFailGetActivity404() throws Exception {
		...
		verify(mockSynapseJavascriptClient).getActivityForEntityVersion(eq(outputEntity.getId()),
				eq(outputEntity.getVersionNumber()), any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient).getEntityHeaderBatchFromReferences(anyList(),
				any(AsyncCallback.class));
		ProvGraph graph = captureGraph();
	...
	public void testBuildTreeFailHeaderBatch() throws Exception {
		...
		verify(mockSynapseJavascriptClient).getActivityForEntityVersion(eq(outputEntity.getId()),
				eq(outputEntity.getVersionNumber()), any(AsyncCallback.class));
		verify(mockSynapseJavascriptClient).getEntityHeaderBatchFromReferences(eq(referenceList),
				any(AsyncCallback.class));
		ProvGraph graph = captureGraph();
	...
	public void testFindOldVersionsNotFoundException() throws Exception {
		...
		when(mockJsniUtils.nChartlayout(any(NChartLayersArray.class), any(NChartCharacters.class)))
				.thenReturn(new LayoutResultTestImpl());
		provenanceWidget = new ProvenanceWidget(mockView, mockJsniUtils, jsoProvider, mockClientCache,
				mockDateTimeUtils, mockSynapseJavascriptClient, mockSynAlert);

		String message = "entity syn999 was not found";
		AsyncMockStubber.callFailureWith(new NotFoundException(message))
	...
	public void testFindOldVersions() throws Exception {
		...
		ArrayList<EntityHeader> currentVersionBatch =
				new ArrayList<EntityHeader>(Arrays.asList(new EntityHeader[] {header456, header123}));
		// new mocks for batch call
		AsyncMockStubber.callSuccessWith(currentVersionBatch).when(mockSynapseJavascriptClient)
				.getEntityHeaderBatchFromReferences(anyList(), any(AsyncCallback.class));
