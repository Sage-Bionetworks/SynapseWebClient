...
public class NChartUtilTest {
  ...
  public void testCreateLayersSimple() {
    ...
    assertEquals(
      DEFAULT_DURATION,
      ((NChartLayerTestImpl) layers.get(layerIdx)).getDuration()
    );
    // layer 1
    layerIdx = 1;
    layerNodes = ((NChartLayerTestImpl) layers.get(layerIdx)).getNodes();
  ...
  public void testCreateLayersDifferentStarts() {
    ...
    layerNodes = ((NChartLayerTestImpl) layers.get(layerIdx)).getNodes();
    assertEquals(2, layerNodes.size());
    Integer lnAidx = ((NChartLayerNodeTestImpl) layerNodes.get(0)).getEvent()
        .equals(lnA.getEvent())
      ? 0
      : 1;
    Integer lnDidx = lnAidx == 0 ? 1 : 0;
    assertChartLayerNodeEqual(
      lnA,
      (NChartLayerNodeTestImpl) layerNodes.get(lnAidx)
    );
    assertChartLayerNodeEqual(
      lnD,
      (NChartLayerNodeTestImpl) layerNodes.get(lnDidx)
    );
    assertEquals(
      DEFAULT_DURATION,
      ((NChartLayerTestImpl) layers.get(layerIdx)).getDuration()
  ...
  public void testCreateNChartCharacters() {
    EntityGraphNode d1 = new EntityGraphNode(
      "d1",
      null,
      null,
      null,
    ...
    EntityGraphNode d2 = new EntityGraphNode(
      "d2",
      null,
      null,
      null,
    ...
    ActivityGraphNode a = new ActivityGraphNode(
      "A",
      "1",
      "Step A",
      "description a",
  ...
  public void testCreateActivityLayerNode() {
    ...
    NChartLayerNode actNode = NChartUtil.createActivityLayerNode(
      jsoProvider,
      a,
      graphNodes
    );

    List<String> subnodes = Arrays.asList(
      new String[] { d1.getId(), d2.getId() }
    );
  ...
  public void testFillPositions() {
    ...
    idToPoints.put(
      a.getId(),
      Arrays.asList(new XYPoint[] { new XYPointTestImpl(100, 1) })
    );
    idToPoints.put(
      d1.getId(),
      Arrays.asList(new XYPoint[] { new XYPointTestImpl(0, 1) })
    );
   ...
   * Private Methods
   */
  private void assertChartLayerNodeEqual(
    NChartLayerNodeTestImpl expected,
