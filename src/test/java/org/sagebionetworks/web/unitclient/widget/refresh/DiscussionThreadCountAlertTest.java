...
public class DiscussionThreadCountAlertTest {
	...
	public void before() {
		...
		refreshAlert = new DiscussionThreadCountAlert(mockView, mockGWTWrapper,
				mockGlobalApplicationState, mockSynapseJSNIUtils, mockSynapseJavascriptClient);
		when(mockView.isAttached()).thenReturn(true);
		AsyncMockStubber.callSuccessWith(count).when(mockSynapseJavascriptClient)
				.getThreadCountForForum(anyString(), any(DiscussionFilter.class), any(AsyncCallback.class));
	...
	public void testConfigureNotAttached() {
		...
		verify(mockSynapseJavascriptClient, never()).getThreadCountForForum(anyString(),
	...
	public void testConfiguredAndAttached() {
		...
		verify(mockSynapseJavascriptClient).getThreadCountForForum(anyString(),
				any(DiscussionFilter.class), any(AsyncCallback.class));
		// and will call this again later
		verify(mockGWTWrapper).scheduleExecution(any(Callback.class),
