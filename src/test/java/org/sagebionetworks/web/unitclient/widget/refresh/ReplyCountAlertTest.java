...
public class ReplyCountAlertTest {
	...
	public void before() {
		...
		refreshAlert = new ReplyCountAlert(mockView, mockGWTWrapper, mockGlobalApplicationState,
				mockSynapseJSNIUtils, mockSynapseJavascriptClient);
		when(mockView.isAttached()).thenReturn(true);
		AsyncMockStubber.callSuccessWith(count).when(mockSynapseJavascriptClient)
				.getReplyCountForThread(anyString(), any(DiscussionFilter.class), any(AsyncCallback.class));
	}
	...
	public void testConfigureNotAttached() {
		...
		verify(mockSynapseJavascriptClient, never()).getReplyCountForThread(anyString(),
	...
	public void testConfiguredAndAttached() {
		...
		verify(mockSynapseJavascriptClient).getReplyCountForThread(anyString(),
				any(DiscussionFilter.class), any(AsyncCallback.class));
