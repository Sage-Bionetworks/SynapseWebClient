...
public class AccessControlListEditorTest {
	...
	public void setUp() throws JSONObjectAdapterException {
		...
		when(mockAuthenticationController.getCurrentUserPrincipalId())
				.thenReturn(new Long(ADMIN_ID).toString());
		AsyncMockStubber.callSuccessWith(userGroupHeaderRP).when(mockSynapseJavascriptClient)
				.getUserGroupHeadersById(Matchers.<ArrayList<String>>any(), any(AsyncCallback.class));

		AsyncMockStubber.callSuccessWith("").when(mockSynapseClient).sendMessage(anySet(), anyString(),
				anyString(), anyString(), any(AsyncCallback.class));

		// instantiate the ACLEditor
		acle = new AccessControlListEditor(mockACLEView, mockSynapseClient,
				mockAuthenticationController, mockSynapseProperties, mockGwt, mockSynapseJavascriptClient,
				mockSynAlert, mockPortalGinInjector);
	...
	public void createAclTest() throws Exception {
		// create response ACL
		// configure mocks
		AsyncMockStubber.callSuccessWith(entityBundleTransport_inheritedACL)
				.when(mockSynapseJavascriptClient)
				.getEntityBundle(anyString(), any(EntityBundleRequest.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(localACL).when(mockSynapseClient)
				.createAcl(any(AccessControlList.class), any(AsyncCallback.class));
		...
		verify(mockACLEView).buildWindow(anyBoolean(), anyBoolean(), eq(INHERITED_ACL_ID), anyBoolean(),
				anyBoolean(), eq(PermissionLevel.CAN_DOWNLOAD), anyBoolean());
		verify(mockACLEView, times(2)).buildWindow(anyBoolean(), anyBoolean(), eq(ENTITY_ID),
				anyBoolean(), anyBoolean(), eq(PermissionLevel.CAN_DOWNLOAD), anyBoolean());

		verify(mockACLEView).setPublicAclPrincipalId(any(Long.class));

		verify(mockSynapseClient, never()).sendMessage(anySet(), anyString(), anyString(), anyString(),
	...
	public void addAccessTest() throws Exception {
		...
		// configure mocks
		AsyncMockStubber.callSuccessWith(entityBundleTransport_localACL)
				.when(mockSynapseJavascriptClient)
				.getEntityBundle(anyString(), any(EntityBundleRequest.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(localACL).when(mockSynapseClient)
				.updateAcl(any(AccessControlList.class), anyBoolean(), any(AsyncCallback.class));
		ArgumentCaptor<AccessControlList> captor = ArgumentCaptor.forClass(AccessControlList.class);
		...
		assertTrue(
				"setting access to the public principal didn't update the user entity permissions (ACL editor view might be wrong)",
				canPublicRead);
		acle.removeAccess(TEST_PUBLIC_PRINCIPAL_ID);
		canPublicRead = acle.getUserEntityPermissions().getCanPublicRead();
		assertFalse(
				"removing access to the public principal didn't update the user entity permissions (ACL editor view might be wrong)",
				canPublicRead);

		assertEquals("Updated ACL is invalid", localACL, returnedACL);
		verify(mockACLEView, never()).showErrorMessage(anyString());
		verify(mockACLEView, times(6)).buildWindow(anyBoolean(), anyBoolean(), anyString(),
				anyBoolean(), anyBoolean(), eq(PermissionLevel.CAN_DOWNLOAD), anyBoolean());
		verify(mockACLEView).setPublicAclPrincipalId(any(Long.class));

		ArgumentCaptor<Set> recipientSetCaptor = ArgumentCaptor.forClass(Set.class);
		verify(mockSynapseClient).sendMessage(recipientSetCaptor.capture(), anyString(), anyString(),
				eq(HOST_PAGE_BASE_URL), any(AsyncCallback.class));
	...
	public void addPublicAccessTest() throws Exception {
		...
		verify(mockSynapseJavascriptClient).getUserGroupHeadersById(listCaptor.capture(),
				any(AsyncCallback.class));
	...
	public void changeAccessTest() throws Exception {
		...
		verify(mockACLEView, never()).showErrorMessage(anyString());
		verify(mockACLEView, times(3)).buildWindow(anyBoolean(), anyBoolean(), anyString(),
				anyBoolean(), anyBoolean(), eq(PermissionLevel.CAN_DOWNLOAD), anyBoolean());
	...
	public void deleteAclTest() throws Exception {
		...
		AsyncMockStubber.callSuccessWith(entityBundleTransport_localACL)
				.when(mockSynapseJavascriptClient)
				.getEntityBundle(anyString(), any(EntityBundleRequest.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(inheritedACL).when(mockSynapseClient).deleteAcl(eq(ENTITY_ID),
				any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(inheritedACL).when(mockSynapseClient)
	...
	public void setAdminAccessTest() throws Exception {
		...
		verify(mockACLEView, times(2)).buildWindow(eq(isProject), eq(isInherited), eq(aclEntityId),
				eq(canEnableInheritance), eq(canChangePermission), eq(PermissionLevel.CAN_DOWNLOAD),
				eq(isLoggedIn));
	}
	...
	public void pushNoChangesTest() throws Exception {
		...
		verify(mockACLEView).buildWindow(anyBoolean(), anyBoolean(), anyString(), anyBoolean(),
				anyBoolean(), eq(PermissionLevel.CAN_DOWNLOAD), anyBoolean());
	}
	...
	public void addDownloadAccessToAuthenticatedUsersNotCertified() throws Exception {
		...
		AsyncMockStubber
				.callFailureWith(new ForbiddenException(
						"Only certified users can allow authenticated users to download.",
						ErrorResponseCode.USER_CERTIFICATION_REQUIRED))
