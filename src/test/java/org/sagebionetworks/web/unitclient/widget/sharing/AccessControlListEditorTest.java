...
public class AccessControlListEditorTest {
  ...
  public void setUp() throws JSONObjectAdapterException {
    ...
    entityBundleTransport_inheritedACL =
      createEBT(inheritedAclClone, createUEP());
    userGroupHeaderRP = createUGHRP();

    // set up mocks
    mockAuthenticationController =
      mock(AuthenticationController.class, RETURNS_DEEP_STUBS);
    ...
    when(mockPublicPrincipalIds.getPublicAclPrincipalId())
      .thenReturn(TEST_PUBLIC_PRINCIPAL_ID);
    ...
    when(mockPortalGinInjector.getQuizInfoDialog())
      .thenReturn(mockQuizInfoDialog);
  }
  ...
  private static EntityBundle createEBT(
    AccessControlList acl,
    UserEntityPermissions uep
  ) {
    try {
  ...
  public void createAclTest() throws Exception {
    ...
    // configure mocks
    AsyncMockStubber
      .callSuccessWith(entityBundleTransport_inheritedACL)
      .when(mockSynapseJavascriptClient)
      .getEntityBundle(
        anyString(),
    ...
    AsyncMockStubber
      .callSuccessWith(localACL)
      .when(mockSynapseClient)
      .createAcl(any(AccessControlList.class), any(AsyncCallback.class));
    ArgumentCaptor<AccessControlList> captor = ArgumentCaptor.forClass(
    ...
    verify(mockSynapseClient)
      .createAcl(captor.capture(), any(AsyncCallback.class));
    ...
    verify(mockACLEView)
      .buildWindow(
        ...
        anyBoolean(),
        anyBoolean(),
        eq(PermissionLevel.CAN_DOWNLOAD),
        anyBoolean()
      );
    verify(mockACLEView, times(2))
      .buildWindow(
        anyBoolean(),
        anyBoolean(),
        eq(ENTITY_ID),
  ...
  public void addAccessTest() throws Exception {
    ...
    AsyncMockStubber
      .callSuccessWith(localACL)
      .when(mockSynapseClient)
      .updateAcl(any(AccessControlList.class), any(AsyncCallback.class));
    ArgumentCaptor<AccessControlList> captor = ArgumentCaptor.forClass(
      AccessControlList.class
    );
    ...
    verify(mockSynapseClient)
      .updateAcl(captor.capture(), any(AsyncCallback.class));
    AccessControlList returnedACL = captor.getValue();
    localACL.setCreationDate(returnedACL.getCreationDate());
    ...
    verify(mockSynapseClient)
      .sendMessage(
        recipientSetCaptor.capture(),
        anyString(),
        anyString(),
        eq(HOST_PAGE_BASE_URL),
        any(AsyncCallback.class)
      );
  ...
  public void addPublicAccessTest() throws Exception {
    ...
    verify(mockSynapseJavascriptClient)
      .getUserGroupHeadersById(listCaptor.capture(), any(AsyncCallback.class));
  ...
  public void changeAccessTest() throws Exception {
    ...
    for (ResourceAccess resourceAccess : localACL.getResourceAccess()) if (
      resourceAccess.getPrincipalId().equals(USER_ID)
    ) resourceAccess.setAccessType(
      AclUtils.getACCESS_TYPEs(PermissionLevel.CAN_VIEW)
    );
  ...
  public void setAdminAccessTest() throws Exception {
    ...
    verify(mockACLEView, times(2))
      .buildWindow(
        ...
        eq(isInherited),
        eq(aclEntityId),
        eq(canEnableInheritance),
        eq(canChangePermission),
        eq(isOpenData),
        eq(PermissionLevel.CAN_DOWNLOAD),
        eq(isLoggedIn)
      );
  ...
  @Test
  public void addDownloadAccessToAuthenticatedUsersNotCertified()
    throws Exception {
    // configure mocks
    AsyncMockStubber
      .callSuccessWith(entityBundleTransport_localACL)
