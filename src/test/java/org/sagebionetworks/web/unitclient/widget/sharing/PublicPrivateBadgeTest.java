...
public class PublicPrivateBadgeTest {
	...
	public void setup() {
		...
		publicPrivateBadge =
				new PublicPrivateBadge(mockView, mockSynapseJavascriptClient, mockUserService);
		acl = new AccessControlList();
		acl.setResourceAccess(resourceAccessSet);
		testEntity = new FileEntity();
		testEntity.setId("syn12345");
		publicPrincipalIds = new PublicPrincipalIds(TEST_PUBLIC_PRINCIPAL_ID,
		...
		AsyncMockStubber.callSuccessWith(transport).when(mockSynapseJavascriptClient)
				.getEntityBundle(anyString(), any(EntityBundleRequest.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(publicPrincipalIds).when(mockUserService)
	...
	public void testGetPublicPrincipalIdsFailure() {
		AsyncMockStubber.callFailureWith(new IllegalArgumentException()).when(mockUserService)
				.getPublicAndAuthenticatedGroupPrincipalIds(any(AsyncCallback.class));
		publicPrivateBadge.configure(testEntity);
		verify(mockView).showErrorMessage(anyString());
	}
