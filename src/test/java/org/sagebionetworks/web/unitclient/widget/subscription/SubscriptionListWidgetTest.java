...
public class SubscriptionListWidgetTest {
	...
	public void setUp() throws Exception {
		...
		widget = new SubscriptionListWidget(mockView, mockJsClient, mockPortalGinInjector, mockSynAlert,
				mockAuthenticationController, mockDetailedPaginationWidget);
		AsyncMockStubber.callSuccessWith(mockSubscriptionPagedResults).when(mockJsClient)
				.getAllSubscriptions(any(SubscriptionObjectType.class), anyLong(), anyLong(),
						any(SortByType.class), any(SortDirection.class), any(AsyncCallback.class));
	...
	public void testConfigureAnonymous() {
		...
		verify(mockJsClient, never()).getAllSubscriptions(any(SubscriptionObjectType.class), anyLong(),
				anyLong(), any(SortByType.class), any(SortDirection.class), any(AsyncCallback.class));
	}
	...
	public void testConfigure() {
		...
		verify(mockJsClient).getAllSubscriptions(eq(expectedFilter), eq(SubscriptionListWidget.LIMIT),
				eq(expectedOffset), any(SortByType.class), any(SortDirection.class),
				any(AsyncCallback.class));
		verify(mockSynAlert).clear();

		// full result set. create a TopicRowWidget for each subscription
		verify(mockDetailedPaginationWidget).configure(SubscriptionListWidget.LIMIT, expectedOffset,
				expectedTotalCount, widget);
		verify(mockTopicRowWidget, times(SubscriptionListWidget.LIMIT.intValue()))
				.configure(any(Subscription.class));
		verify(mockView, times(SubscriptionListWidget.LIMIT.intValue()))
				.addNewSubscription(any(Widget.class));
		...
		verify(mockJsClient).getAllSubscriptions(eq(expectedFilter), eq(SubscriptionListWidget.LIMIT),
				eq(expectedOffset), eq(SortByType.CREATED_ON), eq(SortDirection.ASC),
				any(AsyncCallback.class));
	...
	public void testOnFilter() {
		...
		verify(mockJsClient).getAllSubscriptions(eq(expectedFilter), eq(SubscriptionListWidget.LIMIT),
				eq(expectedOffset), eq(SortByType.CREATED_ON), eq(expectedSortDirection),
				any(AsyncCallback.class));
		verify(mockSynAlert).clear();
	...
	public void testGetAllSubscriptionsError() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).getAllSubscriptions(
				any(SubscriptionObjectType.class), anyLong(), anyLong(), any(SortByType.class),
