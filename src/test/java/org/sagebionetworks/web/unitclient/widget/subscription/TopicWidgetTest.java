...
public class TopicWidgetTest {
	...
	public void testConfigureForum() {
		AsyncMockStubber.callSuccessWith(mockProject).when(mockForumClient).getForumProject(anyString(),
				any(AsyncCallback.class));
	...
	public void testConfigureForumFailure() {
		String errorMessage = "error";
		Exception ex = new Exception(errorMessage);
		AsyncMockStubber.callFailureWith(ex).when(mockForumClient).getForumProject(anyString(),
				any(AsyncCallback.class));
