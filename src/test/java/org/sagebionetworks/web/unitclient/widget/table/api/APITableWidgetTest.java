...
public class APITableWidgetTest {
	...
	public void setup() throws JSONObjectAdapterException {
		...
		fillInResult(result1, new String[] {"field1", "field2"},
				new String[] {"result1 value 1", "result1 value 2"});
		JSONObjectAdapter result2 = testReturnJSONObject.createNew();
		fillInResult(result2, new String[] {"field1", "field2"},
				new String[] {"result2 value 1", "result2 value 2"});
		...
		AsyncMockStubber.callSuccessWith(testReturnJSONObject).when(mockSynapseJavascriptClient)
				.getJSON(anyString(), any(AsyncCallback.class));
		widget = new APITableWidget(mockView, mockSynapseJavascriptClient, mockGinInjector,
				mockGlobalApplicationState, mockAuthenticationController, mockSynAlert, mockCellFactory);
	...
	private void fillInResult(JSONObjectAdapter result, String[] fieldNames, String[] fieldValues)
	...
	public void testServiceCallFailure() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseJavascriptClient).getJSON(anyString(),
	...
	public void testTableUnavailableFailure() throws JSONObjectAdapterException {
		AsyncMockStubber.callFailureWith(new TableUnavilableException())
				.when(mockSynapseJavascriptClient).getJSON(anyString(), any(AsyncCallback.class));
		widget.configure(testWikiKey, descriptor, null, null);
	...
	public void testQueryServicePagingURISubmissionSearch() throws JSONObjectAdapterException {
		...
		String testServiceCall =
				ClientProperties.EVALUATION_QUERY_SERVICE_PREFIX + "select+*+from+evaluation_1234";
		String pagedURI = widget.getPagedURI(testServiceCall);
		assertEquals(testServiceCall + "+limit+10+offset+" + expectedOffset, pagedURI.toLowerCase());
	}
	...
	public void testCurrentUserVariable() throws JSONObjectAdapterException {
		String testServiceCall = ClientProperties.QUERY_SERVICE_PREFIX
				+ "select+*+from+project+where+userId==" + APITableWidget.CURRENT_USER_SQL_VARIABLE;
	...
	public void testCurrentUserVariableEncoded() throws JSONObjectAdapterException {
		String testServiceCall = ClientProperties.QUERY_SERVICE_PREFIX
				+ "select+*+from+project+where+userId==" + APITableWidget.ENCODED_CURRENT_USER_SQL_VARIABLE;
		descriptor.put(WidgetConstants.API_TABLE_WIDGET_PATH_KEY, testServiceCall);
		descriptor.put(WidgetConstants.API_TABLE_WIDGET_PAGING_KEY, "false");
		when(mockAuthenticationController.isLoggedIn()).thenReturn(true);
		String testUserId = "12345test";
	...
	public void testOrderByURIWithQueryService() throws JSONObjectAdapterException {
		...
		inputUri =
				ClientProperties.QUERY_SERVICE_PREFIX + "select+*+from+folder+where+projectId='syn123'";
	...
	public void testGuessRendererFriendlyName() {
		APITableConfig tableConfig = getTableConfig();
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_NONE,
				widget.guessRendererFriendlyName(null, tableConfig));
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_NONE,
				widget.guessRendererFriendlyName("foo", tableConfig));
		// case should not matter
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_USER_ID.toUpperCase(),
						tableConfig));
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_USER_ID.toLowerCase(),
						tableConfig));
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_MODIFIED_BY_PRINCIPAL_ID,
						tableConfig));
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_EPOCH_DATE,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_CREATED_ON, tableConfig));

		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_ENTITY_ID, tableConfig));
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_PARENT_ID, tableConfig));

		// next, check "id". If node query service, assume it's a synapse id. Otherwise, do not render
		// in a special way.
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_NONE,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_ID, tableConfig));
		tableConfig.setUri(ClientProperties.QUERY_SERVICE_PREFIX + "select+*+from+project");
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_ID, tableConfig));
		tableConfig
				.setUri(ClientProperties.EVALUATION_QUERY_SERVICE_PREFIX + "select+*+from+evaluation_123");
		assertEquals(WidgetConstants.API_TABLE_COLUMN_RENDERER_NONE,
				widget.guessRendererFriendlyName(WebConstants.DEFAULT_COL_NAME_ID, tableConfig));
	}
	...
	public void testGetColumnTypeFromRendererName() {
		...
		assertEquals(ApiTableColumnType.USERID,
				APITableWidget.getColumnType(WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID));
		assertEquals(ApiTableColumnType.DATE,
				APITableWidget.getColumnType(WidgetConstants.API_TABLE_COLUMN_RENDERER_EPOCH_DATE));
		assertEquals(ApiTableColumnType.MARKDOWN_LINK,
				APITableWidget.getColumnType(WidgetConstants.API_TABLE_COLUMN_RENDERER_LINK));
		assertEquals(ApiTableColumnType.ENTITYID,
				APITableWidget.getColumnType(WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID));
		assertEquals(ApiTableColumnType.LARGETEXT,
				APITableWidget.getColumnType(WidgetConstants.API_TABLE_COLUMN_RENDERER_ANNOTATIONS));
		assertEquals(ApiTableColumnType.CANCEL_CONTROL,
				APITableWidget.getColumnType(WidgetConstants.API_TABLE_COLUMN_RENDERER_CANCEL_CONTROL));
	}
