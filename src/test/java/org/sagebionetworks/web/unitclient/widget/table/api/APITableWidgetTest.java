...
public class APITableWidgetTest {
  ...
  WikiPageKey testWikiKey = new WikiPageKey(
    "",
  ...
  public void setup() throws JSONObjectAdapterException {
    when(mockCellFactory.createRenderer(any(ColumnType.class)))
      .thenReturn(mockCell);
    when(mockGinInjector.getCancelControlWidget())
      .thenReturn(mockCancelControlWidget);
    ...
    fillInResult(
      result1,
      new String[] { col1Name, col2Name },
      new String[] { COL_1_RESULT_VALUE_1, "result1 value 2" }
    );
    ...
    AsyncMockStubber
      .callSuccessWith(testReturnJSONObject)
  ...
  public void testConfigureWithTableConfigValidColumnName() {
    ...
    APITableConfigEditor.updateDescriptorWithColumnConfigs(
      descriptor,
      columnConfigs
    );
    String uri =
      ClientProperties.EVALUATION_QUERY_SERVICE_PREFIX +
    ...
    verify(mockSynapseJavascriptClient)
      .getJSON(stringCaptor.capture(), any(AsyncCallback.class));
  ...
  @Test
  public void testEmptyResultsEmptyColumnConfiguration()
    throws JSONObjectAdapterException {
  ...
  public void testTableUnavailableFailure() throws JSONObjectAdapterException {
    AsyncMockStubber
      .callFailureWith(new TableUnavilableException())
      .when(mockSynapseJavascriptClient)
      .getJSON(anyString(), any(AsyncCallback.class));
    widget.configure(testWikiKey, descriptor, null, null);
  ...
  public void testPagingURI() throws JSONObjectAdapterException {
    ...
    assertEquals(
      TESTSERVICE_PATH + "?limit=10&offset=0",
      pagedURI.toLowerCase()
    );
  }

  @Test
  public void testQueryServicePagingURINodeSearch()
    throws JSONObjectAdapterException {
    String expectedOffset = "1";
    widget.configure(testWikiKey, descriptor, null, null);
    String testServiceCall =
      ClientProperties.QUERY_SERVICE_PREFIX + "select+*+from+project";
    String pagedURI = widget.getPagedURI(testServiceCall);
    assertEquals(
      testServiceCall + "+limit+10+offset+" + expectedOffset,
      pagedURI.toLowerCase()
    );
  }

  @Test
  public void testQueryServicePagingURISubmissionSearch()
    throws JSONObjectAdapterException {
    String expectedOffset = ServiceConstants.DEFAULT_PAGINATION_OFFSET_PARAM;
    widget.configure(testWikiKey, descriptor, null, null);
    String testServiceCall =
      ClientProperties.EVALUATION_QUERY_SERVICE_PREFIX +
      "select+*+from+evaluation_1234";
    String pagedURI = widget.getPagedURI(testServiceCall);
    assertEquals(
      testServiceCall + "+limit+10+offset+" + expectedOffset,
      pagedURI.toLowerCase()
  ...
  public void testCurrentUserVariable() throws JSONObjectAdapterException {
    String testServiceCall =
      ClientProperties.QUERY_SERVICE_PREFIX +
      "select+*+from+project+where+userId==" +
      APITableWidget.CURRENT_USER_SQL_VARIABLE;
    ...
    verify(mockSynapseJavascriptClient)
      .getJSON(arg.capture(), any(AsyncCallback.class));

    assertTrue(arg.getValue().endsWith(testUserId));
  }

  @Test
  public void testCurrentUserVariableEncoded()
    throws JSONObjectAdapterException {
    String testServiceCall =
      ClientProperties.QUERY_SERVICE_PREFIX +
      "select+*+from+project+where+userId==" +
      APITableWidget.ENCODED_CURRENT_USER_SQL_VARIABLE;
    descriptor.put(WidgetConstants.API_TABLE_WIDGET_PATH_KEY, testServiceCall);
    descriptor.put(WidgetConstants.API_TABLE_WIDGET_PAGING_KEY, "false");
    when(mockAuthenticationController.isLoggedIn()).thenReturn(true);
    String testUserId = "12345test";
  ...
  private APITableConfig getTableConfig() {
    ...
    columnConfig.setRendererFriendlyName(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID
    );
  ...
  @Test
  public void testOrderByURINotQueryService()
    throws JSONObjectAdapterException {
  ...
  @Test
  public void testOrderByURIWithQueryService()
    throws JSONObjectAdapterException {
    APITableConfig tableConfig = getTableConfig();
    // and set a sort column
    APITableColumnConfig sortColumnConfig = tableConfig
      .getColumnConfigs()
      .get(1);
    sortColumnConfig.setSort(COLUMN_SORT_TYPE.DESC);
    String inputUri =
      ClientProperties.QUERY_SERVICE_PREFIX + "select+*+from+project";
    String outputUri = widget
      .getOrderedByURI(inputUri, tableConfig)
      .toLowerCase();
    ...
    inputUri =
      ClientProperties.QUERY_SERVICE_PREFIX +
      "select+*+from+folder+where+projectId='syn123'";
  ...
  public void testRemoveFirstToken() {
    ...
    assertEquals(
      "Annotation",
      APITableWidget.removeFirstToken("data.Annotation")
    );
  ...
  public void testGuessRendererFriendlyName() {
    APITableConfig tableConfig = getTableConfig();
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_NONE,
    ...
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID,
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_USER_ID.toUpperCase(),
        tableConfig
      )
    );
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID,
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_USER_ID.toLowerCase(),
        tableConfig
      )
    );
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID,
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_MODIFIED_BY_PRINCIPAL_ID,
        tableConfig
      )
    );
    assertEquals(
      ...
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_CREATED_ON,
        tableConfig
      )
    );
    ...
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID,
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_ENTITY_ID,
        tableConfig
      )
    );
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID,
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_PARENT_ID,
        tableConfig
      )
    );
    ...
    assertEquals(
      WidgetConstants.API_TABLE_COLUMN_RENDERER_NONE,
      widget.guessRendererFriendlyName(
        WebConstants.DEFAULT_COL_NAME_ID,
        tableConfig
      )
    );
    tableConfig.setUri(
  ...
  public void testGetColumnValueLong() throws JSONObjectAdapterException {
    ...
    when(row.getString(anyString()))
      .thenThrow(new JSONObjectAdapterException("invalid string"));
    when(row.getLong(anyString())).thenReturn(testValue);
    assertEquals(
      testValue.toString(),
      APITableWidget.getColumnValue(row, "key")
    );
  }
  ...
  public void testGetColumnValueDouble() throws JSONObjectAdapterException {
    ...
    when(row.getLong(anyString()))
      .thenThrow(new JSONObjectAdapterException("invalid long"));
  ...
  public void testGetColumnValueArray() throws JSONObjectAdapterException {
    ...
    when(row.getLong(anyString()))
      .thenThrow(new JSONObjectAdapterException("invalid long"));
    when(row.get(anyString()))
      .thenThrow(new JSONObjectAdapterException("invalid json object"));
  ...
  public void testGetSelectColumns() {
    String selectColumns = widget.getSelectColumns(
      getTableConfig().getColumnConfigs()
    );
  ...
  public void testGetColumnTypeFromRendererName() {
    ...
    assertEquals(
      ApiTableColumnType.USERID,
      APITableWidget.getColumnType(
        WidgetConstants.API_TABLE_COLUMN_RENDERER_USER_ID
      )
    );
    assertEquals(
      ApiTableColumnType.DATE,
      APITableWidget.getColumnType(
        WidgetConstants.API_TABLE_COLUMN_RENDERER_EPOCH_DATE
      )
    );
    assertEquals(
      ApiTableColumnType.MARKDOWN_LINK,
      APITableWidget.getColumnType(
        WidgetConstants.API_TABLE_COLUMN_RENDERER_LINK
      )
    );
    assertEquals(
      ApiTableColumnType.ENTITYID,
      APITableWidget.getColumnType(
        WidgetConstants.API_TABLE_COLUMN_RENDERER_SYNAPSE_ID
      )
    );
    assertEquals(
      ApiTableColumnType.LARGETEXT,
      APITableWidget.getColumnType(
        WidgetConstants.API_TABLE_COLUMN_RENDERER_ANNOTATIONS
      )
    );
    assertEquals(
      ApiTableColumnType.CANCEL_CONTROL,
      APITableWidget.getColumnType(
        WidgetConstants.API_TABLE_COLUMN_RENDERER_CANCEL_CONTROL
      )
    );
  }
  ...
  public void testGetNewCellCellRenderer() {
    ...
    value = "2";
    IsWidget cancelControlCell = widget.getNewCell(
      ApiTableColumnType.CANCEL_CONTROL,
      value
    );
    ...
    value = "3";
    IsWidget markdownLinkCell = widget.getNewCell(
      ApiTableColumnType.MARKDOWN_LINK,
      value
    );
