...
public class CreateTableViewWizardStep1Test {
  ...
  public void before() {
    ...
    when(mockEntityContainerListWidget.getEntityIds())
      .thenReturn(entityScopeIds);
    evaluationScopeIds = Collections.singletonList("8278743");
    when(mockSubmissionViewScope.getEvaluationIds())
      .thenReturn(evaluationScopeIds);
    when(mockGlobalAppState.getPlaceChanger()).thenReturn(mockPlaceChanger);
    widget =
      new CreateTableViewWizardStep1(
        mockView,
        mockJsClient,
        mockEntityContainerListWidget,
        mockSubmissionViewScope,
        mockStep2,
        mockGlobalAppState
      );
  ...
  public void testNullName() {
    ...
    widget.onPrimary();
    verify(mockWizardPresenter)
      .setErrorMessage(
        CreateTableViewWizardStep1.NAME_MUST_INCLUDE_AT_LEAST_ONE_CHARACTER
      );
  ...
  public void testConfigureSubmissionView() {
    ...
    when(mockJsClient.createEntity(captor.capture()))
      .thenReturn(getDoneFuture(table));
  ...
  public void testCreateFileView() {
    ...
    assertEquals(
      (Long) ViewTypeMask.File.getMask(),
      capturedFileView.getViewTypeMask()
    );
    verify(mockWizardPresenter, never()).setErrorMessage(anyString());
  ...
  public void testCreateFileFolderTableView() {
    ...
    assertNull(capturedFileView.getType());
    assertEquals(
      new Long(filesFoldersTablesView.getViewTypeMask()),
      capturedFileView.getViewTypeMask()
    );
    verify(mockWizardPresenter, never()).setErrorMessage(anyString());
  ...
  public void testCreateFileViewEmptyScope() {
    ...
    table.setName(tableName);
    table.setId("syn57");
    when(mockJsClient.createEntity(any(Entity.class)))
      .thenReturn(getDoneFuture(table));
    when(mockView.getName()).thenReturn(tableName);
    when(mockEntityContainerListWidget.getEntityIds())
  ...
  public void testCreateProjectView() {
    ...
    assertNull(capturedFileView.getType());
    assertEquals(
      (Long) ViewTypeMask.Project.getMask(),
      capturedFileView.getViewTypeMask()
    );
    verify(mockWizardPresenter, never()).setErrorMessage(anyString());
  ...
  public void testCreateDataset() {
    ...
    when(mockJsClient.createEntity(captor.capture()))
      .thenReturn(getDoneFuture(dataset));
    when(mockJsClient.getDefaultColumnsForView(ViewEntityType.dataset))
      .thenReturn(getDoneFuture(defaultColumns));
    when(mockView.getName()).thenReturn(tableName);
    widget.onPrimary();

    // Verify that default columns are added
    assertEquals(
      captor.getValue().getColumnIds().size(),
      defaultColumns.size()
    );
    ...
    verify(mockWizardPresenter, never()).setErrorMessage(anyString());
    // We shouldn't go to step 2 for datasets
  ...
  public void testCreateDatasetCollection() {
    widget.configure(parentId, TableType.dataset_collection);
    verify(mockView).setEntityViewScopeWidgetVisible(false);
    verify(mockView).setViewTypeOptionsVisible(false);
    String tableName = "a name";
    DatasetCollection collection = new DatasetCollection();
    collection.setName(tableName);
    collection.setId("syn57");
    ...
    ArgumentCaptor<DatasetCollection> captor = ArgumentCaptor.forClass(
      DatasetCollection.class
    );
    when(mockJsClient.createEntity(captor.capture()))
      .thenReturn(getDoneFuture(collection));
    when(
    ...
    verify(mockWizardPresenter, never()).setErrorMessage(anyString());
    // We shouldn't go to step 2 for dataset collections
    verify(mockStep2, never()).configure(any(), any());
    verify(mockWizardPresenter, never()).setNextActivePage(mockStep2);
    verify(mockPlaceChanger).goTo(any());
    verify(mockWizardPresenter).onFinished();
  ...
  public void testCreateFailed() {
    ...
    when(mockJsClient.createEntity(any(Entity.class)))
      .thenReturn(getFailedFuture(new Throwable(error)));
    when(mockView.getName()).thenReturn(tableName);
    widget.onPrimary();
