...
public class CreateTableViewWizardStep2Test {
  ...
  public void before() {
    ...
    when(mockFileViewDefaultColumns.getDefaultViewColumns(TableType.file_view))
      .thenReturn(mockDefaultColumnModels);
    when(
      mockFileViewDefaultColumns.getDefaultViewColumns(filesFoldersTablesView)
    )
      .thenReturn(mockDefaultColumnModels);
    when(
      mockFileViewDefaultColumns.getDefaultViewColumns(TableType.project_view)
    )
      .thenReturn(mockDefaultProjectColumnModels);
    when(
      mockFileViewDefaultColumns.getDefaultViewColumns(
  ...
  public void testGetPossibleColumnModelsForViewScope() {
    ...
    AsynchronousProgressHandler handler = verifyViewColumnModelRequest(
      mockViewScopeIds,
      TableType.file_view.getViewTypeMask().longValue(),
      firstPageToken
    );
  ...
  public void testGetPossibleColumnModelsForProjectViewScope() {
    ...
    AsynchronousProgressHandler handler = verifyViewColumnModelRequest(
      mockViewScopeIds,
      TableType.project_view.getViewTypeMask().longValue(),
      firstPageToken
    );
    handler.onComplete(mockViewColumnModelResponsePage1);
  ...
  public void testGetPossibleColumnModelsForViewScopeFailure() {
    ...
    AsynchronousProgressHandler handler = verifyViewColumnModelRequest(
      mockViewScopeIds,
      filesFoldersTablesView.getViewTypeMask().longValue(),
      firstPageToken
    );
  ...
  public void testOnPrimaryInvalid() {
    ...
    verify(mockWizardPresenter)
      .setErrorMessage(ColumnModelsWidget.SEE_THE_ERROR_S_ABOVE);
  }
  ...
  public void testOnCancelSuccessfulCleanup() {
    ...
    verify(mockJsClient)
      .deleteEntityById(eq(ENTITY_ID), eq(true), any(AsyncCallback.class));
    verify(mockJsniUtils)
      .consoleLog(DELETE_PLACEHOLDER_SUCCESS_MESSAGE + ENTITY_ID);
  }
  ...
  public void testOnCancelFailedToCleanup() {
    ...
    verify(mockJsniUtils)
      .consoleError(
        DELETE_PLACEHOLDER_FAILURE_MESSAGE + ENTITY_ID + ": " + errorMessage
      );
  }
  ...
  public void testOnPrimaryAsyncCancelled() {
    ...
    verify(mockEditor).validate();
    verify(mockWizardPresenter)
      .setErrorMessage(CreateTableViewWizardStep2.SCHEMA_UPDATE_CANCELLED);
