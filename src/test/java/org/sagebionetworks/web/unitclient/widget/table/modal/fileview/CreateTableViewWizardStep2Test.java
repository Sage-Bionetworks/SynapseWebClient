...
public class CreateTableViewWizardStep2Test {
	...
	public void before() {
		...
		widget = new CreateTableViewWizardStep2(mockView, mockEditor, mockSynapseClient,
				mockJobTrackingWidget, mockFileViewDefaultColumns, mockJsClient, mockJsniUtils);
		...
		when(mockFileViewDefaultColumns.getDefaultViewColumns(eq(true), anyBoolean()))
				.thenReturn(mockDefaultColumnModels);
		when(mockFileViewDefaultColumns.getDefaultViewColumns(eq(false), anyBoolean()))
				.thenReturn(mockDefaultProjectColumnModels);
	...
	public void testOnCancelSuccessfulCleanup() {
		AsyncMockStubber.callSuccessWith(null).when(mockJsClient).deleteEntityById(anyString(),
	...
	public void testOnCancelFailedToCleanup() {
		...
		AsyncMockStubber.callFailureWith(new Exception(errorMessage)).when(mockJsClient)
		...
		verify(mockJsniUtils)
				.consoleError(DELETE_PLACEHOLDER_FAILURE_MESSAGE + ENTITY_ID + ": " + errorMessage);
	}
	...
	public void testOnPrimary() {
		...
		verify(mockJsClient, never()).deleteEntityById(anyString(), anyBoolean(),
				any(AsyncCallback.class));
	}
