...
public class EntityContainerListWidgetTest {
	...
	public void before() {
		...
		widget = new EntityContainerListWidget(mockView, mockEntityFinder, mockSynapseJavascriptClient,
				mockSynapseAlert);
	...
	public void testConfigureHappyCase() {
		...
		AsyncMockStubber.callSuccessWith(entityHeaders).when(mockSynapseJavascriptClient)
		...
		verify(mockEntityFinder).configureMulti(eq(EntityFilter.CONTAINER), eq(showVersions),
	...
	public void testMultipleConfigure() {
		...
		boolean showVersions = false;
		verify(mockEntityFinder).configureMulti(eq(EntityFilter.PROJECT), eq(showVersions),
				any(SelectedHandler.class));
	...
	public void testConfigureNoIdsNoEdit() {
		...
		verify(mockSynapseJavascriptClient, never()).getEntityHeaderBatch(anyList(),
	...
	public void testOnAddProjectId() {
		...
		AsyncMockStubber.callSuccessWith(returnList).when(mockSynapseJavascriptClient)
				.getEntityHeaderBatch(anyList(), any(AsyncCallback.class));
	...
	public void testSetValueInvalidResponse() {
		AsyncMockStubber.callSuccessWith(new ArrayList<EntityHeader>())
