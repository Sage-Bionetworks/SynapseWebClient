...
public class ScopeWidgetTest {
	...
	public void before() {
		...
		widget = new ScopeWidget(mockView, mockJsClient, mockViewScopeWidget, mockEditScopeWidget,
				mockSynapseAlert, mockEventBus);
		...
		AsyncMockStubber.callSuccessWith(mockUpdatedEntityView).when(mockJsClient)
				.updateEntity(any(Table.class), anyString(), anyBoolean(), any(AsyncCallback.class));
	...
	public void testConfigureHappyCase() {
		...
		verify(mockEntityView)
				.setViewTypeMask(ViewTypeMask.getMaskForDepricatedType(ViewType.file_and_table));
		verify(mockJsClient).updateEntity(any(Table.class), anyString(), anyBoolean(),
	...
	public void testConfigureUnsupportedViewTypeMask() {
		...
		when(mockEntityView.getViewTypeMask())
				.thenReturn(new Long(WebConstants.PROJECT | WebConstants.FILE));
		boolean isEditable = true;
		widget.configure(mockBundle, isEditable);
	...
	public void testOnSaveFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockJsClient).updateEntity(any(Table.class),
