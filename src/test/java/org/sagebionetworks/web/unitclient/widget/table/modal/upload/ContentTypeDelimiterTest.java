...
public class ContentTypeDelimiterTest {
	...
	public void testAllTypes() {
		...
		for (ContentTypeDelimiter ctd : ContentTypeDelimiter.values()) {
			assertEquals(ctd,
					ContentTypeDelimiter.findByContentType(ctd.getContentType().toUpperCase(), null));
		}
	}
		...
		for (ContentTypeDelimiter ctd : ContentTypeDelimiter.values()) {
			if (ctd.getExtentions() != null) {
				for (String extention : ctd.getExtentions()) {
					assertEquals(ctd,
							ContentTypeDelimiter.findByContentType("unknonwn", "foo." + extention.toUpperCase()));
				}
			}
		}
	}
	...
	public void testFindUnknownNullName() {
		assertEquals(ContentTypeDelimiter.TEXT,
				ContentTypeDelimiter.findByContentType("some/kind/of/type", null));
	}
}
