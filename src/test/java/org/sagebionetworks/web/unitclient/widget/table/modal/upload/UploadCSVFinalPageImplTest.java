...
public class UploadCSVFinalPageImplTest {
	...
	public void before() {
		...
		page = new UploadCSVFinishPageImpl(mockView, mockSynapseClient, mockJsClient,
				mockPortalGinInjector, jobTrackingWidgetStub, mockKeyboardNavigationHandler);
		...
		parentId = "syn123";
		fileHandleId = "456";
		when(mockPortalGinInjector.createColumnModelEditorWidget())
	...
	public void testOnPrimaryFailedColumnCreate() {
		...
		AsyncMockStubber.callFailureWith(new IllegalArgumentException(error)).when(mockSynapseClient)
	...
	public void testOnPrimarySuccess() {
		...
		AsyncMockStubber.callSuccessWith(columns).when(mockSynapseClient)
				.createTableColumns(any(List.class), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(table).when(mockJsClient).createEntity(any(TableEntity.class),
