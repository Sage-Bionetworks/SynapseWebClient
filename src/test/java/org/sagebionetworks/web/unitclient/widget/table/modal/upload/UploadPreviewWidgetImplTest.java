...
public class UploadPreviewWidgetImplTest {
	...
	public void testConfigureNullColumnName() {
		...
		widget.configure(preview);
		verify(mockView).setPreviewMessage(UploadPreviewWidgetImpl.PREVIEW_MESSAGE_PREFIX
				+ preview.getRowsScanned() + UploadPreviewWidgetImpl.PREVIEW_MESSAGE_SUFFIX);
	...
	public void testConfigureNullNoValues() {
		...
		widget.configure(preview);
		verify(mockView)
				.showEmptyPreviewMessage(UploadPreviewWidgetImpl.NO_ROWS_WERE_FOUND_IN_THE_FILE);
		verify(mockView).setEmptyMessageVisible(true);
		verify(mockView, never()).addRow(any(List.class));
	}
