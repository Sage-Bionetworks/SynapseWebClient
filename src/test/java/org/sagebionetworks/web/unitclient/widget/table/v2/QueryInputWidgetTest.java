...
public class QueryInputWidgetTest {
	...
	public void testExecuteValidateSuccess() {
		...
		AsyncMockStubber.callSuccessWith(null).when(mockSynapseClient).validateTableQuery(anyString(),
	...
	public void testExecuteValidateFailure() {
		...
		AsyncMockStubber.callFailureWith(new ParseException(errorMessage)).when(mockSynapseClient)
				.validateTableQuery(anyString(), any(AsyncCallback.class));
		widget.onExecuteQuery();
		verify(mockView).setQueryInputLoading(true);
		verify(mockView).showInputError(false);
		verify(mockView).setQueryInputLoading(true);
