...
public class TableEntityWidgetTest {
	...
	public void before() {
		...
		widget = new TableEntityWidget(mockView, mockQueryResultsWidget, mockQueryInputWidget,
				mockPreflightController, mockSynapseClient, mockFileViewClientsHelp, mockAddToDownloadList,
				mockPortalGinInjector, mockSessionStorage);

		AsyncMockStubber.callSuccessWith(FACET_SQL).when(mockSynapseClient)
				.generateSqlWithFacets(anyString(), anyList(), anyList(), any(AsyncCallback.class));
		// The test bundle
	...
	public void testShowSchema() {
		boolean canEdit = true;
		widget.configure(entityBundle, versionNumber, canEdit, mockQueryChangeHandler, mockActionMenu);
		verify(mockActionMenu).setActionListener(eq(Action.SHOW_TABLE_SCHEMA),
		...
		verify(mockActionMenu, times(2)).setActionText(Action.SHOW_TABLE_SCHEMA,
				SHOW + "Table" + SCHEMA);
	}
	...
	public void testShowViewScope() {
		boolean canEdit = true;
		widget.configure(entityBundle, versionNumber, canEdit, mockQueryChangeHandler, mockActionMenu);
		verify(mockActionMenu).setActionListener(eq(Action.SHOW_VIEW_SCOPE),
				actionListenerCaptor.capture());
		ActionListener listener = actionListenerCaptor.getValue();
	...
	public void testOnStartingnewQuery() {
		...
		verify(mockQueryResultsWidget, never()).configure(any(Query.class), anyBoolean(),
				any(TableType.class), any(QueryResultsListener.class));
	}
	...
	public void testCanEditViewResults() {
		configureBundleWithView(ViewType.file);
		// we can edit the view, but verify that the query input widget should be told that editing
	...
	public void testInitAdvancedQueryStateUI2() {
		...
		widget.onShowSimpleSearch();
		verify(mockView).showConfirmDialog(eq(TableEntityWidget.RESET_SEARCH_QUERY),
				eq(TableEntityWidget.RESET_SEARCH_QUERY_MESSAGE), callbackCaptor.capture());

		startQuery.setSql(" select pos, count(*) as c from syn123 group BY pos    ");
		widget.onShowSimpleSearch();
		verify(mockView, times(2)).showConfirmDialog(eq(TableEntityWidget.RESET_SEARCH_QUERY),
				eq(TableEntityWidget.RESET_SEARCH_QUERY_MESSAGE), callbackCaptor.capture());

		startQuery.setSql(" select * FROM syn123 LIMIT 2 offset 1 ");
		widget.onShowSimpleSearch();
		verify(mockView, times(3)).showConfirmDialog(eq(TableEntityWidget.RESET_SEARCH_QUERY),
				eq(TableEntityWidget.RESET_SEARCH_QUERY_MESSAGE), callbackCaptor.capture());
	...
	public void testInitAdvancedQueryStateUI3() {
		...
		widget.onShowSimpleSearch();
		verify(mockView, never()).showConfirmDialog(eq(TableEntityWidget.RESET_SEARCH_QUERY),
				eq(TableEntityWidget.RESET_SEARCH_QUERY_MESSAGE), any(Callback.class));
		verifySimpleSearchUI();
	}
