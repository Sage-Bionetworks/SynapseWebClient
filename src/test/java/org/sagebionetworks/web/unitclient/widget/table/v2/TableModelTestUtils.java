...
public class TableModelTestUtils {
	...
	private static void createPartialRow(List<ColumnModel> cms, PartialRow toUpdate, int i,
			boolean useDateStrings) {
		// Add a value for each column
	...
	private static PartialRow updatePartialRow(List<ColumnModel> cms, Row toUpdate, int i,
			boolean useDateStrings) {
		// Add a value for each column
		Map<String, String> values = Maps.newHashMap();
	...
	private static void updateExpectedPartialRow(List<ColumnModel> cms, Row toUpdate, int i,
			boolean isUpdate, boolean useDateStrings) {
		// Add a value for each column
		List<String> values = new LinkedList<String>();
		int cmIndex = 0;
		for (ColumnModel cm : cms) {
	...
	private static String getValue(ColumnModel cm, int i, boolean isUpdate, boolean useDateStrings,
			boolean isExpected) {
		if (cm.getColumnType() == null)
			throw new IllegalArgumentException("ColumnType cannot be null");
	 ...
	 * @throws JSONObjectAdapterException
	 */
	public static <T extends JSONEntity> List<T> cloneObject(List<T> toClone, Class<T> clazz)
			throws JSONObjectAdapterException {
