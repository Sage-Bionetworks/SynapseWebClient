...
public class TableModelTestUtils {
  ...
  public static List<PartialRow> createPartialRows(
    List<ColumnModel> cms,
    int count
  ) {
  ...
  public static List<Row> createExpectedFullRows(
    List<ColumnModel> cms,
    int count
  ) {
  ...
  public static PartialRow updatePartialRow(
  ...
  private static void createPartialRow(
    List<ColumnModel> cms,
    PartialRow toUpdate,
    int i,
    boolean useDateStrings
    ...
    int cmIndex = 0;
    for (ColumnModel cm : cms) {
      if (cm.getColumnType() == null) throw new IllegalArgumentException(
  ...
  private static PartialRow updatePartialRow(
    List<ColumnModel> cms,
    Row toUpdate,
    int i,
    boolean useDateStrings
  ...
  private static void updateExpectedPartialRow(
    List<ColumnModel> cms,
    Row toUpdate,
    int i,
    boolean isUpdate,
  ...
  private static String getValue(
    ...
    switch (cm.getColumnType()) {
      case STRING_LIST:
        return (
          (isUpdate ? "updatestring, updatestring2" : "string1, string2") + i
        );
      case STRING:
      ...
      case DATE_LIST:
        if (!isExpected && useDateStrings && i % 2 == 0) {
          return (
            gmtDateFormatter.format(
              new Date(i + 4000 + (isUpdate ? 10000 : 0))
            ) +
            "," +
          ...
          return (
            (i + 4000 + (isUpdate ? 10000 : 0)) +
            "," +
            (i + 4001 + (isUpdate ? 10001 : 0))
          );
        }
      case DATE:
        if (!isExpected && useDateStrings && i % 2 == 0) {
          return gmtDateFormatter.format(
            new Date(i + 4000 + (isUpdate ? 10000 : 0))
          );
        } else {
  ...
  public static PartialRow createPartialRow(
    Long rowId,
   ...
   * @return
   */
  public static List<SelectColumn> buildSelectColumns(
   ...
   */
  public static <T extends JSONEntity> T cloneObject(T toClone, Class<T> clazz)
    throws JSONObjectAdapterException {
   ...
   * @throws JSONObjectAdapterException
   */
  public static <T extends JSONEntity> List<T> cloneObject(
    List<T> toClone,
    Class<T> clazz
  ) throws JSONObjectAdapterException {
