...
public class QueryResultEditorWidgetTest {
  ...
  public void before() throws JSONObjectAdapterException {
    ...
    when(mockTableUpdateTransactionResponse.getResults())
      .thenReturn(tableUpdateResults);
  ...
  public void testOnCancelWithChangesConfirmOkay() {
    ...
    AsyncMockStubber
      .callWithInvoke()
      .when(mockView)
      .showConfirmDialog(anyString(), any(Callback.class));
  ...
  public void testOnSaveWithChagnesNotValid() {
    ...
    verify(mockView).hideProgress();
    verify(mockView)
      .showErrorMessage(QueryResultEditorWidget.SEE_THE_ERRORS_ABOVE);
    verify(mockView).setSaveButtonLoading(false);
    // update should not be invoked
    verify(mockEventBus, never()).fireEvent(any(EntityUpdatedEvent.class));
  }
  ...
  public void testOnSaveWithChangesValidJobSuccessfulIsView() {
    ...
    entityUpdateResults.setUpdateResults(
      Collections.singletonList(entityUpdateResult)
    );
  ...
  public void testGetEntityUpdateResultsEmpty() {
    assertNull(
      QueryResultEditorWidget.getEntityUpdateResults(
  ...
  public void testGetEntityUpdateResultsFailureSuccessIndex() {
    ...
    when(mockEntityUpdateResult1.getFailureCode())
      .thenReturn(EntityUpdateFailureCode.NOT_FOUND);
    when(mockEntityUpdateResult1.getEntityId()).thenReturn("syn29292");
    when(mockEntityUpdateResult1.getFailureMessage())
      .thenReturn("Not there, buddy");
    assertEquals(
      "<p>syn29292 (NOT_FOUND): Not there, buddy</p>",
    ...
    assertEquals(
      1,
      QueryResultEditorWidget.getFirstIndexOfEntityUpdateResultSuccess(
        mockTableUpdateTransactionResponse
      )
    );
  }
  ...
  public void testGetEntityUpdateResultsNoFailures() {
    EntityUpdateResults results = new EntityUpdateResults();
    results.setUpdateResults(
      Collections.singletonList(mockEntityUpdateResult1)
    );
    tableUpdateResults.add(results);
  ...
  public void testGetEntityUpdateResultsNoSuccess() {
    ...
    tableUpdateResults.add(results);
    when(mockEntityUpdateResult1.getFailureCode())
      .thenReturn(EntityUpdateFailureCode.UNAUTHORIZED);
    when(mockEntityUpdateResult1.getEntityId()).thenReturn("syn29292");
    assertEquals(
      -1,
      QueryResultEditorWidget.getFirstIndexOfEntityUpdateResultSuccess(
