...
public class RowWidgetTest {
	...
	public void testConfigureAndGet() {
		...
		assertFalse("The extracted row must not be same instance as the configured row.",
				aRow == extracted);
	...
	public void testTakesAddressCell() {
		...
		boolean isEditor = false;
		rowWidget.configure(tableId, types, isEditor, tableType, aRow, mockListner);
		verify(mockTakesAddress).setCellAddresss(new CellAddress(tableId, types.get(0), aRow.getRowId(),
				aRow.getVersionNumber(), tableType));
		verify(mockTakesAddress).setCellAddresss(new CellAddress(tableId, types.get(1), aRow.getRowId(),
				aRow.getVersionNumber(), tableType));
	}
