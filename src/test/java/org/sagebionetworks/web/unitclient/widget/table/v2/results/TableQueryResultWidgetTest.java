...
public class TableQueryResultWidgetTest {
	...
	public void before() {
		...
		widget = new TableQueryResultWidget(mockView, mockSynapseClient, mockGinInjector,
				mockSynapseAlert, mockClientCache, mockGWT, mockFacetsWidget, mockPopupUtils);
		query = new Query();
	...
	public void testConfigureSuccessEditable() {
		...
		verify(mockFacetsWidget).configure(eq(query), mockFacetChangedHandlerCaptor.capture(),
				callbackCaptor.capture());
		verify(mockJobTrackingWidget).startAndTrackJob(eq(TableQueryResultWidget.RUNNING_QUERY_MESSAGE),
				eq(false), eq(AsynchType.TableQuery), qbrCaptor.capture(),
				asyncProgressHandlerCaptor.capture());

		AsynchronousProgressHandler progressHandler1 = asyncProgressHandlerCaptor.getValue();
		// reconfigure before previous job is done (verify previous job is no longer tracked, it uses a
		// new tracking widget).
		widget.configure(query, isEditable, tableType, mockListner);
		verify(mockJobTrackingWidget2).startAndTrackJob(
				eq(TableQueryResultWidget.RUNNING_QUERY_MESSAGE), eq(false), eq(AsynchType.TableQuery),
				any(QueryBundleRequest.class), asyncProgressHandlerCaptor.capture());
		...
		verify(mockPageWidget, never()).configure(any(QueryResultBundle.class), any(Query.class),
				anyList(), anyBoolean(), any(TableType.class), any(RowSelectionListener.class),
				any(TableQueryResultWidget.class), any(CallbackP.class));
		...
		verify(mockPageWidget).configure(eq(bundle), eq(widget.getStartingQuery()), eq(sortList),
				eq(false), eq(tableType), any(RowSelectionListener.class), eq(widget),
				mockFacetChangedHandlerCaptor.capture());
		...
		CallbackP<FacetColumnRequest> facetChangeRequestHandler =
				mockFacetChangedHandlerCaptor.getValue();
		...
		// verify that if the column name is the same, then the selected facet is updated for that
		// column
	...
	public void testOnPageChange() {
		...
		Long expectedPartsMask = BUNDLE_MASK_QUERY_RESULTS | BUNDLE_MASK_QUERY_COLUMN_MODELS
		...
		verify(mockJobTrackingWidget2).startAndTrackJob(
				eq(TableQueryResultWidget.RUNNING_QUERY_MESSAGE), eq(false), eq(AsynchType.TableQuery),
				qbrCaptor.capture(), asyncProgressHandlerCaptor.capture());
	...
	public void testSchemaChange() {
		...
		asyncProgressHandlerCaptor.getValue().onFailure(
				new BadRequestException(TableQueryResultWidget.FACET_COLUMNS_CHANGED_MESSAGE, code));
	...
	public void testConfigureSuccessNotEditable() {
		...
		widget.configure(query, isEditable, tableType, mockListner);
		verify(mockJobTrackingWidget).startAndTrackJob(eq(TableQueryResultWidget.RUNNING_QUERY_MESSAGE),
				eq(false), eq(AsynchType.TableQuery), any(QueryBundleRequest.class),
				asyncProgressHandlerCaptor.capture());
		// invoke a success
	...
	public void testConfigureEmptyResultsIsConsistentFalse() {
		...
		verify(mockJobTrackingWidget2).startAndTrackJob(
				eq(TableQueryResultWidget.RUNNING_QUERY_MESSAGE), eq(false), eq(AsynchType.TableQuery),
				any(QueryBundleRequest.class), any(AsynchronousProgressHandler.class));
	...
	public void testConfigurError() {
		...
		verify(mockView).setErrorVisible(true);
		verify(mockPageWidget, times(2)).setTableVisible(false);
		// note that if not logged in, synapse alert will show prompt the user to login in order to run
		// the query.
	...
	public void testVerifyEtagUpdated() {
		...
		when(mockClientCache.get(ENTITY_ID + QueryResultEditorWidget.VIEW_RECENTLY_CHANGED_KEY))
				.thenReturn(etag);
		...
		verify(mockJobTrackingWidget).startAndTrackJob(
				eq(TableQueryResultWidget.VERIFYING_ETAG_MESSAGE), eq(false), eq(AsynchType.TableQuery),
				any(QueryBundleRequest.class), asyncProgressHandlerCaptor.capture());
	...
	public void testVerifyEtagOld() {
		...
		verify(mockClientCache, never())
				.remove(ENTITY_ID + QueryResultEditorWidget.VIEW_RECENTLY_CHANGED_KEY);
		verify(mockGWT).scheduleExecution(callbackCaptor.capture(),
				eq(TableQueryResultWidget.ETAG_CHECK_DELAY_MS));
