...
public class TableQueryResultWikiWidgetTest {
	...
	@Before
	public void before() {
		widget = new TableQueryResultWikiWidget(mockView, mockActionMenu, mockEntityActionController,
				mockSynapseJSNIUtils, mockSynapseJavascriptClient, mockSynAlert, mockGWT, mockGinInjector);
	...
	public void testConfigure() {
		...
		verify(mockSynapseJavascriptClient).getEntityBundleFromCache(eq(tableId),
		...
		verify(mockEntityActionController).configure(mockActionMenu, mockEntityBundle, isCurrentVersion,
				wikiPageRootId, EntityArea.TABLES);
		boolean canEdit = false;
		verify(mockTableEntityWidget).configure(mockEntityBundle, tableVersionNumber, canEdit, widget,
				mockActionMenu);
	...
	public void testLoadMultipleTables() {
		// do not typically use reset(), but other tests need mockSynapseJavascriptClient response. in
		// this test, we do not want it to respond (until we manually invoke)
		reset(mockSynapseJavascriptClient);
		TableQueryResultWikiWidget widget1 = new TableQueryResultWikiWidget(mockView, mockActionMenu,
				mockEntityActionController, mockSynapseJSNIUtils, mockSynapseJavascriptClient, mockSynAlert,
				mockGWT, mockGinInjector);
		TableQueryResultWikiWidget widget2 = new TableQueryResultWikiWidget(mockView, mockActionMenu,
				mockEntityActionController, mockSynapseJSNIUtils, mockSynapseJavascriptClient, mockSynAlert,
				mockGWT, mockGinInjector);
		...
		verifyZeroInteractions(mockGWT);
		verify(mockSynapseJavascriptClient).getEntityBundleFromCache(anyString(),
		...
		// invoke the async callback to complete widget1 load, and verify that widget2 would begin to
		// load (on callback execution).
		...
		assertTrue(TableQueryResultWikiWidget.isLoading);
		verify(mockSynapseJavascriptClient, times(2)).getEntityBundleFromCache(anyString(),
				any(AsyncCallback.class));
