...
public class EditJSONListModalTest{
	...
	@Before
	public void setup(){
		when(mockCellFactory.createEditor(any(ColumnModel.class))).thenReturn(mockCell1, mockCell2, mockCell3);
	...
	public void testConfigure_notJSONArray(){
		...
		when(mockGWTWrapper.parseJSONStrict(jsonString)).thenReturn(new JSONObject());
	...
	public void testConfigure_invalidJSON(){
		...
		when(mockGWTWrapper.parseJSONStrict(jsonString)).thenThrow(JSONException.class);
	...
	public void testOnClickPasteNewValues(){
		...
		when(mockInjector.getCommaSeparatedValuesParser()).thenReturn(mockCommaSeparatedValuesParser);
