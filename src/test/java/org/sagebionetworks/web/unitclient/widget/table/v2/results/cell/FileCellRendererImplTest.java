...
public class FileCellRendererImplTest {
	...
	public void testCreateAnchorHref() {
		...
		String expectedHref =
				"/Portal/filehandleassociation?associatedObjectId=syn123&associatedObjectType=TableEntity&fileHandleId="
						+ fileHandleId;
	...
	public void testCreateAnchorHrefView() {
		...
		renderer.setCellAddresss(address);
		renderer.setValue(fileHandleId);
		String expectedHref = "/Portal/filehandleassociation?associatedObjectId=" + rowId
				+ "&associatedObjectType=FileEntity&fileHandleId=" + fileHandleId;
		assertEquals(expectedHref, renderer.createAnchorHref());
	}
	...
	public void testCreateAnchorHrefFileViewMissingRowId() {
		...
		renderer.setCellAddresss(address);
		renderer.setValue(fileHandleId);
		verify(mockFileHandleAsyncHandler, never()).getFileResult(any(FileHandleAssociation.class),
	...
	public void testSetValueSuccessUnauthorized() {
		...
		// hide loading
		verify(mockView).setLoadingVisible(false);
		verify(mockView).setErrorText(FileCellRenderer.UNABLE_TO_LOAD_FILE_DATA + ": "
				+ FileResultFailureCode.UNAUTHORIZED.toString());
	}
	...
	public void testSetValueFailureAttached() {
		...
		AsyncMockStubber.callFailureWith(error).when(mockFileHandleAsyncHandler)
				.getFileResult(any(FileHandleAssociation.class), any(AsyncCallback.class));
		// attached.
		when(mockView.isAttached()).thenReturn(true);
		renderer.setValue(fileHandleId);
