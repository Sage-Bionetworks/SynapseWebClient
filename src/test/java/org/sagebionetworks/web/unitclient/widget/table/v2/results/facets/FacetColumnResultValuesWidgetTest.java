...
public class FacetColumnResultValuesWidgetTest {
  ...
  public void setUp() throws Exception {
    ...
    when(mockPortalGinInjector.getUserIdCellRenderer())
      .thenReturn(mockUserBadge);
    when(mockPortalGinInjector.getEntityIdCellRenderer())
  ...
  public void testConfigureEntityIdValue() {
    ...
    verify(mockEntityIdCellRenderer)
      .setValue(eq(VALUE), any(ClickHandler.class), eq(true));
    verify(mockView)
      .addValue(
        eq(DEFAULT_SELECTED),
        any(Widget.class),
  ...
  public void testOverflow() {
    int numberOfFacets =
      FacetColumnResultValuesWidget.MAX_VISIBLE_FACET_VALUES + 20;
    for (int i = 0; i < numberOfFacets; i++) {
      FacetColumnResultValueCount valuesCount = Mockito.mock(
        FacetColumnResultValueCount.class
      );
    ...
    verify(
      mockView,
      times(FacetColumnResultValuesWidget.MAX_VISIBLE_FACET_VALUES)
    )
      .addValue(anyBoolean(), any(Widget.class), anyLong(), anyString());
    verify(
      mockView,
      times(
        numberOfFacets - FacetColumnResultValuesWidget.MAX_VISIBLE_FACET_VALUES
      )
    )
      .addValueToOverflow(
        anyBoolean(),
    ...
    verify(mockView)
      .setShowAllButtonText(
        FacetColumnResultValuesWidget.SHOW_ALL + numberOfFacets
      );
  ...
  public void testSelectedFacetInOverflow() {
    ...
    for (int i = 0; i < numberOfFacets; i++) {
      ...
      when(valuesCount.getIsSelected())
        .thenReturn(
          i == FacetColumnResultValuesWidget.MAX_VISIBLE_FACET_VALUES
        );
      facetValues.add(valuesCount);
    }
    widget.configure(mockFacet, ColumnType.INTEGER, mockOnFacetRequest);
    verify(mockView).setColumnName(COLUMN_NAME);
    verify(mockView, times(numberOfFacets))
      .addValue(anyBoolean(), any(Widget.class), anyLong(), anyString());
    verify(mockView).setShowAllButtonVisible(false);
  }
