...
public class ColumnModelTableRowEditorWidgetTest {
  ...
  public void testConfigure() {
    ...
    verify(mockView)
      .setMaxSize("" + ColumnModelTableRowEditorWidgetImpl.DEFAULT_STRING_SIZE);
  ...
  public void testChangeToLink() {
    ...
    // It should keep the original value
    verify(mockView, times(1))
      .setMaxSize("" + ColumnModelTableRowEditorWidgetImpl.MAX_STRING_SIZE);
  ...
  public void testCanHaveSize() {
    Set<ColumnTypeViewEnum> expectedTrue = new HashSet<ColumnTypeViewEnum>(
      Arrays.asList(
        ColumnTypeViewEnum.String,
        ColumnTypeViewEnum.StringList,
        ColumnTypeViewEnum.Link
      )
    );
    for (ColumnTypeViewEnum type : ColumnTypeViewEnum.values()) {
      assertEquals(
        "Unexpected for type: " + type,
        expectedTrue.contains(type),
        editor.canHaveSize(type)
      );
    }
  }
  ...
  public void testCanHaveRestricted() {
    ...
    for (ColumnTypeViewEnum type : ColumnTypeViewEnum.values()) {
      assertEquals(
        "Unexpected for type: " + type,
        expectedTrue.contains(type),
        editor.canHaveRestrictedValues(type)
      );
    }
  }
  ...
  public void testValidateNameNull() {
    ...
    verify(mockView)
      .setNameError(ColumnModelTableRowEditorWidgetImpl.NAME_CANNOT_BE_EMPTY);
  }
  ...
  public void testValidateSize() {
    ...
    when(mockView.getMaxSize())
      .thenThrow(new IllegalArgumentException("Should not be hit"));
  ...
  public void testValidateStringTooSmall() {
    ...
    verify(mockView)
      .setSizeError(ColumnModelTableRowEditorWidgetImpl.MUST_BE_A_NUMBER);
  ...
  public void testValidateStringTooLarge() {
    ...
    when(mockView.getColumnType()).thenReturn(ColumnTypeViewEnum.String);
    when(mockView.getMaxSize())
      .thenReturn("" + ColumnModelTableRowEditorWidgetImpl.MAX_STRING_SIZE + 1);
    assertFalse(editor.validateSize());
    verify(mockView)
      .setSizeError(ColumnModelTableRowEditorWidgetImpl.MUST_BE_A_NUMBER);
    verify(mockView, never()).clearSizeError();
  ...
  public void testValidateString() {
    ...
    when(mockView.getColumnType()).thenReturn(ColumnTypeViewEnum.String);
    when(mockView.getMaxSize())
      .thenReturn("" + ColumnModelTableRowEditorWidgetImpl.MAX_STRING_SIZE);
    assertTrue(editor.validateSize());
  ...
  public void testValidateListLength() {
    ...
    when(mockView.getColumnType()).thenReturn(ColumnTypeViewEnum.Integer);
    when(mockView.getMaxListLength())
      .thenThrow(new IllegalArgumentException("Should not be hit"));
  ...
  public void testValidateListLengthStringTooSmall() {
    ...
    verify(mockView)
      .setMaxListLengthError(
        ColumnModelTableRowEditorWidgetImpl.MUST_BE_A_NUMBER_ONE_AND_HUNDRED
      );
  ...
  public void testValidateListLengthStringTooLarge() {
    ...
    when(mockView.getMaxListLength())
      .thenReturn("" + ColumnModelTableRowEditorWidgetImpl.MAX_LIST_LENGTH + 1);
    assertFalse(editor.validateMaxListLength());
    verify(mockView)
      .setMaxListLengthError(
        ColumnModelTableRowEditorWidgetImpl.MUST_BE_A_NUMBER_ONE_AND_HUNDRED
  ...
  public void testValidateListLengthString() {
    ...
    when(mockView.getColumnType()).thenReturn(ColumnTypeViewEnum.StringList);
    when(mockView.getMaxListLength())
      .thenReturn("" + ColumnModelTableRowEditorWidgetImpl.MAX_LIST_LENGTH);
    assertTrue(editor.validateMaxListLength());
  ...
  public void testConfigureFacetsForTypeInteger() {
    ...
    verify(mockView)
      .setFacetValues(None.toString(), Values.toString(), Range.toString());
    verify(mockView).setFacetVisible(true);
  }
