...
public class ColumnModelsEditorWidgetTest {
	...
	public void before() {
		...
		when(mockFileViewDefaultColumns.getDefaultViewColumnNames(anyBoolean()))
				.thenReturn(nonEditableColumnNames);
		widget =
				new ColumnModelsEditorWidget(mockGinInjector, adapterFactory, mockFileViewDefaultColumns);
	...
	public void testConfigure() {
		...
		// select all, delete, and add schema again (verify all columns from the schema have been
		// re-added).
		widget.selectAll();
	...
	public void testFileView() {
		...
		when(mockGinInjector.createColumnModelEditorWidget())
				.thenReturn(mockColumnModelTableRowEditorWidget1, mockColumnModelTableRowEditorWidget2);
	...
	public void testAddNewColumn() {
		...
		verify(mockKeyboardNavigationHandler, times(schema.size() + 1))
				.bindRow(any(RowOfWidgets.class));
	...
	public void testSelectAll() {
		widget.configure(TableType.table, schema);
		// checks selection state each time a column editor is added, and once when columns are
		// initialized.
