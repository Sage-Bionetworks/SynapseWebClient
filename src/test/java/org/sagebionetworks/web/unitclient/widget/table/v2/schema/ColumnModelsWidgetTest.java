...
public class ColumnModelsWidgetTest {
	...
	public void before() {
		...
		when(mockGinInjector.createColumnModelEditorWidget())
				.thenAnswer(new Answer<ColumnModelTableRowEditorWidget>() {
					@Override
		...
		when(mockGinInjector.createNewColumnModelTableRowViewer())
				.thenAnswer(new Answer<ColumnModelTableRowViewer>() {
					@Override
		...
		widget = new ColumnModelsWidget(mockBaseView, mockGinInjector, mockSynapseClient, mockEditor,
				mockJobTrackingWidget, mockFileViewDefaultColumns, mockSynAlert);
		...
		AsyncMockStubber.callSuccessWith(mockTableSchemaChangeRequest).when(mockSynapseClient)
				.getTableUpdateTransactionRequest(anyString(), anyList(), anyList(),
		...
		AsyncMockStubber.callSuccessWith(mockColumnModelPage1, mockColumnModelPage2)
				.when(mockSynapseClient).getPossibleColumnModelsForViewScope(any(ViewScope.class),
	...
	public void testGetDefaultColumnsForView() {
		...
		when(mockFileViewDefaultColumns.getDefaultViewColumns(anyBoolean(), anyBoolean()))
	...
	public void testGetPossibleColumnModelsForViewScope() {
		...
		verify(mockSynapseClient).getPossibleColumnModelsForViewScope(viewScopeCaptor.capture(),
				eq(firstPageToken), any(AsyncCallback.class));
		// verify scope
		...
		verify(mockSynapseClient).getPossibleColumnModelsForViewScope(any(ViewScope.class),
				eq(NEXT_PAGE_TOKEN), any(AsyncCallback.class));
	...
	public void testGetPossibleColumnModelsForViewScopeWithViewType() {
		...
		List<ColumnModel> schema = TableModelTestUtils.createOneOfEachType(true);
		tableBundle.setColumnModels(schema);
		org.sagebionetworks.repo.model.table.ViewType viewtype =
	...
	public void testGetPossibleColumnModelsForViewScopeFailure() {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient)
				.getPossibleColumnModelsForViewScope(any(ViewScope.class), anyString(),
						any(AsyncCallback.class));
		String firstPageToken = null;
		widget.getPossibleColumnModelsForViewScope(firstPageToken);
		verify(mockSynapseClient).getPossibleColumnModelsForViewScope(any(ViewScope.class),
	...
	private AsynchronousProgressHandler onSave() {
		...
		verify(mockJobTrackingWidget).startAndTrackJob(eq(ColumnModelsWidget.UPDATING_SCHEMA),
				eq(isDeterminate), eq(AsynchType.TableTransaction), eq(mockTableSchemaChangeRequest),
				captor.capture());
	...
	public void testOnSaveFailure() throws JSONObjectAdapterException {
		...
		AsyncMockStubber.callFailureWith(ex).when(mockSynapseClient).getTableUpdateTransactionRequest(
				anyString(), anyList(), anyList(), any(AsyncCallback.class));
		// Now call save
		widget.onSave();
	...
	public void testGetTableType() {
		when(mockView.getViewTypeMask()).thenReturn(
				ViewTypeMask.getMaskForDepricatedType(org.sagebionetworks.repo.model.table.ViewType.file));
		assertEquals(TableType.files, TableType.getTableType(mockView));

		when(mockView.getViewTypeMask()).thenReturn(ViewTypeMask
				.getMaskForDepricatedType(org.sagebionetworks.repo.model.table.ViewType.file_and_table));
		assertEquals(TableType.files_tables, TableType.getTableType(mockView));

		when(mockView.getViewTypeMask()).thenReturn(ViewTypeMask
				.getMaskForDepricatedType(org.sagebionetworks.repo.model.table.ViewType.project));
