...
public class FileHandleUploadWidgetImplTest {
	...
	public void testFileSelected() {
		...
		verify(mockMultipartUploader).uploadFile(anyString(), anyString(), any(JavaScriptObject.class),
				handleCaptor.capture(), any(Long.class), eq(mockView));
	...
	public void testMultiFileSelected() {
		...
		// The progress should be updated with scaled values based on the presence of two files to
		// upload
		verify(mockMultipartUploader).uploadFile(anyString(), anyString(), any(JavaScriptObject.class),
				handleCaptor.capture(), any(Long.class), eq(mockView));
		verify(mockView).showProgress(true);
	...
	public void testFileSelectedFailedWithValidationCallback() {
		...
		}).when(mockMultipartUploader).uploadFile(anyString(), anyString(), any(JavaScriptObject.class),
				handleCaptor.capture(), any(Long.class), eq(mockView));
