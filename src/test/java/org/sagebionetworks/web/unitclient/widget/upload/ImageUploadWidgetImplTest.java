...
public class ImageUploadWidgetImplTest {
	...
	public void before() {
		...
		widget = new ImageUploadWidget(mockMultipartUploader, mockJSNIUtils, mockSynAlert,
	...
	public void testFileSelected() {
		...
		verify(mockMultipartUploader).uploadFile(anyString(), eq("image/png"),
				any(JavaScriptObject.class), handleCaptor.capture(), any(Long.class), eq(mockView));
	...
	public void testFileSelectedForceContentType() {
		...
		verify(mockMultipartUploader).uploadFile(anyString(), eq("image/jpeg"),
				any(JavaScriptObject.class), handleCaptor.capture(), any(Long.class), eq(mockView));
	}
