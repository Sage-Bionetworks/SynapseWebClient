...
public class MultipartUploaderTest {
	...
	public void before() throws Exception {
		...
		when(synapseJsniUtils.getContentType(any(JavaScriptObject.class), anyInt()))
				.thenReturn("image/png");
		AsyncMockStubber.callSuccessWith(mockMultipartUploadStatus).when(mockJsClient)
				.startMultipartUpload(any(MultipartUploadRequest.class), anyBoolean(),
		...
		when(mockPartPresignedUrl.getUploadPresignedUrl())
				.thenReturn("http://fakepresignedurl.uploader.test");
		AsyncMockStubber.callSuccessWith(mockBatchPresignedUploadUrlResponse).when(mockJsClient)
		...
		AsyncMockStubber.callSuccessWith(mockAddPartResponse).when(mockJsClient)
				.addPartToMultipartUpload(anyString(), anyInt(), anyString(), any(AsyncCallback.class));
		AsyncMockStubber.callSuccessWith(mockMultipartUploadStatus).when(mockJsClient)
				.completeMultipartUpload(anyString(), any(AsyncCallback.class));
		...
		}).when(synapseJsniUtils).getFilePartMd5(any(JavaScriptObject.class), anyInt(), anyLong(),
				any(MD5Callback.class));
		...
		when(synapseJsniUtils.getMultipleUploadFileNames(any(JavaScriptObject.class)))
	...
	public void testDirectUploadFolder() throws Exception {
		...
		uploader.uploadFile(FILE_NAME, CONTENT_TYPE, mockFileBlob, mockHandler, storageLocationId,
				mockView);
	...
	public void testDirectUploadAllPartsExist() throws Exception {
		...
		uploader.uploadFile(FILE_NAME, CONTENT_TYPE, mockFileBlob, mockHandler, storageLocationId,
				mockView);
		verify(mockJsClient).startMultipartUpload(any(MultipartUploadRequest.class), anyBoolean(),
				any(AsyncCallback.class));

		// never tries to get a presigned url, since all parts are uploaded.
		verify(mockJsClient, never()).getMultipartPresignedUrlBatch(
				any(BatchPresignedUploadUrlRequest.class), any(AsyncCallback.class));
		verify(synapseJsniUtils, never()).uploadFileChunk(anyString(), any(JavaScriptObject.class),
				anyLong(), anyLong(), anyString(), any(XMLHttpRequest.class), any(ProgressCallback.class));
		// combine parts
	...
	public void testDirectUploadEmptyFile() throws Exception {
		...
		uploader.uploadFile(FILE_NAME, CONTENT_TYPE, mockFileBlob, mockHandler, storageLocationId,
				mockView);
		verify(mockJsClient, never()).startMultipartUpload(any(MultipartUploadRequest.class),
	...
	public void testDirectUploadSinglePart() throws Exception {
		...
		verify(synapseJsniUtils).uploadFileChunk(eq(MultipartUploaderImpl.BINARY_CONTENT_TYPE),
				any(JavaScriptObject.class), anyLong(), anyLong(), anyString(), any(XMLHttpRequest.class),
		...
		verify(mockJsClient).addPartToMultipartUpload(anyString(), anyInt(), anyString(),
				any(AsyncCallback.class));
		verify(mockJsClient).completeMultipartUpload(anyString(), any(AsyncCallback.class));
		// the handler should get the id.
		verify(mockHandler).uploadSuccess(RESULT_FILE_HANDLE_ID);
	...
	public void testDirectUploadSingleSecondPart() throws Exception {
		...
		verify(mockJsClient).startMultipartUpload(any(MultipartUploadRequest.class), anyBoolean(),
				any(AsyncCallback.class));
		verify(mockJsClient).getMultipartPresignedUrlBatch(any(BatchPresignedUploadUrlRequest.class),
				any(AsyncCallback.class));
		verify(synapseJsniUtils).uploadFileChunk(anyString(), any(JavaScriptObject.class), anyLong(),
				anyLong(), anyString(), any(XMLHttpRequest.class), any(ProgressCallback.class));
		// manually call the method that's invoked with a successful xhr put (upload)
		uploader.addCurrentPartToMultipartUpload();
		verify(mockJsClient).addPartToMultipartUpload(anyString(), anyInt(), anyString(),
				any(AsyncCallback.class));
	...
	public void testDirectUploadMultiPart() throws Exception {
		...
		verify(synapseJsniUtils, times(2)).getFileMd5(any(JavaScriptObject.class),
				any(MD5Callback.class));
	...
	public void testUploadCanceled() throws Exception {
		...
		// never updated the handler because the upload has been canceled (can't verify the abort(),
		// since xhr is a js object).
	...
	public void testStartMultipartUploadFailure() throws Exception {
		String error = "failed";
		AsyncMockStubber.callFailureWith(new IllegalArgumentException(error)).when(mockJsClient)
				.startMultipartUpload(any(MultipartUploadRequest.class), anyBoolean(),
						any(AsyncCallback.class));
		setPartsState("00");
		uploader.uploadFile(FILE_NAME, CONTENT_TYPE, mockFileBlob, mockHandler, storageLocationId,
	...
	public void testStartMultipartUploadAttemptsExceeded() throws Exception {
		...
		verify(mockJsClient, times(12)).startMultipartUpload(any(MultipartUploadRequest.class),
				anyBoolean(), any(AsyncCallback.class));
	...
	public void testCompleteMultipartUploadFailure() throws Exception {
		...
		verify(mockJsClient, times(2)).startMultipartUpload(any(MultipartUploadRequest.class),
				anyBoolean(), any(AsyncCallback.class));
	}
	...
	public void testFixingDefaultContentType() throws RestServiceException {
		...
		inputContentType = "";
		assertEquals(ContentTypeUtils.PLAIN_TEXT,
		...
		assertEquals(WebConstants.TEXT_TAB_SEPARATED_VALUES,
				fixDefaultContentType(inputContentType, inputFilename));
		...
		assertEquals(WebConstants.TEXT_COMMA_SEPARATED_VALUES,
				fixDefaultContentType(inputContentType, inputFilename));
		...
		inputContentType = null;
		inputFilename = "";
		assertEquals(ContentTypeUtils.APPLICATION_OCTET_STREAM,
				fixDefaultContentType(inputContentType, inputFilename));
	}
