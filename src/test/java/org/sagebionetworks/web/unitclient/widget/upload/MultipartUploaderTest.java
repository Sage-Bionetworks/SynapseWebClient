...
public class MultipartUploaderTest {
  ...
  public void before() throws Exception {
    ...
    AsyncMockStubber
      .callSuccessWith(mockMultipartUploadStatus)
      .when(mockJsClient)
    ...
    when(mockMultipartUploadStatus.getResultFileHandleId())
      .thenReturn(RESULT_FILE_HANDLE_ID);
    List<PartPresignedUrl> presignedUrlList = new ArrayList<PartPresignedUrl>();
    when(mockBatchPresignedUploadUrlResponse.getPartPresignedUrls())
      .thenReturn(presignedUrlList);
    ...
    AsyncMockStubber
      .callSuccessWith(mockBatchPresignedUploadUrlResponse)
      .when(mockJsClient)
      .getMultipartPresignedUrlBatch(
        any(BatchPresignedUploadUrlRequest.class),
        any(AsyncCallback.class)
      );
    when(mockAddPartResponse.getAddPartState())
      .thenReturn(AddPartState.ADD_SUCCESS);
    AsyncMockStubber
      .callSuccessWith(mockAddPartResponse)
      .when(mockJsClient)
      .addPartToMultipartUpload(
        anyString(),
        anyInt(),
        anyString(),
        any(AsyncCallback.class)
  ...
  public void testDirectUploadSinglePart() throws Exception {
    ...
    assertEquals(
      MultipartUploaderImpl.BINARY_CONTENT_TYPE,
      captor.getValue().getContentType()
    );
    verify(synapseJsniUtils)
      .uploadFileChunk(
  ...
  public void testDirectUploadMultiPart() throws Exception {
    ...
    verify(synapseJsniUtils, times(2))
      .getFileMd5(any(JavaScriptObject.class), any(MD5Callback.class));
  ...
  public void testStartMultipartUploadFailure() throws Exception {
    String error = "failed";
    AsyncMockStubber
      .callFailureWith(new IllegalArgumentException(error))
      .when(mockJsClient)
      .startMultipartUpload(
        any(MultipartUploadRequest.class),
        anyBoolean(),
  ...
  public void testByteRange() {
    ...
    range =
      new ByteRange(
        1,
        PartUtils.MAX_PART_SIZE_BYTES - 1024,
        PartUtils.MAX_PART_SIZE_BYTES
      );
