...
public class TeamBadgeTest {
	...
	public void before() {
		...
		when(mockSnapseJsniUtils.getFileHandleAssociationUrl(anyString(),
				any(FileHandleAssociateType.class), anyString())).thenReturn(TEAM_ICON_URL);
		when(mockSynapseProperties.getSynapseProperty(WebConstants.PUBLIC_ACL_PRINCIPAL_ID))
		...
		badge =
				new TeamBadge(mockView, mockTeamAsyncHandler, mockSynapseProperties, mockSnapseJsniUtils);
	...
	public void testConfigureAsyncCustomClickHandler() throws Exception {
		AsyncMockStubber.callSuccessWith(team).when(mockTeamAsyncHandler).getTeam(eq(principalId),
	...
	public void testConfigureAsyncFail() throws Exception {
		AsyncMockStubber.callFailureWith(new Exception()).when(mockTeamAsyncHandler)
