...
public class UserBadgeTest {
	...
	public void before() {
		...
		when(mockSynapseJSNIUtils.getFileHandleAssociationUrl(anyString(),
				any(FileHandleAssociateType.class), anyString())).thenReturn(PICTURE_URL);
		userBadge = new UserBadge(mockView, mockSynapseClient, mockGlobalApplicationState,
				mockSynapseJSNIUtils, mockCache, mockUserProfileAsyncHandler, adapterFactory);
	}

	@Test
	public void testConfigureAsync() throws Exception {
		AsyncMockStubber.callSuccessWith(profile).when(mockUserProfileAsyncHandler)
				.getUserProfile(eq(principalId), any(AsyncCallback.class));
	...
	public void testConfigureAsyncFail() throws Exception {
		AsyncMockStubber.callFailureWith(new Exception()).when(mockUserProfileAsyncHandler)
				.getUserProfile(eq(principalId), any(AsyncCallback.class));
	...
	public void testConfigureSuccessFromCache() throws Exception {
		...
		userBadge.configure(principalId);
		verify(mockCache).get(anyString());
		verify(mockUserProfileAsyncHandler, never()).getUserProfile(eq(principalId),
