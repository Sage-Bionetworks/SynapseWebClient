...
public class UserBadgeTest {
  ...
  public static final String SMALL_PICTURE_URL =
    "https://s3.url.to.preview.picture";
  ...
  public void before() {
    ...
    when(
      mockSynapseJSNIUtils.getFileHandleAssociationUrl(
        anyString(),
        any(FileHandleAssociateType.class),
        anyString()
      )
    )
      .thenReturn(FULL_PICTURE_URL);
    when(mockJsClient.getProfilePicturePreviewURL(anyString(), anyBoolean()))
      .thenReturn(SMALL_PICTURE_URL);
    userBadge =
      new UserBadge(
        mockView,
        mockSynapseClient,
  ...
  public void testConfigureAsync() throws Exception {
    ...
    verify(mockUserProfileAsyncHandler)
      .getUserProfile(eq(principalId), any(AsyncCallback.class));
  ...
  public void testConfigureSuccessFromCache() throws Exception {
    ...
    verify(mockCache).get(anyString());
    verify(mockUserProfileAsyncHandler, never())
      .getUserProfile(eq(principalId), any(AsyncCallback.class));
    verify(mockView).configure(profile, FULL_PICTURE_URL, null, null);
  }
}
