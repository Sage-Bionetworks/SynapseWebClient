...
public class SynapseClientImplTest {
	...
	public void before() throws SynapseException, JSONObjectAdapterException {
		...
		when(mockSynapse.getAccessRequirements(any(RestrictableObjectDescriptor.class), anyLong(),
				anyLong())).thenReturn(ars);
		when(mockSynapse.getUnmetAccessRequirements(any(RestrictableObjectDescriptor.class),
		...
		when(mockSynapse.getOpenMembershipInvitations(anyString(), anyString(), anyLong(), anyLong()))
		...
		when(mockSynapse.getOpenMembershipRequests(anyString(), anyString(), anyLong(), anyLong()))
				.thenReturn(openRequests);
		...
		when(mockSynapse.getMyProjects(any(ProjectListType.class), any(ProjectListSortColumn.class),
				any(SortDirection.class), anyInt(), anyInt())).thenReturn(headers);
		when(mockSynapse.getProjectsFromUser(anyLong(), any(ProjectListSortColumn.class),
				any(SortDirection.class), anyInt(), anyInt())).thenReturn(headers);
		when(mockSynapse.getProjectsForTeam(anyLong(), any(ProjectListSortColumn.class),
				any(SortDirection.class), anyInt(), anyInt())).thenReturn(headers);
		...
		encodedNotificationSettingsToken =
				SerializationUtils.serializeAndHexEncode(notificationSettingsToken);

		when(mockSynapse.copyFileHandles(any(BatchFileHandleCopyRequest.class)))
				.thenReturn(mockBatchCopyResults);
	...
	private void setupTeamInvitations() throws SynapseException {
		...
		PaginatedResults<MembershipInvitation> paginatedInvitations =
				new PaginatedResults<MembershipInvitation>();
		paginatedInvitations.setResults(testInvitations);
		when(mockSynapse.getOpenMembershipInvitationSubmissions(anyString(), anyString(), anyLong(),
				anyLong())).thenReturn(paginatedInvitations);

		inviteeUserProfile = new UserProfile();
		inviteeUserProfile.setUserName("Invitee User");
		inviteeUserProfile.setOwnerId(USER_ID);
		when(mockSynapse.listUserProfiles(anyList()))
				.thenReturn(Collections.singletonList(inviteeUserProfile));
	}
	...
	private void setupOpenTeamRequest() throws SynapseException {
		...
		PaginatedResults<MembershipRequest> paginatedInvitations =
	...
	public void testMoveEntity()
			throws JSONObjectAdapterException, RestServiceException, SynapseException {
	...
	public void testGetWikiAttachmentHandles() throws Exception {
		...
		synapseClient
				.getWikiAttachmentHandles(new WikiPageKey("syn123", ObjectType.ENTITY.toString(), "20"));
		verify(mockSynapse)
				.getWikiAttachmenthHandles(any(org.sagebionetworks.repo.model.dao.WikiPageKey.class));
	}
	...
	public void testRestoreV2WikiPage() throws Exception {
		...
		Mockito.when(mockSynapse.restoreV2WikiPage(anyString(), any(ObjectType.class),
				any(String.class), anyLong())).thenReturn(v2Page);
		synapseClient.restoreV2WikiPage("ownerId", ObjectType.ENTITY.toString(), wikiId, new Long(2));
		verify(mockSynapse).restoreV2WikiPage(anyString(), any(ObjectType.class), any(String.class),
				anyLong());
	}
	...
	public void testGetV2WikiHistory() throws Exception {
		PaginatedResults<V2WikiHistorySnapshot> historyResults =
				new PaginatedResults<V2WikiHistorySnapshot>();
		when(mockSynapse.getV2WikiHistory(any(org.sagebionetworks.repo.model.dao.WikiPageKey.class),
				any(Long.class), any(Long.class))).thenReturn(historyResults);
		synapseClient.getV2WikiHistory(new WikiPageKey("syn123", ObjectType.ENTITY.toString(), "20"),
				new Long(10), new Long(0));
		verify(mockSynapse).getV2WikiHistory(any(org.sagebionetworks.repo.model.dao.WikiPageKey.class),
	...
	public void testCreateV2WikiPageWithV1() throws Exception {
		Mockito
				.when(mockSynapse.createWikiPage(anyString(), any(ObjectType.class), any(WikiPage.class)))
				.thenReturn(page);
	...
	public void testUpdateV2WikiPageWithV1() throws Exception {
		Mockito
				.when(mockSynapse.updateWikiPage(anyString(), any(ObjectType.class), any(WikiPage.class)))
				.thenReturn(page);
	...
	public void getV2WikiPageAsV1() throws Exception {
		Mockito.when(mockSynapse.getWikiPage(any(org.sagebionetworks.repo.model.dao.WikiPageKey.class)))
				.thenReturn(page);
		Mockito
				.when(mockSynapse.getV2WikiPage(any(org.sagebionetworks.repo.model.dao.WikiPageKey.class)))
				.thenReturn(v2Page);
	...
	public void testHtmlTeamMembersCache() throws Exception {
		...
		// the cache should be ready, it should not ask for the team members from the synapse client
		// again for an hour
	...
	private void resetUpdateExternalFileHandleMocks(String testId, FileEntity file,
			ExternalFileHandle handle) throws SynapseException, JSONObjectAdapterException {
		reset(mockSynapse);
	...
	public void testUpdateExternalFileHandle() throws Exception {
		...
		synapseClient.updateExternalFile(testId, testUrl, myFileName, contentType, fileSize, md5,
				storageLocationId);
		...
		// an exception
		when(mockSynapse.putEntity(any(FileEntity.class))).thenReturn(file)
				.thenThrow(new IllegalArgumentException("invalid name for some reason"));
		synapseClient.updateExternalFile(testId, testUrl, myFileName, contentType, fileSize, md5,
				storageLocationId);
	...
	public void testCreateExternalFile() throws Exception {
		...
		String contentType = "text/plain";
		Long fileSize = 1024L;
		when(mockSynapse.createExternalFileHandle(any(ExternalFileHandle.class)))
				.thenReturn(new ExternalFileHandle());
		when(mockSynapse.createEntity(any(FileEntity.class))).thenReturn(new FileEntity());
		synapseClient.createExternalFile(parentEntityId, externalUrl, fileName, contentType, fileSize,
				md5, storageLocationId);
		ArgumentCaptor<ExternalFileHandle> captor = ArgumentCaptor.forClass(ExternalFileHandle.class);
		verify(mockSynapse).createExternalFileHandle(captor.capture());
	...
	/**
	 * Direct upload tests. Most of the methods are simple pass-throughs to the Java Synapse client,
	 * but completeUpload has additional logic
	 * 
	...
	public void testGetFileEntityIdWithSameNameNotFound()
			throws JSONObjectAdapterException, SynapseException, RestServiceException, JSONException {

		// Have results returned in query.
		when(mockSynapse.lookupChild(anyString(), eq(testFileName)))
	...
	@Test
	public void testGetFileEntityIdWithSameNameFound()
			throws JSONException, JSONObjectAdapterException, SynapseException, RestServiceException {
	...
	public void testInviteMemberOpenInvitations()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		...
		verify(mockSynapse, Mockito.times(0)).addTeamMember(anyString(), anyString(), anyString(),
				anyString());
		verify(mockSynapse, Mockito.times(0))
				.createMembershipInvitation(any(MembershipInvitation.class), anyString(), anyString());

	}

	@Test
	public void testRequestMemberOpenRequests()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		...
		verify(mockSynapse, Mockito.times(0)).addTeamMember(anyString(), anyString(),
				eq(TEST_HOME_PAGE_BASE + "#!Team:"), eq(TEST_HOME_PAGE_BASE + "#!SignedToken:"));
		ArgumentCaptor<MembershipRequest> captor = ArgumentCaptor.forClass(MembershipRequest.class);
		verify(mockSynapse, Mockito.times(0)).createMembershipRequest(captor.capture(), anyString(),
				anyString());
	}
	...
	public void testInviteMemberCanJoin()
		...
		verify(mockSynapse).addTeamMember(anyString(), anyString(), eq(TEST_HOME_PAGE_BASE + "#!Team:"),
				eq(TEST_HOME_PAGE_BASE + "#!SignedToken:"));
	}

	@Test
	public void testRequestMembershipCanJoin()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		membershipStatus.setCanJoin(true);
	...
	public void testInviteMember()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		synapseClient.inviteMember("123", "a team", "", TEST_HOME_PAGE_BASE);
		verify(mockSynapse).createMembershipInvitation(any(MembershipInvitation.class),
				eq(TEST_HOME_PAGE_BASE + "#!SignedToken:"), eq(TEST_HOME_PAGE_BASE + "#!SignedToken:"));
	}
	...
	public void testRequestMembership()
		...
		verify(mockSynapse).createMembershipRequest(captor.capture(),
				eq(TEST_HOME_PAGE_BASE + "#!SignedToken:"), eq(TEST_HOME_PAGE_BASE + "#!SignedToken:"));
	...
	@Test
	public void testRequestMembershipWithExpiresOn()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		ArgumentCaptor<MembershipRequest> captor = ArgumentCaptor.forClass(MembershipRequest.class);
		verify(mockSynapse, Mockito.times(0)).createMembershipRequest(captor.capture(), anyString(),
				anyString());
	...
	@Test
	public void testGetOpenTeamInvitations()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		setupTeamInvitations();
		int limit = 55;
		int offset = 2;
		String teamId = "132";
		List<OpenTeamInvitationBundle> invitationBundles =
				synapseClient.getOpenTeamInvitations(teamId, limit, offset);
		verify(mockSynapse).getOpenMembershipInvitationSubmissions(eq(teamId), anyString(),
				eq((long) limit), eq((long) offset));
	...
	public void testGetOpenRequests()
		...
		String teamId = "132";
		List<MembershipRequestBundle> bundles = synapseClient.getOpenRequests(teamId);
		verify(mockSynapse).getOpenMembershipRequests(eq(teamId), anyString(), anyLong(), anyLong());
	...
	@Test
	public void testGetTeamBundle() throws SynapseException, RestServiceException,
			MalformedURLException, JSONObjectAdapterException {
	...
	@Test
	public void testGetTeamMembers() throws SynapseException, RestServiceException,
			MalformedURLException, JSONObjectAdapterException {
		...
		when(mockSynapse.getTeamMembers(anyString(), anyString(),
				any(TeamMemberTypeFilterOptions.class), anyLong(), anyLong())).thenReturn(allMembers);
		...
		// make the call
		TeamMemberPagedResults results = synapseClient.getTeamMembers("myTeamId", "search term",
				TeamMemberTypeFilterOptions.ALL, 100, 0);
	...
	public void testIsTeamMember()
			throws NumberFormatException, RestServiceException, SynapseException {
	...
	public void testSendMessage()
		...
		when(mockSynapse.sendStringMessage(any(MessageToUser.class), eq(messageBody)))
				.thenReturn(mockMessageToUser);
		when(mockMessageToUser.writeToJSONObject(any(JSONObjectAdapter.class)))
				.thenReturn(mockJSONObjAd);
	...
	@Test
	public void testSendMessageToEntityOwner()
			throws SynapseException, RestServiceException, JSONObjectAdapterException {
		ArgumentCaptor<MessageToUser> arg = ArgumentCaptor.forClass(MessageToUser.class);
		...
		when(mockSynapse.sendStringMessage(any(MessageToUser.class), eq(entityId), eq(messageBody)))
				.thenReturn(mockMessageToUser);
		when(mockMessageToUser.writeToJSONObject(any(JSONObjectAdapter.class)))
				.thenReturn(mockJSONObjAd);
	...
	public void testGetCertifiedUserPassingRecord()
			throws RestServiceException, SynapseException, JSONObjectAdapterException {
		PassingRecord passingRecord = new PassingRecord();
		passingRecord.setPassed(true);
		passingRecord.setQuizId(1238L);
		String passingRecordJson =
				passingRecord.writeToJSONObject(adapterFactory.createNew()).toJSONString();
	...
	public void testUserNeverAttemptedCertification() throws RestServiceException, SynapseException {
		when(mockSynapse.getCertifiedUserPassingRecord(anyString()))
				.thenThrow(new SynapseNotFoundException("PassingRecord not found"));
	...
	public void testSubmitCertificationQuizResponse()
		...
		when(mockSynapse.submitCertifiedUserTestResponse(any(QuizResponse.class)))
	...
	public void testFilterAccessRequirements() throws Exception {
		...
		filteredAccessRequirements = AccessRequirementUtils
				.filterAccessRequirements(unfilteredAccessRequirements, ACCESS_TYPE.UPDATE);
		...
		// 1 download
		filteredAccessRequirements = AccessRequirementUtils
				.filterAccessRequirements(unfilteredAccessRequirements, ACCESS_TYPE.DOWNLOAD);
		assertEquals(1, filteredAccessRequirements.size());
		// 2 submit
		filteredAccessRequirements = AccessRequirementUtils
				.filterAccessRequirements(unfilteredAccessRequirements, ACCESS_TYPE.SUBMIT);
		...
		filteredAccessRequirements =
				AccessRequirementUtils.filterAccessRequirements(null, ACCESS_TYPE.SUBMIT);
	...
	public void testGetEntityUnmetAccessRequirements() throws Exception {
		...
		verify(mockSynapse).getUnmetAccessRequirements(any(RestrictableObjectDescriptor.class),
				any(ACCESS_TYPE.class), anyLong(), anyLong());
	}
	...
	public void testGetAllEntityAccessRequirements() throws Exception {
		...
		verify(mockSynapse).getAccessRequirements(any(RestrictableObjectDescriptor.class), anyLong(),
				anyLong());
	}
	...
	public void testGetAllEntityAccessRequirementsTwoPage() throws Exception {
		...
		when(mockSynapse.getUnmetAccessRequirements(any(RestrictableObjectDescriptor.class),
				any(ACCESS_TYPE.class), anyLong(), anyLong())).thenReturn(page1, page2);
		...
		org.sagebionetworks.web.shared.PaginatedResults<AccessRequirement> results =
				synapseClient.getEntityAccessRequirements(entityId, unmetOnly, null);
		// 1 full page of results
		assertEquals(SynapseClientImpl.LIMIT_50 + 1, results.getResults().size());
		verify(mockSynapse).getUnmetAccessRequirements(any(RestrictableObjectDescriptor.class),
				any(ACCESS_TYPE.class), eq(SynapseClientImpl.LIMIT_50),
				eq(SynapseClientImpl.ZERO_OFFSET.longValue()));
		verify(mockSynapse).getUnmetAccessRequirements(any(RestrictableObjectDescriptor.class),
				any(ACCESS_TYPE.class), eq(SynapseClientImpl.LIMIT_50), eq(SynapseClientImpl.LIMIT_50));
	}
	...
	public void testHandleSignedTokenJoinTeam() throws RestServiceException, SynapseException {
		...
		verify(mockSynapse).addTeamMember(joinTeamToken, TEST_HOME_PAGE_BASE + "#!Team:",
	...
	@Test
	public void testHandleSignedTokenNotificationSettings()
			throws RestServiceException, SynapseException {
		SignedTokenInterface token =
				synapseClient.hexDecodeAndDeserialize(encodedNotificationSettingsToken);
		synapseClient.handleSignedToken(token, TEST_HOME_PAGE_BASE);
		verify(mockSynapse).updateNotificationSettings(notificationSettingsToken);
	}

	@Test(expected = BadRequestException.class)
	public void testHandleSignedTokenInvalidNotificationSettings()
			throws RestServiceException, SynapseException {
		SignedTokenInterface token = synapseClient.hexDecodeAndDeserialize("invalid token");
	}
	...
	@Test
	public void testGetStorageLocationSettingNullSetting()
			throws SynapseException, RestServiceException {
	...
	@Test
	public void testGetStorageLocationSettingNullUploadDestination()
			throws SynapseException, RestServiceException {
		assertNull(synapseClient.getStorageLocationSetting(entityId));
	}
	...
	@Test
	public void testCreateStorageLocationSettingFoundStorageAndProjectSetting()
			throws SynapseException, RestServiceException {
		setupGetMyLocationSettings();
		...
		when(mockSynapse.getProjectSetting(entityId, ProjectSettingsType.upload))
				.thenReturn(projectSetting);
		...
		verify(mockSynapse, Mockito.never())
				.createStorageLocationSetting(any(StorageLocationSetting.class));
		...
		UploadDestinationListSetting updatedProjectSetting =
				(UploadDestinationListSetting) captor.getValue();
	...
	@Test
	public void testCreateStorageLocationSettingNewStorageAndProjectSetting()
			throws SynapseException, RestServiceException {
		setupGetMyLocationSettings();
		when(mockSynapse.getProjectSetting(entityId, ProjectSettingsType.upload)).thenReturn(null);
		...
		when(mockSynapse.createStorageLocationSetting(any(StorageLocationSetting.class)))
		...
		// verify creates new project setting, and the new location list is a single value (id of the
	...
	@Test(expected = Exception.class)
	public void testCreateStorageLocationSettingFailure()
			throws SynapseException, RestServiceException {
		when(mockSynapse.getMyStorageLocationSetting(anyLong())).thenThrow(new Exception());
	...
	private void setupVersionedEntityBundle(String entityId, Long latestVersionNumber)
			throws SynapseException {
		...
		when(mockSynapse.getEntityBundleV2(anyString(), anyLong(), any(EntityBundleRequest.class)))
				.thenReturn(eb);
		...
		when(mockSynapse.getEntityVersions(anyString(), anyInt(), anyInt()))
				.thenReturn(versionInfoPaginatedResults);
		when(mockSynapse.getEntityById(anyString())).thenReturn(file);
	}

	@Test
	public void testGetTableUpdateTransactionRequestNoChange()
			throws RestServiceException, SynapseException {
		...
		TableUpdateTransactionRequest request =
				synapseClient.getTableUpdateTransactionRequest(tableId, oldColumnModels, newColumnModels);
		assertEquals(1, request.getChanges().size());
		TableSchemaChangeRequest tableUpdateRequest =
				(TableSchemaChangeRequest) request.getChanges().get(0);
	...
	@Test
	public void testGetTableUpdateTransactionRequestNewColumn()
			throws RestServiceException, SynapseException {
		String tableId = "syn93939";
		List<ColumnModel> oldColumnModels = new ArrayList<ColumnModel>();
		when(mockSynapse.createColumnModels(anyList()))
				.thenReturn(Collections.singletonList(mockNewColumnModelAfterCreate));
	...
	@Test
	public void testGetTableUpdateTransactionRequestFullTest()
			throws RestServiceException, SynapseException {
		...
		TableUpdateTransactionRequest request =
				synapseClient.getTableUpdateTransactionRequest(tableId, oldSchema, proposedNewSchema);
		verify(mockSynapse).createColumnModels(anyList());
		assertEquals(tableId, request.getEntityId());
		List<TableUpdateRequest> tableUpdates = request.getChanges();
		assertEquals(1, tableUpdates.size());
	...
	public void testUpdateFileEntityWrongResponseSize()
			throws RestServiceException, SynapseException {
		synapseClient.updateFileEntity(mockFileEntity, mockFileHandleCopyRequest);
	}

	@Test(expected = UnknownErrorException.class)
	public void testUpdateFileEntityWrongResponseSizeTooMany()
			throws RestServiceException, SynapseException {
	...
	public void testGenerateSqlWithFacetsError() throws RestServiceException, SynapseException {
		when(mockSynapse.transformSqlRequest(any(SqlTransformRequest.class)))
				.thenThrow(new SynapseBadRequestException());
	...
	public void testGenerateSqlWithFacets() throws RestServiceException, SynapseException {
		...
		when(mockSynapse.transformSqlRequest(any(SqlTransformRequest.class)))
				.thenReturn(expectedTransformedSql);
		...
		TransformSqlWithFacetsRequest transformRequest =
				(TransformSqlWithFacetsRequest) sqlTransformRequestCaptor.getValue();
	...
	public void testGetFileNameFromLocationPath() {
		...
		assertEquals(name, SynapseClientImpl
				.getFileNameFromExternalUrl("http://some.really.long.com/path/to/a/file/" + name));
		assertEquals(name, SynapseClientImpl.getFileNameFromExternalUrl(
				"http://some.really.long.com/path/to/a/file/" + name + "?param1=value&param2=value"));
	...
	public void testIsChallenge() throws RestServiceException, SynapseException {
		...
		when(mockSynapse.getEvaluationByContentSource(anyString(), anyInt(), anyInt()))
				.thenReturn(testResults);
		...
		// "Before" junit test setup configured so this user to have the ability to change permissions
		// on eval 2, but not on eval 1
		assertTrue(synapseClient.isChallenge("syn123"));
	...
	public void testResendTeamInvitation() throws RestServiceException, SynapseException {
		...
		verify(mockSynapse).createMembershipInvitation(eq(mockMembershipInvitation),
				stringCaptor1.capture(), stringCaptor2.capture());
