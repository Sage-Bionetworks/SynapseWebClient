...
public class AliasRedirectorServletTest {
  ...
  public void setup()
    ...
    when(mockSynapse.getUserGroupHeadersByAliases(anyList()))
      .thenReturn(Collections.singletonList(mockUserGroupHeader));

    // when asking to encode the redirect URL, just return the input param.
    when(mockResponse.encodeRedirectURL(anyString()))
      .thenAnswer(
        new Answer<String>() {
  ...
  public void testDoGetUserAlias() throws Exception {
    ...
    String ownerId = "123";
    when(mockRequest.getParameter(WebConstants.ALIAS_PARAM_KEY))
      .thenReturn(alias);
    ...
    verify(mockSynapse)
      .getUserGroupHeadersByAliases(stringListCaptor.capture());
