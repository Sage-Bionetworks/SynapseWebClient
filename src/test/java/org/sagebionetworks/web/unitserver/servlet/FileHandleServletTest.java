...
public class FileHandleServletTest {
	...
	public void setup() throws IOException, SynapseException {
		...
		when(mockSynapse.getV2WikiAttachmentPreviewTemporaryUrl(any(WikiPageKey.class), anyString()))
				.thenReturn(resolvedUrl);
		when(mockSynapse.getV2WikiAttachmentTemporaryUrl(any(WikiPageKey.class), anyString()))
				.thenReturn(resolvedUrl);

		when(mockSynapse.getFileEntityPreviewTemporaryUrlForVersion(anyString(), anyLong()))
				.thenReturn(resolvedUrl);
		when(mockSynapse.getFileEntityTemporaryUrlForVersion(anyString(), anyLong()))
				.thenReturn(resolvedUrl);
		when(mockSynapse.getFileEntityPreviewTemporaryUrlForCurrentVersion(anyString()))
				.thenReturn(resolvedUrl);
		when(mockSynapse.getFileEntityTemporaryUrlForCurrentVersion(anyString()))
				.thenReturn(resolvedUrl);
		when(mockSynapse.getTableFileHandlePreviewTemporaryUrl(anyString(), any(RowReference.class),
				anyString())).thenReturn(resolvedUrl);
		when(mockSynapse.getTableFileHandleTemporaryUrl(anyString(), any(RowReference.class),
				anyString())).thenReturn(resolvedUrl);
	...
	public void testDoGetLoggedInWikiAttachmentPreview() throws Exception {
		...
		servlet.doGet(mockRequest, mockResponse);
		verify(mockSynapse).getVersionOfV2WikiAttachmentPreviewTemporaryUrl(any(WikiPageKey.class),
				anyString(), anyLong());
		verify(mockResponse).sendRedirect(anyString());
	}
	...
	public void testDoGetLoggedInWikiAttachment() throws Exception {
		...
		when(mockRequest.getParameter(WebConstants.WIKI_VERSION_PARAM_KEY)).thenReturn("1");
		servlet.doGet(mockRequest, mockResponse);
		verify(mockSynapse).getVersionOfV2WikiAttachmentTemporaryUrl(any(WikiPageKey.class),
				anyString(), anyLong());
		verify(mockResponse).sendRedirect(anyString());
	}
	...
	public void testDoGetLoggedInTableRowPreview() throws Exception {
		...
		when(mockRequest.getCookies()).thenReturn(cookies);
		servlet.doGet(mockRequest, mockResponse);
		verify(mockSynapse).getTableFileHandlePreviewTemporaryUrl(anyString(), any(RowReference.class),
				anyString());
		verify(mockResponse).sendRedirect(anyString());
	}
	...
	public void testDoGetLoggedInTableRow() throws Exception {
		...
		when(mockRequest.getCookies()).thenReturn(cookies);
		servlet.doGet(mockRequest, mockResponse);
		verify(mockSynapse).getTableFileHandleTemporaryUrl(anyString(), any(RowReference.class),
				anyString());
		verify(mockResponse).sendRedirect(anyString());
	}
	...
	public void testNoCacheHeaders() throws Exception {
		...
		verify(mockResponse).setHeader(eq(WebConstants.CACHE_CONTROL_KEY),
