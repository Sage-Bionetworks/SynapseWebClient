...
public class FileHandleServletTest {
  ...
  public void setup() throws IOException, SynapseException {
    ...
    when(
      mockSynapse.getV2WikiAttachmentPreviewTemporaryUrl(
    ...
    when(
      mockSynapse.getV2WikiAttachmentTemporaryUrl(
        any(WikiPageKey.class),
        anyString()
      )
    )
    ...
    when(
      mockSynapse.getFileEntityPreviewTemporaryUrlForVersion(
        anyString(),
        anyLong()
      )
    )
      .thenReturn(resolvedUrl);
    when(
      mockSynapse.getFileEntityTemporaryUrlForVersion(anyString(), anyLong())
    )
      .thenReturn(resolvedUrl);
    when(
      mockSynapse.getFileEntityPreviewTemporaryUrlForCurrentVersion(anyString())
    )
      .thenReturn(resolvedUrl);
    when(mockSynapse.getFileEntityTemporaryUrlForCurrentVersion(anyString()))
      .thenReturn(resolvedUrl);
    when(
      mockSynapse.getTableFileHandlePreviewTemporaryUrl(
        anyString(),
        any(RowReference.class),
        anyString()
      )
    )
      .thenReturn(resolvedUrl);
    when(
      mockSynapse.getTableFileHandleTemporaryUrl(
        anyString(),
        any(RowReference.class),
        anyString()
      )
  ...
  private void setupWiki() {
    when(mockRequest.getParameter(WebConstants.WIKI_OWNER_ID_PARAM_KEY))
      .thenReturn("syn296531");
    when(mockRequest.getParameter(WebConstants.WIKI_OWNER_TYPE_PARAM_KEY))
      .thenReturn(ObjectType.ENTITY.toString());
    when(mockRequest.getParameter(WebConstants.WIKI_ID_PARAM_KEY))
      .thenReturn("2");
    when(mockRequest.getParameter(WebConstants.WIKI_FILENAME_PARAM_KEY))
      .thenReturn("file.png");
  }

  private void setupFileEntity() {
    when(mockRequest.getParameter(WebConstants.ENTITY_PARAM_KEY))
      .thenReturn("syn296531");
    when(mockRequest.getParameter(WebConstants.ENTITY_VERSION_PARAM_KEY))
      .thenReturn("20");
  }
  ...
  private void setupTableEntityRow() {
    when(mockRequest.getParameter(WebConstants.ENTITY_PARAM_KEY))
      .thenReturn("syn296531");
    when(mockRequest.getParameter(WebConstants.TABLE_COLUMN_ID))
      .thenReturn("123");
    when(mockRequest.getParameter(WebConstants.TABLE_ROW_ID)).thenReturn("456");
    when(mockRequest.getParameter(WebConstants.TABLE_ROW_VERSION_NUMBER))
      .thenReturn("789");
  }

  private void setupTeam() {
    when(mockRequest.getParameter(WebConstants.TEAM_PARAM_KEY))
      .thenReturn("36");
  }
  ...
  public void testDoGetLoggedInWikiAttachmentPreview() throws Exception {
    setupWiki();
    when(mockRequest.getParameter(WebConstants.FILE_HANDLE_PREVIEW_PARAM_KEY))
      .thenReturn("true");
    Cookie[] cookies = { new Cookie(CookieKeys.USER_LOGIN_TOKEN, "fake") };
    when(mockRequest.getCookies()).thenReturn(cookies);
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getV2WikiAttachmentPreviewTemporaryUrl(
        any(WikiPageKey.class),
        anyString()
      );
    verify(mockResponse).sendRedirect(anyString());

    when(mockRequest.getParameter(WebConstants.WIKI_VERSION_PARAM_KEY))
      .thenReturn("1");
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getVersionOfV2WikiAttachmentPreviewTemporaryUrl(
        any(WikiPageKey.class),
        anyString(),
        anyLong()
      );
  ...
  public void testDoGetLoggedInWikiAttachment() throws Exception {
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getV2WikiAttachmentTemporaryUrl(any(WikiPageKey.class), anyString());
    verify(mockResponse).sendRedirect(anyString());
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getVersionOfV2WikiAttachmentTemporaryUrl(
        any(WikiPageKey.class),
        anyString(),
        anyLong()
      );
  ...
  public void testDoGetLoggedInFileEntityPreview() throws Exception {
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getFileEntityPreviewTemporaryUrlForVersion(anyString(), anyLong());
    verify(mockResponse).sendRedirect(anyString());
  ...
  public void testDoGetLoggedInFileEntity() throws Exception {
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getFileEntityTemporaryUrlForVersion(anyString(), anyLong());
    verify(mockResponse).sendRedirect(anyString());
  }

  @Test
  public void testDoGetLoggedInFileEntityPreviewCurrentVersion()
    throws Exception {
    setupFileEntity();
    when(mockRequest.getParameter(WebConstants.ENTITY_VERSION_PARAM_KEY))
      .thenReturn(null);
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getFileEntityPreviewTemporaryUrlForCurrentVersion(anyString());
    verify(mockResponse).sendRedirect(anyString());
  }
  ...
  public void testDoGetLoggedInTableRowPreview() throws Exception {
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getTableFileHandlePreviewTemporaryUrl(
        anyString(),
        any(RowReference.class),
        anyString()
      );
  ...
  public void testDoGetLoggedInTableRow() throws Exception {
    ...
    servlet.doGet(mockRequest, mockResponse);
    verify(mockSynapse)
      .getTableFileHandleTemporaryUrl(
        anyString(),
        any(RowReference.class),
        anyString()
      );
  ...
  @Test
  public void testDoGetLoggedInTableRowPreviewNotFullySpecifiedParams()
    throws Exception {
    setupTableEntityRow();
    when(mockRequest.getParameter(WebConstants.TABLE_COLUMN_ID))
      .thenReturn(null);
  ...
  @Test
  public void testDoGetLoggedInTableRowPreviewIllegalArgument()
    throws Exception {
    setupTableEntityRow();
    when(mockRequest.getParameter(WebConstants.TABLE_ROW_ID))
      .thenReturn("a problem");
    Cookie[] cookies = { new Cookie(CookieKeys.USER_LOGIN_TOKEN, "fake") };
    when(mockRequest.getCookies()).thenReturn(cookies);
    try {
      servlet.doGet(mockRequest, mockResponse);
